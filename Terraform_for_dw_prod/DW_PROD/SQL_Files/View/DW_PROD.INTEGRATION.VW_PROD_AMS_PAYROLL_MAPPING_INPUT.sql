create or replace view DW_PROD.INTEGRATION.VW_PROD_AMS_PAYROLL_MAPPING_INPUT(
	ID,
	MAP_ID,
	EMPLOYEE_ID,
	SYSTEM_CODE,
...
--AMS
 SELECT DISTINCT EMPLOYEE_KEY AS ID,
  ...
  REGEXP_REPLACE(COALESCE(DW_PROD.PUBLIC.REPLACE_NON_UTF8_CHARS(EMPLOYEE_HOME_PHONE),EMPLOYEE_CELL_PHONE,EMPLOYEE_WORK_PHONE), '[^0-9]','') PHONE,
  REGEXP_REPLACE(TRIM(DW_PROD.PUBLIC.REPLACE_NON_UTF8_CHARS(EMPLOYEE_HOME_PHONE)), '[^0-9]','') EMPLOYEE_HOME_PHONE,
  ...
  UPPER(COALESCE(DW_PROD.PUBLIC.REPLACE_NON_UTF8_CHARS(EMPLOYEE_ADDRESS1),EMPLOYEE_ADDRESS2)) ADDRESS,UPPER(TRIM(EMPLOYEE_CITY)) AS CITY,
  ...
  UPPER(REGEXP_REPLACE(EMPLOYEE_FIRST_NAME, '[^A-Za-z]', '')) AS EMPLOYEE_FIRST_NAME,UPPER(REGEXP_REPLACE(EMPLOYEE_LAST_NAME, '[^A-Za-z]', '')) AS EMPLOYEE_LAST_NAME,
  TRIM(HEX_DECODE_STRING(EMPLOYEE_PID)) AS EMPLOYEE_PID,TRIM(EMPLOYEE_DOB) AS EMPLOYEE_DOB,
  REGEXP_REPLACE(TRIM(COALESCE(DW_PROD.PUBLIC.REPLACE_NON_UTF8_CHARS(EMPLOYEE_HOME_PHONE),
  EMPLOYEE_CELL_PHONE,EMPLOYEE_WORK_PHONE)), '[^0-9]','') PHONE,
  REGEXP_REPLACE(TRIM(DW_PROD.PUBLIC.REPLACE_NON_UTF8_CHARS(EMPLOYEE_HOME_PHONE)), '[^0-9]','') EMPLOYEE_HOME_PHONE,
  REGEXP_REPLACE(TRIM(EMPLOYEE_CELL_PHONE), '[^0-9]','') EMPLOYEE_CELL_PHONE,UPPER(TRIM(COALESCE(EMPLOYEE_PERSONAL_EMAIL,EMPLOYEE_WORK_EMAIL))) EMAIL,
  UPPER(TRIM(COALESCE(DW_PROD.PUBLIC.REPLACE_NON_UTF8_CHARS(EMPLOYEE_ADDRESS1),EMPLOYEE_ADDRESS2))) ADDRESS,UPPER(TRIM(EMPLOYEE_CITY)) AS CITY,
