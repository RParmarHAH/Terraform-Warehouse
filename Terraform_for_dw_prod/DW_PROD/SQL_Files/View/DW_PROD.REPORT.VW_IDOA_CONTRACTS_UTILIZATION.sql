create or replace view DW_PROD.REPORT.VW_IDOA_CONTRACTS_UTILIZATION(
	CLIENT_KEY,
	CLIENT_ID,
	FIRSTNAME,
	LASTNAME,
	...
	SUPERVISOR_CODE,
	AUTH_DATE_REVISED,
	MONTHLYMAXHOURS_REVISED,
	OLDAUTHORIZEDHOURS,
	WEEKLY_HOURS_AUTHORIZED
) as
-------------------------------------------------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------------------------------------
...
JOIN DISC_PROD.DATAFLEXSYNCDATA.DFTIMESHEETS TS ON TS.CLIENTNUMBER = C.NUMBER AND TS.DBNAME = C.DBNAME 
JOIN DISC_PROD.DATAFLEXSYNCDATA.DFTIMESHEETHOURS TSH ON TS.REFERENCENO = TSH.REFERENCENO AND TS.DBNAME = TSH.DBNAME
JOIN DISC_PROD.DATAFLEXSYNCDATA.DFOFFICES O ON TS.OFFCENO = O.OFFICENUMBER AND TS.DBNAME = O.DBNAME
LEFT JOIN DW_PROD.HAH.FACT_VISIT  VV ON VV.VISIT_KEY = MD5(TSH.DBNAME || '-' || TSH.REFERENCENO || '-' || TSH.WEEKENDDATE || '-' || TSH.SERVICEDATE::date || '-' || 'DATAFLEXSYNCDATA')
WHERE 
TS.CONTRACTCODE = '2'
AND TSH.DBNAME = 'IL'
...
FROM DISC_PROD.DATAFLEXSYNCDATA.DFCLIENTCONTRACTS AUTH
JOIN DISC_PROD.DATAFLEXSYNCDATA.DFCLIENTS C ON C.DBNAME = AUTH.DBNAME AND C.NUMBER = AUTH.CLIENTNUMBER
LEFT JOIN CLIENT_DATA cd ON cd.CLIENT_NUMBER = c."NUMBER" AND cd.DBNAME = c.DBNAME
JOIN DISC_PROD.DATAFLEXSYNCDATA.DFCONTRACTS CON ON CON.DBNAME = AUTH.DBNAME AND CON.CONTRACTCODE = AUTH.CONTRACTCODE
JOIN DISC_PROD.DATAFLEXSYNCDATA.DFCLIENTCONTRACTAUTHORIZATIONS CA ON CA.DBNAME = AUTH.DBNAME AND CA.CLIENTNUMBER = AUTH.CLIENTNUMBER 
...
LEFT JOIN DW_PROD.HAH.DIM_SUPERVISOR SS ON v.SUPERVISOR_KEY = SS.SUPERVISOR_KEY 
)
--SELECT *
--FROM fin  
