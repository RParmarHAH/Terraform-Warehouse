create or replace view DW_PROD.STAGE.VW_CURRENT_ALAYACARE_DIM_INVOICE(
	INVOICE_KEY
) as
WITH INVOICE_ITEMS AS (
	...
	LEFT JOIN DISC_DEV.ALAYACARE.VISIT AS VISIT
		ON VISIT.VISIT_ID = INVOICE_ITEM.VISIT_ID 
	LEFT JOIN DISC_DEV.ALAYACARE.SERVICE AS SERVICE
		ON SERVICE.SERVICE_ID = VISIT.SERVICE_ID
	LEFT JOIN DISC_DEDUPE_DEV.ALAYACARE.CLIENT_MATCH_LIST   CM
		ON CM.ID = SERVICE.SERVICE_CLIENT_ID
	LEFT JOIN DISC_DEDUPE_DEV.ALAYACARE.CLIENT_MASTER_LIST  C	
...
--			AND VISIT.BRANCH_ID=C.BRANCH_ID 
	...
	LEFT JOIN DISC_DEV.ALAYACARE.GROUPS AS G
...
SELECT MD5(NVL(TRIM(B.PROPERTIES_TBL_GT_ACCOUNT_DESCRIPTION), 'Unknown') || ' (' || NVL(INVOICE_ITEMS.BRANCH_ID,-1) || ')' || '-' || INVOICE_DETAILS.INVOICE_ID || '-' || 'ALAYACARE') AS INVOICE_KEY,
	...
	FROM DISC_DEV.ALAYACARE.INVOICE_DETAILS AS INVOICE_DETAILS
	LEFT JOIN INVOICE_ITEMS AS INVOICE_ITEMS
		ON INVOICE_ITEMS.INVOICE_ID = INVOICE_DETAILS.INVOICE_ID
	LEFT JOIN DISC_DEV.ALAYACARE.FUNDER AS FUNDERS
	...
	LEFT JOIN DISC_DEV.ALAYACARE.BRANCH AS B 
