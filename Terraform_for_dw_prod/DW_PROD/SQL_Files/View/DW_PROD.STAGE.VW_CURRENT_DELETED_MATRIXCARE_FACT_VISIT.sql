create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_MATRIXCARE_FACT_VISIT(
	VISIT_KEY
) as 
...
,ADM_AUTH AS 
		( 
		 ...
		 LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_AUTHORIZATION AUTH ON ADM.ADM_ID =AUTH_ADMISSIONID
		)	
, INVOICE_PAYERS AS
	(	
...
FROM DISC_PROD.MATRIXCARE.HIST_STVHC_T_SCHEDULES AS SCH
...
LEFT JOIN DISC_PROD.MATRIXCARE.MATRIXCARE_MASTER_EMPLOYEE_MAPPING_ALL AS EMPLOYEE_MAPPING ON EMPLOYEE_MAPPING.EMPLOYEE_NUMBER = SCH.SCH_CAREGIVERID 
LEFT JOIN DISC_PROD.MATRIXCARE.MATRIXCARE_MASTER_CLIENT_MAPPING_ALL AS CLIENT_MAPPING ON CLIENT_MAPPING.CLIENT_NUMBER = SCH.SCH_CLIENTID 
...
LEFT JOIN DISC_PROD.MATRIXCARE.MATRIXCARE_MASTER_BRANCH_MAPPING BRANCH_MAPPING ON BRANCH_MAPPING.OFFICE_CODE = SCH.SCH_BRANCHID::STRING AND BRANCH_MAPPING.SOURCE_SYSTEM_ID = 7 AND BRANCH_MAPPING.SYSTEM_CODE = 'MATRIXCARE' 
...
WHERE SCH.ETL_DELETED_FLAG = 1 AND SCH.ETL_LAST_UPDATED_DATE::DATE IN (SELECT MAX(ETL_LAST_UPDATED_DATE)::DATE FROM DISC_PROD.MATRIXCARE.HIST_STVHC_T_SCHEDULES );