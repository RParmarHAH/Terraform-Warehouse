create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_DATAFLEXSYNCDATA_FACT_VISIT(
	VISIT_KEY
) as 
WITH 
CODES_DESC AS (													
...
FROM DISC_PROD.DATAFLEXSYNCDATA.DFTIMESHEETS TS1 
...
RIGHT JOIN DISC_PROD.DATAFLEXSYNCDATA.HIST_DFTIMESHEETHOURS f
...
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFTIMESHEETS TS 
...
FROM 
DISC_PROD.DATAFLEXSYNCDATA.DFTIMESHEETS TS1 
LEFT JOIN DF_IMPORT_VISITS DFI 																		-- ONLY IL AND MO CAN COME USING THIS JOIN FROM SANDATA
	ON TS1.TRACKINGID = DFI.TRACKINGID 
	AND TS1.WEEKENDDATE = DFI.WEEKENDDATE 
	AND TS1.DBNAME = DFI.STATE 
...
RIGHT JOIN DISC_PROD.DATAFLEXSYNCDATA.HIST_DFTIMESHEETHOURS f
	...
	AND F.SERVICEDATE = IFF(F.SERVICEDATE = COALESCE(SV.DATE,SCH.SCHEDULEDATE),COALESCE(SV.DATE,SCH.SCHEDULEDATE),F.SERVICEDATE) -- ONLY JOIN ON servicedate WHEN mapping IS possible, otherwise will GET TS.DATA ONLY FOR visits mapped WITH sd AND rest will have NULL
	AND F.SERVICEDATE = IFF(F.SERVICEDATE = DFI.SERVICEDATE,DFI.SERVICEDATE,NVL(DFI.SERVICEDATE,F.SERVICEDATE))					--  To avoid getting 1 - many join between TSH and Mapping table DFI
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFTIMESHEETS TS 															-- This join will only be used to get fields, the previous TS1 join's purpose is to just act as mapping between TABLES, using same TS table for both causes issues 
...
AND VISIT_KEY NOT IN (SELECT VISIT_KEY FROM DISC_PROD.PUBLIC.VW_DATAFLEX_SANDATA_PENDING_VISITS))
, FINAL AS 
(
	SELECT L.*
	FROM (
