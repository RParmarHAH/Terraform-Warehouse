create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_COSTALSYNCDATA_FACT_PAYROLL(
	PAYROLL_KEY
) as
WITH ServicePayCode
AS 
...
(
	...
	FROM DISC_PROD.COSTALSYNCDATA.HIST_GPALLPAYCHECKSBASE pcb
	...
	WHERE pcb.ETL_DELETED_FLAG = TRUE
        AND CAST(pcb.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_PROD.COSTALSYNCDATA.HIST_GPALLPAYCHECKSBASE)
)
