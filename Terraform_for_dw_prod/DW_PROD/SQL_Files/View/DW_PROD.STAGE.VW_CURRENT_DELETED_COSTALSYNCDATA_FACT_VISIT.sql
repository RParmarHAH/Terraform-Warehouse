create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_COSTALSYNCDATA_FACT_VISIT(
	VISIT_KEY
) as 
...
(
	...
	FROM DISC_PROD.COSTALSYNCDATA.HIST_CV_Schedules s 
	LEFT JOIN HOUR_UNIT_MAPPING um
	ON um.Plan_Code = s.Plan_Code
	AND um.Db = s.Db
WHERE s.ETL_DELETED_FLAG = TRUE
		AND CAST(s.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_PROD.COSTALSYNCDATA.HIST_CV_Schedules)
)
, Visit_Agg
AS
(
