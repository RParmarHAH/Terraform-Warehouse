create or replace view DW_PROD.PUBLIC.MS_CON(
	SOURCE_SYSTEM_ID,
	CLIENT_KEY,
...
--FROM DW_DEV.HAH.FACT_PARTNER_CONTRACT_SERVICE 
FROM DW_PROD.INTEGRATION.FACT_VISIT_MERGED a 
...
FROM dat a
JOIN DW_DEV.HAH.CONTRACT_MAPPING b ON (a.KEY = b.KEY1 OR a.KEY = b.KEY2 )
GROUP BY ALL 
ORDER BY CLIENT_KEY
), REVENUE_DATAFLEX AS (
...
--, sum(u.HOURS_SERVED) AS hs , sum(u.VISITS_ALL ) va , sum(u.hours_billed)hb,sum(VISITS_CLEAN_SHIFTS)cs,sum(VISITS_NEED_MAINTENANCE)vm
 FROM aug_visits a
JOIN DW_PROD.HAH.DIM_CONTRACT b ON a.CONTRACT_KEY =b.CONTRACT_KEY 
...
FROM DW_PROD.REPORT.UTILIZATION_2_0 uu
...
FROM DW_PROD.REPORT.UTILIZATION_1_0 uu
