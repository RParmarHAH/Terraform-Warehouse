create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_MATRIXCARE_DIM_CONTRACT(
	CONTRACT_KEY
) as
WITH PAYER_SERVICES AS (
	...
	SELECT MD5('MATRIXCARE' || '-' || PAYERS.PAY_ID::STRING || '-' || NVL(PAYER_SERVICES.SERVICECODEID::STRING, '') || '-' || 'MATRIXCARE') AS CONTRACT_KEY,
	...
	FROM DISC_PROD.MATRIXCARE.HIST_STVHC_T_PAYERS AS PAYERS
	LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_PAYERTYPE AS PAYERTYPES
	...
	LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_SERVICECODE AS SERVICES
	...
	LEFT JOIN DISC_PROD."STAGE".MATRIXCARE_SERVICE_REVENUE_CATEGORY_MAPPING AS SERVICE_MAPPING
		ON SERVICE_MAPPING.SVCC_ID = PAYER_SERVICES.SERVICECODEID 
	WHERE (PAYERS.ETL_LAST_UPDATED_DATE >= '1900-01-01'
		OR PAYERTYPES.ETL_LAST_UPDATED_DATE >= '1900-01-01'
		OR PAYER_SERVICES.ETL_LAST_UPDATED_DATE >= '1900-01-01'
		OR SERVICES.ETL_LAST_UPDATED_DATE >= '1900-01-01'
		OR CONTRACT_MAPPING.ETL_LAST_UPDATED_DATE >= '1900-01-01'
		OR SERVICE_MAPPING.ETL_LAST_UPDATED_DATE >= '1900-01-01')
		AND PAYERS.ETL_DELETED_FLAG = TRUE
		AND CAST(PAYERS.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_PROD.MATRIXCARE.HIST_STVHC_T_PAYERS)
)
