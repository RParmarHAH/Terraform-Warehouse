create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_GPSYNCDATA_DIM_EMPLOYEE(
	EMPLOYEE_KEY
) as
...
LEFT OUTER JOIN DW_PROD.HAH.DIM_State AS geo ON UPPER( TRIM( emp.State)) = geo.State_ISO_Code OR UPPER( TRIM( emp.State)) = UPPER( geo.State_Name)
LEFT OUTER JOIN DW_PROD.HAH.DIM_Branch AS brnch ON ( LPAD( CAST( brnch.Office_Number AS VARCHAR), 3, '0') = LPAD( TRIM( emp.Office_NO), 3, '0') AND brnch.Source_System_ID = 3 AND emp.Office_No NOT IN (908, 950, 951, 17, 52, 53, 60, -1)) 
...
DISC_PROD.GPSyncData.HIST_GPAllEmployeesBase);