create or replace view DW_PROD.REPORT.VW_DAYFALL_CALC_WIP(
	STATE,
	TYPE,
	REVENUE_CATEGORY,
	MONTH,
...
FROM DW_PROD.HAH.FACT_BRANCH_PAYROLL_PERIODS o       
...
select SERVICE_DATE,NULL AS TYPE,CONTRACT.REVENUE_CATEGORY,
  ...
  '0010d523d5db5f536745453dc9f21c60' ELSE VISIT.BRANCH_KEY END AS BRANCH_KEY,
    ...
    (
      select distinct contract_KEY from DW_PROD.hah.dim_contract where upper(contract_name) like '%DCFS%' or 
...
select SERVICE_DATE,PCS.SCHEDULE_TYPE AS TYPE,PCS.REVENUE_CATEGORY,
  ...
  '0010d523d5db5f536745453dc9f21c60' ELSE VISIT.BRANCH_KEY END AS BRANCH_KEY,
    ...
    (SELECT PARTNER_CONTRACT_SERVICE_KEY,PRNTER_CNTCT.SCHEDULE_TYPE,
     ...
     FROM DW_PROD.HAH.FACT_PARTNER_CONTRACT_SERVICE PRNTER_CNTCT 
     LEFT JOIN DW_PROD.HAH.DIM_SERVICES  DIM_SERV ON PRNTER_CNTCT.SERVICE_KEY = DIM_SERV.SERVICE_KEY)PCS
     ON PCS.PARTNER_CONTRACT_SERVICE_KEY = VISIT.PARTNER_CONTRACT_SERVICE_KEY 
    	AND BILLABLE_FLAG = TRUE and NVL(MILEAGE_FLAG,FALSE) = FALSE
    	AND PCS.PARTNER_CONTRACT_SERVICE_KEY NOT IN 
    (
      SELECT DISTINCT  PARTNER_CONTRACT_SERVICE_KEY  FROM  DW_PROD.HAH.FACT_PARTNER_CONTRACT_SERVICE
