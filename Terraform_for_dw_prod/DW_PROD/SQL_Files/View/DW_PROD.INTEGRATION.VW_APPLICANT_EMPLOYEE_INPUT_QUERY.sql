create or replace view DW_PROD.INTEGRATION.VW_APPLICANT_EMPLOYEE_INPUT_QUERY(
	ID,
	EMPLOYEE_KEY,
	EMPLOYEE_ID,
	SYSTEM_CODE,
...
EMPLOYEE_LAST_WORKED_DATE,EMPLOYEE_HIRE_DATE,SOURCE_SYSTEM FROM 
	...
	FROM DW_PROD.INTEGRATION.VW_JAZZHR_PARADOX_MASTER_LIST VW 
	LEFT JOIN DW_PROD.INTEGRATION.VW_WORKDAY_PARADOX_APPLICANT_MAPPING PWM
...
FROM DW_PROD.HAH.FACT_APPLICANT_STATUS FAS
LEFT JOIN  DW_PROD.HAH.DIM_APPLICANT APP
    ON APP.APPLICANT_KEY  = FAS.APPLICANT_KEY    
LEFT JOIN  DW_PROD.HAH.DIM_APPLICANT APP_WDAY
    ON FAS.CANDIDATE_ID = APP_WDAY.LINKED_ID 
LEFT JOIN DW_PROD.HAH.DIM_EMPLOYEE DE
