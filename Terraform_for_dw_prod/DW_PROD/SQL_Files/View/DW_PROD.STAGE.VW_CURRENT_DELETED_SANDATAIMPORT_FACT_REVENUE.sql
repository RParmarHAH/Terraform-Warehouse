create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_SANDATAIMPORT_FACT_REVENUE(
	REVENUE_KEY
) as
WITH VISITS AS (
	...
	FROM DISC_PROD.SANDATAIMPORT.HIST_SANDATA_INVOICEHEADER H
    ...
    WHERE H.ETL_DELETED_FLAG = TRUE 
        AND CAST(H.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_PROD.SANDATAIMPORT.HIST_SANDATA_INVOICEHEADER)
), CHANGED_DATA AS (
