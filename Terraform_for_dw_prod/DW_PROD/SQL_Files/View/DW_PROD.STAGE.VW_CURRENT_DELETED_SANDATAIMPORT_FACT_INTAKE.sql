create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_SANDATAIMPORT_FACT_INTAKE(
	INTAKE_KEY
) as
WITH 
CLIENT AS
...
FROM (
	...
	FROM DISC_PROD.SANDATAIMPORT.HIST_SANDATA_AUTHORIZATIONS A
    ...
    AND CAST(A.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_PROD.SANDATAIMPORT.HIST_SANDATA_AUTHORIZATIONS)
) LIST
