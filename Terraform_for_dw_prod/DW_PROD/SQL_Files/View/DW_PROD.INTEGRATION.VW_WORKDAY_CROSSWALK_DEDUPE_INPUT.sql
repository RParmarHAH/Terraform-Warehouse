create or replace view DW_PROD.INTEGRATION.VW_WORKDAY_CROSSWALK_DEDUPE_INPUT(
	ID1,
	ID2,
	FIRST_NAME_MATCH,
	LAST_NAME_MATCH,
...
(select COALESCE(WORKDAY_INTERNAL_ID,RANDOM()) fROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK  where WORKDAY_INTERNAL_ID is not null AND INACTIVE = FALSE and DUP_FLAG = FALSE) -- DATASET 1 - WITH ALL NEW RECORDS FROM THE WORKDAY
 )
,CROSS_WALK_DATA AS (
