create or replace view DW_PROD.STAGE.MI_DF_VISIT_CLAIMS(
	SERVICE_DATE,
	PAYROLL_DATE,
	BRANCH_NAME,
	CLIENT_NUMBER,
...
SELECT DISTINCT SCHCP_SCHEDULEID
	...
	FROM DISC_DEV.MATRIXCARE.STVHC_T_SCHEDULEPAYERS 
	)
, CLIENT_PAYERS AS
	(
		...
		FROM DISC_DEV.MATRIXCARE.STVHC_T_CLIENTPAYERS 
	)
, PAYER_BRANCH AS
		 (
			...
			FROM DISC_DEV.MATRIXCARE.STVHC_T_PAYERBRANCHES 
		) 	
,ADM_AUTH AS 
		( 
		 ...
		 FROM DISC_DEV.MATRIXCARE.STVHC_T_ADMISSIONS ADM 
		 LEFT JOIN DISC_DEV.MATRIXCARE.STVHC_T_AUTHORIZATION AUTH ON ADM.ADM_ID =AUTH_ADMISSIONID
		)	
, INVOICE_PAYERS AS
	(	
		...
		FROM DISC_DEV.MATRIXCARE.STVHC_T_INVOICEHEADER INH 
	 	JOIN  DISC_DEV.MATRIXCARE.STVHC_T_INVOICEDETAILS IND ON INH.INVH_ID = IND.INVD_INVHID 
	)
, BILL_RATES AS
	(
		SELECT * FROM 
		...
		FROM DISC_DEV.MATRIXCARE.STVHC_T_RATEBILLSHIFTS 
		) WHERE ROWRBS_ID =1
	)
SELECT DISTINCT
...
FROM DISC_DEV.MATRIXCARE.STVHC_T_SCHEDULES AS SCH
LEFT JOIN DISC_DEV.MATRIXCARE.STVHC_T_SCHEDULESTATUS AS SCHSTATUS ON SCH.SCH_SCHEDULESTATUSID = SCHSTATUS.SCHST_ID 
LEFT JOIN DISC_DEV.MATRIXCARE.STVHC_T_CAREGIVER AS CARE ON CARE.CAR_ID = SCH.SCH_CAREGIVERID
LEFT JOIN DISC_DEV.MATRIXCARE.MATRIXCARE_MASTER_EMPLOYEE_MAPPING_ALL AS EMPLOYEE_MAPPING ON EMPLOYEE_MAPPING.EMPLOYEE_NUMBER = SCH.SCH_CAREGIVERID 
LEFT JOIN DISC_DEV.MATRIXCARE.MATRIXCARE_MASTER_CLIENT_MAPPING_ALL AS CLIENT_MAPPING ON CLIENT_MAPPING.CLIENT_NUMBER = SCH.SCH_CLIENTID 
LEFT JOIN BILL_RATES BR ON BR.SCHEDULEID = SCH.SCH_ID
LEFT JOIN DISC_DEV.MATRIXCARE.STVHC_T_BRANCHES AS BRANCH ON BRANCH.BR_ID = SCH.SCH_BRANCHID 
LEFT JOIN DISC_DEV.MATRIXCARE.MATRIXCARE_MASTER_BRANCH_MAPPING BRANCH_MAPPING ON BRANCH_MAPPING.OFFICE_CODE = SCH.SCH_BRANCHID::STRING AND BRANCH_MAPPING.SOURCE_SYSTEM_ID = 7 AND BRANCH_MAPPING.SYSTEM_CODE = 'MATRIXCARE' 
...
LEFT JOIN DW_PROD.HAH.DIM_CONTRACT CN ON CN.CONTRACT_KEY = VIS.CONTRACT_KEY 
LEFT OUTER JOIN DW_PROD.HAH.DIM_Services AS srv ON
	CN.Service_Key = srv.Service_Key
LEFT OUTER JOIN DW_PROD.HAH.DIM_Client AS cl ON
vis.Client_Key = cl.Client_Key
LEFT JOIN DW_PROD.INTEGRATION.DATAFLEXSYNCDATA_MATRIXCARE_EMPLOYEE_MAPPING AS EMPLOYEE_MAPPING 
