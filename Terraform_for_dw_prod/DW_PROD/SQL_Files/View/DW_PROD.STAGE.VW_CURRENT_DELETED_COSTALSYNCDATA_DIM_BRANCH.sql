create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_COSTALSYNCDATA_DIM_BRANCH(
	BRANCH_KEY
) as 
...
--		CAST(NULL AS VARCHAR(10)) AS SECONDARY_BRANCH_MANAGER_DERIVED_LAST_NAME,
	...
	WHERE OFFICES.STATE IN ('GA', 'SC')
		...
		AND OFFICES.ETL_DELETED_FLAG = TRUE
		AND CAST(OFFICES.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_PROD.HAHUSERS.LOGIN_OFFICE)
),
...
SELECT --MD5( TRIM( loc.DB) || '-' || '1' || '-' || TRIM( loc.Location_Code) || '-' || TRIM( loc.Location_Name) || '-' || 'CostalSyncData') AS Parent_Branch_Key,
	MD5( TRIM( loc.DB) || '-' || TRIM( loc.Location_Code) || '-' || 'COSTALSYNCDATA') AS Parent_Branch_Key,
       ...
       CASE WHEN TRIM( loc.Location_Code) = Parent_Office_Code THEN TRUE
            ELSE FALSE
       END AS Parent_Flag
FROM DISC_PROD.CostalSyncData.CV_Locations AS loc
LEFT OUTER JOIN DISC_PROD.CostalSyncData.CV_LocationOfficeBreakdown AS lhier ON TRIM( loc.DB) = TRIM( lhier.DB) AND TRIM( loc.Location_Code) = TRIM( lhier.LocationCode)
...
OFFICE_LIST AS (
	...
	FROM DISC_PROD.CostalSyncData.HIST_CV_Locations 
    WHERE ETL_DELETED_FLAG = TRUE
        AND CAST(ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_PROD.CostalSyncData.HIST_CV_Locations)
--	UNION ALL
