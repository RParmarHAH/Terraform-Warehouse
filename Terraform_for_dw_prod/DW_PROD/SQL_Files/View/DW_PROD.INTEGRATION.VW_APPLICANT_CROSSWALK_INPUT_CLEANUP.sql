create or replace view DW_PROD.INTEGRATION.VW_APPLICANT_CROSSWALK_INPUT_CLEANUP(
	ID1,
	ID2,
	LINKED_ID_MATCH,
	FIRST_NAME_MATCH,
...
where APPLICANT_ID not in (select COALESCE(WORKDAY_INTERNAL_ID,RANDOM()) from DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_CLONE_FOR_CLEANUP WHERE WORKDAY_INTERNAL_ID IS NOT NULL)
  ) 
,CROSS_WALK_DATA AS (
