create or replace view DW_PROD.INTEGRATION.VW_DIM_CLIENT_AL(
	ID,
	CLIENT_NUMBER,
	SYSTEM_CODE,
	SOURCE_SYSTEM_ID,
...
WHEN split_part(LINKED_ID::varchar,'-',2) = 'IL' THEN 'IL' END          as LINKED_ID_source,   split_part(LINKED_ID::varchar,'-',3) as LINKED_ID_match,      * from DW_PROD.HAH.DIM_CLIENT       WHERE (SOURCE_SYSTEM_ID =9 AND SYSTEM_CODE ='ALABAMA (3554)')    
union select system_code as LINKED_ID_source, CLIENT_NUMBER::varchar as LINKED_ID_match,* from DW_PROD.HAH.DIM_CLIENT WHERE (SOURCE_SYSTEM_ID =3 AND SYSTEM_CODE ='AL'));