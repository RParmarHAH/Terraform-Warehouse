create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_SANDATAIMPORT_DIM_BRANCH(
	BRANCH_KEY
) as
WITH RISKCONNECT_HIERARCHY AS (
	...
	FROM (
		...
		FROM DISC_PROD.SANDATAIMPORT.HIST_SANDATA_LOCATIONS AS SANDATA_LOCATIONS 
		LEFT JOIN (
			SELECT CASE DBNAME 
					WHEN 'IL' THEN '8380' 
					WHEN 'IN' THEN '38004'
		...
		WHERE SANDATA_LOCATIONS.AGENCYID = '8485'
			...
			AND CAST(SANDATA_LOCATIONS.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_PROD.SANDATAIMPORT.HIST_SANDATA_LOCATIONS)
	) AS LIST
