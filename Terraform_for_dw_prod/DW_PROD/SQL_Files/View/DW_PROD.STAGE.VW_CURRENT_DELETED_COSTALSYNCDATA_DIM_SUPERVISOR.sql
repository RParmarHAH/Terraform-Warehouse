create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_COSTALSYNCDATA_DIM_SUPERVISOR(
	SUPERVISOR_KEY
) as
WITH SC_CLIENT_SUPERVISORS AS (
	...
	FROM DISC_PROD.COSTALSYNCDATA.HIST_CV_DEPARTMENTS AS DEPARTMENTS
	...
	WHERE (DEPARTMENTS.ETL_LAST_UPDATED_DATE >= '1900-01-01' OR SC_CLIENT_SUPERVISORS.ETL_LAST_UPDATED_DATE >= '1900-01-01')
		...
		AND CAST(DEPARTMENTS.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_PROD.COSTALSYNCDATA.HIST_CV_DEPARTMENTS)
)
SELECT
