create or replace view DW_PROD.REPORT.VW_HOURS_AND_CLIENT_CENSUS_MONTHLY_WIP_NEW(
	PERIOD_BEGIN_DATE,
	PERIOD_NAME,
	TYPE,
	STATE,
...
( SELECT PERIOD_BEGIN_DATE,PERIOD_NAME,TYPE,STATE,CLIENT_KEY,BRANCH_KEY,	CONTRACT_KEY,PARTNER_CONTRACT_SERVICE_KEY,SUPERVISOR_KEY,
	CLIENT_NUMBER,PARTNER_CODE,CONTRACT_CODE,SERVICE_CODE,PARTNER_NAME,CONTRACT_NAME,SERVICE_NAME,OFFICE_NUMBER,OFFICE_NAME,
	SUPERVISOR_CODE,ABSOLUTE_FIRST_SERVICE_DATE,ABSOLUTE_LAST_SERVICE_DATE,
	ORIGINAL_SOURCE_SYSTEM_ID,SOURCE_SYSTEM_ID,AUTH_START_DATE,AUTH_END_DATE,HOURS_SCHEDULED,HOURS_SERVED_ALL,
	HOURS_CANCELLED,HOURS_IN_REVIEW,HOURS_MISSED,HOURS_RESCHEDULED,FUTURE_HOURS,FUTURE_HOURS_NO_CAREGIVER,FUTURE_CANCELLED_HOURS,
	FUTURE_HOLD_HOURS,VISITS_SCHEDULED,VISITS_COMPLETED,VISITS_CANCELLED,VISITS_IN_REVIEW,VISITS_MISSED,VISITS_RESCHEDULED,FUTURE_VISITS,
	FUTURE_VISITS_NO_CAREGIVER,FUTURE_CANCELLED_VISITS,
	FUTURE_HOLD_VISITS,NON_STANDARD_UNITS,NUMBER_OF_DAYS,
	VISITS_CLEAN_SHIFTS, HOURS_UNITS_SERVED_VALID_AUTHS,HOURS_UNITS_SERVED,HOURS_BILLED_WHERE_AUTH_EXISTS,AUTH_AMOUNT_VALID,HOURS_BILLED_ALL,
	UNITS_AUTH_NON_ADJUSTED,REVENUE_CATEGORY,CLIENT_ACQUIRED_FLAG,CLIENT_ACQUISITION_NAME,CLIENT_SERVED_FLAG,
	UNIT_OVER_OR_UNDER_SERVED,
	VALID_AUTH,
	POTENTIAL_UNIT,
	POTENTIAL_UNIT_SERVED_WHERE_AUTH_EXISTS,
...
(WITH DATES AS 
	...
	--			NULL AS SYSTEM_CODE,
				...
				COALESCE(CONTRACT.CONTRACT_NAME,DC.CONTRACT_NAME) AS CONTRACT_NAME,
				--CC.INCLUDE_FOR_CLIENTS,
				--CONTRACT.INCLUDE_FOR_HOURS,
				CONTRACT.REVENUE_CATEGORY,
				NVL(CASE WHEN CONTRACT.INCLUDE_FOR_HOURS = 1 THEN DATA.HOURS_SERVED END, 0) AS HOURS_UNITS_SERVED,
				...
				COALESCE(CASE WHEN CONTRACT.INCLUDE_FOR_OPS_PERF_HOURS=1 THEN DATA.HOURS_AUTHORIZED_NON_ADJUSTED END,0) AS UNITS_AUTH_NON_ADJUSTED,
				...
				NVL(CASE WHEN CONTRACT.INCLUDE_FOR_HOURS = 1 AND NOT (NVL(DATA.HOURS_AUTHORIZED, 0) = 0 OR DATA.HOURS_AUTHORIZED > 900) THEN DATA.HOURS_SERVED END, 0) HOURS_UNITS_SERVED_VALID_AUTHS,
				...
				NVL(CASE WHEN CONTRACT.INCLUDE_FOR_HOURS = 1 AND NOT (NVL(DATA.HOURS_AUTHORIZED, 0) = 0 OR DATA.HOURS_AUTHORIZED > 900) THEN DATA.HOURS_AUTHORIZED END, 0) AUTH_AMOUNT_VALID,
		   ...
		   CASE WHEN COALESCE(CCSI_BRANCH.CLIENT_KEY,CCSI_ACQUIRED_CLIENT.CLIENT_KEY) IS NOT NULL THEN 'CCSI' 
				...
				-- NULL AS CONTRACT_NAME,
				 ...
				 HOURS_SERVED_ALL - UNITS_AUTH_NON_ADJUSTED AS UNIT_OVER_OR_UNDER_SERVED,
				CASE WHEN AUTH_AMOUNT_VALID >= 900 THEN 'INVALID_AUTH'
					WHEN AUTH_AMOUNT_VALID <> 0 THEN 'VALID AUTH'
				--WHEN (HOURS_SERVED_ALL=0 AND UNITS_AUTH_NON_ADJUSTED=0 AND CLIENT_SERVED_FLAG = TRUE) THEN 'NA/VISIT BASED'
				WHEN AUTH_AMOUNT_VALID=0 AND (UNITS_AUTH_NON_ADJUSTED < 900 AND UNITS_AUTH_NON_ADJUSTED >0) THEN 'ON HOLD'
				ELSE 'INVALID_AUTH' END AS VALID_AUTH,
				(HOURS_IN_REVIEW + HOURS_SERVED_ALL+FUTURE_HOURS) AS POTENTIAL_UNIT,
				IFF(VALID_AUTH = 'VALID AUTH' ,POTENTIAL_UNIT, 0) AS POTENTIAL_UNIT_SERVED_WHERE_AUTH_EXISTS,
				POTENTIAL_UNIT_SERVED_WHERE_AUTH_EXISTS - UNITS_AUTH_NON_ADJUSTED AS POTENTIAL_UNIT_OVER_OR_UNDER_SERVED,
	...
	-- ELSE FALSE END AS ACQUISITION_AS_OF_DATE
			...
			LEFT JOIN HAH.DIM_CONTRACT DC ON DC.CONTRACT_KEY = DATA.CONTRACT_KEY
			JOIN DATES DATES 
	...
	SELECT DISTINCT PERIOD_BEGIN_DATE,PERIOD_NAME,TYPE,STATE,	CLIENT_KEY,	BRANCH_KEY,	CONTRACT_KEY,NULL as	PARTNER_CONTRACT_SERVICE_KEY,	SUPERVISOR_KEY,
	CLIENT_NUMBER,PARTNER_CODE,CONTRACT_CODE,SERVICE_CODE,PARTNER_NAME,CONTRACT_NAME,SERVICE_NAME,OFFICE_NUMBER,OFFICE_NAME,
	SUPERVISOR_CODE,ABSOLUTE_FIRST_SERVICE_DATE,ABSOLUTE_LAST_SERVICE_DATE,
	--CLIENT_KEY_DATA,
	ORIGINAL_SOURCE_SYSTEM_ID,SOURCE_SYSTEM_ID,AUTH_START_DATE,AUTH_END_DATE,HOURS_SCHEDULED,HOURS_SERVED_ALL,
	...
	-- )
		,UTILIZATION_DATA_2 AS (
			SELECT DATA.PERIOD_BEGIN_DATE,
				DATA.PERIOD_BEGIN_DATE AS REPORT_DATE,
				DATES.PERIOD_NAME,
				IFF(DATA.SOURCE_SYSTEM_ID=3 AND S.REVENUE_SUBCATEGORY_CODE IN ('DDM','NSH'),'non standard',DATA.TYPE) AS TYPE,
				DATA.BRANCH_KEY,
				DATA.STATE,  
				DATA.OFFICE_NUMBER,
				DATA.OFFICE_NAME,
	...
	--			PCS.SYSTEM_CODE ,
				...
				S.REVENUE_CATEGORY AS REVENUE_CATEGORY,
				DATA.CLIENT_KEY,
				...
				IFF(DATA.SOURCE_SYSTEM_ID=3 AND S.REVENUE_SUBCATEGORY_CODE IN ('DDM','NSH') ,0,IFF(DATA.TYPE='Visit',COALESCE(DATA.VISITS_ALL,0),DATA.HOURS_SERVED)) AS HOURS_UNITS_SERVED,
			--	DATA.HOURS_AUTHORIZED AS HOURS_AUTHORIZED_ALL,
				DATA.OVER_AUTHED_HOURS AS OVER_AUTHED_HOURS,
				COALESCE(IFF(DATA.TYPE='Visit',ROUND(DATA.HOURS_AUTHORIZED_NON_ADJUSTED),DATA.HOURS_AUTHORIZED_NON_ADJUSTED),0) AS UNITS_AUTH_NON_ADJUSTED,
				...
				THEN IFF(DATA.SOURCE_SYSTEM_ID=3 AND S.REVENUE_SUBCATEGORY_CODE IN ('DDM','NSH') ,0, IFF(DATA.TYPE='Visit',COALESCE(DATA.VISITS_ALL,0),DATA.HOURS_SERVED))END, 0) HOURS_UNITS_SERVED_VALID_AUTHS,
				...
				NVL(CASE WHEN NOT (NVL(DATA.HOURS_AUTHORIZED, 0) = 0 OR DATA.HOURS_AUTHORIZED > 900) THEN IFF(DATA.TYPE='Visit',ROUND(DATA.HOURS_AUTHORIZED),DATA.HOURS_AUTHORIZED) END, 0) AUTH_AMOUNT_VALID,
			--	NVL(CASE WHEN NOT (NVL(DATA.HOURS_AUTHORIZED_NON_ADJUSTED, 0) = 0 OR DATA.HOURS_AUTHORIZED_NON_ADJUSTED > 900) THEN DATA.HOURS_AUTHORIZED_NON_ADJUSTED END, 0) HOURS_AUTHORIZED_NON_ADJUSTED_WHERE_AUTH_EXISTS,
				DATA.ETL_LAST_UPDATED_DATE,
				DATA.VISITS_CLEAN_SHIFTS as VISITS_CLEAN_SHIFTS,
		   ...
		   CASE WHEN COALESCE(CCSI_BRANCH.CLIENT_KEY,CCSI_ACQUIRED_CLIENT.CLIENT_KEY) IS NOT NULL THEN 'CCSI' 
				...
				DATA.VISITS_RESCHEDULED,
				DATA.FUTURE_VISITS,
				DATA.FUTURE_VISITS_NO_CAREGIVER,
				DATA.FUTURE_CANCELLED_VISITS,
				DATA.FUTURE_HOLD_VISITS,
				DATA.CLIENT_SERVED_FLAG,
				 DATA.CONTRACT_NAME,
				 ...
				 DATA.AUTH_END_DATE,
				 DATA.NUMBER_OF_DAYS,
				iff(DATA.TYPE='Visit',DATA.VISITS_ALL,HOURS_SERVED_ALL) - UNITS_AUTH_NON_ADJUSTED AS UNIT_OVER_OR_UNDER_SERVED,
				CASE WHEN AUTH_AMOUNT_VALID >= 900 THEN 'INVALID_AUTH'
				WHEN AUTH_AMOUNT_VALID <> 0 THEN 'VALID AUTH'
--				WHEN (HOURS_SERVED_ALL=0 AND UNITS_AUTH_NON_ADJUSTED=0 AND CLIENT_SERVED_FLAG = TRUE) THEN 'NA/VISIT BASED'
--				WHEN DATA.TYPE='Visit' THEN 'VISIT BASED'							 
				WHEN AUTH_AMOUNT_VALID=0 AND (UNITS_AUTH_NON_ADJUSTED < 900 AND UNITS_AUTH_NON_ADJUSTED >0) THEN 'ON HOLD'
				ELSE 'INVALID_AUTH' END AS VALID_AUTH,
				IFF(DATA.TYPE='Visit',(DATA.VISITS_IN_REVIEW+DATA.VISITS_COMPLETED+DATA.FUTURE_VISITS),(HOURS_IN_REVIEW + HOURS_SERVED_ALL+FUTURE_HOURS)) AS POTENTIAL_UNIT,
	...
	-- WHEN CLIENT_ACQUISITION_NAME = 'PROSPERCARE'  AND PERIOD_BEGIN_DATE >= DATE(2022-08-01)THEN TRUE
	-- WHEN CLIENT_ACQUISITION_NAME = 'ALL GENERATIONS'  AND PERIOD_BEGIN_DATE >= DATE(2022-04-01) THEN TRUE
	-- WHEN CLIENT_ACQUISITION_NAME = 'ROMEO'  AND PERIOD_BEGIN_DATE >= DATE(1900-01-01) THEN TRUE
	-- WHEN COALESCE(CLIENT_ACQUISITION_NAME ,'HAH') ='HAH' THEN TRUE
	-- ELSE FALSE END AS ACQUISITION_AS_OF_DATE
			FROM REPORT.UTILIZATION_2_0_WIP_NEW AS DATA
	...
	LEFT OUTER JOIN HAH.FACT_CLIENT_ACQUIRED_2_0 RECCO_CLIENT
      ...
      RECCO_CLIENT.AQUISITION_NAME ='RECCO'
	WHERE PERIOD_BEGIN_DATE <= DATEADD( 'MONTH', 2, CURRENT_DATE)
	AND IFF(DATA.CLIENT_SERVED_FLAG = TRUE, coalesce(DATA.HOURS_SERVED, 0), 1) > 0 --OR INVALID_FLAG=TRUE)
	--AND TYPE='Hourly' 
	AND REVENUE_CATEGORY NOT IN ('CLS','NA','CC') AND PCS.BILLABLE_FLAG = TRUE
	--AND DATA.SOURCE_SYSTEM_ID IN (17,7,4)-- AND DATA.STATE='DE'
		)
		 ,
	FINAL_UTILIZATION_2 AS (
		SELECT DISTINCT PERIOD_BEGIN_DATE,PERIOD_NAME,TYPE,STATE,	CLIENT_KEY,	BRANCH_KEY,	CONTRACT_KEY,	PARTNER_CONTRACT_SERVICE_KEY,	SUPERVISOR_KEY,
	CLIENT_NUMBER,PARTNER_CODE,CONTRACT_CODE,SERVICE_CODE,PARTNER_NAME,CONTRACT_NAME,SERVICE_NAME,OFFICE_NUMBER,OFFICE_NAME,
	SUPERVISOR_CODE,ABSOLUTE_FIRST_SERVICE_DATE,ABSOLUTE_LAST_SERVICE_DATE,
	--CLIENT_KEY_DATA,
	ORIGINAL_SOURCE_SYSTEM_ID,SOURCE_SYSTEM_ID,AUTH_START_DATE,AUTH_END_DATE,HOURS_SCHEDULED,HOURS_SERVED_ALL,
	HOURS_CANCELLED,HOURS_IN_REVIEW,HOURS_MISSED,HOURS_RESCHEDULED,FUTURE_HOURS,FUTURE_HOURS_NO_CAREGIVER,FUTURE_CANCELLED_HOURS,
	FUTURE_HOLD_HOURS,VISITS_SCHEDULED,VISITS_COMPLETED,VISITS_CANCELLED,VISITS_IN_REVIEW,VISITS_MISSED,VISITS_RESCHEDULED,
	FUTURE_VISITS,FUTURE_VISITS_NO_CAREGIVER,FUTURE_CANCELLED_VISITS,
	FUTURE_HOLD_VISITS,NON_STANDARD_UNITS,NUMBER_OF_DAYS,
	VISITS_CLEAN_SHIFTS, HOURS_UNITS_SERVED_VALID_AUTHS,HOURS_UNITS_SERVED,HOURS_BILLED_WHERE_AUTH_EXISTS,AUTH_AMOUNT_VALID,HOURS_BILLED_ALL,
	UNITS_AUTH_NON_ADJUSTED,REVENUE_CATEGORY,CLIENT_ACQUIRED_FLAG,CLIENT_ACQUISITION_NAME,CLIENT_SERVED_FLAG,
	--NPS,
...
SELECT DISTINCT PERIOD_BEGIN_DATE,PERIOD_NAME,TYPE,STATE,	CLIENT_KEY,	BRANCH_KEY,	CONTRACT_KEY,	PARTNER_CONTRACT_SERVICE_KEY,	SUPERVISOR_KEY,
	CLIENT_NUMBER,PARTNER_CODE,CONTRACT_CODE,SERVICE_CODE,PARTNER_NAME,CONTRACT_NAME,SERVICE_NAME,OFFICE_NUMBER,OFFICE_NAME,
	SUPERVISOR_CODE,SERVICEDATES.ABSOLUTE_FIRST_SERVICE_DATE,SERVICEDATES.ABSOLUTE_LAST_SERVICE_DATE,
	--CLIENT_KEY_DATA,
	ORIGINAL_SOURCE_SYSTEM_ID,SOURCE_SYSTEM_ID,AUTH_START_DATE,AUTH_END_DATE,HOURS_SCHEDULED,HOURS_SERVED_ALL,
	HOURS_CANCELLED,HOURS_IN_REVIEW,HOURS_MISSED,HOURS_RESCHEDULED,FUTURE_HOURS,FUTURE_HOURS_NO_CAREGIVER,FUTURE_CANCELLED_HOURS,
	FUTURE_HOLD_HOURS,VISITS_SCHEDULED,VISITS_COMPLETED,VISITS_CANCELLED,VISITS_IN_REVIEW,VISITS_MISSED,VISITS_RESCHEDULED,FUTURE_VISITS,
