create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_COSTALSYNCDATA_DIM_EMPLOYEE(
	EMPLOYEE_KEY
) as
...
LEFT OUTER JOIN HAH.DIM_State AS st ON st.State_ISO_Code = UPPER( SUBSTRING( gpe.State, 1, 2))
LEFT OUTER JOIN HAH.DIM_Zip_Code AS zip ON UPPER( zip.Zip_Code) = UPPER( SUBSTRING( gpe.Zip, 1, 5))
LEFT OUTER JOIN DISC_PROD.STAGE.COASTAL_OFFICE_MAPPING_PAYROLL AS BRANCH_MAPPING ON TRIM(BRANCH_MAPPING.DB) = TRIM(gpe.DB) AND TRIM(BRANCH_MAPPING.DEPARTMENT) = TRIM(gpe.DEPARTMENT)
