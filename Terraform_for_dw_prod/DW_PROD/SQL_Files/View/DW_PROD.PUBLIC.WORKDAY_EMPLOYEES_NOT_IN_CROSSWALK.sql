create or replace view DW_PROD.PUBLIC.WORKDAY_EMPLOYEES_NOT_IN_CROSSWALK(
	EMPLOYEE_ENTERPRISE_ID,
	CROSSWALK_FIRST_NAME,
	CROSSWALK_LAST_NAME,
	WORKDAY_INTERNAL_ID,
	POTENIAL_AMS_KEY,
	POTENTIAL_AMS_ENTERPRISE_ID,
	POTENTIAL_AMS_WORKDAY_INTERNAL_ID,
	EMPLOYEE_HIRE_DATE,
	APPLICANT_HIRED_DATETIME,
	PARADOX_STATUS,
	REMARK
) as 
...
WITH DISC AS 
	...
	WITH NO_POS AS 
	    ...
	    FROM DISC_PROD.WORKDAY.HIST_WORKER_POSITION_DATA NP
	...
	,NO_ORG AS 
		( -- FOR employees who haven'T been assigned a POSITION AS OF now (last known position is fetched)
	    SELECT EMPLOYEE_ID, ORGANIZATION_NAME
	    FROM DISC_PROD.WORKDAY.HIST_WORKER_ORGANIZATION_DATA
	    WHERE ORGANIZATION_TYPE = 'Location_Hierarchy'
	        AND ORGANIZATION_SUBTYPE = 'Geographic_Division'
	...
	INNER JOIN DISC_PROD.WORKDAY.WORKER_ADDITIONAL_INFORMATION AD
	    ON AD.EMPLOYEE_ID = S.EMPLOYEE_ID
	LEFT JOIN DISC_PROD.WORKDAY.WORKER_POSITION_DATA P
	    ON P.EMPLOYEE_ID = S.EMPLOYEE_ID
	LEFT JOIN DISC_PROD.WORKDAY.WORKER_NAME_DATA NM
	    ON NM.EMPLOYEE_ID = S.EMPLOYEE_ID AND Nm.NAME_TYPE = 'Legal'
	LEFT JOIN DISC_PROD.WORKDAY.WORKER_ADDRESS_DATA A
	...
	LEFT JOIN DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK CROS
	    ON CROS.WORKDAY_INTERNAL_ID = S.EMPLOYEE_ID
	LEFT JOIN DISC_PROD.WORKDAY.WORKER_EXCLUDE_LIST EX
...
,CTE AS 
    ...
    FROM DISC W
    LEFT JOIN DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK C 
    	ON C.WORKDAY_INTERNAL_ID = W.EMPLOYEE_ID 
    	AND c.DUP_FLAG = FALSE AND C.INACTIVE = FALSE
    LEFT JOIN DISC_PROD.WORKDAY.WORKER_IDENTIFICATION_DATA I 
    	ON I.EMPLOYEE_ID = W.EMPLOYEE_ID AND I.ID_TYPE = 'USA-SSN'
    LEFT JOIN DISC_PROD.WORKDAY.WORKER_PERSONAL P 
    	ON P.EMPLOYEE_ID = W.EMPLOYEE_ID 
    LEFT JOIN DW_PROD.HAH.DIM_SOURCE_SYSTEM SS 
    	ON C.AMS_SOURCE_SYSTEM_ID = SS.SOURCE_SYSTEM_ID 
    LEFT JOIN DW_PROD.INTEGRATION.DIM_EMPLOYEE_AMS_MERGED E 
...
,cte_2 AS
	...
	FROM CTE
	LEFT JOIN DW_PROD.INTEGRATION.DIM_EMPLOYEE_AMS_MERGED DE 
...
FROM cte_2 C
LEFT JOIN DW_PROD.INTEGRATION.VW_WORKDAY_PARADOX_APPLICANT_MAPPING VW 
	ON VW.WORKDAY_ID = C.EXPECTED_WORKDAY_ID
LEFT JOIN APPLICANT FA 
	ON FA.APPLICANT_KEY = VW.PARADOX_APPLICANT_KEY
LEFT JOIN DW_PROD.HAH.DIM_APPLICANT AP 
	ON VW.PARADOX_APPLICANT_KEY = AP.APPLICANT_KEY 
LEFT JOIN DW_PROD.INTEGRATION.VW_APPLICANT_EMPLOYEE_INPUT_QUERY VW_AEM 
