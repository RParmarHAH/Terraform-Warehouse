create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_COSTALSYNCDATA_DIM_CLIENT(
	CLIENT_KEY
) as
WITH 
    main as
    (
        ...
        FROM "DISC_PROD"."COSTALSYNCDATA"."CV_PATIENTS" P 
        LEFT JOIN "DISC_PROD"."COSTALSYNCDATA"."CV_LOCATIONS" L ON TRIM(P.DB) = TRIM(L.DB) AND TRIM(P.LOCATION_CODE) = TRIM(L.LOCATION_CODE)
        LEFT JOIN HAH.DIM_STATE ST ON ST.STATE_ISO_CODE = NULLIF(TRIM(L.STATE_CODE), '')
        LEFT JOIN "DISC_PROD"."COSTALSYNCDATA"."CV_DEPARTMENTS" D ON TRIM(P.DB) = TRIM(D.DB) AND TRIM(P.DEPARTMENT__CODE) = TRIM(D.DEPARTMENT__CODE)
        LEFT JOIN "DISC_PROD"."COSTALSYNCDATA"."CV_ADMISSIONS" A ON TRIM(P.PATIENT_NUMBER) = TRIM(A.PATIENT_NUMBER) AND TRIM(P.DB)=TRIM(A.DB)
  ...
  FROM main f
  JOIN "DISC_PROD"."COSTALSYNCDATA"."HIST_CV_PATIENTS" P ON f.ClientNumber = P.PATIENT_NUMBER AND f.DB=P.DB
  ...
  WHERE f.ETL_LAST_UPDATED_DATE >= '1900-01-01' 
  	...
  	AND CAST(P.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM "DISC_PROD"."COSTALSYNCDATA"."HIST_CV_PATIENTS");