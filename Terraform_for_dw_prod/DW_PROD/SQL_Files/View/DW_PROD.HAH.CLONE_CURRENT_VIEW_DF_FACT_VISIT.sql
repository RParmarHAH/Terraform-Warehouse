create or replace view DW_PROD.HAH.CLONE_CURRENT_VIEW_DF_FACT_VISIT(
	VISIT_KEY,
	REPORT_DATE,
	BRANCH_KEY,
	CLIENT_KEY,
...
with SD_Visits as (
    ...
    JOIN DISC_PROD.SANDATAIMPORT.SANDATA_SCHEDULES sch ON f.AGENCYID = sch.AGENCYID AND f.SCHEDULEID = sch.SCHEDULEID
  ...
  code_desc as (
      ...
      from DISC_PROD.SANDATAIMPORT.SANDATA_VISITS
      where REASONCODE <> ''
      group by 1
), CLIENT_DATA AS
(
...
FROM (
	...
	FROM DISC_PROD.DataFlexSyncData.DfTimeSheetHours f
	LEFT JOIN DISC_PROD.SANDATAIMPORT.SANDATA_SCHEDULES sch ON f.DbName = sch.AGENCYID AND f.ReferenceNo = sch.SCHEDULEID
	LEFT JOIN DISC_PROD.SANDATAIMPORT.SANDATA_VISITS SV ON f.DbName = SV.AGENCYID AND f.ReferenceNo = SV.SCHEDULEID
	...
	LEFT OUTER JOIN DISC_PROD.DATAFLEXSYNCDATA.DERIVED_BILLRATE Billrate_Contract
	...
	LEFT OUTER JOIN DISC_PROD.DATAFLEXSYNCDATA.DERIVED_BILLRATE Billrate_Contract_Client
	...
	LEFT OUTER JOIN DISC_PROD.DATAFLEXSYNCDATA.DERIVED_BILLRATE Billrate_Contract_billcode
	...
	LEFT OUTER JOIN DISC_PROD.DATAFLEXSYNCDATA.DERIVED_BILLRATE Billrate_Contract_overheadrate
