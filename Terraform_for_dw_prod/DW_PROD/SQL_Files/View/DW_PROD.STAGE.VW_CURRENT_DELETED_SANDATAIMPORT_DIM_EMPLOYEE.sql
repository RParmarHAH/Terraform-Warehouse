create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_SANDATAIMPORT_DIM_EMPLOYEE(
	EMPLOYEE_KEY
) as
WITH 
-- offbrnch AS
...
-- FROM DISC_PROD.SandataImport.Sandata_Staffs AS stf
-- LEFT OUTER JOIN DISC_PROD.BI_Repository.EXTERNALIDS AS eid ON stf.HomeLocation = eid.ExtID AND eid.Typ = 'L'
...
FROM DISC_PROD.SanDataImport.SanData_Schedules AS ss
INNER JOIN DISC_PROD.SanDataImport.SanData_SchedulesStaffs AS sst ON ss.AgencyID = sst.AgencyID AND ss.ScheduleID = sst.ScheduleID AND sst.Status IN ( '05')
...
FROM DISC_PROD.SandataImport.HIST_Sandata_Staffs AS stf
...
LEFT OUTER JOIN DISC_PROD.TrustPointData.Employees AS tp ON tp.COMPANY = 'EXCEL' AND stf.AGENCYID = '8485' AND TRY_CAST(tp.EMPLOYEE_NUMBER AS INT) = TRY_CAST(stf.STAFFAGENCYID AS INTEGER) --stf.ID = tp.ID
...
AND CAST(stf.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_PROD.SandataImport.HIST_Sandata_Staffs)
UNION
SELECT
...
FROM DISC_PROD.TrustPointData.HIST_Employees AS tp
LEFT OUTER JOIN DISC_PROD.SandataImport.Sandata_Staffs AS stf ON stf.AGENCYID = '8485' AND stf.Person = 'TRUE' AND TRY_CAST(tp.EMPLOYEE_NUMBER AS INT) = TRY_CAST(stf.STAFFAGENCYID AS INTEGER)
...
WHERE tp.COMPANY = 'EXCEL' AND tp.Employee_Number NOT IN (SELECT CAST( emp.STAFFAGENCYID AS INTEGER)
                                 FROM DISC_PROD.SanDataImport.Sandata_Staffs AS emp
                                 WHERE AgencyID = 8485
                                 AND emp.Person = 'TRUE')
AND tp.ETL_DELETED_FLAG = TRUE
AND CAST(tp.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_PROD.TrustPointData.HIST_Employees);