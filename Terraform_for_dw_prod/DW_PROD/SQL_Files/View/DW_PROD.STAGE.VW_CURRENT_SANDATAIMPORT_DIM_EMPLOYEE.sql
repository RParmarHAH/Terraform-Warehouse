create or replace view DW_PROD.STAGE.VW_CURRENT_SANDATAIMPORT_DIM_EMPLOYEE(
	EMPLOYEE_KEY
) as
WITH 
-- offbrnch AS
...
AND stf.Person = 'TRUE'
AND (stf.ETL_LAST_UPDATED_DATE >= '1900-01-01' OR tp.ETL_LAST_UPDATED_DATE >= '1900-01-01')
UNION
SELECT
...
FROM DISC_PROD.TrustPointData.Employees AS tp
...
WHERE tp.COMPANY = 'EXCEL' AND tp.Employee_Number NOT IN (SELECT CAST( emp.STAFFAGENCYID AS INTEGER)
                                 ...
                                 WHERE AgencyID = 8485
                                 AND emp.Person = 'TRUE')
AND tp.ETL_LAST_UPDATED_DATE >= '1900-01-01';