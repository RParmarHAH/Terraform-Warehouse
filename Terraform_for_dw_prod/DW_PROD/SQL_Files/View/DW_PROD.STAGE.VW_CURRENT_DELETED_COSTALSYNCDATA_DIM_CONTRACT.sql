create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_COSTALSYNCDATA_DIM_CONTRACT(
	CONTRACT_KEY
) as
...
FROM DISC_PROD.COSTALSYNCDATA.HIST_CV_PLANS AS PLANS
...
WHERE PLANS.ETL_DELETED_FLAG = TRUE
	AND CAST(PLANS.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_PROD.COSTALSYNCDATA.HIST_CV_PLANS);