create or replace view DW_PROD.INTEGRATION.VW_APPLICANT_CROSSWALK_INPUT(
	ID1,
	ID2,
	LINKED_ID_MATCH,
	FIRST_NAME_MATCH,
...
from DW_PROD.stage.workday_dim_applicant 
  where APPLICANT_ID not in (select COALESCE(WORKDAY_INTERNAL_ID,RANDOM()) from DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK WHERE WORKDAY_INTERNAL_ID IS NOT NULL AND INACTIVE = FALSE and DUP_FLAG = FALSE)
  ) 
,CROSS_WALK_DATA AS (
