create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_DATAFLEXSYNCDATA_DIM_INVOICE(
	INVOICE_KEY
) as
...
FROM DISC_PROD.DATAFLEXSYNCDATA.HIST_DFINVOICES inv
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFINVOICEDETAILS det
...
WHERE inv.ETL_DELETED_FLAG = TRUE
    AND CAST(inv.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_PROD.DATAFLEXSYNCDATA.HIST_DFINVOICES)
