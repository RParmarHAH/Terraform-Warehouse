create or replace view DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TEST(
...
WITH MAPPED_EMPLOYEES AS 
	(
	    ...
	    FROM DW_PROD.INTEGRATION.FACT_AMS_PAYROLL_MAPPING APM
	    FULL OUTER JOIN DW_PROD.INTEGRATION.APPLICANT_WORKDAY_EMPLOYEE_MAPPING APP 
...
, BR_STATE_NULL_EMPLOYEES AS 
	(
	    ...
	    FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_PAYROLL_MERGE_DEDUPE E
	    INNER JOIN MAPPED_EMPLOYEES M
	        ON M.PAYROLL_EMPLOYEE_KEY = E.EMPLOYEE_KEY
	    LEFT JOIN DW_PROD.INTEGRATION.DIM_BRANCH_MERGED BR
...
, ACROSS_DUPS_EXCLUDE AS 
	(
		...
		,NOT_IN_AMS_P_MAPPING AS
			...
			WHERE ID NOT IN 
				(SELECT DISTINCT AMS_EMPLOYEE_KEY FROM DW_PROD.INTEGRATION.FACT_AMS_PAYROLL_MAPPING
				UNION 
				SELECT DISTINCT PAYROLL_EMPLOYEE_KEY FROM DW_PROD.INTEGRATION.FACT_AMS_PAYROLL_MAPPING)
			)
...
--                    AND NULL_BR.EMPLOYEE_KEY IS NULL  --to make exception flag true for null branch_state employees
        ...
        LEFT JOIN DW_PROD.INTEGRATION.DIM_EMPLOYEE_PAYROLL_MERGE_DEDUPE EMP
        ...
        LEFT OUTER JOIN DW_PROD.INTEGRATION.DIM_Branch_Merged AS BR_AMS
            ON COALESCE(EM.PRIMARY_BRANCH_KEY,FP.BRANCH_KEY) = BR_AMS.ORIGINAL_Branch_Key 
        LEFT OUTER JOIN DW_PROD.INTEGRATION.DIM_Branch_Merged AS BR_PAYROLL
        ...
        LEFT JOIN APP_DB_PROD.BROADSPIRE.VW_BROADSPIRE_WEEKLY_REPORT VI
...
--          COALESCE(BR_PAYROLL.Office_State_Code,EMP.PRIMARY_BRANCH_STATE ,BR_AMS.Office_State_Code) IN ('IN','PA','IL','MS','MO','AL','MI','GA','OH','DE','KY','TN','FL','SC')
          AND TRIM(COALESCE(BR_PAYROLL.Office_State_Code,EMP.WORK_STATE, EMP.PRIMARY_BRANCH_STATE ,BR_AMS.Office_State_Code, '')) NOT IN ('NY', 'DC','FL') AND M.AMS_EMPLOYEE_KEY NOT IN (SELECT AMS_EMPLOYEE_KEY FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING) AND M.PAYROLL_EMPLOYEE_KEY NOT IN (SELECT PAYROLL_EMPLOYEE_KEY FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
     ---------------
     UNION
     ---------------
        SELECT DISTINCT 
...
--            BR.Office_State_Code IN ('IN','PA','IL','MS','MO','AL','MI','GA','OH','DE','KY','TN','FL','SC')
            ...
            AND EMP.EMPLOYEE_KEY NOT IN (SELECT DISTINCT PAYROLL_EMPLOYEE_KEY FROM DW_PROD.INTEGRATION.FACT_AMS_PAYROLL_MAPPING ) 
--         -- Added on 22th nov to exclude across dups which were not identified at integration layer but falls in same cluster
            ...
            AND EMP.EMPLOYEE_KEY NOT IN (SELECT PAYROLL_EMPLOYEE_KEY FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
	-------------------
    UNION
    -------------------
        SELECT DISTINCT 
        ...
        FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_AMS_MERGED EM
        INNER JOIN DW_PROD.HAH.DIM_SOURCE_SYSTEM SS
        ...
        LEFT JOIN DW_PROD.INTEGRATION.APPLICANT_WORKDAY_EMPLOYEE_MAPPING APP 
...
--        WHERE BR.Office_State_Code IN ('IN','PA','IL','MS','MO','AL','MI','GA','OH','DE','KY','TN','FL','SC') 
        ...
        WHERE EM.EMPLOYEE_KEY NOT IN (SELECT DISTINCT AMS_EMPLOYEE_KEY FROM DW_PROD.INTEGRATION.FACT_AMS_PAYROLL_MAPPING)
	-- Added on 22th nov to exclude across dups which were not identified at integration layer but falls in same cluster
		AND EM.EMPLOYEE_KEY NOT IN (SELECT DISTINCT ID FROM ACROSS_DUPS_EXCLUDE WHERE SOURCE_SYSTEM = 'AMS')
        AND EM.EMPLOYEE_KEY NOT IN (SELECT AMS_EMPLOYEE_KEY FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
        AND APP.APPLICANT_KEY NOT IN (SELECT APPLICANT_EMPLOYEE_KEY FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
    UNION
        SELECT DISTINCT 
        ...
        FROM DW_PROD.HAH.FACT_APPLICANT_STATUS STATUS
        INNER JOIN DW_PROD.HAH.DIM_APPLICANT APPLICANT
            ON STATUS.APPLICANT_KEY = APPLICANT.APPLICANT_KEY
        WHERE
            STATUS.HIRED_DATETIME IS NOT NULL
            AND STATUS.APPLICANT_KEY NOT IN ( SELECT DISTINCT APPLICANT_KEY FROM DW_PROD.INTEGRATION.APPLICANT_WORKDAY_EMPLOYEE_MAPPING APP ) -- 3223
            AND STATUS.APPLICANT_KEY NOT IN (SELECT APPLICANT_EMPLOYEE_KEY FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
            UNION
SELECT DISTINCT 
        	EMP.EMPLOYEE_FIRST_NAME FIRST_NAME, 
            EMP.EMPLOYEE_LAST_NAME LAST_NAME, 
            COALESCE(BR_PAYROLL.Office_State_Code,EMP.PRIMARY_BRANCH_STATE,BR_AMS.Office_State_Code) AS BRANCH_STATE_CODE,
            VI.EMP_ID AS BROADSPIRE_ID,
            VECAM.AMS_EMPLOYEE_KEY AS AMS_EMPLOYEE_KEY,
            EM.SOURCE_SYSTEM_ID AS AMS_SOURCE_SYSTEM_ID,
			EM.SYSTEM_CODE AS AMS_SYSTEM_CODE,
			EM.EMPLOYEE_ID AS AMS_EMPLOYEE_ID,
            OBJECT_CONSTRUCT_KEEP_NULL
            ...
            VECAM.APPLICANT_EMPLOYEE_KEY AS APPLICANT_EMPLOYEE_KEY,
            STATUS.SOURCE_SYSTEM_ID  AS APPLICANT_SOURCE_SYSTEM_ID,
			STATUS.SYSTEM_CODE AS APPLICANT_SYSTEM_CODE,
			APPLICANT.APPLICANT_ID AS APPLICANT_EMPLOYEE_ID, 
            CASE 
              WHEN VECAM.AMS_EMPLOYEE_KEY IS NOT NULL AND VECAM.PAYROLL_EMPLOYEE_KEY IS NOT NULL
--                    AND NULL_BR.EMPLOYEE_KEY IS NULL  --to make exception flag true for null branch_state employees
              THEN FALSE 
              ELSE TRUE 
            END AS EXCEPTION_FLAG,
            EM.PAYROLL_ID AS AMS_PAYROLL_ID,
            COALESCE(EMP.EMPLOYEE_PID, EM.EMPLOYEE_PID) AS EMPLOYEE_SSN
        FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING VECAM
        LEFT JOIN DW_PROD.INTEGRATION.DIM_EMPLOYEE_AMS_MERGED EM
            ON VECAM.AMS_EMPLOYEE_KEY = EM.EMPLOYEE_KEY 
        LEFT JOIN DW_PROD.INTEGRATION.DIM_EMPLOYEE_PAYROLL_MERGE_DEDUPE EMP
        	ON VECAM.PAYROLL_EMPLOYEE_KEY = EMP.EMPLOYEE_KEY
        LEFT JOIN ACTIVE_PAYROLL_EMPLOYEE AS FP 
            ON FP.EMPLOYEE_KEY = VECAM.PAYROLL_EMPLOYEE_KEY
        LEFT JOIN DW_PROD.HAH.DIM_APPLICANT APPLICANT 
            ON APPLICANT.APPLICANT_KEY = VECAM.APPLICANT_EMPLOYEE_KEY
        LEFT JOIN DW_PROD.HAH.FACT_APPLICANT_STATUS STATUS 
            ON STATUS.APPLICANT_KEY = APPLICANT.APPLICANT_KEY
        LEFT OUTER JOIN DW_PROD.INTEGRATION.DIM_Branch_Merged AS BR_AMS
            ON COALESCE(EM.PRIMARY_BRANCH_KEY,FP.BRANCH_KEY) = BR_AMS.ORIGINAL_Branch_Key 
        LEFT OUTER JOIN DW_PROD.INTEGRATION.DIM_Branch_Merged AS BR_PAYROLL
            ON COALESCE(EMP.PRIMARY_BRANCH_KEY,FP.BRANCH_KEY) = BR_PAYROLL.ORIGINAL_Branch_Key
        LEFT JOIN APP_DB_PROD.BROADSPIRE.VW_BROADSPIRE_WEEKLY_REPORT VI
        ON VI.EMPLOYEE_KEY = VECAM.PAYROLL_EMPLOYEE_KEY
        AND VI.BAD_DATA_FLAG = 0
)
SELECT COALESCE((SELECT MAX(EMPLOYEE_ENTERPRISE_ID) FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK WHERE EMPLOYEE_ENTERPRISE_ID < 50000000),10000000)+ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS EMPLOYEE_ENTERPRISE_ID
		,*
FROM FINAL;