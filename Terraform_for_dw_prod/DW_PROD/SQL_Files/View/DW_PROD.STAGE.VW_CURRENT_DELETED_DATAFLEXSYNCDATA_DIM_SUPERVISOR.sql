create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_DATAFLEXSYNCDATA_DIM_SUPERVISOR(
	SUPERVISOR_KEY
) as
WITH BULK_BILLING_SUPERVISORS AS (
	...
	FROM DISC_PROD.DATAFLEXSYNCDATA.HIST_DfSupervisors f
	WHERE f.ETL_DELETED_FLAG = TRUE
        AND CAST(f.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_PROD.DATAFLEXSYNCDATA.HIST_DfSupervisors)
	UNION ALL
