create or replace view DW_PROD.STAGE.VW_CURRENT_DELETED_MATRIXCARE_FACT_VISIT_EXCEPTION(
	VISIT_EXCEPTION_KEY
) as
WITH CALLS AS (
	SELECT 
	...
	FROM (
		...
		FROM DISC_PROD.MATRIXCARE.STVHC_T_TELEPHONYCALLS VISITS
...
INNER JOIN -- TO ONLY GET exceptions OF the visits that ARE IN FACT_VISIT AND EXCLUDE ORPHANS
	DISC_PROD.MATRIXCARE.HIST_STVHC_T_SCHEDULES VISITS
	ON CALLS.TCL_SCHEDULEID = VISITS.SCH_ID
WHERE 
	VISITS.ETL_DELETED_FLAG = TRUE AND VISITS.ETL_LAST_UPDATED_DATE::DATE IN (SELECT MAX(ETL_LAST_UPDATED_DATE)::DATE FROM DISC_PROD.MATRIXCARE.HIST_STVHC_T_SCHEDULES )
)
SELECT
