CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_OPENSYSTEMS_FACT_ADMISSION("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
SELECT * FROM (SELECT ROW_NUMBER () OVER(PARTITION BY TC.COORDINATORNAME ORDER BY TC.COORDINATORID  DESC) AS RN,
			* FROM DISC_PROD.HHAEXCHANGEOPENSYSTEMS.TBLCOORDINATOR_REPL TC) WHERE RN=1
),	
ADMISSIONS_DATE AS(
SELECT * FROM (SELECT ROW_NUMBER () OVER(PARTITION BY P.PATIENTID  ORDER BY P.SERVICESTARTDATE  DESC) AS RN,
			* FROM DISC_PROD.HHAEXCHANGEOPENSYSTEMS.PATIENTS P) WHERE RN=1
),
VISITS AS (
SELECT * FROM ( SELECT V.PATIENTID,VI.PRIMARYSERVICECODEID AS SERVICECODEID FROM  DISC_PROD.HHAEXCHANGEOPENSYSTEMS.VISITS V 
LEFT JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.VISITINFO_REPL VI ON VI.VISITID = V.VISITID
LEFT JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.TBLVISITS_REPL TBLV ON TBLV.VISITID = VI.VISITID
UNION ALL 
 SELECT PA.PATIENTID,SERVICECODEID FROM  DISC_PROD.HHAEXCHANGEOPENSYSTEMS.PATIENTAUTHORIZATION  PA)
 GROUP BY 1,2
)
...
FROM CLIENT
LEFT JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.PATIENTS P
ON P.PATIENTID = CLIENT.PATIENTID
LEFT JOIN VISITS V ON V.PATIENTID = P.PATIENTID 
LEFT JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.PATIENTCONTRACTS_REPL PC ON P.PATIENTID = PC.PATIENTID 
LEFT JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.REFERRAL  R
ON P.ADMISSIONID = R.ADMISSIONID 
LEFT JOIN ADMISSIONS_DATE AS ADM
ON ADM.PATIENTID = P.PATIENTID 
LEFT JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.REFERRALSOURCE  RS
...
LEFT JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.OFFICE_OFFICES_REPL O ON CLIENT.OFFICEID = O.OFFICEID
 JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.SERVICECODES SC ON SC.SERVICECODEID  = V.SERVICECODEID 
