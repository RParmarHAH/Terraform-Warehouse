CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_AXXESS_DIM_SUPERVISOR("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
(
  SELECT * FROM 
	(
		...
		FROM DISC_DEDUPE_PROD.AXXESS.EMPLOYEE_MASTER_LIST  --DISC_DEDUPE_PROD.ASR.CLIENT_MASTER_LIST 
	)
	UNION
	SELECT * FROM 
	(
		SELECT  MASTER_ID, EMPLOYEE_ID, (FIRST_NAME || ''  '' || LAST_NAME) AS EMP_NAME, ADDRESS_STATECODE
		FROM DISC_DEDUPE_PROD.AXXESS.EMPLOYEE_MATCH_LIST  --DISC_DEDUPE_PROD.ASR.CLIENT_MATCH_LIST 
