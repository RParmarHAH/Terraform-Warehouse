CREATE OR REPLACE PROCEDURE DW_PROD.REPORT.GET_REPORT_CLIENTS_REFERRED_TEST("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
CREATE OR REPLACE TEMPORARY TABLE DW_PROD."PUBLIC".CONFIRMED_VISITS_DATA
as
SELECT 
...
CREATE OR REPLACE TEMPORARY TABLE DW_PROD."PUBLIC".CLIENTS_SERVICE_DATES
AS
SELECT V.CLIENT_KEY 
...
LEFT JOIN DW_PROD.HAH.DIM_SERVICES S ON FPS.SERVICE_KEY = S.SERVICE_KEY 
...
CREATE OR REPLACE TEMPORARY TABLE DW_PROD.PUBLIC.CLIENT_METRIC
AS
...
FROM DW_PROD."PUBLIC".CLIENTS_SERVICE_DATES A
JOIN DW_PROD.INTEGRATION.FACT_VISIT_MERGED V ON A.CLIENT_KEY  = V.CLIENT_KEY AND A.FIRST_SERVICE_DATE = V.REPORT_DATE 
JOIN DW_PROD.INTEGRATION.FACT_VISIT_MERGED V2 ON A.CLIENT_KEY  = V2.CLIENT_KEY ;



CREATE OR REPLACE  TEMPORARY  TABLE DW_PROD."PUBLIC".auth_test
as
--with AUTHORIZATION AS (
	...
	FROM DW_PROD.REPORT.CLIENT_SERVICE_MONTHLY_AUTHORIZATION 
--	WHERE CLIENT_KEY IN (SELECT CLIENT_KEY FROM CONFIRMED_VISITS_DATA)
	GROUP BY 1,2,3,4,5;
--)

	
CREATE OR REPLACE  TEMPORARY  TABLE DW_PROD."PUBLIC".FUTURE_VISITS 
as
--, FUTURE_VISITS AS 
--(
SELECT
...
CREATE OR REPLACE TEMPORARY  TABLE DW_PROD."PUBLIC".FUTURE_VISITS_EVV 
as
...
CREATE OR REPLACE TEMPORARY  TABLE DW_PROD."PUBLIC".BILLABLE_VISITS 
as
--, BILLABLE_VISITS AS (
SELECT
	CLIENT_KEY,
...
LEFT JOIN DW_PROD."PUBLIC".auth_test auth ON auth.client_key = c.client_key
LEFT JOIN DW_PROD."PUBLIC".FUTURE_VISITS FV 
	ON FV.CLIENT_KEY=C.CLIENT_KEY
LEFT JOIN DW_PROD."PUBLIC".FUTURE_VISITS_EVV FEV 
