CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_PREFERRED_FACT_ADMISSION("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
PATIENTID, MASTER_ID, AGENCYID, LASTNAME, FIRSTNAME, OFFICEID
	 FROM DISC_DEDUPE_PROD.HHAEXCHANGEPREFERRED.CLIENT_MATCH_LIST CML
...
SELECT * FROM (SELECT ROW_NUMBER () OVER(PARTITION BY TC.COORDINATORNAME ORDER BY TC.COORDINATORID  DESC) AS RN,
			* FROM DISC_PROD.HHAEXCHANGEPREFERRED.TBLCOORDINATOR_REPL TC) WHERE RN=1
),
ADMISSIONS_DATE AS(
SELECT * FROM (SELECT ROW_NUMBER () OVER(PARTITION BY P.PATIENTID  ORDER BY P.SERVICESTARTDATE  DESC) AS RN,
			* FROM DISC_PROD.HHAEXCHANGEPREFERRED.STAGE_PATIENTS P) WHERE RN=1
),
VISITS AS (
    SELECT * FROM DISC_PROD.HHAEXCHANGEPREFERRED.VISITS V QUALIFY row_number() OVER (partition by v.visitid order by v.primaryservicecode) = 1
                )
...
LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.STAGE_PATIENTS P
...
--LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.VISITINFO_REPL VI ON VI.VISITID = V.VISITID
--LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.REVENUE_MAPPING  vor ON VI.VISITID  = VOR.VISITID
--LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.TBLVISITS_REPL TBLV ON TBLV.VISITID = VI.VISITID
LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.STAGE_PATIENTCONTRACTS_REPL PC ON P.PATIENTID = PC.PATIENTID 
LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.REFERRAL  R
ON P.ADMISSIONID = R.ADMISSIONID 
LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.REFERRALSOURCE  RS
ON P.REFERRALSOURCEID = RS.REFERRALSOURCEID  
--JOIN DISC_PROD.HHAEXCHANGEPREFERRED.SERVICECODES SC ON SC.CONTRACTID = PC.CONTRACTID
JOIN DISC_PROD.HHAEXCHANGEPREFERRED.SERVICECODES SC ON SC.CONTRACTID = PA.CONTRACTID
   	 	 AND SC.SERVICECODEID = PA.SERVICECODEID
  LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.REVENUE_MAPPING  vor ON VOR.SERVICECODEID  = SC.SERVICECODEID 	 	 
