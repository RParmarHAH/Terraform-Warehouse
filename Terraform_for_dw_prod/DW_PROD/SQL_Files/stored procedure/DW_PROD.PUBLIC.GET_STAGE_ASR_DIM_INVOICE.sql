CREATE OR REPLACE PROCEDURE DW_PROD.PUBLIC.GET_STAGE_ASR_DIM_INVOICE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INSERT OVERWRITE INTO DW_DEV.STAGE.ASR_DIM_INVOICE
WITH CLIENT AS
(
    SELECT * FROM
    (
        SELECT MASTER_ID, CUSTOMERID,SUPERVISORID
        FROM DISC_DEDUPE_DEV.ASR.CLIENT_MASTER_LIST
    )
    UNION
    SELECT * FROM
    (
        SELECT DISTINCT MASTER_ID, CUSTOMERID, NULL AS SUPERVISORID
        FROM DISC_DEDUPE_DEV.ASR.CLIENT_MATCH_LIST
        WHERE CUSTOMERID NOT IN (SELECT CUSTOMERID FROM DISC_DEDUPE_DEV.ASR.CLIENT_MASTER_LIST)
    )
)
SELECT
...
FROM
     DISC_DEV.ASR.ASR_REVENUE BSVR
LEFT JOIN
    CLIENT CLI ON BSVR.CUSTOMER_ID = CLI.CUSTOMERID
LEFT JOIN
    DISC_DEV.ASR.ASR_PAYERSOURCES P ON UPPER(P.PAYERSOURCENAME) = UPPER(BSVR.PAYER_SOURCE)
LEFT JOIN
    DISC_DEV.ASR.BRANCH_MAPPING BM ON TRIM(UPPER(BM.OLD_LOCATION_NAME)) = TRIM(UPPER(BSVR.LOCATION))
LEFT JOIN
     DISC_DEV.ASR.ASR_SUPERVISORS S ON CLI.SUPERVISORID =S.SUPERVISORID
