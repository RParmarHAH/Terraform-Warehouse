CREATE OR REPLACE PROCEDURE DW_PROD.INTEGRATION.GET_EMPLOYEE_KEYRING_DATA_TEST("ENVIRONMENT" VARCHAR(10), "JSON_STRING" VARCHAR(16777216))
RETURNS VARIANT
LANGUAGE JAVASCRIPT
EXECUTE AS OWNER
AS '
...
try {
    ...
    for (var i = 0; i < jsonDataArr.length; i++) {
        ...
        for (var key in jsonData) {
            ...
            if (!COL_NAMES.some(val => val.toUpperCase() === key.toUpperCase()) && !PAYROLL_KEYS.some(val => val.toUpperCase() === key.toUpperCase()) && AMS_KEYS_VALIDATION === false && PAYROLL_KEYS_VALIDATION === false && AMS_EMPLOYEE_KEY === false && PAYROLL_EMPLOYEE_KEY === false && EMPLOYEE_ENTERPRISE_ID === false && AMS_EMPLOYEE_ID === false && PAYROLL_EMPLOYEE_ID === false && AMS_PUBLIC_ID === false && AMS_PAYROLL_ID === false) {
                COLUMNCHECK_VALIDATION = true;
            }
        }
        if ((AMS_KEYS_VALIDATION === true || PAYROLL_KEYS_VALIDATION === true) && AMS_EMPLOYEE_KEY === false && PAYROLL_EMPLOYEE_KEY === false && EMPLOYEE_ENTERPRISE_ID === false && AMS_EMPLOYEE_ID === false && PAYROLL_EMPLOYEE_ID === false && AMS_PUBLIC_ID === false && AMS_PAYROLL_ID === false) {
            errorObj = {
                KEYRING_MSG_CODE: ''2'',
                KEYRING_MSG_DESC: ''EMPLOYEE_ID or PUBLIC_ID IS mandatory along with AMS_SOURCE_SYSTEM or PAYROLL_SOURCE_SYSTEM.''
            };
        } else if (AMS_EMPLOYEE_KEY === false && PAYROLL_EMPLOYEE_KEY === false && EMPLOYEE_ENTERPRISE_ID === false && AMS_EMPLOYEE_ID === false && PAYROLL_EMPLOYEE_ID === false && AMS_PUBLIC_ID === false && AMS_PAYROLL_ID === false) {
            errorObj = {
                KEYRING_MSG_CODE: ''2'',
                KEYRING_MSG_DESC: ''AMS_EMPLOYEE_KEY,PAYROLL_EMPLOYEE_KEY or EMPLOYEE_ENTERPRISE_ID is mandatory.''
            };
        } else if (AMS_KEYS_VALIDATION === false && PAYROLL_KEYS_VALIDATION === true && AMS_EMPLOYEE_KEY === false && PAYROLL_EMPLOYEE_KEY === false && EMPLOYEE_ENTERPRISE_ID === false && AMS_EMPLOYEE_ID === false && PAYROLL_EMPLOYEE_ID === false && AMS_PUBLIC_ID === false && AMS_PAYROLL_ID === false) {
            errorObj = {
                KEYRING_MSG_CODE: ''2'',
                KEYRING_MSG_DESC: ''Please provide a valid value for PAYROLL_SOURCE_SYSTEM.''
            };
        } else if (AMS_KEYS_VALIDATION === true && PAYROLL_KEYS_VALIDATION === false && AMS_EMPLOYEE_KEY === false && PAYROLL_EMPLOYEE_KEY === false && EMPLOYEE_ENTERPRISE_ID === false && AMS_EMPLOYEE_ID === false && PAYROLL_EMPLOYEE_ID === false && AMS_PUBLIC_ID === false && AMS_PAYROLL_ID === false) {
            errorObj = {
                KEYRING_MSG_CODE: ''2'',
         ...
         else if (AMS_KEYS_VALIDATION === false && PAYROLL_KEYS_VALIDATION === false && AMS_EMPLOYEE_KEY === false && PAYROLL_EMPLOYEE_KEY === false && EMPLOYEE_ENTERPRISE_ID === false && (AMS_EMPLOYEE_ID === true || AMS_PUBLIC_ID === true || AMS_PAYROLL_ID === true) && PAYROLL_EMPLOYEE_ID === false) {
            errorObj = {
                KEYRING_MSG_CODE: ''2'',
                KEYRING_MSG_DESC: ''Please provide a valid value for AMS_SOURCE_SYSTEM.''
            };
          }
        else if (AMS_KEYS_VALIDATION === false && PAYROLL_KEYS_VALIDATION === false && AMS_EMPLOYEE_KEY === false && PAYROLL_EMPLOYEE_KEY === false && EMPLOYEE_ENTERPRISE_ID === false && AMS_EMPLOYEE_ID === false && PAYROLL_EMPLOYEE_ID === true && AMS_PUBLIC_ID === false && AMS_PAYROLL_ID === false) {
            errorObj = {
                KEYRING_MSG_CODE: ''2'',
                KEYRING_MSG_DESC: ''Please provide a valid value for PAYROLL_SOURCE_SYSTEM.''
            };
