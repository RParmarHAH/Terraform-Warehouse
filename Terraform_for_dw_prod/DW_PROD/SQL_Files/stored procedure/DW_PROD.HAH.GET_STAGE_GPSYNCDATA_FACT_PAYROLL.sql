CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_GPSYNCDATA_FACT_PAYROLL("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
( SELECT m1.CheckID, CAST( SUM( dl1.AMOUNT) AS DECIMAL( 12,2)) AS Service_Wages, CAST( SUM( dl1.UNITS) AS DECIMAL( 12,2)) AS Service_Hours, MAX(DL1.RATE) AS RATE
  FROM DISC_PROD.GPSYNCDATA.Paystub_Master AS m1
  INNER JOIN DISC_PROD.GPSYNCDATA.Paystub_DetailLine dl1 ON m1.CheckID = dl1.CheckID
...
( SELECT CheckID, CAST( SUM( AMOUNT) AS DECIMAL( 12,2)) AS Taxes
  FROM DISC_PROD.GPSYNCDATA.Paystub_DetailLine 
  WHERE IsTax = ''TRUE''
  GROUP BY CheckID)
SELECT DISTINCT
...
--LEFT JOIN DW_PROD.HAH.DIM_BRANCH BRANCH2 ON ( LPAD( CAST( BRANCH2.Office_Number AS VARCHAR), 3, ''0'') = LPAD( TRIM( EMP.Office_NO), 3, ''0'') AND BRANCH2.Source_System_ID = 3 AND EMP.Office_No NOT IN (908, 950, 951, 17, 52, 53, 60, -1)) 
...
LEFT JOIN DW_PROD.HAH.DIM_BRANCH BRANCH ON  TRIM(BRANCH.OFFICE_NUMBER) = TRIM(COALESCE(EMP.OFFICE_NO, M1.OFFICENUMBER))::INTEGER AND BRANCH.SOURCE_SYSTEM_ID = 3 AND TRIM(UPPER(BRANCH.OFFICE_STATE_CODE)) = UPPER(TRIM(COALESCE(CASE WHEN EMP.OFFICE_NO = 53 THEN ''IN'' ELSE EMP.WORK_STATE END, M1.WORKSTATE)))
