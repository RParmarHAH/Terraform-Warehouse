CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_DATAFLEXSYNCDATA_FACT_INTAKE_BKP_BFR_AUTH_08082022("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
WITH MAXREAUTH AS (
 ...
 (
     SELECT MIN(PAYROLLDATE) AS TIMESHEET_DATE
          ...
          , DBNAME
          , CLIENTNUMBER
     FROM DISC_PROD.DATAFLEXSYNCDATA.DFTIMESHEETS
...
--                                          MIRA.CONTRACTCODE = F.CONTRACTCODE
	...
	WHERE F.ETL_LAST_UPDATED_DATE >= ''`; 
sql += STR_CDC_START
sql += `''::timestamp_ntz
			OR C.ETL_LAST_UPDATED_DATE >= ''`; 
sql += STR_CDC_START
sql += `''::timestamp_ntz
...
SELECT L.INTAKE_KEY, L.REPORT_DATE, L.BRANCH_KEY, L.CLIENT_KEY, L.CONTRACT_KEY, L.SOURCE_SYSTEM_ID, L.REAUTHORIZED_DATE,
