CREATE OR REPLACE PROCEDURE DW_PROD.PUBLIC.GET_STAGE_ALAYACARE_FACT_VISIT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
(
	...
	FROM DISC_DEV.ALAYACARE.INVOICE_ITEM
	GROUP BY VISIT_ID 
), VISITS AS (
	SELECT MD5(NVL(TRIM(B.PROPERTIES_TBL_GT_ACCOUNT_DESCRIPTION), ''Unknown'') || '' ('' || NVL(VISITS.BRANCH_ID,-1) || '')'' || ''-'' || VISITS.VISIT_ID|| ''-'' || ''ALAYACARE'') AS VISIT_KEY,
			...
			NULL AS TRACKING_ID
	FROM DISC_DEV.ALAYACARE.VISIT AS VISITS
	...
	LEFT JOIN DISC_DEV.ALAYACARE.SERVICE AS SERVICES
