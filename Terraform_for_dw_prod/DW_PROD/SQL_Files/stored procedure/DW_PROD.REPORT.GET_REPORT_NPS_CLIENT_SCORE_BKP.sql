CREATE OR REPLACE PROCEDURE DW_PROD.REPORT.GET_REPORT_NPS_CLIENT_SCORE_BKP("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
WITH SURVEY_MAPPING AS (
    SELECT CLIENT_KEY, TO_DATE(RECORDED_DATE) RECORDED_DATE, TRIM(SPLIT_PART(COMMENTS, ''='', 2)) AS NPS, SURVEY_RESPONSE_HEADER_KEY
    FROM DW_PROD.INTEGRATION.FACT_SURVEY_QUESTION_RESPONSE_MAPPING WHERE QUESTION_ID = ''QID4''
  )
  , CLIENT_CONTRACTS AS (
      SELECT 
        DISTINCT SM.CLIENT_KEY, SM.RECORDED_DATE, SM.NPS, SM.SURVEY_RESPONSE_HEADER_KEY, FV.BRANCH_KEY, FV.CONTRACT_KEY
      FROM SURVEY_MAPPING SM
      INNER JOIN DW_PROD.HAH.FACT_VISIT FV
        ON SM.CLIENT_KEY = FV.CLIENT_KEY
        AND YEAR(SM.RECORDED_DATE) = YEAR(FV.SERVICE_DATE)
        AND MONTH(SM.RECORDED_DATE) = MONTH(FV.SERVICE_DATE)
  ...
  , t1 as(
  SELECT 
    C.CLIENT_KEY, C.BRANCH_KEY, C.SURVEY_RESPONSE_HEADER_KEY, RECORDED_DATE,NPS --, CONTRACT_KEY, QUESTION_ID, QUESTION_TEXT, 
...
batch AS (
SELECT client_key,SURVEY_RESPONSE_HEADER_KEY,CAST(CONCAT(''20'',SUBSTRING(BATCH_NUMBER,3,2),
  CASE 
