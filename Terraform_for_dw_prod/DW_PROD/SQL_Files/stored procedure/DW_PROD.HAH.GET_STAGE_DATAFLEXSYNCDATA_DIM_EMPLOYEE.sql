CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_DATAFLEXSYNCDATA_DIM_EMPLOYEE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
FROM DISC_DEDUPE_PROD.DATAFLEXSYNCDATA.EMPLOYEE_MASTER_LIST AS E
LEFT JOIN DISC_DEDUPE_PROD.DATAFLEXSYNCDATA.EMPLOYEE_MATCH_LIST AS ML
	ON ML.MASTER_ID = E.MASTER_ID 
JOIN DISC_PROD.DataFlexSyncData.ALL_DFTIMESHEETS AS TS
	ON TS.DBNAME = COALESCE(ML.DBNAME, E.DBNAME) AND TS.EMPLOYEENUMBER = COALESCE(ML."NUMBER", E."NUMBER")
INNER JOIN DISC_PROD.DataFlexSyncData.ALL_TIMESHEETSHOURS AS tsh ON ts.DBName = tsh.DBName AND ts.ReferenceNo = tsh.ReferenceNo
...
FROM DISC_DEDUPE_PROD.DATAFLEXSYNCDATA.EMPLOYEE_MASTER_LIST e
...
DISC_DEDUPE_PROD.DATAFLEXSYNCDATA.EMPLOYEE_MASTER_LIST),
WORKDAY_EXCLUDE_EMPLOYEES AS(
SELECT ADD_INFO.EMPLOYEE_ID, ADD_INFO.GP_ID
FROM DISC_PROD.WORKDAY.WORKER_ADDITIONAL_INFORMATION AS ADD_INFO
LEFT JOIN DISC_PROD.WORKDAY.WORKER_EXCLUDE_LIST WE
...
LEFT JOIN DISC_PROD.WORKDAY.WORKER_POSITION_DATA AS POS ON WEE.EMPLOYEE_ID = POS.EMPLOYEE_ID
