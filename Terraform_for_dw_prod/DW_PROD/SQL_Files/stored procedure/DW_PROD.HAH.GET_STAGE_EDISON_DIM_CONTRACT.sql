CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_EDISON_DIM_CONTRACT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
WITH CTE AS (
        ...
        FROM
            DISC_PROD.HHAEXCHANGEEDISON.VisitInfo_REPL VI
            JOIN DISC_PROD.HHAEXCHANGEEDISON.SERVICETYPE_REPL ST ON ST.SERVICETYPEID = VI.SERVICETYPEID
            AND ST.AGENCYID = VI.AGENCYID
            JOIN DISC_PROD.HHAEXCHANGEEDISON.SERVICECATEGORY_REPL SC ON SC.SERVICECATEGORYID = VI.SERVICECATEGORYID
            and st.SERVICECATEGORYID
            LEFT JOIN DISC_PROD.HHAEXCHANGEEDISON.SERVICECODES S ON S.SERVICECODEID = VI.PRIMARYSERVICECODEID
            ...
            JOIN DISC_PROD.HHAEXCHANGEEDISON.CONTRACTRATES CR ON CR.CONTRACTID = C.CONTRACTID
            AND CR.AGENCYID = C.AGENCYID
            AND CR.SERVICECODEID = S.SERVICECODEID
            AND CR.TODATE :: DATE > GETDATE()
            AND CR.FROMDATE :: DATE <= GETDATE()
            LEFT JOIN DISC_PROD.HHAEXCHANGEEDISON.TBLVISITS_REPL TV ON TV.VISITID = VI.VISITID
            AND TV.AGENCYID = VI.AGENCYID
            LEFT JOIN DISC_PROD.HHAEXCHANGEEDISON.VISITCONTRACTS_REPL VC ON VC.VISITID = TV.VISITID
            AND TV.AGENCYID = VC.AGENCYID
            LEFT JOIN DISC_PROD.HHAEXCHANGEEDISON.PATIENTCONTRACTS_REPL PC ON PC.PATIENTCONTRACTID = VC.CONTRACTID
...
FROM
    DISC_PROD.HHAEXCHANGEEDISON.SERVICECODES SC
LEFT JOIN EDISON_REVENUE_SEG RS
    ...
    LEFT JOIN DISC_PROD.HHAEXCHANGEEDISON.CONTRACTRATES CR ON CR.CONTRACTID = SC.CONTRACTID
