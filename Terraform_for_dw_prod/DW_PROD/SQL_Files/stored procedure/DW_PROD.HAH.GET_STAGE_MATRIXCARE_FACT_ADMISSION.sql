CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_MATRIXCARE_FACT_ADMISSION("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_ADMISSIONSDISCHARGEREASONS ADMDISC ON ADMDISC.ADMDCR_ID = ADM.ADM_DISCHARGEREASONID
LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_REFERRALTYPES REFCAMP ON REFCAMP.REFTYP_ID = ADM.ADM_REFERRALCAMPAIGNID
LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_ADMISSIONSTATUS ADMSTS ON ADMSTS.ADMSTS_ID = ADM.ADM_STATUSID
...
MD5(
   ...
   as CLIENT_ADMISSION_KEY
	...
	,:STR_ETL_TASK_KEY AS ETL_TASK_KEY,
     ...
     CURRENT_USER                                              as ETL_LAST_UPDATED_BY,
     0                                                         as ETL_DELETED_FLAG
    FROM DISC_PROD.MATRIXCARE.STVHC_T_AUTHORIZATION AU
    LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_ADMISSIONS ADM ON adm.adm_id = au.AUTH_ADMISSIONID
    LEFT JOIN CLIENT AS CLI ON CLI.CLI_ID = ADM.ADM_CLIENTID
    LEFT JOIN DISC_PROD.matrixcare.STVHC_T_SERVICECODE sc on sc.svcc_id = au.AUTH_SERVICECODEID
    LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_ADMISSIONSDISCHARGEREASONS ADMDISC ON ADMDISC.ADMDCR_ID = ADM.ADM_DISCHARGEREASONID
	LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_REFERRALTYPES REFCAMP ON REFCAMP.REFTYP_ID = ADM.ADM_REFERRALCAMPAIGNID
	LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_ADMISSIONSTATUS ADMSTS ON ADMSTS.ADMSTS_ID = ADM.ADM_STATUSID
...
MD5(
   ...
   ,MD5(
    ...
    LEFT JOIN CLIENT AS CLI ON CLI.CLI_ID = ADM.ADM_CLIENTID
    LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_AUTHORIZATION AU ON adm.adm_id = au.AUTH_ADMISSIONID
    LEFT JOIN DISC_PROD.matrixcare.STVHC_T_SERVICECODE sc on sc.svcc_id = au.AUTH_SERVICECODEID
    LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_ADMISSIONSDISCHARGEREASONS ADMDISC ON ADMDISC.ADMDCR_ID = ADM.ADM_DISCHARGEREASONID
	LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_REFERRALTYPES REFCAMP ON REFCAMP.REFTYP_ID = ADM.ADM_REFERRALCAMPAIGNID
	LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_ADMISSIONSTATUS ADMSTS ON ADMSTS.ADMSTS_ID = ADM.ADM_STATUSID;
