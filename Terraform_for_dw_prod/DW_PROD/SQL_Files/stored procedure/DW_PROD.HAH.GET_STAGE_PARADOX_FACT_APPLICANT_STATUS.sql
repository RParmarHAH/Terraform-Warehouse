CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_PARADOX_FACT_APPLICANT_STATUS("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--*****************************************************************************************************************************
--
INSERT OVERWRITE INTO DW_PROD.STAGE.PARADOX_FACT_APPLICANT_STATUS
WITH UNIQUE_APPLICANTS_STATUS_HISTORY_LATEST_LOCATION AS (
    ...
    FROM DISC_PROD.PARADOX.CANDIDATE_JOURNEY_STATUS_HISTORY CJ
...
, CANDIDATE_JOURNEY_HISTORY AS (
    SELECT 
        ...
        MAX(CASE WHEN CJ.STATUS IN (''Hire: Hired'', ''Orientation/ Hired: Hired'', ''Orientation/Hired: Hired'',''Hired: Hired'') THEN iff(RANK = 1,CJ.STATUS_CHANGE_DATE,NULL) END)  HIRED_DATETIME,
...
//            THEN CJ.STATUS_CHANGE_DATE END) REJECTED_OR_OFFER_DECLINED_DATETIME,
        ...
        MAX(CASE WHEN CJ.STATUS IN( ''Onboarding: Send Onboarding'',''Onboarding: Onboarding Sent'') THEN CJ.STATUS_CHANGE_DATE END) ONBOARDING_SENT_DATETIME,
        MAX(CASE WHEN CJ.STATUS = ''Onboarding: Onboarding Complete'' THEN CJ.STATUS_CHANGE_DATE END) ONBOARDING_COMPLETED_DATETIME,
        CASE WHEN ONBOARDING_COMPLETED_DATETIME IS NOT NULL THEN ''TRUE'' ELSE ''FALSE'' END AS ONBOARDING_COMPLETE_FLAG,
        CASE WHEN HIRED_DATETIME IS NOT NULL THEN ''TRUE'' ELSE ''FALSE'' END AS SEND_TO_AMS,
        MAX(CASE WHEN CJ.STATUS = ''Preboarding: Preboarding Complete'' THEN CJ.STATUS_CHANGE_DATE END) PREBOARDING_COMPLETED_DATETIME,
        CASE WHEN PREBOARDING_COMPLETED_DATETIME IS NOT NULL THEN ''TRUE'' ELSE ''FALSE'' END AS PRE_BOARDING_COMPLETION, 
...
, CANDIDATE_JOURNEY_HISTORY_WITH_REJECTION_REASON AS (
  ...
  LEFT JOIN DISC_PROD.PARADOX.CANDIDATE_JOURNEY_STATUS_HISTORY CJ 
...
SELECT
	...
	PRE_BOARDING_COMPLETION::BOOLEAN PRE_BOARDING_COMPLETION,
	...
	ONBOARDING_COMPLETE_FLAG ONBOARDING_COMPLETE_FLAG,
    SEND_TO_AMS SEND_TO_AMS,
...
FROM DISC_PROD.PARADOX.CANDIDATE_JOURNEY CJ
    ...
    INNER JOIN DISC_PROD.PARADOX.CANDIDATE_SPECIFIC CSS ON CJ.CANDIDATE_ID = CSS.CANDIDATE_ID
    LEFT JOIN DW_PROD.HAH.AMS_PARADOX_BRANCH_MAPPING CS 
    ON (UPPER(TRIM(CS.CANDIDATE_LOCATION__LEVEL_1)) = UPPER(TRIM(CSS.CANDIDATE_LOCATION__LEVEL_1))
	AND UPPER(TRIM(CS.CANDIDATE_LOCATION__LEVEL_2)) = UPPER(TRIM(CSS.CANDIDATE_LOCATION__LEVEL_2))
	AND  ifnull(CS.CANDIDATE_LOCATION__LEVEL_3,''-1'') = upper(ifnull(CSS.CANDIDATE_LOCATION__LEVEL_3,''-1'')))
	OR (UPPER(TRIM(CS.CANDIDATE_LOCATION__LEVEL_1)) = UPPER(TRIM(CSS.CANDIDATE_LOCATION__LEVEL_1))
	AND UPPER(TRIM(CS.CANDIDATE_LOCATION__LEVEL_2)) = UPPER(TRIM(CSS.CANDIDATE_LOCATION__LEVEL_2))
	)
