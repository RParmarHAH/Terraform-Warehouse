CREATE OR REPLACE PROCEDURE DW_PROD.INTEGRATION.GET_EMPLOYEE_KEYRING_DATA("ENVIRONMENT" VARCHAR(10), "JSON_STRING" VARCHAR(16777216))
RETURNS VARIANT
LANGUAGE JAVASCRIPT
EXECUTE AS OWNER
AS '
...
try {
    ...
    for (var i = 0; i < jsonDataArr.length; i++) {
        ...
        var AMS_PAYROLL_ID = false;
		var EMPLOYEE_SSN = false;					   
        var resultString = '''';
        var keyValuePairs = [];
        for (var key in jsonData) {
            ...
            if (key.toUpperCase() === ''AMS_PAYROLL_ID'') {
                AMS_PAYROLL_ID = true;
            }
			if (key.toUpperCase() === ''EMPLOYEE_SSN'') {
                EMPLOYEE_SSN = true;
            }
            if (!COL_NAMES.some(val => val.toUpperCase() === key.toUpperCase()) && !PAYROLL_KEYS.some(val => val.toUpperCase() === key.toUpperCase()) && AMS_KEYS_VALIDATION === false && PAYROLL_KEYS_VALIDATION === false && AMS_EMPLOYEE_KEY === false && PAYROLL_EMPLOYEE_KEY === false && EMPLOYEE_ENTERPRISE_ID === false && EMPLOYEE_SSN === false && AMS_EMPLOYEE_ID === false && PAYROLL_EMPLOYEE_ID === false && AMS_PUBLIC_ID === false && AMS_PAYROLL_ID === false) {
                COLUMNCHECK_VALIDATION = true;
            }
        }
        if ((AMS_KEYS_VALIDATION === true || PAYROLL_KEYS_VALIDATION === true) && AMS_EMPLOYEE_KEY === false && PAYROLL_EMPLOYEE_KEY === false && EMPLOYEE_ENTERPRISE_ID === false && EMPLOYEE_SSN === false && AMS_EMPLOYEE_ID === false && PAYROLL_EMPLOYEE_ID === false && AMS_PUBLIC_ID === false && AMS_PAYROLL_ID === false) {
            errorObj = {
                KEYRING_MSG_CODE: ''2'',
                KEYRING_MSG_DESC: ''EMPLOYEE_ID or PUBLIC_ID IS mandatory along with AMS_SOURCE_SYSTEM or PAYROLL_SOURCE_SYSTEM.''
            };
        } else if (AMS_EMPLOYEE_KEY === false && PAYROLL_EMPLOYEE_KEY === false && EMPLOYEE_ENTERPRISE_ID === false && EMPLOYEE_SSN === false && AMS_EMPLOYEE_ID === false && PAYROLL_EMPLOYEE_ID === false && AMS_PUBLIC_ID === false && AMS_PAYROLL_ID === false) {
            errorObj = {
                KEYRING_MSG_CODE: ''2'',
                KEYRING_MSG_DESC: ''AMS_EMPLOYEE_KEY,PAYROLL_EMPLOYEE_KEY, EMPLOYEE_ENTERPRISE_ID or EMPLOYEE_SSN is mandatory.''
            };
        } else if (AMS_KEYS_VALIDATION === false && PAYROLL_KEYS_VALIDATION === true && AMS_EMPLOYEE_KEY === false && PAYROLL_EMPLOYEE_KEY === false && EMPLOYEE_ENTERPRISE_ID === false && EMPLOYEE_SSN === false && AMS_EMPLOYEE_ID === false && PAYROLL_EMPLOYEE_ID === false && AMS_PUBLIC_ID === false && AMS_PAYROLL_ID === false) {
            errorObj = {
                KEYRING_MSG_CODE: ''2'',
                KEYRING_MSG_DESC: ''Please provide a valid value for PAYROLL_SOURCE_SYSTEM.''
            };
        } else if (AMS_KEYS_VALIDATION === true && PAYROLL_KEYS_VALIDATION === false && AMS_EMPLOYEE_KEY === false && PAYROLL_EMPLOYEE_KEY === false && EMPLOYEE_ENTERPRISE_ID === false && EMPLOYEE_SSN === false && AMS_EMPLOYEE_ID === false && PAYROLL_EMPLOYEE_ID === false && AMS_PUBLIC_ID === false && AMS_PAYROLL_ID === false) {
            errorObj = {
                KEYRING_MSG_CODE: ''2'',
         ...
         else if (AMS_KEYS_VALIDATION === false && PAYROLL_KEYS_VALIDATION === false && AMS_EMPLOYEE_KEY === false && PAYROLL_EMPLOYEE_KEY === false && EMPLOYEE_ENTERPRISE_ID === false && EMPLOYEE_SSN === false && (AMS_EMPLOYEE_ID === true || AMS_PUBLIC_ID === true || AMS_PAYROLL_ID === true) && PAYROLL_EMPLOYEE_ID === false) {
            errorObj = {
                KEYRING_MSG_CODE: ''2'',
                KEYRING_MSG_DESC: ''Please provide a valid value for AMS_SOURCE_SYSTEM.''
            };
          }
        else if (AMS_KEYS_VALIDATION === false && PAYROLL_KEYS_VALIDATION === false && AMS_EMPLOYEE_KEY === false && PAYROLL_EMPLOYEE_KEY === false && EMPLOYEE_ENTERPRISE_ID === false && EMPLOYEE_SSN === false && AMS_EMPLOYEE_ID === false && PAYROLL_EMPLOYEE_ID === true && AMS_PUBLIC_ID === false && AMS_PAYROLL_ID === false) {
            errorObj = {
                KEYRING_MSG_CODE: ''2'',
                KEYRING_MSG_DESC: ''Please provide a valid value for PAYROLL_SOURCE_SYSTEM.''
            };
    ...
    if (conditions.length > 0) {
        ...
        for (key in input_dict) {
            ...
            for (var i = 0; i < result_arr.length; i++) {
                if (parseInt(result_arr[i][''EMPLOYEE_ENTERPRISE_ID'']) === parseInt(input_dict[key][''EMPLOYEE_ENTERPRISE_ID''])) {
				...
				} else if (String(result_arr[i][''EMPLOYEE_SSN'']) === String(input_dict[key][''EMPLOYEE_SSN''])) {
                    res.push(result_arr[i]);																											   											
                    ...
                    res.push(result_arr[i]);                
                        ...
                        (String(result_arr[i][''AMS_PUBLIC_ID'']) === String(input_dict[key][''AMS_PUBLIC_ID'']) || 
                         String(result_arr[i][''AMS_PAYROLL_ID'']) === String(input_dict[key][''AMS_PAYROLL_ID''])) && ( 
                        (String(result_arr[i][''AMS_SOURCE_SYSTEM_ID'']) === String(input_dict[key][''AMS_SOURCE_SYSTEM_ID''])) ||
                        ([String(result_arr[i][''AMS_SYSTEM_CODE'']),String(result_arr[i][''AMS_SOURCE_SYSTEM''])].includes(String(input_dict[key][''AMS_SOURCE_SYSTEM'']).toUpperCase())) ||
                        ([String(result_arr[i][''AMS_SYSTEM_CODE'']),String(result_arr[i][''AMS_SOURCE_SYSTEM''])].includes(String(input_dict[key][''AMS_SYSTEM_CODE'']).toUpperCase())) ||
                        (String(result_arr[i][''PAYROLL_SOURCE_SYSTEM_ID'']) === String(input_dict[key][''PAYROLL_SOURCE_SYSTEM_ID''])) ||
