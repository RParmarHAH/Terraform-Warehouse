CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_EDISON_DIM_EMPLOYEE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
(
	...
	AS  LAST_CHECK_DATE
	FROM  DISC_PROD.HHAEXCHANGEEDISON.PAYROLL_DETAILS_REPL
	GROUP BY 1
)
SELECT DISTINCT
...
FROM DISC_DEDUPE_PROD.HHAEXCHANGEEDISON.EMPLOYEE_MASTER_LIST EM
LEFT JOIN DISC_PROD.HHAEXCHANGEEDISON.LANGUAGES L ON L.LANGUAGEID = EM.LANGUAGE1
LEFT JOIN DISC_PROD.HHAEXCHANGEEDISON.OFFICE_OFFICES_REPL  O ON EM.OFFICEID = O.OFFICEID AND EM.AGENCYID = O.AGENCYID 
LEFT  JOIN DISC_PROD.HHAEXCHANGEEDISON.PAYROLL_DETAILS_REPL PD ON PD.CAREGIVERID = EM.CAREGIVERID 
