CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_MATRIXCARE_FACT_CLIENT_SERVICE_BKCP1310("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--05/09/2023    Rajat Sapkal			 Modified Authorization max units logic																	  
--*****************************************************************************************************************************
INSERT OVERWRITE INTO STAGE.MATRIXCARE_FACT_CLIENT_SERVICE_bkcp1310
WITH CLIENT AS
(
	SELECT * FROM
	(
...
CASE 
    ...
    --DIV0(authorization_max_units_by_uom * AUTHDTL_FREQCOUNT, CASE WHEN AUTHDTL_FREQINTERVAL = 1 THEN 2 ELSE 1 END) AS authorization_max_units,
       -- 
        CASE WHEN AUTHDTL_FREQTYPE IN (1,2,7) THEN (authorization_max_units_by_uom*(EFFECTIVE_DAYS/ 7))  
         ...
         WHEN AUTHDTL_FREQTYPE IN(4,8) THEN (authorization_max_units_by_uom*MONTHS_BETWEEN(AUTHORIZATION_PERIOD_END_DATE,AUTHORIZATION_PERIOD_START_DATE))
...
LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_AUTHORIZATIONDETAIL AD
	ON AU.AUTH_ID =AD.AUTHDTL_AUTHORIZATIONID 
LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_SERVICECODE SC
