CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_CLEARCARE_DIM_INVOICE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INNER JOIN DISC_PROD.CLEARCARE.RECEIVABLES_PAYMENTDESIGNATION rpt ON rpt.INVOICE_ID = inv.id
INNER JOIN DISC_PROD.CLEARCARE.PAYMENT_TRANSACTION pt ON pt.id = rpt.PAYMENT_ID 
...
FROM DISC_PROD.CLEARCARE.INVOICE_TRANSACTION INV
INNER JOIN DISC_PROD.CLEARCARE.PROFILE_PROFILE p ON p.ID = INV.client_id
INNER JOIN DISC_PROD.CLEARCARE.PATIENT_PATIENT pp ON pp.ID  = p.PATIENT_ID 
INNER JOIN DISC_PROD.CLEARCARE.AGENCY_AGENCYLOCATION ag ON ag.id = pp.location_id
