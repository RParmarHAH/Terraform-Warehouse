CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_CLEARCARE_DIM_VISIT_STATUS_MAPPING("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
c.id
FROM DISC_PROD.CLEARCARE.CARELOGS_CARELOG c
LEFT JOIN DISC_PROD.CLEARCARE.SCHEDULES_SHIFT s ON s.id = c.SHIFT_ID 
LEFT JOIN DISC_PROD.CLEARCARE.RECEIVABLES_INVOICE_CARELOGS  ric ON ric.CARELOG_ID = c.ID 
LEFT JOIN DISC_PROD.CLEARCARE.INVOICE_TRANSACTION iv ON iv.id = ric.INVOICE_ID 
LEFT JOIN DISC_PROD.CLEARCARE.SCHEDULES_SCHEDULE sc ON sc.id = s.SCHEDULE_ID )
