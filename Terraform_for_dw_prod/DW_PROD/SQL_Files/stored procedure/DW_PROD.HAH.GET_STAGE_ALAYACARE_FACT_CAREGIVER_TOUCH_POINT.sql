CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_ALAYACARE_FACT_CAREGIVER_TOUCH_POINT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO DW_PROD.STAGE.ALAYACARE_FACT_CAREGIVER_TOUCH_POINT
WITH EMPLOYEE AS 
(
SELECT * FROM 
(
...
FROM DISC_PROD.ALAYACARE.EMPLOYEE  AS employee_tier_5
LEFT JOIN DISC_PROD.ALAYACARE.BRANCH  AS employee_branch ON (employee_tier_5."BRANCH_ID")=(employee_branch."BRANCH_ID")
LEFT JOIN EMPLOYEE E ON
employee_tier_5.EMPLOYEE_ID = E.EMPLOYEE_ID
AND employee_branch.BRANCH_ID = E.BRANCH_ID
LEFT JOIN DISC_DEDUPE_PROD.ALAYACARE.EMPLOYEE_MASTER_LIST EMP ON EMP.MASTER_ID = E.MASTER_ID
LEFT JOIN DISC_PROD.ALAYACARE.PROFILE  AS employee_attributes_generic ON employee_tier_5.profile_id=employee_attributes_generic.profile_id
LEFT JOIN DISC_PROD.ALAYACARE.tbl_guid_cost_centre_tier_4  AS employee_cost_centre_tier_4 ON employee_tier_5.guid=employee_cost_centre_tier_4.guid_to
LEFT JOIN DISC_PROD.ALAYACARE.tbl_cost_centres_tier_4  AS cost_centre_tier_4 ON employee_cost_centre_tier_4.cost_centre_id=cost_centre_tier_4.id
LEFT JOIN DISC_PROD.ALAYACARE.tbl_journal_entries_tier_4  AS note_tier_5 ON employee_tier_5.guid=note_tier_5.guid_to
