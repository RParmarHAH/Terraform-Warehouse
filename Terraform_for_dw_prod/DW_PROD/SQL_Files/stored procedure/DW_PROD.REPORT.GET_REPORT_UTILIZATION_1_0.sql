CREATE OR REPLACE PROCEDURE DW_PROD.REPORT.GET_REPORT_UTILIZATION_1_0("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
-- 04/19/23	   Shraddha Sejpal		 Updated original fields logic in VISITS CTE
-- 05/09/23	   Shraddha Sejpal		 Added original_source_system_id filter in VISITS CTE
-- 05/22/23    Nutan Jagnade         Added future_hours_no_caregiver column
-- 29/11/23		Sanket Jain			 Removed CCSI as Shifted TO DW_2_0
-- 12/22/23		Shraddha Sejpal		 Removed Alliance as shifted to DW 2_0
--*****************************************************************************************************************************
--NOTE:			If Manual Refresh is required then SCHEDULE_METRICS_MONTHLY should be refreshed first before running this object
--*****************************************************************************************************************************
	INSERT OVERWRITE INTO REPORT.UTILIZATION_1_0
	(PERIOD_BEGIN_DATE, STATE, BRANCH_KEY, ORIGINAL_BRANCH_KEY, OFFICE_NUMBER, OFFICE_NAME, SUPERVISOR_KEY, SUPERVISOR_CODE,ABSOLUTE_FIRST_SERVICE_DATE,
	...
	VISITS_ALL, VISITS_CLEAN_SHIFTS, VISITS_NEED_MAINTENANCE, CLIENT_SERVED_FLAG,
		...
		,Absolute_Last_Service_Date, NPS,PARTNER_CONTRACT_SERVICE_KEY)
		--
		WITH VISITS AS (		
		...
		FIRST_VALUE(VISIT.ORIGINAL_BRANCH_KEY) OVER(PARTITION BY DATE_TRUNC(MONTH, VISIT.REPORT_DATE), VISIT.CLIENT_KEY, VISIT.CONTRACT_KEY ORDER BY CASE WHEN VISIT.CLIENT_KEY = VISIT.ORIGINAL_CLIENT_KEY THEN 1 ELSE 0 END, IFF(MAX(VISIT.CONFIRMED_FLAG) = ''YES'', 0, 1), MAX(VISIT.REPORT_DATE) DESC,  NVL(SUM(VISIT.HOURS_SERVED), 0) DESC) ORIGINAL_BRANCH_KEY,		--
	...
	FROM INTEGRATION.FACT_VISIT_MERGED AS VISIT WHERE VISIT.ORIGINAL_SOURCE_SYSTEM_ID NOT IN (4,7,17,13,9,3,19,8,16,1,2) 
	--WHERE VISIT.CONFIRMED_FLAG = ''YES''  -- Only confirmed visits 
	...
	--AND INVOICE.INVOICE_STATUS IN (''PAID'',''BILLED'',''PARTIAL PAY'') --Modified on 30/05/23 By Pinkal
	...
	FROM INTEGRATION.FACT_INTAKE_MERGED INTAKE WHERE ORIGINAL_SOURCE_SYSTEM_ID NOT IN (4,7,17,13,9,3,19,8,16,1,2)
...
--DERIVED_LAST_SERVICE_DATE,
	...
	0 as ETL_DELETED_FLAG, Absolute_Last_Service_Date, NPS, NULL AS PARTNER_CONTRACT_SERVICE_KEY
