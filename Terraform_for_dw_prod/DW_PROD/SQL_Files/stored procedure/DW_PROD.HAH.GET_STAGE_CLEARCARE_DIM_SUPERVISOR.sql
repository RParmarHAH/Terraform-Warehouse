CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_CLEARCARE_DIM_SUPERVISOR("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
FROM DISC_PROD.CLEARCARE.PROFILE_PATIENT_AGENCYLOCATION PP
WHERE
PP.ID IN (SELECT DISTINCT PRIMARY_MANAGER_ID
FROM DISC_PROD.CLEARCARE.PROFILE_PATIENT_AGENCYLOCATION) OR
PP.ID IN (SELECT DISTINCT SECONDARY_MANAGER_ID
FROM DISC_PROD.CLEARCARE.PROFILE_PATIENT_AGENCYLOCATION)
