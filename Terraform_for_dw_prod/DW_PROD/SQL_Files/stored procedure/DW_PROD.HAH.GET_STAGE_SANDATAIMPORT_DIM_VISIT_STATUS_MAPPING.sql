CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_SANDATAIMPORT_DIM_VISIT_STATUS_MAPPING("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
SELECT AGENCYID,SCHEDULEID, 
   CASE WHEN SV.STATUS =''01'' AND SV.STAFFAGENCYID IS NULL AND SV.SCHEDULEDATE > CURRENT_DATE() THEN ''01OPENFUTURE'' 
		...
		ELSE SV.STATUS END
		AS DERIVED_DISC_STATUS FROM DISC_PROD.BI_REPOSITORY.SANDATAVISITS SV
		WHERE AGENCYID =''8485''
)
SELECT DISTINCT
