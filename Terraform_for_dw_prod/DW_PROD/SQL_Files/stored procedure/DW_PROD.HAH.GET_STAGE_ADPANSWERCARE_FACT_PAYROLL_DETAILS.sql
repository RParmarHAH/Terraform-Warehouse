CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_ADPANSWERCARE_FACT_PAYROLL_DETAILS("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
WITH EMPLOYEE AS (
	...
	FROM DISC_DEDUPE_PROD.ADP.ANSWERCARE_EMPLOYEE_MASTER_LIST
	UNION
	SELECT
		ID, MASTER_ID, MD5(''ADP'' || ''-'' || MASTER_ID || ''ANSWERCARE'')
	FROM DISC_DEDUPE_PROD.ADP.ANSWERCARE_EMPLOYEE_MATCH_LIST
	WHERE ID NOT IN (SELECT DISTINCT ID FROM DISC_DEDUPE_PROD.ADP.ANSWERCARE_EMPLOYEE_MASTER_LIST)
)
SELECT DISTINCT
...
FROM DISC_PROD.ADP.ANSWERCARE_PAYROLL_DETAIL AS APD
...
LEFT JOIN DISC_DEDUPE_PROD.ADP.ANSWERCARE_EMPLOYEE_MASTER_LIST EML
	ON EML.EMPLOYEE_SSN = REPLACE(APD.TAX_ID, ''-'')
LEFT JOIN DW_PROD.HAH.DIM_EMPLOYEE CDE
