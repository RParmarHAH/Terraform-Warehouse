CREATE OR REPLACE PROCEDURE DW_PROD.REPORT.GET_REPORT_EMPLOYEE_DERIVED_MONTHLY_VISIT_METRICS("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--***********************************************************************************************************************************
INSERT OVERWRITE INTO DW_PROD.Report.EMPLOYEE_DERIVED_MONTHLY_VISIT_METRICS
...
JOIN DW_PROD.INTEGRATION.FACT_AMS_PAYROLL_MAPPING AMS_PAYROL
...
FROM 
DW_PROD.INTEGRATION.FACT_PAYROLL_MERGED 
...
FROM 
DW_PROD.INTEGRATION.FACT_PAYROLL_DETAILS_MERGED 
...
JOIN DW_PROD.HAH.DIM_CONTRACT AS C ON C.CONTRACT_KEY = VIS.CONTRACT_KEY
WHERE VIS.SERVICE_DATE BETWEEN (SELECT MIN( Calendar_Month) FROM Calendar_Months) AND (SELECT MAX( Calendar_Month) FROM Calendar_Months)
...
LEFT JOIN DW_PROD.REPORT.EMPLOYEE_DERIVED_METRICS EDM ON EDM.EMPLOYEE_KEY = ellm.Employee_Key
...
LEFT JOIN (SELECT DISTINCT UPPER(TRIM(DFE.DBNAME)) SYSTEM_CODE, UPPER(TRIM(DFE.NUMBER)) EMPLOYEE_ID FROM DISC_DEDUPE_PROD.GPSYNCDATA.EMPLOYEE_MASTER_LIST M  
	LEFT JOIN DISC_PROD.GPSYNCDATA.Paystub_Master AS m1 ON m1.DB||''-''||m1.EMPLOYEEID = M.MASTER_ID
	LEFT JOIN DISC_PROD.GPSYNCDATA.Paystub_DetailLine dl1 ON m1.CheckID = dl1.CheckID
	LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DfEmployees AS dfe ON TRIM( dfe.PayrollID) = TRIM( M.Employee_ID) AND TRIM( dfe.DBName) = TRIM( M.Office_State) 
