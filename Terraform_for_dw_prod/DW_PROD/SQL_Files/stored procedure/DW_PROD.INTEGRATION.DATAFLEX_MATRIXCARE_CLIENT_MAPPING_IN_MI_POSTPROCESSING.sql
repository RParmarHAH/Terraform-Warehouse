CREATE OR REPLACE PROCEDURE DW_PROD.INTEGRATION.DATAFLEX_MATRIXCARE_CLIENT_MAPPING_IN_MI_POSTPROCESSING("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INSERT OVERWRITE INTO DW_PROD.INTEGRATION.DATAFLEX_MATRIXCARE_CLIENT_MAPPING_IN_MI
WITH DC AS 
(
...
FROM DC
LEFT JOIN (SELECT SSN FROM DISC_PROD.DATAFLEXSYNCDATA.DFCLIENTS WHERE DBNAME IN (''IN'')) D ON MD5(D.SSN)=DC.client_pid
JOIN DC as MC
LEFT JOIN (SELECT SSN FROM DISC_PROD.DATAFLEXSYNCDATA.DFCLIENTS WHERE DBNAME IN (''IN'')) M ON MD5(M.SSN)=MC.client_pid
