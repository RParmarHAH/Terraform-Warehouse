CREATE OR REPLACE PROCEDURE DW_PROD.INTEGRATION.GET_MASTER_DIM_EMPLOYEE_MICH_MERGED_DEDUPE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
(
    ...
    CLUSTER_ID
    FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_MICH_RL_OUTPUT
...
,NEW AS (
    SELECT * FROM (
        SELECT
            ...
            ,NYO.CLUSTER_ID 
        FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_MICH_RL_OUTPUT NYO
        INNER JOIN LWD_NEW 
...
FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_MICH_RL_OUTPUT R
LEFT JOIN OLD O 
...
FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_MICH_RL_OUTPUT U
