CREATE OR REPLACE PROCEDURE DW_PROD.PUBLIC.GET_STAGE_SANDATAIMPORT_FACT_EVV_VISIT_DEV("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
RETURNS VARCHAR(16777216)
LANGUAGE JAVASCRIPT
EXECUTE AS OWNER
AS '
        var sql = `
    INSERT OVERWRITE INTO STAGE.SANDATAIMPORT_FACT_EVV_VISIT_DEV
WITH avg_bill_rate as (
	SELECT agencyid,admissionid,AVG(RATE) as BILL_RATE 
	FROM DISC_DEV.SANDATAIMPORT.SANDATA_SCHEDULESCLIENTS
...
FROM DISC_DEV.BI_REPOSITORY.SANDATAVISITS SV
--LEFT JOIN DISC_DEV.BI_REPOSITORY.EXTERNALIDS EID
...
LEFT JOIN DISC_DEV.SANDATAIMPORT.SANDATA_CLIENTADMISSIONS CAD
...
LEFT JOIN DISC_DEV.SANDATAIMPORT.SANDATA_SCHEDULES sch ON SV.AGENCYID = sch.AGENCYID AND SV.SCHEDULEID = sch.SCHEDULEID 
LEFT JOIN DISC_DEV.SANDATAIMPORT.SANDATA_REJECTIONCODES r ON sch.REJECTIONCODE = r.code AND sch.agencyid = r.AGENCYID 
-------Employee_Key-----
LEFT JOIN DISC_DEV.DATAFLEXSYNCDATA.DFEMPLOYEES AS EMPLOYEE
    ON EMPLOYEE.DBNAME = S.STATE_ISO_CODE AND EMPLOYEE.PAYROLLIDINDEXNO = try_cast(SV.STAFFAGENCYID as integer)
-------Contract_Key-----
LEFT JOIN DISC_DEV.SANDATAEXCHANGE.OPENSAM_CLIENTADMISSIONTYPES CAT
    ON CAT.HAHDFDB = S.STATE_ISO_CODE AND CAT.ADMISSIONTYPEID = SV.ADMISSIONTYPE AND CAT.DEFAULTSERVICE = SV.SERVICEID
LEFT JOIN DISC_DEV.DATAFLEXSYNCDATA.DFCONTRACTS AS CONTRACT
    ON CONTRACT.DBNAME = CAT.HAHDFDB AND CONTRACT.CONTRACTCODE = CAT.CONTRACTCODE
-------Supervisor_Key----
LEFT JOIN DISC_DEV.SANDATAIMPORT.SANDATA_CLIENTS C
    ON C.AGENCYID = CAD.AGENCYID AND C.CLIENTID = CAD.CLIENTID
LEFT JOIN DISC_DEV.DATAFLEXSYNCDATA.DFCLIENTS AS DFCLIENT
    ON DFCLIENT.DBNAME = S.STATE_ISO_CODE AND DFCLIENT."NUMBER" = TRY_CAST(C.OTHERID AS INTEGER)
LEFT JOIN DISC_DEV.DATAFLEXSYNCDATA.DFCLIENTSUPERVISORS DFCLIENTSUPERVISOR
...
-------Branch_Key------
LEFT JOIN DISC_DEV.DATAFLEXSYNCDATA.DFOFFICES AS OFFICE
...
,44,45,46,47,48,49,50,51,52;`;
 ...
 var sql = `MERGE INTO dw_dev.STAGE.SANDATAIMPORT_FACT_EVV_VISIT_DEV AS TARGET USING (
WITH clu as (
    select
    ...
    from DISC_DEV.SANDATAIMPORT.SANDATA_CLIENTS
    group by 1,2
),
max_client as (
...
SELECT
    ...
    CURRENT_USER as ETL_LAST_UPDATED_BY,
    0 as ETL_DELETED_FLAG
FROM DISC_DEV.SANDATAIMPORT.SANDATA_VISITS f
left join clu clu
...
LEFT JOIN DISC_DEV.SANDATAIMPORT.SANDATA_CLIENTADMISSIONS ClientAdmissions
...
LEFT JOIN DISC_DEV.SANDATAIMPORT.SANDATA_SCHEDULES sch ON f.AGENCYID = sch.AGENCYID AND f.SCHEDULEID = sch.SCHEDULEID
LEFT JOIN DISC_DEV.SANDATAIMPORT.SANDATA_REJECTIONCODES r ON sch.REJECTIONCODE = r.code AND sch.agencyid = r.AGENCYID 
LEFT JOIN (SELECT AGENCYID,avg(try_to_number(unittype)) AS unittype,scheduleid,admissionid,avg(rate) as bill_rate
 from DISC_DEV.SANDATAIMPORT.SANDATA_SCHEDULESCLIENTS 
...
WHEN NOT MATCHED THEN
    ...
    CREATE OR REPLACE PROCEDURE DW_DEV.HAH.MERGE_STAGE_SANDATAIMPORT_FACT_EVV_VISIT_DEV()
    RETURNS string 
    LANGUAGE javascript
    AS
    $$
    ...
    try {
      var sqlCmd = `
    MERGE INTO HAH.FACT_EVV_VISIT_DEV TGT 
USING STAGE.SANDATAIMPORT_FACT_EVV_VISIT_DEV STAGE 
