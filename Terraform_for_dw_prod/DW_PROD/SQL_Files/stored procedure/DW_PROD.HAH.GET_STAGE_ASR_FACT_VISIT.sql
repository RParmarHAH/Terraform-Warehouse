CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_ASR_FACT_VISIT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INSERT OVERWRITE INTO DW_PROD.STAGE.ASR_FACT_VISIT
WITH CLIENT AS
(
	SELECT * FROM 
	(
...
, SUPS AS (
	...
	FROM
		CLIENT CLI
	LEFT JOIN DISC_PROD.ASR.ASR_CUSTOMERS RAW ON
		TRIM(CLI.CUSTOMERID) = TRIM(RAW.CUSTOMERID)
	LEFT JOIN DISC_PROD.ASR.ASR_CUSTOMERS DERIVE ON
...
SELECT 
	...
	AND BILLING_TRAN_ID_NUMBER NOT IN (SELECT BILLINGTRANSACTIONID FROM DISC_PROD.ASR.ASR_RECEIVABLES)
)
,RES_PAY_CLI AS(
...
, 				  
AUTH AS (SELECT * FROM DISC_PROD.ASR.ASR_PLANOFSERVICE  S
...
LEFT JOIN DISC_PROD.ASR.BRANCH_MAPPING BM ON V.LOCATIONID = BM.OLD_LOCATION_ID
LEFT JOIN DISC_PROD.ASR.ASR_BILLINGCODES BILL ON V.BILLINGCODEID = BILL.BILLINGCODEID
LEFT JOIN DISC_PROD.ASR.ASR_SERVICE_TYPE_MAPPING SM ON P.SERVICETYPE = SM.SERVICE_TYPE_ID 
LEFT JOIN BILLING_STATUS AG ON V.SERVICEVISITACTUALID = AG.BILLINGTRANSACTIONID 
--LEFT JOIN DISC_PROD.ASR.ASR_RECEIVABLES  ON V.SERVICEVISITACTUALID = .SERVICEVISITACTUALID 
...
LEFT JOIN DISC_PROD.ASR.ASR_PAYERSOURCES AS PAYS ON PAYS.PAYERSOURCEID = P.PAYERSOURCEID 
