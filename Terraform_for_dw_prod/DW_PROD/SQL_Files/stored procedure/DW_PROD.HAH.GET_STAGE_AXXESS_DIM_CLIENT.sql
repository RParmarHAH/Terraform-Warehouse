CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_AXXESS_DIM_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--              LEFT JOIN DW_PROD."STAGE".AXXESS_DIM_VISIT_STATUS_MAPPING VSM 
...
FROM 
	(-- HOME HEALTH :
		SELECT
					...
					PV.BRANCH_ID
	FROM
		DISC_PROD.AXXESS.AXXESS_PATIENTVISITS PV
	...
	--  HOMECARE :			
			SELECT DISTINCT  
					...
					BRANCH_ID  
			FROM DISC_PROD.AXXESS.AXXESS_CAREPERIODS VF
...
-- HOME HEALTH :
	SELECT
		...
		FROM
		DISC_PROD.AXXESS.AXXESS_EPISODESNAPSHOTS ES
	    LEFT JOIN DISC_DEDUPE_PROD.AXXESS.CLIENT_MASTER_LIST CES ON ES.PATIENT_ID = CES.PATIENT_ID  -- CES.MASTER_ID
		LEFT JOIN DISC_PROD.AXXESS.AXXESS_BRANCHINFO BI ON ES.BRANCH_ID = BI.BRANCH_ID 
...
FROM
	DISC_PROD.AXXESS.AXXESS_CAREPERIODS CP
	INNER JOIN DISC_DEDUPE_PROD.AXXESS.CLIENT_MASTER_LIST CLI ON	CLI.PATIENT_ID = CP.CLIENT_ID
),
DISCHARGE_REASON 
AS 
(
...
--COALESCE (CLIENT_RACES, CLIENT_ETHNICITIES ) AS CLIENT_RACES,
 ...
 FROM DISC_PROD.AXXESS.AXXESS_CAREPERIODS CP 
...
-- COALESCE (RACE, ENTHNICITY) AS CLIENT_RACES, 
 ...
 FROM  DISC_PROD.AXXESS.AXXESS_PATIENTINFO CP
...
SELECT DISTINCT
    ...
    -- 	,TRY_TO_DATE(TRIM(BR.BEGIN_DATE ),''MM/DD/YYYY HH12:MI:SS AM +TZH:TZM'') AS BEGIN_DATE
		...
		FROM DISC_DEDUPE_PROD.AXXESS.CLIENT_MASTER_LIST CLI 
		...
		LEFT JOIN DISC_PROD.AXXESS.AXXESS_BRANCH_OFFICE_MAPPING O ON O.BRANCH_ID = TRIM(BR.BRANCH_ID );
RETURN ''SUCCESS'';
end;                        
';