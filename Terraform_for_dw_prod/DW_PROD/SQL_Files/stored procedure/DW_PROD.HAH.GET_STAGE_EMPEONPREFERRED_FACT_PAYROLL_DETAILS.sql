CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_EMPEONPREFERRED_FACT_PAYROLL_DETAILS("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
PAYCODE AS (
	...
	FROM
		DISC_PROD.EMPEONPREFERRED.CDED
	UNION
	SELECT DISTINCT 
		CO AS CO ,
		ECODE AS PAYCODE,
...
REPORT_DATE AS (
	...
	FROM DISC_PROD.EMPEONPREFERRED.EPAYHIST P
...
FROM DISC_PROD.EMPEONPREFERRED.EPAYHISTDETAIL PD
LEFT JOIN REPORT_DATE RD ON
	RD.TRANS = PD.TRANS
LEFT JOIN PAYCODE P ON
	P.PAYCODE = PD.DETCODE
	AND P.CO = RD.CO
LEFT JOIN DISC_PROD.EMPEONPREFERRED.EPAYHIST PH
