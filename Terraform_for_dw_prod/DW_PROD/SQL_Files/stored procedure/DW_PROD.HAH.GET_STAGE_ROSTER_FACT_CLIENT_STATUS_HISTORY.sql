CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_ROSTER_FACT_CLIENT_STATUS_HISTORY("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO DW_PROD.STAGE.ROSTER_FACT_CLIENT_STATUS_HISTORY
...
FROM DISC_PROD.MOLINA.MOLINA_SOURCE_DATA MDC
UNION ALL
SELECT MD5(''CENTENE''  || ''-'' || TRIM(MDC.EXTERNAL_ID)|| ''-'' || TO_DATE(REGEXP_SUBSTR(MDC.FILENAME , ''[0-9]{8}''), ''YYYYMMDD'') ||''-'' || ''CENTENE'') AS FACT_CLIENT_STATUS_HISTORY_KEY
		...
		, CONVERT_TIMEZONE(''UTC'', CURRENT_TIMESTAMP)::TIMESTAMP_NTZ AS ETL_LAST_UPDATED_DATE
		, CURRENT_USER AS ETL_LAST_UPDATED_BY
FROM DISC_PROD.CENTENE.CENTENE_SOURCE_DATA MDC
UNION ALL
SELECT MD5(''AMERIHEALTH''  || ''-'' || TRIM(MDC.EXTERNAL_ID)|| ''-'' || TO_DATE(REGEXP_SUBSTR(MDC.FILENAME , ''[0-9]{8}''), ''YYYYMMDD'') ||''-'' || ''AMERIHEALTH'') AS FACT_CLIENT_STATUS_HISTORY_KEY
		...
		, CONVERT_TIMEZONE(''UTC'', CURRENT_TIMESTAMP)::TIMESTAMP_NTZ AS ETL_LAST_UPDATED_DATE
		, CURRENT_USER AS ETL_LAST_UPDATED_BY 
FROM DISC_PROD.AMERIHEALTH.AMERIHEALTH_SOURCE_DATA MDC;
RETURN ''SUCCESS'';
END;
';