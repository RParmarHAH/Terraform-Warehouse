CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_EDISON_DIM_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
(
	...
	FROM DISC_PROD.HHAEXCHANGEEDISON.PATIENTSTATUSCHANGEACTIVITY 
	),
...
FROM DISC_PROD.HHAEXCHANGEEDISON.PATIENTCONTRACTCHANGES)
,
DERIVED_STATUS AS 
( 
...
FROM DISC_DEDUPE_PROD.HHAEXCHANGEEDISON.CLIENT_MASTER_LIST M
LEFT JOIN SERVICE_DATE D ON D.PATIENTID = M.PATIENTID
--LEFT JOIN BEGIN_DATE S ON S.PATIENTID = M.PATIENTID 
LEFT JOIN DISC_PROD.HHAEXCHANGEEDISON.OFFICE_OFFICES_REPL  O ON M.OFFICEID = O.OFFICEID
...
LEFT JOIN DISC_PROD.HHAEXCHANGEEDISON.REFERRALSOURCE RS ON RS.REFERRALSOURCEID = M.REFERRALSOURCEID
