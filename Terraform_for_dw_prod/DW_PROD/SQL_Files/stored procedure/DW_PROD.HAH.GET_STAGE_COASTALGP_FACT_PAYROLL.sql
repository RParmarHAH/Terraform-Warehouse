CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_COASTALGP_FACT_PAYROLL("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INSERT OVERWRITE INTO STAGE.COASTALGP_FACT_PAYROLL
...
(
	...
	WHERE T.ETL_LAST_UPDATED_DATE >= ''1900-01-01''::timestamp_ntz
)
,srv 
AS 
(
...
(
	SELECT 
		pcb.DB 
		,CASE TRIM(pcb.DB) WHEN ''ALTRU'' THEN ''ALTRU'' WHEN ''SHC'' THEN ''SHC'' END AS SYSTEM_CODE
