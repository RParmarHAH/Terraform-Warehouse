CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_PREFERRED_DIM_CONTRACT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
(
	...
	FROM DISC_PROD.HHAEXCHANGEPREFERRED.VisitInfo_REPL VI
	JOIN DISC_PROD.HHAEXCHANGEPREFERRED.SERVICETYPE_REPL ST ON ST.SERVICETYPEID = VI.SERVICETYPEID
	    	AND ST.AGENCYID = VI.AGENCYID
	JOIN DISC_PROD.HHAEXCHANGEPREFERRED.SERVICECATEGORY_REPL SC ON SC.SERVICECATEGORYID = VI.SERVICECATEGORYID
	...
	LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.SERVICECODES S ON S.SERVICECODEID = VI.PRIMARYSERVICECODEID
	        AND S.AGENCYID = VI.AGENCYID
	LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.CONTRACTS C ON S.CONTRACTID = C.CONTRACTID
	        AND C.AGENCYID = VI.AGENCYID
	JOIN DISC_PROD.HHAEXCHANGEPREFERRED.CONTRACTRATES CR ON CR.CONTRACTID = C.CONTRACTID
	    	AND CR.AGENCYID = C.AGENCYID
	        AND CR.SERVICECODEID = S.SERVICECODEID
	        AND CR.TODATE :: DATE > GETDATE()
	        AND CR.FROMDATE :: DATE <= GETDATE()
	LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.TBLVISITS_REPL TV ON TV.VISITID = VI.VISITID
	        AND TV.AGENCYID = VI.AGENCYID
	LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.VISITCONTRACTS_REPL VC ON VC.VISITID = TV.VISITID
	        AND TV.AGENCYID = VC.AGENCYID
	LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.STAGE_PATIENTCONTRACTS_REPL PC ON PC.PATIENTCONTRACTID = VC.CONTRACTID
...
FROM  DISC_PROD.HHAEXCHANGEPREFERRED.SERVICECODES SC
INNER JOIN DISC_PROD.HHAEXCHANGEPREFERRED.AGENCY_MAPPING AGENCY_CONFIG
	ON AGENCY_CONFIG.AGENCYID = SC.AGENCYID
		AND AGENCY_CONFIG.ISACTIVE = TRUE
LEFT JOIN PREFERRED_REVENUE_SEG PS
		ON PS.CONTRACTID = SC.CONTRACTID 
LEFT JOIN  DISC_PROD.HHAEXCHANGEPREFERRED.CONTRACTS C ON SC.CONTRACTID = C.CONTRACTID
    AND SC.AGENCYID = C.AGENCYID
LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.CONTRACTRATES CR ON CR.CONTRACTID = SC.CONTRACTID
