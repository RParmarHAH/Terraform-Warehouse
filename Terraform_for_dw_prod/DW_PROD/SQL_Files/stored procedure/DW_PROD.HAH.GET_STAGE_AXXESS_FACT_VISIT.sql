CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_AXXESS_FACT_VISIT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
-- 11/02/23    Deepen Gajjar         Updated hours_served logic
-- --------    -------------------   -----------------------------------------------------------------------------------------------
INSERT OVERWRITE INTO STAGE.AXXESS_FACT_VISIT
...
(
  SELECT * FROM 
	...
	(
		...
		WHERE EMPLOYEE_ID NOT IN (SELECT EMPLOYEE_ID FROM DISC_DEDUPE_PROD.AXXESS.EMPLOYEE_MASTER_LIST)		
	)
),
STATUS_MAPPING AS 
(
...
LEFT JOIN DISC_PROD.AXXESS.AXXESS_FINANCIALTRANSACTIONS FT ON TD.CLAIM_ID =FT.CLAIM_ID and  FT.TRANSACTION_ID IS NOT NULL--AND FT.TRANSACTION_DATE =FIN.TRANSACTION_DATE
)T
),
DERIVED_VISIT  AS
(
...
--		,MD5(''PRIME'' || ''-'' || IFNULL(UPPER(TRIM(PV.EPISODE_PRIMARY_INSURANCE)),''UNKNOWN'') || ''-'' || ''AXXESS'') AS CONTRACT_KEY  --Pankti M.
		...
		,MD5(''PRIME'' || ''-'' || UPPER(TRIM(EMP.MASTER_ID)) || ''-'' || ''AXXESS'') AS SCHEDULED_EMPLOYEE_KEY		--Pinkal
...
FROM DISC_PROD.AXXESS.VW_AXXESS_HH_VISITS_UPPER PV
LEFT JOIN DISC_PROD.AXXESS.AXXESS_ARS A ON A.ID = PV.FACT_AR_ID
...
LEFT JOIN DISC_PROD.AXXESS.AXXESS_VISIT_STATUS_MAPPING VM ON PV.VISIT_STATUS = VM.SCHEDULED_TASK_STATUS AND VM.REVENUE_CATEGORY = ''HH''
...
--      )/60,0) AS HOURS_SERVED,
      ,NVL(iff(COALESCE(TIMEDIFF(MINUTE,COALESCE(TRY_to_TIMESTAMP(TD.VISIT_START,''DD-MM-YYYY HH:MI:ss''),TRY_to_TIMESTAMP(TD.VISIT_START,''MM/DD/YYYY HH:MI:ss''))::DATETIME ,
      COALESCE(TRY_to_TIMESTAMP(TD.VISIT_END,''DD-MM-YYYY HH:MI:ss''),TRY_to_TIMESTAMP(TD.VISIT_END,''MM/DD/YYYY HH:MI:ss''))::DATETIME 
      )/60,0) >= 0,
        COALESCE(TIMEDIFF(MINUTE,COALESCE(TRY_to_TIMESTAMP(TD.VISIT_START,''DD-MM-YYYY HH:MI:ss''),TRY_to_TIMESTAMP(TD.VISIT_START,''MM/DD/YYYY HH:MI:ss''))::DATETIME ,
      COALESCE(TRY_to_TIMESTAMP(TD.VISIT_END,''DD-MM-YYYY HH:MI:ss''),TRY_to_TIMESTAMP(TD.VISIT_END,''MM/DD/YYYY HH:MI:ss''))::DATETIME 
      )/60,0),
        0
    ),0) AS HOURS_SERVED		--Modified by Pinkal on 03/02/23
	...
	,NULL AS CONFIRMED_FLAG
FROM
DISC_PROD.AXXESS.VW_AXXESS_HC_TASKDETAILS PV
LEFT JOIN DISC_PROD.AXXESS.AXXESS_FINANCIALTRANSACTIONS FIN ON FIN.CLAIM_ID = PV.CLAIM_ID 
LEFT JOIN DISC_PROD.AXXESS.AXXESS_TASKDETAILS TD ON TD.SCHEDULED_TASK_ID = PV.SCHEDULED_TASK_ID 
...
LEFT JOIN EMP_DEDUPE EMP ON EMP.EMPLOYEE_ID=PV.CLINICIAN_ID
LEFT JOIN DISC_PROD.AXXESS.AXXESS_VISIT_STATUS_MAPPING VM ON PV.SCHEDULED_TASK_STATUS = VM.SCHEDULED_TASK_STATUS AND VM.REVENUE_CATEGORY = ''HC''
...
LEFT JOIN DW_PROD."STAGE".AXXESS_DIM_VISIT_STATUS_MAPPING VSM ON VSM.VISIT_STATUS_KEY = SCH.VISIT_STATUS_KEY
