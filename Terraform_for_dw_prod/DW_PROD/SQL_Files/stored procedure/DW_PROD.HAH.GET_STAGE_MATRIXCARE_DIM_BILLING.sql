CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_MATRIXCARE_DIM_BILLING("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
COALESCE(s2.SVCC_NAME, ''UNKNOWN'') AS BILL_NAME,
	...
	s1.SVCC_ACTIVE AS ACTIVE_FLAG,
	 ...
	 FROM DISC_PROD.MATRIXCARE.STVHC_T_SERVICECODE s1
	 left join DISC_PROD.MATRIXCARE.STVHC_T_SERVICECODE s2 on s2.svcc_id = s1.svcc_categoryid
	 LEFT JOIN (SELECT DISTINCT RATE_SERVICECODEID, RATE_RATE1, RATE_EFFECTIVESTARTDATE, RATE_EFFECTIVEENDDATE 
	 		FROM DISC_PROD.MATRIXCARE.STVHC_T_RATES R
