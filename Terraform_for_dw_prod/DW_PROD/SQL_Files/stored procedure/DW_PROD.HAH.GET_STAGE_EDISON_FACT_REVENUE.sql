CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_EDISON_FACT_REVENUE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
(
	...
	(
		...
		--,BRANCHID,OFFICEID
		FROM DISC_DEDUPE_PROD.HHAEXCHANGEEDISON.EMPLOYEE_MASTER_LIST
	)
	UNION
	SELECT * FROM
	(
		...
		--,BRANCHID,OFFICEID
		FROM DISC_DEDUPE_PROD.HHAEXCHANGEEDISON.EMPLOYEE_MATCH_LIST
...
,VISIT_DATA AS
( SELECT DISTINCT VISITID,AGENCYID,CONTRACTID FROM DISC_PROD.HHAEXCHANGEEDISON.VISITS WHERE AGENCYID = 155)
/*,
	MAXCOORDS AS
(
	SELECT
...
(
	...
	FROM
		CLIENT P
	JOIN DISC_PROD.HHAEXCHANGEEDISON.PROVIDERPATIENTCOORDINATORS PC1 ON
...
FROM 
DISC_PROD.HHAEXCHANGEEDISON.PAYMENTS PAYMENTS
WHERE AGENCYID = 155
...
,PAYERID,SUM(NVL(PAIDAMOUNT, 0) + NVL(ADJUSTMENT, 0) + NVL(TTADJUSTMENT, 0) + NVL(WRITEOFF, 0) + NVL(OTHERADJUSTMENT, 0))
 AS PAIDAMOUNT FROM DISC_PROD.HHAEXCHANGEEDISON.BILLING_PAIDVISITS_REPL 
...
SELECT	DISTINCT
	...
	LEFT JOIN DISC_PROD.HHAEXCHANGEEDISON.OFFICE_OFFICES_REPL O ON O.OFFICEID = INVOICE.OFFICEID
	INNER JOIN DISC_PROD.HHAEXCHANGEEDISON.TBLINVOICEHEADER_REPL INV_HEAD
	ON INVOICE.INVOICEHEADERID = INV_HEAD.INVOICEHEADERID
	INNER JOIN DISC_PROD.HHAEXCHANGEEDISON.tblVisits_REPL VISITS ON INVOICE.VISITID = VISITS.VISITID --INNER JOIN DUE TO THERE 8 VISIT ID IN tblinvoicedetails_REPL BUT NOT IN VisitContracts_REPL
...
INNER JOIN CLIENT C ON INVOICE.PATIENTID = C.PATIENTID
  ...
  DC.CLIENT_KEY = C.CLIENT_KEY
	...
	JOIN DISC_PROD.HHAEXCHANGEEDISON.VISITINFO_REPL VINFO
ON
