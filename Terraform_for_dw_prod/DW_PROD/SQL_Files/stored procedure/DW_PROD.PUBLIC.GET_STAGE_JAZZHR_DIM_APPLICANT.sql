CREATE OR REPLACE PROCEDURE DW_PROD.PUBLIC.GET_STAGE_JAZZHR_DIM_APPLICANT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INNER JOIN DISC_DEDUPE_PROD.JAZZHR.EMPLOYEE_MASTER_LIST M ON AD.ID = M.ID
`;
          try {
                snowflake.execute (
                    {sqlText: sql}
