CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_CLEARCARE_DIM_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
WITH CARE_DATES AS (
    ...
    FROM DISC_PROD.CLEARCARE.CARELOGS_CARELOG CARE
    LEFT JOIN DISC_PROD.CLEARCARE.PROFILE_PATIENT_AGENCYLOCATION CLIENT ON CLIENT.PATIENT_ID = CARE.PATIENT_ID
    GROUP BY 1
)
SELECT 
...
FROM DISC_DEDUPE_PROD.CLEARCARE.CLIENT_MASTER_LIST CL
LEFT JOIN DISC_PROD.CLEARCARE.profile_patient_agencylocation AS PR ON CL.PATIENT_ID = PR.PATIENT_ID
LEFT JOIN DISC_PROD.CLEARCARE.patient_patient AS PA ON PR.patient_id = PA.id 
LEFT JOIN DISC_PROD.CLEARCARE.AGENCY_AGENCYLOCATION AG ON AG.ID = PA.LOCATION_ID
LEFT JOIN DISC_PROD.CLEARCARE.profile_patient_agencylocation PM ON PM.ID = PR.PRIMARY_MANAGER_ID
LEFT JOIN DISC_PROD.CLEARCARE.profile_patient_agencylocation SM ON SM.ID = PR.SECONDARY_MANAGER_ID
