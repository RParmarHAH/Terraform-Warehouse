CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_DATAFLEXSYNCDATA_FACT_VISIT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
-- 09/20/23	   Sanket Jain			Added logic for parner contract service key and client service key. 
--*****************************************************************************************************************************

INSERT OVERWRITE INTO STAGE.DATAFLEXSYNCDATA_FACT_VISIT (VISIT_KEY,	REPORT_DATE, BRANCH_KEY, CLIENT_KEY, CONTRACT_KEY, SCHEDULED_EMPLOYEE_KEY, EMPLOYEE_KEY, 
SOURCE_SYSTEM_ID, SUPERVISOR_KEY, INVOICE_KEY, PAYROLL_KEY,CLIENT_SERVICE_KEY, PARTNER_CONTRACT_SERVICE_KEY,BILLING_KEY,SERVICE_DATE, PAYROLL_DATE,	BRANCH_NAME,
CLIENT_NUMBER, CONTRACT_CODE, SCHEDULED_EMPLOYEE_ID, EMPLOYEE_ID, SYSTEM_CODE, SUPERVISOR_CODE, INVOICE_ID, CHEQUE_NUMBER, CLEAN_SHIFT_FLAG, SCHEDULE_STATUS_NAME,
SCHEDULE_STATUS_CODE, SCHEDULE_STATUS_DESCRIPTION, VISIT_STATUS_NAME, VISIT_STATUS_CODE, VISIT_STATUS_DESCRIPTION, INVOICE_STATUS_NAME, INVOICE_STATUS_CODE,
INVOICE_STATUS_DESCRIPTION, PAYROLL_STATUS_NAME, PAYROLL_STATUS_CODE, PAYROLL_STATUS_DESCRIPTION, CANCEL_REASON_CODE, CANCEL_REASON_DESCRIPTION, CANCEL_REASON_NOTES,
EXCEPTION_REASON_INDICATOR,	RESOLUTION_CODE, RESOLUTION_DESCRIPTION, REJECTION_CODE, REJECTION_DESCRIPTION, BILL_CODE, BILL_RATE, BILL_UNITS_SERVED, BILL_UNIT_TYPE,
OVERHEAD_RATE, SCHEDULE_TIMEIN,	SCHEDULE_TIMEOUT, SCHEDULE_DURATION, ACTUAL_TIMEIN, ACTUAL_TIMEOUT, ACTUAL_DURATION,	ADJUSTED_TIMEIN, ADJUSTED_TIMEOUT, ADJUSTED_DURATION,
HOURS_SERVED, COMMENTS, IS_EVV_FLAG, TIMESHEET_TYPE, TRACKING_ID, ETL_TASK_KEY, ETL_INSERTED_TASK_KEY, ETL_INSERTED_DATE,	ETL_INSERTED_BY, ETL_LAST_UPDATED_DATE,
...
---
		...
		INNER JOIN DISC_PROD.SANDATAIMPORT.SANDATA_VISITS SV 
			ON SV.SCHEDULEID = SCH.SCHEDULEID 
			AND SV.AGENCYID = SCH.AGENCYID
		LEFT JOIN DISC_PROD.SANDATAIMPORT.SANDATA_REJECTIONCODES r 
...
(
	SELECT DISTINCT
		...
		NULL AS PAYROLL_KEY,
		NULL AS INVOICE_ID,
		md5(NVL(F.DBNAME, '''') || ''-'' || COALESCE(c2.MASTER_CLIENT_NUMBER, ts.CLIENTNUMBER,-1) || ''-'' || AUTH.MASTER_RN || ''-'' || NVL(TS.CONTRACTCODE,'''') || ''-'' || TRIM(CON.REVENUESUBCATEGORY)|| ''-'' || ''DATAFLEXSYNCDATA'') AS CLIENT_SERVICE_KEY,
		nvl(n3.PARTNER_CONTRACT_SERVICE_KEY,MD5(f.DBNAME || ''-'' || nvl(ts.CONTRACTCODE,''Unknown'') || ''-'' || nvl(TRIM(CON.REVENUESUBCATEGORY),''Unknown'') || ''-'' || ''DATAFLEXSYNCDATA'')) AS PARTNER_CONTRACT_SERVICE_KEY,
		NULL AS BILLING_KEY,
	    ...
	    md5(COALESCE(e.MASTER_DBNAME, f.DBNAME) || ''-'' || UPPER(COALESCE(e.MASTER_EMPLOYEE_NUMBER, ts.EMPLOYEENUMBER)) || ''-''  ||  ''1900-01-01'' || ''-'' || ''DATAFLEXSYNCDATA'') AS SCHEDULED_EMPLOYEE_KEY,
	    ...
	    COALESCE(c2.CLIENT_NUMBER,ts.CLIENTNUMBER, -1) AS CLIENT_NUMBER,
	    ts.CONTRACTCODE AS CONTRACT_CODE,
	    UPPER(COALESCE(e.EMPLOYEE_NUMBER,ts.EMPLOYEENUMBER, -1)) as EMPLOYEE_ID,
...
-------------------------------------------------------------------------------------------------------------------------------
		...
		SUM(Billrate_Contract_overheadrate.BILL_RATE)/COUNT(1) OVER 
			(PARTITION BY f.DBNAME,COALESCE(c2.CLIENT_NUMBER,ts.CLIENTNUMBER, -1),ts.CONTRACTCODE,DATE_TRUNC(''DAY'',f.SERVICEDATE))
	   		AS Overhead_Rate,
...
LEFT JOIN HAH.FACT_PARTNER_CONTRACT_SERVICE n3
	ON n3.CONTRACT_CODE = ts.CONTRACTCODE 
	AND n3.SYSTEM_CODE = ts.DBNAME
...
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFCONTRACTS CON
	ON CON.CONTRACTCODE = TS.CONTRACTCODE
	AND CON.DBNAME = TS.DBNAME
LEFT JOIN DW_PROD.STAGE.DATAFLEXSYNCDATA_DIM_DERIVED_AUTH_FROM_HISTORY_WITH_MASTER_DEDUP auth
	ON auth.DBNAME = TS1.DBNAME 
	AND AUTH.CLIENTNUMBER = TS1.CLIENTNUMBER 
	AND AUTH.CONTRACTCODE = TS1.CONTRACTCODE 
	AND f.SERVICEDATE BETWEEN AUTH.MASTER_MODIFIED_START_DATE AND AUTH.MASTER_MODIFIED_END_DATE  
	--LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFBILLCODES BC ON TS.BILLCODE = BC.BILLCODE AND BC.DBNAME = TS.DBNAME
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DERIVED_BILLRATE Billrate_Contract 
...
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DERIVED_BILLRATE Billrate_Contract_Client 
...
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DERIVED_BILLRATE Billrate_Contract_billcode 
...
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DERIVED_BILLRATE Billrate_Contract_overheadrate 
...
where (f.ETL_LAST_UPDATED_DATE >=:STR_CDC_START::timestamp_ntz
	...
	OR SV.ETL_LAST_UPDATED_DATE >=:STR_CDC_START::timestamp_ntz
	OR DFI.ETL_LAST_UPDATED_DATE >=:STR_CDC_START::timestamp_ntz)
	GROUP BY VISIT_KEY,F.PAID,F.BILLED,TS.PAIDFLAG,REPORT_DATE,BRANCH_KEY,CLIENT_KEY,CONTRACT_KEY,PARTNER_CONTRACT_SERVICE_KEY,SCHEDULED_EMPLOYEE_KEY ,EMPLOYEE_KEY
	,SUPERVISOR_KEY,SERVICE_DATE,
	PAYROLL_DATE, BRANCH_NAME, B.OFFICENAME, TS.OFFCENO, c2.MASTER_CLIENT_NUMBER, c2.CLIENT_NUMBER, ts.CLIENTNUMBER, TS.CONTRACTCODE, e.MASTER_EMPLOYEE_NUMBER, e.EMPLOYEE_NUMBER, ts.EMPLOYEENUMBER
...
STATUS_MAPPING.PAYROLL_STATUS_CODE,STATUS_MAPPING.PAYROLL_STATUS_NAME,STATUS_MAPPING.PAYROLL_STATUS_DESCRIPTION, STATUS_MAPPING.CONFIRMED_FLAG,AUTH.MASTER_RN,CON.REVENUESUBCATEGORY
)
...
FROM DISC_PROD.DATAFLEXSYNCDATA.DFSUPERVISORS QUALIFY rno = 1
)
, SD_STATUS AS (
...
--		null as CLIENT_SERVICE_KEY,
		md5(NVL(SD.DFDBNAME, '''') || ''-'' || COALESCE(c2.MASTER_CLIENT_NUMBER, SD.CLIENTOTHERID,-1) || ''-'' || AUTH.MASTER_RN || ''-'' || NVL(SD.CONTRACTCODE,'''') || ''-'' || TRIM(CON.REVENUESUBCATEGORY)|| ''-'' || ''DATAFLEXSYNCDATA'') AS CLIENT_SERVICE_KEY,
		nvl(n3.PARTNER_CONTRACT_SERVICE_KEY,md5(SD.DFDBNAME || ''-'' || nvl(SD.CONTRACTCODE,''Unknown'') || ''-'' || nvl(TRIM(CON.REVENUESUBCATEGORY),''Unknown'')|| ''-'' || ''DATAFLEXSYNCDATA'')) AS PARTNER_CONTRACT_SERVICE_KEY,
		NULL AS BILLING_KEY,
		NULL AS CHEQUE_NUMBER,
		...
		md5(COALESCE(e.MASTER_DBNAME, SD.DFDBNAME) || ''-'' || UPPER(COALESCE(e.MASTER_EMPLOYEE_NUMBER, DE1.NUMBER)) || ''-''  ||  ''1900-01-01'' || ''-'' || ''DATAFLEXSYNCDATA'') AS SCHEDULED_EMPLOYEE_KEY,
	    ...
	    COALESCE(b.OfficeName, ''Unknown'') AS BRANCH_NAME,
	    COALESCE(c2.CLIENT_NUMBER,SD.CLIENTOTHERID, -1) AS CLIENT_NUMBER,
	    SD.CONTRACTCODE AS CONTRACT_CODE,
	    --
	    UPPER(COALESCE(e.EMPLOYEE_NUMBER,DE1.NUMBER, -1)) as EMPLOYEE_ID,
		EMPLOYEE_ID AS SCHEDULED_EMPLOYEE_ID,
	    --
...
-------------------------------------------------------------------------------------------------------------------------------
		...
		--
		SUM(Billrate_Contract_overheadrate.BILL_RATE)/COUNT(1) OVER 
			(PARTITION BY SD.DFDBNAME,COALESCE(c2.CLIENT_NUMBER,SD.CLIENTOTHERID, -1),SD.CONTRACTCODE,DATE_TRUNC(''DAY'',SD.SCHEDULEDATE))
	   		AS Overhead_Rate,
...
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFEMPLOYEES DE1
...
LEFT JOIN HAH.FACT_PARTNER_CONTRACT_SERVICE n3 
	ON n3.CONTRACT_CODE = SD.CONTRACTCODE 
	AND n3.SYSTEM_CODE = SD.DFDBNAME
	--
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFEMPLOYEES DE 							--  FOR Supervisor, 1st JOIN IS FROM Sandata visit TO client admission, 
...
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFCONTRACTS CON
	ON CON.CONTRACTCODE = SD.CONTRACTCODE
	AND CON.DBNAME = SD.DBNAME
LEFT JOIN DW_PROD.STAGE.DATAFLEXSYNCDATA_DIM_DERIVED_AUTH_FROM_HISTORY_WITH_MASTER_DEDUP auth
	ON auth.DBNAME = SD.DFDBNAME 
	AND AUTH.CLIENTNUMBER = SD.CLIENTOTHERID 
	AND AUTH.CONTRACTCODE = SD.CONTRACTCODE 
	AND SD.SCHEDULEDATE BETWEEN AUTH.MASTER_MODIFIED_START_DATE AND AUTH.MASTER_MODIFIED_END_DATE 
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DERIVED_BILLRATE Billrate_Contract 
...
where (SD.ETL_LAST_UPDATED_DATE >=:STR_CDC_START::timestamp_ntz
	...
	GROUP BY SD.DFDBNAME, c2.MASTER_CLIENT_NUMBER, c2.CLIENT_NUMBER, SD.CLIENTOTHERID,SD.CONTRACTCODE,SD.SCHEDULEDATE, SD.SCHEDULEID, B2.BRANCH_KEY, SD.INTID, B.OFFICENAME
	,C2.MASTER_DBNAME, N2.CONTRACT_KEY,N3.PARTNER_CONTRACT_SERVICE_KEY, E.MASTER_DBNAME, e.MASTER_EMPLOYEE_NUMBER, e.EMPLOYEE_NUMBER,DE1.NUMBER, SD.COMMENTS ,S2.SUPERVISOR_KEY, SP.CODE, SP2.CODE, SD.BILLCODE
	, Billrate_Contract.BILL_RATE, Billrate_Contract_Client.BILL_RATE, Billrate_Contract_billcode.BILL_RATE, SD.SCHEDULE_TIMEIN, SD.SCHEDULE_TIMEOUT,
	    ...
	    ,SD.HOURS_SERVED, SM.VISIT_STATUS_CODE, SM.VISIT_STATUS_NAME, SM.VISIT_STATUS_DESCRIPTION, SM.SCHEDULE_STATUS_NAME, SM.SCHEDULE_STATUS_CODE, SM.SCHEDULE_STATUS_DESCRIPTION,AUTH.MASTER_RN,CON.REVENUESUBCATEGORY
	) 
, FINAL AS 
(
	SELECT L.*
...
SOURCE_SYSTEM_ID, SUPERVISOR_KEY, INVOICE_KEY, PAYROLL_KEY, CLIENT_SERVICE_KEY, PARTNER_CONTRACT_SERVICE_KEY,BILLING_KEY,SERVICE_DATE, PAYROLL_DATE, BRANCH_NAME, CLIENT_NUMBER,
CONTRACT_CODE, SCHEDULED_EMPLOYEE_ID, EMPLOYEE_ID, SYSTEM_CODE, SUPERVISOR_CODE, INVOICE_ID, CHEQUE_NUMBER, CLEAN_SHIFT_FLAG, SCHEDULE_STATUS_NAME, SCHEDULE_STATUS_CODE,
SCHEDULE_STATUS_DESCRIPTION,VISIT_STATUS_NAME, VISIT_STATUS_CODE, VISIT_STATUS_DESCRIPTION, INVOICE_STATUS_NAME, INVOICE_STATUS_CODE, INVOICE_STATUS_DESCRIPTION,
PAYROLL_STATUS_NAME, PAYROLL_STATUS_CODE, PAYROLL_STATUS_DESCRIPTION, CANCEL_REASON_CODE, CANCEL_REASON_DESCRIPTION, CANCEL_REASON_NOTES, EXCEPTION_REASON_INDICATOR,
RESOLUTION_CODE, RESOLUTION_DESCRIPTION, REJECTION_CODE,	REJECTION_DESCRIPTION, BILL_CODE, BILL_RATE, BILL_UNITS_SERVED, BILL_UNIT_TYPE,	OVERHEAD_RATE, SCHEDULE_TIMEIN,
SCHEDULE_TIMEOUT, SCHEDULE_DURATION, ACTUAL_TIMEIN, ACTUAL_TIMEOUT, ACTUAL_DURATION,	ADJUSTED_TIMEIN, ADJUSTED_TIMEOUT, ADJUSTED_DURATION, HOURS_SERVED,	COMMENTS, IS_EVV_FLAG,
TIMESHEET_TYPE, TRACKING_ID,ETL_TASK_KEY, ETL_INSERTED_TASK_KEY, ETL_INSERTED_DATE,	ETL_INSERTED_BY, ETL_LAST_UPDATED_DATE,	ETL_LAST_UPDATED_BY, ETL_DELETED_FLAG, CONFIRMED_FLAG 
FROM FINAL;
