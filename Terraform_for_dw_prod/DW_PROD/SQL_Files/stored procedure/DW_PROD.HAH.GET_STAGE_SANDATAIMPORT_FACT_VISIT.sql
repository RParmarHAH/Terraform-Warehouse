CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_SANDATAIMPORT_FACT_VISIT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
(
	...
	FROM DISC_PROD.SANDATAIMPORT.SANDATA_SCHEDULESCLIENTS sc
	LEFT JOIN DISC_PROD.SANDATAIMPORT.SANDATA_AUTHORIZATIONS a ON sc.AUTHORIZATIONID = a.AUTHID AND sc.AGENCYID = a.AGENCYID 
...
(
	...
	FROM DISC_PROD.SANDATAIMPORT.SANDATA_SCHEDULESCLIENTS sc 
	JOIN DISC_PROD.BI_REPOSITORY.SANDATAVISITS SV ON SC.SCHEDULEID = SV.SCHEDULEID AND SC.AGENCYID = SV.AGENCYID 
	LEFT JOIN DISC_PROD.SANDATAIMPORT.SANDATA_AUTHORIZATIONS a ON sc.AUTHORIZATIONID = a.AUTHID AND sc.AGENCYID = a.AGENCYID 
...
FROM SCHEDULECLIENT_AUTH S JOIN DISC_PROD.SANDATAIMPORT.SANDATA_AUTHORIZATIONS A
...
AUTH_OVERLAP AS 
	...
	MAX(A.DATEEND) OVER (PARTITION BY A.AGENCYID ,A.ADMISSIONID, A.PAYORID ,A.SERVICEID ) AS DATEEND
		...
		JOIN DISC_PROD.SANDATAIMPORT.SANDATA_AUTHORIZATIONS B ON A.AGENCYID = B.AGENCYID
...
NULL AS BILLING_KEY,
	...
	LEFT JOIN DW_PROD.HAH.DIM_BRANCH AS BRANCH ON BRANCH.SYSTEM_CODE::STRING = f.AGENCYID AND BRANCH.OFFICE_CODE = f.LOCATIONID
