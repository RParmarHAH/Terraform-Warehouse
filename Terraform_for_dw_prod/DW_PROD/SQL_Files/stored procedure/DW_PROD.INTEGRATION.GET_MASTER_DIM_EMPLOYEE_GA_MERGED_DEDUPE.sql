CREATE OR REPLACE PROCEDURE DW_PROD.INTEGRATION.GET_MASTER_DIM_EMPLOYEE_GA_MERGED_DEDUPE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
,CLUSTER_ID FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_GA_RL_OUTPUT WHERE CLUSTER_ID IS NOT NULL
)
...
ID,SOURCE_SYSTEM_ID,SYSTEM_CODE,EMPLOYEE_NUMBER,EMPLOYEE_PID,CLUSTER_ID FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_GA_RL_OUTPUT WHERE CLUSTER_ID IS NOT NULL
) --WHERE min_record_id = 1
) --SELECT * FROM OLD;
...
FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_GA_RL_OUTPUT R
...
FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_GA_RL_OUTPUT U
