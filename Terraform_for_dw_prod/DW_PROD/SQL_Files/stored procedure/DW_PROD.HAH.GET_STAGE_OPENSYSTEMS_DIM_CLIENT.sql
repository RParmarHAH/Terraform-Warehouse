CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_OPENSYSTEMS_DIM_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INSERT OVERWRITE INTO DW_PROD.STAGE.OPENSYSTEMS_DIM_CLIENT
WITH CLIENT AS
(
	SELECT * FROM
	(
...
, VISIT_DATES AS (
	...
	FROM DISC_PROD.HHAEXCHANGEOPENSYSTEMS.TBLVISITS_REPL VIS
...
(
	...
	FROM DISC_PROD.HHAEXCHANGEOPENSYSTEMS.PATIENTSTATUSCHANGEACTIVITY 
	)				 
, CLIENT_STATUS AS (
	SELECT
		  ST.PATIENTID
	...
	FROM DISC_PROD.HHAEXCHANGEOPENSYSTEMS.PATIENTSTATUSCHANGEACTIVITY ST
...
, SUPERVISOR AS (
	...
	FROM DISC_PROD.HHAEXCHANGEOPENSYSTEMS.PATIENTS P
	LEFT JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.PROVIDERPATIENTCOORDINATORS PC1 ON PC1.PATIENTID = P.PATIENTID
	 AND PC1.COORDINATORSRNO = 1
	LEFT JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.TBLCOORDINATOR_REPL C1 ON C1.COORDINATORID = PC1.COORDINATORID
	LEFT JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.PROVIDERPATIENTCOORDINATORS PC2 ON PC2.PATIENTID = P.PATIENTID
	 AND PC2.COORDINATORSRNO = 2
	LEFT JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.TBLCOORDINATOR_REPL C2 ON C2.COORDINATORID = PC2.COORDINATORID
)
, CONTRACT_DATES AS (
	SELECT
	...
	FROM DISC_PROD.HHAEXCHANGEOPENSYSTEMS.PATIENTCONTRACTCHANGES PC
...
, ADMISSION_DATA AS (
	...
	FROM DISC_PROD.HHAEXCHANGEOPENSYSTEMS.ADMISSIONS A
...
FROM DISC_DEDUPE_PROD.HHAEXCHANGEOPENSYSTEMS.CLIENT_MASTER_LIST CLI
INNER JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.OFFICE_OFFICES_REPL OFFICE ON CLI.OFFICEID = OFFICE.OFFICEID
...
LEFT JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.REFERRALSOURCE RS ON RS.REFERRALSOURCEID = CLI.REFERRALSOURCEID 
