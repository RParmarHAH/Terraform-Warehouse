CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_MATRIXCARE_FACT_VISIT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
-- 12/05/2023	Rajat Sapkal			Added CLIENT_SERVICE_KEY,PARTNER_CONTRACT_SERVICE_KEY,BILLING_KEY
--07/03/23		Rajat Sapkal		 	Bill_unit_type change
--09/07/23		Deepen Gajjar		 	Modified Client_payer CTE logic															
--*****************************************************************************************************************************

INSERT OVERWRITE INTO STAGE.MATRIXCARE_FACT_VISIT
...
SELECT  DISTINCT CLIPAY_CLIENTID
		,FIRST_VALUE(CLIPAY_PAYERID) OVER (PARTITION BY CLIPAY_CLIENTID ORDER BY IFF(CNT<>2,CLIPAY_SEQNO, iff(CNT=''2'' AND FLG = ''1'',CLIPAY_SEQNO,-1)) DESC) CLIPAY_PAYERID
		,FIRST_VALUE(COALESCE(ETL_LAST_UPDATED_DATE, ''1/1/1900'')) OVER (PARTITION BY CLIPAY_CLIENTID ORDER BY  IFF(CNT<>2,CLIPAY_SEQNO, iff(CNT=''2'' AND FLG = ''1'',CLIPAY_SEQNO,-1)) DESC) ETL_LAST_UPDATED_DATE
FROM 
(SELECT 
count(*) OVER (PARTITION BY CLIPAY_CLIENTID) CNT ,
IFF(CLIPAY_SEQNO=''999'',0, 1) AS FLG ,
*
...
(
 ...
 left join DISC_PROD.MATRIXCARE.STVHC_T_ADMISSIONS ADM ON ADM_ID = AUTH_ADMISSIONID
 qualify row_number() over (partition by AUTH_ID,AUTH_ADMISSIONID, adm_clientid, auth_payerid, auth_servicecodeid, auth_begindate order by authdtl_modifieddate desc) = 1
 )
SELECT DISTINCT 
...
,CASE  WHEN SCHSTATUS.SCHST_NAME = ''S'' THEN MD5(''MATRIXCARE'' || ''-'' || COALESCE(EMP.MASTER_ID,SCH_CAREGIVERID::STRING) || ''-'' || ''MATRIXCARE'') 
...
,NULL as INVOICE_KEY
,NULL as PAYROLL_KEY
,	MD5(	NVL(au.AUTH_ADMISSIONID,-1) || ''-'' || 
        	NVL(au.auth_servicecodeid,-1) || ''-'' ||
        	au.AUTH_ID || ''-'' ||
...
--, MD5(
        ...
        --''MATRIXCARE''
  -- )                                                                                                       AS SERVICE_KEY
	MD5(''MATRIXCARE'' || ''-'' || COALESCE(MAPPING.OFFICE_STATE, BRANCH.BR_STATEORPROVINCE, ''IN'') || ''-'' ||
        NVL(COALESCE (au.AUTH_PAYERID,SCHPAYER.SCHCP_PAYERID,CP.CLIPAY_PAYERID,INV.INV_PAYERID,PB.PAYBR_PAYERID),-1) || ''-'' ||
        NVL(sc.svcc_id,-1) || ''-'' ||
        ''MATRIXCARE'')                                                                                                       
...
,CASE  WHEN SCHSTATUS.SCHST_NAME = ''S'' THEN COALESCE(EMP.CAR_ID::NUMBER ,SCH_CAREGIVERID::NUMBER) ELSE NULL END AS SCHEDULED_EMPLOYEE_ID
,COALESCE(EMP.CAR_ID::NUMBER ,SCH_CAREGIVERID::NUMBER) AS EMPLOYEE_ID
...
, VSM.VISIT_STATUS_DESCRIPTION  AS VISIT_STATUS_DESCRIPTION
 ...
 , VSM.INVOICE_STATUS_NAME  AS INVOICE_STATUS_NAME
		...
		,INVD.AMOUNT_BILLED AS BILL_UNITS_SERVED
...
LEFT JOIN INVOICE_PAYERS INV ON INV.INVD_SCHEDULEID = SCH.SCH_ID
LEFT JOIN AUTH_BASE AU ON AU.AUTH_ADMISSIONID = SCH_ADMISSIONID AND AU.ADM_CLIENTID = sch_clientid AND AU.AUTH_PAYERID = COALESCE(SCHPAYER.SCHCP_PAYERID,CP.CLIPAY_PAYERID) and au.auth_servicecodeid = sch_servicecodeid and sch_starttime between au.auth_begindate and au.auth_enddate
left join DISC_PROD.matrixcare.STVHC_T_SERVICECODE sc on sc.svcc_id = SCH_SERVICECODEID
LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_INVOICEHEADER AS INVOICE_HEADER ON INVOICE_HEADER.INVH_ID = INV.INVH_ID
LEFT JOIN INVOICE_DETAILS AS INVD ON INVD.INVH_ID=INVOICE_HEADER.INVH_ID
...
:STR_CDC_START::timestamp_ntz
	...
	AND IFF(SCH.SCH_STARTTIME IS NULL OR SCH.SCH_ENDTIME IS NULL, 
        ...
        QUALIFY ROW_NUMBER() OVER (PARTITION BY VISIT_KEY ORDER BY CLIENT_SERVICE_KEY) = 1;
