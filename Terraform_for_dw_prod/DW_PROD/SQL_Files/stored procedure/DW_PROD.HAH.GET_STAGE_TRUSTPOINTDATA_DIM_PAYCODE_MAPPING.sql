CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_TRUSTPOINTDATA_DIM_PAYCODE_MAPPING("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
WITH SALARY_HOURS_USAGE AS (
	...
	FROM @DW_PROD.Stage.AZStage/CSV_Files/DIM_PAYCODE_MAPPING_FILES/TrustPoint_Excel_PayCodes (file_format => DW_PROD.PUBLIC.CSV_Format) pc
	UNION
	SELECT
		DISTINCT 6 AS Source_System_ID,
		UPPER( TRIM( pc.$1)) AS DB,
		UPPER(TRIM( pc.$5)) AS PAY_CODE,
		(CASE WHEN UPPER( TRIM(pc.$4)) = ''BLOCK'' THEN ''FALSE'' ELSE ''TRUE'' END)::BOOLEAN AS CALC_FLAG
	FROM @DW_PROD.Stage.AZStage/CSV_Files/DIM_PAYCODE_MAPPING_FILES/TrustPoint_Coastal_PayCodes_20210930 (file_format => DW_PROD.PUBLIC.CSV_Format) pc
), --same for ss
PAYCODE_USAGE_DATES AS (
	...
	FROM DISC_PROD.TRUSTPOINTDATA.PAYROLL_DETAILS AS PD
	--INNER JOIN DISC_PROD.TRUSTPOINTDATA.EMPLOYEES AS E ON TRIM( dtl.CheckID) = TRIM( m.CheckID)
