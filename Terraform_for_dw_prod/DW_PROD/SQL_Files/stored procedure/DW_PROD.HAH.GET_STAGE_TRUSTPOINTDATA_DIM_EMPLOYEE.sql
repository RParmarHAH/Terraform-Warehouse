CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_TRUSTPOINTDATA_DIM_EMPLOYEE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INSERT OVERWRITE INTO DW_PROD.STAGE.TRUSTPOINTDATA_DIM_EMPLOYEE
...
(
	...
	FROM DISC_PROD.TRUSTPOINTDATA.PAYROLL
...
FROM DISC_DEDUPE_PROD.TRUSTPOINTDATA.EMPLOYEE_MAPPING_MATCH_LIST AS MAP
LEFT JOIN DISC_DEDUPE_PROD.TRUSTPOINTDATA.EMPLOYEE_MASTER_LIST AS ML ON MAP.MASTER_ID = ML.MASTER_ID
...
WITH LATEST_PAYROLL AS 
        (
            SELECT DISTINCT 
                ...
                FROM DISC_PROD.TRUSTPOINTDATA.PAYROLL AS PAY
...
FROM DISC_PROD.TrustPointData.Employees AS emp
...
LEFT OUTER JOIN DW_PROD.HAH.DIM_Branch BR
    ON
...
JOIN_EMPLOYEES_TRUSTPOINT_BRANCHES AS (
    ...
    LEFT JOIN TRUSTPOINT_BRANCHES tb 
        ON E.EMPLOYEE_NUMBER = tb.Employee_Number AND E.COMPANY = tb.Company 
    left JOIN DISC_DEDUPE_PROD.TRUSTPOINTDATA.EMPLOYEE_MASTER_LIST M
...
SALARY_RATE AS (
	...
	FROM DISC_PROD.TRUSTPOINTDATA.PAYROLL PAY
	QUALIFY RANK = 1
)
SELECT
...
FROM DISC_DEDUPE_PROD.TRUSTPOINTDATA.EMPLOYEE_MASTER_LIST AS tp
...
LEFT OUTER JOIN DW_PROD.HAH.DIM_State AS st ON st.State_ISO_Code = UPPER( tp.Employee_State_Code)
LEFT OUTER JOIN DW_PROD.HAH.DIM_Zip_Code AS zip ON UPPER( zip.Zip_Code) = RIGHT(''00000'' || LEFT(UPPER( tp.Employee_Zip),5),5)
