CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_AXXESS_DIM_EMPLOYEE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
WITH V1 AS (
    ...
    (
        ...
        (
            ...
            FROM DISC_PROD.AXXESS.VW_AXXESS_HH_VISITS PV
	    ...
	    (
			...
			FROM DISC_PROD.AXXESS.VW_AXXESS_HC_TASKDETAILS PV
...
(	SELECT MASTER_ID, EMPLOYEE_ID, SSN FROM DISC_DEDUPE_PROD.AXXESS.EMPLOYEE_MASTER_LIST )
	UNION   
	SELECT * FROM (
...
,AXXESS_TO_TRUSTPOINT_MAPPING AS ( 
    ...
    FROM DISC_DEDUPE_PROD.AXXESS.EMPLOYEE_MASTER_LIST M
...
EMPLOYEE_BRANCH AS (
	WITH dat AS (
	SELECT ''HH'' AS tt, *
	FROM DISC_PROD.AXXESS.AXXESS_HH_EMPLOYEE_BRANCH 
	UNION 
	SELECT ''HC'' AS tt,*
	FROM DISC_PROD.AXXESS.AXXESS_HC_EMPLOYEE_BRANCH 
	)
	SELECT
...
FROM DISC_DEDUPE_PROD.AXXESS.EMPLOYEE_MASTER_LIST EI
...
left join DISC_PROD.AXXESS.AXXESS_BRANCH_OFFICE_MAPPING o on COALESCE (EB.BRANCH_ID,pv.branch_id) = o.branch_id
