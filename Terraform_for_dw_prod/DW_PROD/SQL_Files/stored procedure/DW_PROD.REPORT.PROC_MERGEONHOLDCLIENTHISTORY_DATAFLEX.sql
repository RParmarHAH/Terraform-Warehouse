CREATE OR REPLACE PROCEDURE DW_PROD.REPORT.PROC_MERGEONHOLDCLIENTHISTORY_DATAFLEX()
RETURNS VARCHAR(16777216)
LANGUAGE JAVASCRIPT
EXECUTE AS OWNER
AS '
  var rs = snowflake.execute( { sqlText: `MERGE INTO DW_DEV.REPORT.OnHoldClientHistory_dataflex TARGET
                                          USING DW_DEV.REPORT.VW_NEXTONHOLDDATA_DATAFLEX SRC
  ...
  var rs = snowflake.execute( { sqlText: `MERGE INTO DW_DEV.REPORT.OnHoldClientHistory_dataflex TARGET
                                          ...
                                          ,TARGET.ETL_INSERTED_DATE = SRC.ETL_INSERTED_DATE`} );
  var rs = snowflake.execute({  sqlText: `INSERT INTO DW_DEV.REPORT.OnHoldClientHistory_dataflex(CLIENT_KEY,NUMBER,SOURCESYSTEMID,DBNAME,ONHOLDSTARTDATE,ONHOLDENDDATE,
										  ACTIVE,ETL_INSERTED_DATE)
										  SELECT CLIENT_KEY,NUMBER,SOURCESYSTEMID,DBNAME,ONHOLDSTARTDATE,
										  ONHOLDENDDATE,STATUS,ETL_INSERTED_DATE FROM DW_DEV.REPORT.VW_NEXTONHOLDDATA_DATAFLEX
										  WHERE NVL(ONHOLDSTARTDATE,''1900-01-01'') != ''1900-01-01''
										  AND NUMBER || SOURCESYSTEMID || DBNAME 
										  NOT IN (SELECT NUMBER || SOURCESYSTEMID || DBNAME FROM DW_DEV.REPORT.OnHoldClientHistory_dataflex
										  WHERE ACTIVE  =TRUE);`});
  var rs = snowflake.execute({  sqlText: `INSERT INTO DW_DEV.REPORT.OnHoldClientHistory_dataflex
                                          SELECT SRC.CLIENT_KEY,SRC.NUMBER,SRC.SOURCESYSTEMID,SRC.DBNAME,SRC.ONHOLDSTARTDATE,
                                          SRC.ONHOLDENDDATE,SRC.STATUS,SRC.ETL_INSERTED_DATE FROM  DW_DEV.REPORT.OnHoldClientHistory_dataflex TARGET
                                          INNER JOIN DW_DEV.REPORT.VW_NEXTONHOLDDATA_DATAFLEX SRC
                                          ON TARGET.NUMBER = SRC.NUMBER
                                          AND TARGET.SourceSystemID = SRC.SourceSystemID
                                          AND TARGET.DBNAME = SRC.DBNAME
                                          AND TARGET.ACTIVE = TRUE
  ...
  var rs = snowflake.execute({  sqlText:  `UPDATE DW_DEV.REPORT.OnHoldClientHistory_dataflex clienthistory
											SET ACTIVE = FALSE
											FROM
											(
											  SELECT clienthistory.NUMBER,clienthistory.SourceSystemId,clienthistory.dbname,
											  clienthistory.ETL_INSERTED_DATE
											  FROM DW_DEV.REPORT.OnHoldClientHistory_dataflex clienthistory
											  INNER JOIN
											  (
												SELECT * FROM   
												(
													...
													FROM DW_DEV.REPORT.OnHoldClientHistory_dataflex
  ...
  var rs = snowflake.execute({ sqlText: `MERGE INTO DW_DEV.REPORT.OnHoldClientHistory_DATAFLEX TARGET
                                         ...
                                         AND TARGET.ACTIVE = TRUE
                                          ...
                                          ,TARGET.ETL_INSERTED_DATE = SRC.ETL_INSERTED_DATE;`});
  var rs = snowflake.execute({  sqlText:  `update DW_DEV.REPORT.OnHoldClientHistory_dataflex t1
                                           ...
                                           (
                                                ...
                                                from DW_DEV.REPORT.OnHoldClientHistory_dataflex OnHold
                                            inner join 
                                            (
                                                select distinct SYSTEM_CODE,client_key,service_Date
                                                from
                                                DW_PROD.hah.fact_visit 
