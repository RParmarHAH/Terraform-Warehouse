CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_ALLIANCE_DIM_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
WITH CLIENT AS
(
	SELECT * FROM DISC_DEDUPE_PROD.GENERATIONSALLIANCE.CLIENT_MASTER_LIST  
	WHERE MASTER_ID IN 
	(
...
(
	...
	FROM DISC_PROD.GENERATIONSALLIANCE.CLIENT C
	LEFT JOIN DISC_PROD.GENERATIONSALLIANCE.SCHEDULES V ON TRIM(C.CLIENTID) = TRIM(V.CLIENTID)
	GROUP BY ALL
), SUPERVISORS AS 
(
	SELECT * FROM
...
(
	...
	(
		...
		FROM DISC_PROD.GENERATIONSALLIANCE.CLIENT CL
		JOIN DISC_PROD.GENERATIONSALLIANCE.CONTACTS CS
...
(
	...
	FROM DISC_PROD.GENERATIONSALLIANCE.CLIENT CCS 
...
(
	...
	FROM DISC_PROD.GENERATIONSALLIANCE.SERVICEINTERRUPTIONS	C
	GROUP BY CLIENT_ID
)
SELECT DISTINCT MD5(''ALLIANCE'' || ''-'' || CL.CLIENTID || ''-'' || ''GENERATIONS'') AS CLIENT_KEY
				...
				, CASE WHEN DW_PROD.STAGE.SSN_CLEANUP(CL.SSN) NOT IN (SELECT SSN FROM DATA_MANAGEMENT.DATA_QUALITY.INVALID_SSN)
						THEN HEX_ENCODE(DW_PROD.STAGE.SSN_CLEANUP(CL.SSN))
				  END AS CLIENT_PID
...
LEFT JOIN DISC_PROD.GENERATIONSALLIANCE.CLIENTCMS1500 CMS 
	ON CMS.CLIENTID = CL.CLIENTID
LEFT JOIN DISC_PROD.GENERATIONSALLIANCE.CASEMANAGER CMM
...
LEFT JOIN DISC_PROD.GENERATIONSALLIANCE.SERVICEINTERRUPTIONS SI 
