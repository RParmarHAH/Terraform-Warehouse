CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_AXXESS_FACT_REVENUE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
        ...
        WITH DEDUPE_LEVERAGE AS
                 (SELECT *
                  FROM (SELECT MASTER_ID,
                               PATIENT_ID
                        FROM DISC_DEDUPE_PROD.AXXESS.CLIENT_MASTER_LIST)
                  UNION
                  SELECT *
                  FROM (SELECT DISTINCT MASTER_ID,
                                        PATIENT_ID
                        FROM DISC_DEDUPE_PROD.AXXESS.CLIENT_MATCH_LIST
             ...
             EMPLOYEE_DEDUPE AS
                 (SELECT *
                  FROM (SELECT MASTER_ID,
                        ...
                        FROM DISC_DEDUPE_PROD.AXXESS.EMPLOYEE_MASTER_LIST)
                  UNION
                  SELECT *
                  FROM (SELECT DISTINCT MASTER_ID,
                                        EMPLOYEE_ID,
                                        (FIRST_NAME || ''  '' || LAST_NAME) AS EMP_NAME
                        FROM DISC_DEDUPE_PROD.AXXESS.EMPLOYEE_MATCH_LIST
             ...
             SUPERVISOR AS
                 ( -- THERE ARE NO PATIENTS WITH MULTIPLE SUPERVISORS. CHECKED MANUALLY ON 07/11
                     ...
                     FROM DISC_PROD.AXXESS.AXXESS_PATIENTVISITS PV
             ...
             HOMEHEALTH
                 AS (SELECT DISTINCT MD5(''PRIME'' || ''-'' || IFNULL(UPPER(TRIM(ARS.ID)), ''UNKNOWN'') || ''-'' ||
                     ...
                     FROM DISC_PROD.AXXESS.AXXESS_ARS ARS
                              ...
                              LEFT JOIN DISC_PROD.AXXESS.VW_AXXESS_HH_VISITS_UPPER VISIT ON VISIT.FACT_AR_ID = ARS.ID
