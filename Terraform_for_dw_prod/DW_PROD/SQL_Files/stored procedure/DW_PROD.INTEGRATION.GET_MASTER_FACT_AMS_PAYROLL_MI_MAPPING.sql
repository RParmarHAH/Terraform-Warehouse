CREATE OR REPLACE PROCEDURE DW_PROD.INTEGRATION.GET_MASTER_FACT_AMS_PAYROLL_MI_MAPPING()
...
BEGIN
    INSERT INTO DW_DEV.INTEGRATION.FACT_AMS_PAYROLL_MAPPING
    WITH MAPPING_DATA AS (
      SELECT * FROM (
        SELECT *,COUNT(DISTINCT SOURCE_SYSTEM) OVER(PARTITION BY CLUSTER_ID) SYSTEM_COUNT
        FROM DW_DEV.INTEGRATION.AMS_PAYROLL_MI_DEDUPE_OUTPUT) 
