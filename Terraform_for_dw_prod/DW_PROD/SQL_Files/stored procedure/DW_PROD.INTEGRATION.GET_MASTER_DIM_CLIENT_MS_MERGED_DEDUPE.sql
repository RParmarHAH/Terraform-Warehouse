CREATE OR REPLACE PROCEDURE DW_PROD.INTEGRATION.GET_MASTER_DIM_CLIENT_MS_MERGED_DEDUPE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
WITH LWD_NEW AS (
    ...
    CLUSTER_ID
    FROM DW_PROD.INTEGRATION.DIM_CLIENT_MS_RL_OUTPUT
)
, NEW AS (
    SELECT * FROM (
        SELECT 
        ...
        FROM DW_PROD.INTEGRATION.DIM_CLIENT_MS_RL_OUTPUT PA
...
FROM DW_PROD.INTEGRATION.DIM_CLIENT_MS_RL_OUTPUT R
...
FROM DW_PROD.INTEGRATION.DIM_CLIENT_MS_RL_OUTPUT U
