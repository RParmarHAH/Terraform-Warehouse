CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_OPENSYSTEMS_FACT_VISIT_EXCEPTION("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO DW_PROD.STAGE.OPENSYSTEMS_FACT_VISIT_EXCEPTION
WITH CALLS AS (
	SELECT 
		V.VISITID,
	...
	FROM 
		DISC_PROD.HHAEXCHANGEOPENSYSTEMS.INREVIEWVISITS CM
	LEFT JOIN 
		DISC_PROD.HHAEXCHANGEOPENSYSTEMS.VISITS V
		ON CM.AGENCYID = V.AGENCYID 
			AND CM.VISITID = V.VISITID
	LEFT JOIN
		DISC_PROD.HHAEXCHANGEOPENSYSTEMS.OFFICE_OFFICES_REPL OFFICE ON CM.OFFICEID = OFFICE.OFFICEID
	LEFT JOIN 
		DISC_PROD.HHAEXCHANGEOPENSYSTEMS.TBLVISITS_REPL CRN 
		ON CRN.AGENCYID = V.AGENCYID
			AND CRN.VISITID = V.VISITID
	LEFT JOIN
		DISC_PROD.HHAEXCHANGEOPENSYSTEMS.REASONS_REPL RS
