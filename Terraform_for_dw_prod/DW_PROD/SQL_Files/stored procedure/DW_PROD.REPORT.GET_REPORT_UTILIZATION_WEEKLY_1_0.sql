CREATE OR REPLACE PROCEDURE DW_PROD.REPORT.GET_REPORT_UTILIZATION_WEEKLY_1_0("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--  06/07/23            Sandesh Gosavi      Excluded 4,7,17 source system from visit based cte  
--	06/09/23 			Sanket Jain			COMMENTED PAYROLL DATE AND RESPECTIVE FILEDS TO ACCOMODATE AUTHORIZATION in VW 
--  08/31/23            Divesh Aneja        added mycare code.  
--  09/29/23			Jashvant Patel		Added acqusition code for BERKSHIRE  
--  10/03/23			Shikhar Saxena		Added Acquisition code for ROMEO 
--  10/11/23			Shikhar Saxena		Updated logic of Romeo acquisition code
--  01/03/24	   		Rajat Sapkal		Removed Carevoyant 
-- *****************************************************************************************************************************
--NOTE:			If Manual Refresh is required then SCHEDULE_METRICS_WEEKLY should be refreshed first before running this object
-- *****************************************************************************************************************************

INSERT OVERWRITE INTO REPORT.UTILIZATION_WEEKLY_1_0
WITH  CONTRACTS_CLIENTS AS (
 ...
 VISIT_DATA AS
    (
        SELECT DISTINCT
-------COMMENTED PAYROLL DATE AND RESPECTIVE FILEDS TO ACCOMODATE AUTHORIZATION ----------------------
...
--        AND VISIT.PAYROLL_DATE >= PAYROLL_NEXTCHECKDATE.PreviousCheckDate AND PAYROLL_NEXTCHECKDATE.ORIGINAL_BRANCH_KEY =VISIT.ORIGINAL_BRANCH_KEY 
   WHERE  NVL(VISIT.BILL_UNIT_TYPE, ''Hourly'') = ''Hourly'' AND-- VISIT.CONFIRMED_FLAG = ''YES'' 	-- Only confirmed visits	-- Need all visits now
   VISIT.ORIGINAL_SOURCE_SYSTEM_ID NOT IN (4,7,17,13,9,3,8,19,16,1,2)   
   GROUP BY
...
--            LAST_DAY(DATA.SERVICE_WEEK,"WEEK") AS SERVICE_END_OF_WEEK,
            ...
            NULL AS HOURS_SERVED_WHERE_AUTH_EXISTS,
			--NVL(CASE WHEN CONTRACT.INCLUDE_FOR_HOURS = 1 AND NOT (NVL(AUTH.HOURS_AUTHORIZED, 0) = 0 OR AUTH.HOURS_AUTHORIZED > 900) THEN DATA.HOURS_SERVED END, 0) HOURS_SERVED_WHERE_AUTH_EXISTS,
      ...
      --IFF(COALESCE(CCSI_BRANCH.BRANCH_KEY,CCSI_ACQUIRED_CLIENT.CLIENT_KEY) IS NULL, FALSE, TRUE) AS CLIENT_ACQUIRED_FLAG,
        ...
        WHEN COALESCE(MYCARE_CLIENT.CLIENT_KEY,MYCARE_ACQUIRED_CLIENT.CLIENT_KEY) IS NOT NULL THEN ''MYCARE'' 
...
LEFT OUTER JOIN (SELECT CLIENT_KEY FROM HAH.FACT_CLIENT_ACQUIRED_WEEKLY WHERE AQUISITION_NAME = ''MYCARE'' ) MYCARE_ACQUIRED_CLIENT
          ON MYCARE_ACQUIRED_CLIENT.CLIENT_KEY = DATA.CLIENT_KEY AND DATA.SERVICE_WEEK >= ''2023-07-24''
