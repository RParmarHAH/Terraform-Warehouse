CREATE OR REPLACE PROCEDURE DW_PROD.REPORT.GET_REPORT_EMPLOYEE_DERIVED_METRICS("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INNER JOIN DW_PROD.HAH.Fact_Payroll AS pyrl ON pyrl.Employee_Key = emp.Employee_Key AND pyrl.Source_System_ID IN (17)
...
SELECT emp.Employee_Key, emp.Employee_ID, emp.System_Code, emp.Source_System_ID, emp.Employee_PID, emp.Employee_Number,
    ...
    --    DATEDIFF( ''DAY'', Derived_Last_Check_Date, CURRENT_DATE) AS Days_Since_Last_Paycheck
FROM DW_PROD.HAH.DIM_Employee AS emp
INNER JOIN DW_PROD.HAH.Fact_Payroll AS pyrl ON pyrl.Employee_ID = emp.PAYROLL_ID  AND pyrl.Source_System_ID IN (6)
...
INNER JOIN DW_PROD.HAH.DIM_Contract AS cntrct ON vis.Contract_Key = cntrct.Contract_Key AND NVL( cntrct.Mileage_Flag, ''FALSE'') = ''FALSE'' AND UPPER( cntrct.Revenue_Subcategory_Name ) NOT LIKE ''%MILE%'' AND (NVL( vis.Bill_Rate, 1) >= 1 OR vis.Bill_Rate = 0) AND cntrct.Revenue_Category IN (''OTHER'') 
INNER JOIN Employee_Service_Line_List AS esll ON vis.Employee_Key = esll.Employee_Key
WHERE vis.CONFIRMED_FLAG = ''YES''
AND vis.Employee_Key NOT IN ( SELECT Employee_Key FROM Bill_Only_Employees)
),
...
,APPLICANT_DATA AS (
	...
	FROM DW_PROD.INTEGRATION.FACT_APPLICANT_EMPLOYEE_MAPPING AEM
	INNER JOIN DW_PROD.HAH.FACT_APPLICANT_STATUS FAS
...
SERVICE_AFTER_REHIRE AS (
	...
	LEFT JOIN DW_PROD.HAH.FACT_VISIT FV ON FV.EMPLOYEE_KEY = ETD.EMPLOYEE_KEY  
...
FROM DW_PROD.INTEGRATION.FACT_VISIT_MERGED vis
)
