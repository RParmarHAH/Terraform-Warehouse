CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_PREFERRED_DIM_EMPLOYEE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--
-- PURPOSE: Creates one row per employee_id/database according to caregiver data from Preferred
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
-- 05/04/22    Jashvant Patel        Initial development
-- 11/01/22    Mitul Panchal		Added STATE Code for QUEENS
...
--*****************************************************************************************************************************
INSERT OVERWRITE  INTO STAGE.PREFERRED_DIM_EMPLOYEE
WITH CHECK_DATE AS 
(
	SELECT CAREGIVERID, TO_DATE(MIN(VISITDATE)) AS FIRST_CHECK_DATE, TO_DATE(MAX(VISITDATE)) AS LAST_CHECK_DATE
	FROM DISC_PROD.HHAEXCHANGEPREFERRED.PAYROLL_DETAILS_REPL
	GROUP BY 1
)
SELECT DISTINCT MD5(''PREFERRED'' || ''-'' || EM.MASTER_ID  || ''-'' || ''PREFERRED'') AS EMPLOYEE_KEY
				, TO_NUMBER(NVL(EM.MASTER_ID, EM.CAREGIVERID)) AS EMPLOYEE_ID
				...
				, CASE WHEN UPPER(REGEXP_REPLACE(EM.STATE,''([ .]){1,}'','''')) =''NEWYORK'' THEN ''NY'' 
                       WHEN (UPPER(TRIM(EM.STATE)) IN (''NEWYORK'',''CORONA'', ''NUEVA YORK'', ''BRONX'', ''QUEENS VILLAGE'',''BLOOKYN'',''BROOKLYN'',''QUEENS'',''NYY'',''N.Y'', ''NEW YORK'',''WESTCHESTER'', ''NYC'',''NY - NEW YORK'',''NEW YORRK'',''N. Y'', ''N.Y.'',''NEW'',''NEWYORK NY'',''N YN.A'',''NY   NEW YORK'',''MANHATTAN'',''UNITED STATE'',''NEW YORK,'',''N Y'',''917-963-4969'',''ALABAMA'',''MNY'',''N.A'')
                           OR LEN(UPPER(REGEXP_REPLACE(EM.STATE,''([ .]){1,}'','''')))>2) 
                           AND UPPER(REGEXP_REPLACE(EM.STATE,''([ .]){1,}'','''')) !=''NEWJERSEY'' 
                           THEN ''NY'' 
                       WHEN UPPER(REGEXP_REPLACE(EM.STATE,''([ .]){1,}'','''')) =''NEWJERSEY'' THEN ''NJ'' 
                       ELSE UPPER(REGEXP_REPLACE(EM.STATE,''([ .]){1,}'',''''))
 ...
 ,IFF(LENGTH(REGEXP_REPLACE(TRIM(EM.PHONE2),''[^[:digit:]$]'')) = 10,REGEXP_REPLACE(TRIM(EM.PHONE2),''[^[:digit:]$]''), NULL )  AS EMPLOYEE_CELL_PHONE
	,IFF(LENGTH(REGEXP_REPLACE(TRIM(EM.PHONE3),''[^[:digit:]$]'')) = 10,REGEXP_REPLACE(TRIM(EM.PHONE3),''[^[:digit:]$]''), NULL )  AS EMPLOYEE_WORK_PHONE
				, EM.NOTIFICATIONEMAIL AS EMPLOYEE_PERSONAL_EMAIL     
				, EM.NOTIFICATIONEMAIL AS EMPLOYEE_WORK_EMAIL            
	     ...
	     WHEN  UPPER(TRIM(EM.ETHNICITY)) IN (''NATIVE HAWAIIAN'',''PACIFIC ISLANDER'') THEN ''NATIVE HAWAIIAN OR OTHER PACIFIC ISLANDER''
				  WHEN TRIM(EM.ETHNICITY) = ''Two or More Races'' THEN ''TWO OR MORE''
	...
	,IFF(TRIM(EM.ETHNICITY) ILIKE ANY(''HISPANIC%'',''LATINO%'',''HISPANIC OR LATINO%''), TRUE, FALSE) AS HISPANIC_OR_LATINO
				, L."LANGUAGE"  AS EMPLOYEE_LANGUAGE
				...
				, CASE WHEN UPPER(REGEXP_REPLACE(EM.STATE,''([ .]){1,}'','''')) =''NEWYORK'' THEN ''NY'' 
                       ...
                       WHEN UPPER(REGEXP_REPLACE(EM.STATE,''([ .]){1,}'','''')) =''NEWJERSEY'' THEN ''NJ'' 
                       ELSE UPPER(REGEXP_REPLACE(EM.STATE,''([ .]){1,}'',''''))
                  END AS  WORK_STATE         ---DO WE HAVE THIS? TAKE CAREGIVER STATE IF BLANK STATE THEN TAKE ZIP STATE
				, NULL AS SUTA_STATE
				, NULL AS EMPLOYEE_DEPARTMENT        
				, NULL AS EMPLOYEE_OFFICE_CODE      
				, MD5(''PREFERRED'' || ''-'' || ''PREFERRED CORPORATE OFFICE'' || ''-'' || ''PREFERRED'') AS PRIMARY_BRANCH_KEY, ''NY'' AS PRIMARY_BRANCH_STATE 
				, ''PREFERRED CORPORATE OFFICE'' AS PRIMARY_BRANCH_NAME 
				, ''PREFERRED'' AS PRIMARY_BRANCH_SYSTEM_CODE
...
FROM DISC_DEDUPE_PROD.HHAEXCHANGEPREFERRED.EMPLOYEE_MASTER_LIST EM
INNER JOIN DISC_PROD.HHAEXCHANGEPREFERRED.AGENCY_MAPPING AGENCY_CONFIG
	ON AGENCY_CONFIG.AGENCYID = EM.AGENCYID
		AND AGENCY_CONFIG.ISACTIVE = TRUE
LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.LANGUAGES L ON L.LANGUAGEID = EM.LANGUAGE1
-- LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.OFFICE_OFFICES_REPL  O ON EM.OFFICEID = O.OFFICEID AND EM.AGENCYID = O.AGENCYID 
LEFT  JOIN DISC_PROD.HHAEXCHANGEPREFERRED.PAYROLL_DETAILS_REPL PD ON PD.CAREGIVERID = EM.CAREGIVERID 
