CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_EDISON_FACT_VISIT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
COORDINATOR AS (
    ...
    FROM CLIENT P
    JOIN DISC_PROD.HHAEXCHANGEEDISON.PROVIDERPATIENTCOORDINATORS PC1 
...
(
	SELECT * FROM DISC_PROD.HHAEXCHANGEEDISON.VISITS V
...
FROM DISC_PROD.HHAEXCHANGEEDISON.VISITAUTHORIZATIONS_REPL VAR
JOIN DISC_PROD.HHAEXCHANGEEDISON.PATIENTAUTHORIZATION PA ON PA.AUTHORIZATIONID = VAR.AUTHORIZATIONID 
...
FROM DISC_PROD.HHAEXCHANGEEDISON.PAYMENTS
)--SELECT * FROM Latest_Check_Number;
,AUTO_CONFIRMED_VISITS AS (
SELECT UV.VISITID
FROM DISC_PROD.HHAEXCHANGEEDISON.UNTOUCHEDVISITS UV
LEFT JOIN DISC_PROD.HHAEXCHANGEEDISON.INREVIEWVISITS IV
...
LEFT JOIN DISC_DEDUPE_PROD.HHAEXCHANGEEDISON.EMPLOYEE_MASTER_LIST  EM
	ON E.MASTER_ID = EM.MASTER_ID 
LEFT JOIN DISC_PROD.HHAEXCHANGEEDISON.OFFICE_OFFICES_REPL O 
...
LEFT JOIN DISC_PROD.HHAEXCHANGEEDISON.CONTRACTS 
...
LEFT JOIN DISC_PROD.HHAEXCHANGEEDISON.PAYROLL_VISITS_REPL PAYROLL 
