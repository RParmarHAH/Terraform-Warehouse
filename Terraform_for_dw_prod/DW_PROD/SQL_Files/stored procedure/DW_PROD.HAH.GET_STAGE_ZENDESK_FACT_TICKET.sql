CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_ZENDESK_FACT_TICKET("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
 ...
 MAX( CASE WHEN f.value:id::string = ''7924876074007'' AND TRIM(f.value:value::string)<>'''' THEN f.value:value::string 
      ...
      END )as SYSTEM,
       MAX( CASE WHEN f.value:id::string = ''17893646264087'' AND TRIM(f.value:value::string)<>'''' THEN f.value:value::string 
          ELSE NULL
      END )as PROBLEM_TYPE
    FROM DISC_PROD.ZENDESK.TICKETS T,
...
--     END AS FR_TICKET_TYPE
    ...
    FROM
   DISC_PROD.ZENDESK.TICKET_FORMS TF
    )
SELECT DISTINCT 
...
--
FROM DISC_PROD.ZENDESK.TICKETS T
LEFT JOIN DISC_PROD.ZENDESK.TICKET_METRICS TM
    ON TRIM(T.ID)= TRIM(TM.TICKET_ID) AND TM."SOURCE" =T.SOURCE
LEFT JOIN DISC_PROD.ZENDESK.USER U
	ON T.REQUESTER_ID = U.ID 
LEFT JOIN DISC_PROD.ZENDESK.GROUPS G
	ON T.GROUP_ID = G.ID
...
LEFT JOIN DISC_PROD.ZENDESK.VW_ORGANIZATION_BRANCH_MAPPING BM 
