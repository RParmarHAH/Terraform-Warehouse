CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_DIM_SURVEY_RESPONSE_HEADER("SURVEY_ID" VARCHAR(16777216))
...
||IFF("''CLIENT_KEY''" IS NOT NULL,''LEFT JOIN DISC_PROD.QUALTRICS_SURVEYS.QUALTRICS_CLIENT_MAPPING QCM ON LOWER(f.value:values."''||"''CLIENT_KEY''"||''"::STRING) = LOWER(QCM.ORIGINAL_CLIENT_NUMBER)\\n'','''')
...
'','''')		
||IFF("''CLIENT_KEY''" IS NOT NULL,''LEFT JOIN DW_PROD.INTEGRATION.DIM_CLIENT_MERGED DCM ON LOWER(DCM.ORIGINAL_CLIENT_KEY) =
CASE 
    WHEN LENGTH(LOWER(f.value:values.''||"''CLIENT_KEY''"||''::STRING))>=32
        THEN LOWER(f.value:values.''||"''CLIENT_KEY''"||''::STRING)
    WHEN LENGTH(LOWER(f.value:values.''||"''CLIENT_KEY''"||''::STRING))<32
