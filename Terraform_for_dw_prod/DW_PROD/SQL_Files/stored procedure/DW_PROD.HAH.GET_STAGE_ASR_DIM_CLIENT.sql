CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_ASR_DIM_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
,BILLING_STATUS AS (
	...
	FROM (
		...
		FROM DISC_PROD.ASR.ASR_AGINGREPORT AG
...
LEFT JOIN DISC_PROD.ASR.ASR_PLANOFSERVICE P ON V.SERVICEVISITID  = P.SERVICEVISITID
...
FROM DISC_DEDUPE_PROD.ASR.CLIENT_MASTER_LIST C
...
LEFT JOIN DISC_PROD.ASR.ASR_LOCATIONS L ON TRIM(C.LOCATIONID) = TRIM(L.LOCATIONID)
...
LEFT JOIN DISC_PROD.ASR.ASR_VISITS V ON C.CUSTOMERID = V.CUSTOMERID 
LEFT JOIN DISC_PROD.ASR.ASR_CUSTOMERSCOMMUNICATIONLOG CCL ON V.COMMLOGRECORDID = CCL.RECORDID
LEFT JOIN DISC_PROD.ASR.ASR_CUSTOMERSTATUSES CS ON c.CUSTOMERSTATUS = CS.CUSTOMER_STATUS_ID 																						   
