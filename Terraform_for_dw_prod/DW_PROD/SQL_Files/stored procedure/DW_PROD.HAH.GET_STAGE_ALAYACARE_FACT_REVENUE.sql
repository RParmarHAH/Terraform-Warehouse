CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_ALAYACARE_FACT_REVENUE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
-- Changes made on 25/04/23 for RCM by Pinkal
	...
	INNER JOIN DISC_PROD.ALAYACARE.INVOICE_DETAILS AS INVOICE_DETAIL ON INVOICE_ITEM.INVOICE_ID = INVOICE_DETAIL.INVOICE_ID		-- Added on 20/01/23
	LEFT JOIN DISC_PROD.ALAYACARE.VISIT AS VISITS ON VISITS.VISIT_ID = INVOICE_ITEM.VISIT_ID 
    LEFT JOIN DISC_PROD.ALAYACARE.SERVICE AS SERVICES ON SERVICES.SERVICE_ID = COALESCE(INVOICE_ITEM.SERVICE_ID,VISITS.SERVICE_ID)
    -- ON SERVICES.SERVICE_ID = INVOICE_ITEM.SERVICE_ID		--Changes made on 23/06/23 by Pinkal
	LEFT JOIN DISC_PROD.ALAYACARE.SERVICE_CODE_BILL_CODE AS SERVICE_CODE_BILL_CODE
	...
	LEFT JOIN DISC_PROD.ALAYACARE.BILL_CODE BC ON INVOICE_ITEM.FUNDER_ID = BC.FUNDER_ID AND INVOICE_ITEM.ITEM_BILLITEM_ID = BC.BILL_CODE_ID
