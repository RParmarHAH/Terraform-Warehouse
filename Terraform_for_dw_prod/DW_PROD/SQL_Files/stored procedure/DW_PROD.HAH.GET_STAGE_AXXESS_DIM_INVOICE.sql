CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_AXXESS_DIM_INVOICE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
AS(
	...
	FROM
		 (	SELECT 
			...
			FROM
			DISC_PROD.AXXESS.AXXESS_ARS a	LEFT JOIN DEDUPE_LEVERAGE CLI ON	A.PATIENT_ID = CLI.PATIENT_ID
			GROUP BY MASTER_ID, BRANCH_ID
		) F
	LEFT JOIN DISC_PROD.AXXESS.AXXESS_ARS a ON a.BRANCH_ID = F.BRANCH_ID 
	AND LEFT(
		NVL(TRY_to_timestamp(COALESCE(a.FINAL_OR_MANAGEDCARE_BILL_DATE,a.RAP_BILL_DATE, ''01/01/1999 12:00:00 AM +00:00''), ''mm/dd/yyyy HH12:MI:SS AM TZH:TZM''),
			TRY_to_timestamp(COALESCE(a.FINAL_OR_MANAGEDCARE_BILL_DATE,a.RAP_BILL_DATE, ''01/01/1999 12:00:00 AM +00:00''), ''YYYY-MM-DD''))
...
FROM DISC_PROD.AXXESS.AXXESS_ARS a
--LEFT JOIN DISC_PROD.AXXESS.AXXESS_INVOICE_STATUS_MAPPING M ON
...
LEFT JOIN DISC_PROD.AXXESS.AXXESS_EPISODESNAPSHOTS e ON a.EPISODE_ID = e.EPISODE_ID
...
FROM DISC_PROD.AXXESS.AXXESS_FINANCIALTRANSACTIONS
...
LEFT JOIN DISC_PROD.AXXESS.AXXESS_FINANCIALTRANSACTIONS FIN ON F.CLIENT_ID = FIN.CLIENT_ID  AND F.BRANCH_ID = FIN.BRANCH_ID
)
SELECT DISTINCT 
