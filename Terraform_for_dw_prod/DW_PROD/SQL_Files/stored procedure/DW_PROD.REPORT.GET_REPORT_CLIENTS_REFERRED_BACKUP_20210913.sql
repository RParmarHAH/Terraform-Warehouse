CREATE OR REPLACE PROCEDURE DW_PROD.REPORT.GET_REPORT_CLIENTS_REFERRED_BACKUP_20210913("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
(
	...
	FROM CLIENT_INFO CI
	LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFCLIENTCONTRACTS CC ON CI.DF_SYSTEM_CODE=CC.DBNAME AND CI.DF_CLIENT_NUMBER=CC.CLIENTNUMBER AND CI.LATEST_CONTRACT_CODE=CC.CONTRACTCODE
)
,OLD_VIEW AS 
(
SELECT DISTINCT
...
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFSERVICES S1 ON S1.DBNAME = C.SYSTEM_CODE AND S1.AREA = C.SERVICEAREA
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFSERVICES S2 ON S2.DBNAME = DFC.DBNAME AND S2.AREA = DFC.SERVICEAREA
)
,NEW_VIEW AS 
(
SELECT DISTINCT 
