CREATE OR REPLACE PROCEDURE DW_PROD.REPORT.GET_REPORT_CLIENT_GAPS_IN_CARE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO DW_PROD.REPORT.CLIENT_GAPS_IN_CARE
...
(
	...
	FROM DW_PROD.HAH.FACT_GAPS_IN_CARE GAPS
	INNER JOIN DW_PROD.INTEGRATION.DIM_CLIENT_MERGED ROSTER
		ON ROSTER.ORIGINAL_CLIENT_KEY = GAPS.CLIENT_KEY 
	-- Added MOLINA, CENTENE, And AMERIHEALTH
	LEFT JOIN DW_PROD.INTEGRATION.CLIENT_CROSS_WALK MOLINA_CW
		ON MOLINA_CW.MOLINA_CLIENT = GAPS.CLIENT_KEY 
	LEFT JOIN DW_PROD.INTEGRATION.CLIENT_CROSS_WALK CENTENE_CW
		ON CENTENE_CW.CENTENE_CLIENT = GAPS.CLIENT_KEY
	LEFT JOIN DW_PROD.INTEGRATION.CLIENT_CROSS_WALK AMERIHEALTH_CW
		ON AMERIHEALTH_CW.AMERIHEALTH_CLIENT = GAPS.CLIENT_KEY 
	INNER JOIN DW_PROD.INTEGRATION.DIM_CLIENT_MERGED CLIENT_MERGED
	...
	LEFT JOIN APP_DB_PROD.CARE_COORDINATION.CLIENT_MATCH_LIST GATOR
		ON GATOR.DW_CLIENT_KEY = CLIENT.CLIENT_KEY 
			AND GATOR.DW_CLIENT_KEY != GATOR.GATOR_CLIENT_KEY
	LEFT JOIN DW_PROD.INTEGRATION.FACT_VISIT_MERGED VISIT
...
FROM DW_PROD.HAH.FACT_GAPS_IN_CARE GP 
