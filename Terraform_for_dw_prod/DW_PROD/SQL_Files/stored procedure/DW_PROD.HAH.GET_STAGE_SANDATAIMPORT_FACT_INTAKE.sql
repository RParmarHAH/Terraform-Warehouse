CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_SANDATAIMPORT_FACT_INTAKE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
-- 02/24/20     Rachel Stewart      Initial PRODelopment
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO DW_PROD.STAGE.SANDATAIMPORT_FACT_INTAKE
WITH 
CLIENT AS
(
	SELECT * FROM
...
FROM (
	...
	AND A.ETL_LAST_UPDATED_DATE >= :STR_CDC_START::timestamp_ntz
) LIST
...
(
 ...
 SELECT
 CASE WHEN AUTHS.SERVICEID IN (''CARECO'',''VBPCG'')
	THEN md5(NVL(''CC_''||AUTHS.SOURCE_SYSTEM_CODE, '''') || ''-'' || NVL(AUTHS.REPORT_DATE::string,'''') || ''-'' || AUTHS.BRANCH_NAME || ''-'' || NVL(AUTHS.CLIENT_CODE,-1) || ''-'' || NVL(AUTHS.CONTRACT_CODE,'''') || ''-''|| NVL(AUTHS.AUTHID,-1)
 ...
 ,AUTHS.REPORT_DATE
 ,md5(nvl(AUTHS.SOURCE_SYSTEM_CODE,''S'') || ''-'' || AUTHS.OFFICE_CODE || ''-'' ||  ''SANDATAIMPORT'') AS BRANCH_KEY
,COALESCE(md5(CL.AGENCYID || ''-'' || CL.MASTER_ID || ''-''  ||  ''SANDATAIMPORT'' ),md5(nvl(CL.AGENCYID,''S'')|| ''-'' ||  NVL(CL.MASTER_ID, -1) || ''-''  ||  ''SANDATAIMPORT'' )) AS CLIENT_KEY
 
  ,CASE WHEN AUTHS.SERVICEID IN (''CARECO'',''VBPCG'')
	THEN md5(nvl(''CC_''||AUTHS.SOURCE_SYSTEM_CODE,''S'') || ''-'' || AUTHS.CONTRACT_CODE || ''-''  ||  ''SANDATAIMPORT'') 
	ELSE md5(nvl(AUTHS.SOURCE_SYSTEM_CODE,''S'') || ''-'' || AUTHS.CONTRACT_CODE || ''-''  ||  ''SANDATAIMPORT'')  END AS CONTRACT_KEY
...
,Auths.PRE_AUTH_NUMBER -- Added BY PJShah ON 19042022
 ...
 ,COALESCE(CL.MASTER_ID,AUTHS.CLIENT_CODE) AS CLIENT_NUMBER
 ...
 ,COALESCE(
  ...
  ,CASE WHEN AUTHS.SERVICEID IN (''CARECO'',''VBPCG'')
	...
	ELSE AUTHS.SOURCE_SYSTEM_CODE  END AS SYSTEM_CODE
