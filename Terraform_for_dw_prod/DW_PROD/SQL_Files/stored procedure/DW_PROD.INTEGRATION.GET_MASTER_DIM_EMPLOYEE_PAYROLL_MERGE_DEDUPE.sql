CREATE OR REPLACE PROCEDURE DW_PROD.INTEGRATION.GET_MASTER_DIM_EMPLOYEE_PAYROLL_MERGE_DEDUPE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
BEGIN
INSERT OVERWRITE INTO DW_PROD.INTEGRATION.DIM_EMPLOYEE_PAYROLL_MERGE_DEDUPE
SELECT 
*
from(
  WITH LWD_NEW AS
...
FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_PAYROLL_RL_OUTPUT R
left join DW_PROD.HAH.DIM_EMPLOYEE E ON R.ID = E.EMPLOYEE_KEY
...
FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_PAYROLL_RL_OUTPUT
WHERE CLUSTER_ID IS NOT NULL
),
NEW AS (
SELECT *
...
EMPLOYEE_HIRE_DATE desc,
    EMPLOYEE_ID desc
        ...
        PA.CLUSTER_ID
    FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_PAYROLL_RL_OUTPUT PA
    LEFT JOIN LWD_NEW 
            ON LWD_NEW.ID = PA.ID
...
LEFT JOIN DW_PROD.HAH.DIM_EMPLOYEE e on N.id = e.employee_key
...
FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_PAYROLL_RL_OUTPUT U
LEFT JOIN DW_PROD.HAH.DIM_EMPLOYEE e on U.id = e.employee_key
