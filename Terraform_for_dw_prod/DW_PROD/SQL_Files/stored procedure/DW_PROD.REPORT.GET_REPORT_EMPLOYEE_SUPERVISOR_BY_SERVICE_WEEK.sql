CREATE OR REPLACE PROCEDURE DW_PROD.REPORT.GET_REPORT_EMPLOYEE_SUPERVISOR_BY_SERVICE_WEEK("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--
-- PURPOSE: Creates a record for each employee-supervisor combination for each service week
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------        
...
INSERT OVERWRITE INTO Report.Employee_Supervisor_By_Service_Week
SELECT DISTINCT HAH_CURRENT_WEEK_FIRST_DAY AS Service_Week, 
       vis.Employee_Key,
...
on dt.calendar_date = vis.service_date
WHERE SERVICE_DATE BETWEEN CAST( ''1950-01-01'' AS DATE) AND DATEADD( ''DAY'', 7, CURRENT_DATE)
ORDER BY 1, 2;
