CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_DATAFLEXSYNCDATA_FACT_REVENUE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INSERT OVERWRITE INTO STAGE.DataFlexSyncData_FACT_REVENUE
      WITH CLIENT_DATA AS (SELECT *
             ...
             VIS2 AS (SELECT MAX(TH.REFERENCENO)                   AS REFERENCENO,
                      ...
                      FROM DISC_PROD.DATAFLEXSYNCDATA.ALL_TIMESHEETSHOURS TH
                               LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.ALL_DFTIMESHEETS TS1
        ...
        FROM DISC_PROD.DATAFLEXSYNCDATA.ALL_DFINVOICEDETAILS det
            ...
            AND inv.ETL_DELETED_FLAG = FALSE
                 LEFT JOIN (SELECT SUM(DFPD.AMOUNT)      AS AMOUNT,
                            ...
                            FROM DISC_PROD.DATAFLEXSYNCDATA.DFPAYMENTDETAILS DFPD
                                     LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFPAYMENTS AS DFP
            ...
            AND det.INVOICENO = pay.INVOICENO
                 LEFT JOIN (SELECT DFIP.INVOICENO,
                            ...
                            FROM DISC_PROD.DATAFLEXSYNCDATA.DFINVOICEPAYMENTS DFIP
                            WHERE DFIP.CODE <> ''WO''
            ...
            AND con.ETL_DELETED_FLAG = FALSE
                 LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFCLIENTSUPERVISORS spr ON det.DBNAME = spr.DBNAME
        ...
        INSERT INTO DW_PROD.STAGE.DATAFLEXSYNCDATA_FACT_REVENUE
        ...
        FROM AR_DIM_INVOICE AS INV
                 LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFCONTRACTS con --Added on 14/06/23 for RCM by Pinkal
