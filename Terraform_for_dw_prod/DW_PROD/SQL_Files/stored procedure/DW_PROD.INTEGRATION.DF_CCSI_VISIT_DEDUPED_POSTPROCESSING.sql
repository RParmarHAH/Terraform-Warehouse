CREATE OR REPLACE PROCEDURE DW_PROD.INTEGRATION.DF_CCSI_VISIT_DEDUPED_POSTPROCESSING("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
BEGIN
	...
	CALL DW_PROD.INTEGRATION.DF_CCSI_VISIT_DEDUPE_MERGED_POSTPROCESSING(1,''1999-01-01'',''9999-12-31'');
	
	DELETE from DW_PROD.HAH.FACT_VISIT where visit_key in (select CCSI_VISIT_KEY from DW_PROD.INTEGRATION.DF_CCSI_VISIT_DEDUPE_MERGED where CCSI_SOURCE_SYSTEM_ID = 8);
	
	SELECT CONCAT(''Message : '',"number of rows deleted",'' Rows Deleted.'') into :return_result_temp FROM TABLE(RESULT_SCAN(LAST_QUERY_ID()));

	MERGE into DW_PROD.HAH.FACT_VISIT AS target
using (
SELECT DISTINCT DF_VISIT_KEY, DUMMY_EMPLOYEE_KEY, ORIGINAL_EMPLOYEE_KEY, EMPLOYEE_KEY, EMPLOYEE_ID FROM (
  ...
  DF_VISIT_KEY,
        ...
        FROM DW_PROD.INTEGRATION.DF_CCSI_VISIT_DEDUPE_MERGED V
        LEFT JOIN DW_PROD.integration.dim_employee_merged E ON V.CCSI_EMPLOYEE_KEY = E.ORIGINAL_EMPLOYEE_KEY
        LEFT JOIN DW_PROD.INTEGRATION.DF_CCSI_VISIT_DEDUPE D ON V.CCSI_VISIT_KEY = D.VISIT_KEY AND D.SOURCE_SYSTEM_ID = 8 
