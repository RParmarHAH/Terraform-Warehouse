CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_OSHAH_FACT_VISIT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
FROM CLIENT P
JOIN DISC_PROD.HHAEXCHANGEOSHAH.PROVIDERPATIENTCOORDINATORS PC
...
FROM 	DISC_PROD.HHAEXCHANGEOSHAH.CALLMAINTENANCE CM
INNER JOIN DISC_PROD.HHAEXCHANGEOSHAH.VISITS V
...
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.CALENDARREASONNOTES CRN 
...
FROM DISC_PROD.HHAEXCHANGEOSHAH.PAYMENTS
)
,
...
FROM DISC_PROD.HHAEXCHANGEOSHAH.VISITAUTHORIZATIONS_REPL VAR
JOIN DISC_PROD.HHAEXCHANGEOSHAH.PATIENTAUTHORIZATION PA ON PA.AUTHORIZATIONID = VAR.AUTHORIZATIONID
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.PATIENTS P ON P.PATIENTID = VAR.PATIENTID 
...
SELECT UV.VISITID
FROM DISC_PROD.HHAEXCHANGEOSHAH.UNTOUCHEDVISITS UV
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.INREVIEWVISITS IV
...
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.PATIENTS P ON P.PATIENTID = CLI.PATIENTID
...
LEFT JOIN   DW_PROD."STAGE".OSHAH_DIM_VISIT_STATUS_MAPPING VSM 
			ON DS.VISIT_STATUS_KEY=VSM.VISIT_STATUS_KEY
LEFT JOIN 	DISC_PROD.HHAEXCHANGEOSHAH.PAYROLL_VISITS_REPL PAYROLL 
			ON VISIT.VISITID = PAYROLL.VISITID
LEFT JOIN 	DISC_PROD.HHAEXCHANGEOSHAH.CONTRACTS CONTRACT
