CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_OSHAH_FACT_ADMISSION("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
SELECT * FROM
    (SELECT DISTINCT PATIENTID, MASTER_ID, AGENCYID, LASTNAME, FIRSTNAME, OFFICEID
	 FROM DISC_DEDUPE_PROD.HHAEXCHANGEOSHAH.CLIENT_MATCH_LIST CML
...
SELECT * FROM (SELECT ROW_NUMBER () OVER(PARTITION BY TC.COORDINATORNAME ORDER BY TC.COORDINATORID  DESC) AS RN,
			* FROM DISC_PROD.HHAEXCHANGEOSHAH.TBLCOORDINATOR_REPL TC) WHERE RN=1
),	
ADMISSIONS_DATE AS(
SELECT * FROM (SELECT ROW_NUMBER () OVER(PARTITION BY P.PATIENTID  ORDER BY P.SERVICESTARTDATE  DESC) AS RN,
			* FROM DISC_PROD.HHAEXCHANGEOSHAH.PATIENTS P) WHERE RN=1
),
VISITS AS (
SELECT * FROM ( SELECT V.PATIENTID,VI.PRIMARYSERVICECODEID AS SERVICECODEID FROM  DISC_PROD.HHAEXCHANGEOSHAH.VISITS V 
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.VISITINFO_REPL VI ON VI.VISITID = V.VISITID
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.TBLVISITS_REPL TBLV ON TBLV.VISITID = VI.VISITID
UNION ALL 
 SELECT PA.PATIENTID,SERVICECODEID FROM  DISC_PROD.HHAEXCHANGEOSHAH.PATIENTAUTHORIZATION  PA)
 GROUP BY 1,2)
...
FROM CLIENT
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.PATIENTS P
ON P.PATIENTID = CLIENT.PATIENTID
LEFT JOIN VISITS V ON V.PATIENTID = CLIENT.PATIENTID
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.PATIENTCONTRACTS_REPL PC ON P.PATIENTID = PC.PATIENTID 
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.REFERRAL  R
ON P.ADMISSIONID = R.ADMISSIONID 
LEFT JOIN ADMISSIONS_DATE AS ADM
ON ADM.PATIENTID = P.PATIENTID 
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.REFERRALSOURCE  RS
ON P.REFERRALSOURCEID = RS.REFERRALSOURCEID  
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.OFFICE_OFFICES_REPL O ON CLIENT.OFFICEID = O.OFFICEID
 JOIN DISC_PROD.HHAEXCHANGEOSHAH.SERVICECODES SC ON SC.SERVICECODEID  = V.SERVICECODEID 
