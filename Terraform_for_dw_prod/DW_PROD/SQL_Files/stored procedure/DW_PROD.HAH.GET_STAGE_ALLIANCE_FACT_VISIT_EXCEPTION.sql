CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_ALLIANCE_FACT_VISIT_EXCEPTION("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
, EXCEPTION_DUP_HANDLING AS (
	SELECT
		...
		ACKNOWLEDGED
	FROM 
		DISC_PROD.GENERATIONSALLIANCE.TELEPHONYEXCEPTIONS
)
, CALLS AS (
	SELECT DISTINCT
		TE.SCHEDULEID,
	...
	LEFT JOIN
		DISC_PROD.GENERATIONSALLIANCE.TELEPHONYREASONS TREA
		ON TE.REASONID = TREA.REASONID
	LEFT JOIN 
		DISC_PROD.GENERATIONSALLIANCE.TELEPHONYRESOLUTIONS TRESO
		ON TE.RESOLUTIONID = TRESO.RESOLUTIONID
	INNER JOIN 
		DISC_PROD.GENERATIONSALLIANCE.SCHEDULES SCH
