CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_AXXESS_FACT_INVOICE_TRANSACTIONS("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
        ...
        WITH HOMECARE AS
                 (SELECT MD5(''PRIME''
                  ...
                  FROM DISC_PROD.AXXESS.AXXESS_FINANCIALTRANSACTIONS FIN
                           LEFT JOIN DISC_PROD.AXXESS.AXXESS_VISITFINANCIALS VS ON FIN.CLAIM_ID = VS.CLAIM_ID
             ...
             trans as (select row_number() over (partition by fact_ar_id order by fact_ar_id) row_num, *
                       from disc_PROD.AXXESS.axxess_transactions),
             HOMEHEALTH AS (SELECT distinct MD5(''PRIME'' || ''-'' || IFNULL(UPPER(TRIM(TRANS.FACT_AR_ID))
                 ...
                 || ''-'' || ''AXXESS'')                                                      AS TRANSACTION_KEY
                     ...
                     , CURRENT_TIMESTAMP)::TIMESTAMP_NTZ                                      AS ETL_LAST_UPDATED_DATE
                                          ...
                                          , FALSE                                             AS ETL_DELETED_FLAG
                            FROM TRANS
                                     LEFT JOIN DISC_PROD.AXXESS.VW_AXXESS_HH_VISITS_UPPER VISIT
