CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_CLEARCARE_FACT_VISIT_BKCP_0525("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO STAGE.CLEARCARE_FACT_VISIT_BKCP_0525
WITH EMPLOYEE AS 
(
	SELECT
		*
...
DISC_PROD.CLEARCARE.carelogs_carelog 
...
SELECT
	...
	,MD5(
        ''CLEARCARE'' ||''-''|| CL.MASTER_ID::INT ||''-''|| ''CLEARCARE'' 
    ) as CLIENT_KEY
...
LEFT JOIN DISC_PROD.CLEARCARE.profile_patient_agencylocation AS PP ON CA.patient_id = PP.patient_id
LEFT JOIN DISC_PROD.CLEARCARE.BILLING_CLIENTAUTHORIZATION AU ON AU.ID = CA.AUTHORIZATION_ID
LEFT JOIN DISC_PROD.CLEARCARE.PROFILE_PARENTPAYER PY ON PY.ID = AU.PARENT_PAYER_ID
LEFT JOIN DISC_PROD.CLEARCARE.PROFILE_PARENTPAYERSERVICE SC ON SC.ID = AU.SERVICE_ID
LEFT JOIN DISC_PROD.CLEARCARE.SCHEDULES_SHIFT S ON S.ID = CA.SHIFT_ID
--KEY JOINS
...
LEFT JOIN DISC_PROD.CLEARCARE.RECEIVABLES_INVOICE_CARELOGS  R on R.CARELOG_ID  = CA.id
LEFT JOIN DISC_PROD.CLEARCARE.INVOICE_TRANSACTION INV on INV.id = R.INVOICE_ID 
