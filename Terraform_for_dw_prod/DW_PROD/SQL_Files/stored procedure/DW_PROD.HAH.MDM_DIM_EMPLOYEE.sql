CREATE OR REPLACE PROCEDURE DW_PROD.HAH.MDM_DIM_EMPLOYEE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
RETURNS VARCHAR(16777216)
LANGUAGE JAVASCRIPT
EXECUTE AS OWNER
AS '
        var sql = `
    INSERT OVERWRITE INTO DW_PROD.HAH.MDM_DIM_EMPLOYEE
with employee_gp as 
(SELECT *
...
, emp.LAST_CHECK_DATE DESC, emp.HIRE_DATE DESC, emp.FIRST_CHECK_DATE DESC, ETL_LAST_UPDATED_DATE DESC, ETL_INSERTED_DATE DESC) 
as rnk_pay
FROM DISC_PROD.DATA_VAULT.VW_SAT_EMPLOYEE_GP emp),
...
(
    ...
    FROM DISC_PROD.DATA_VAULT.VW_SAT_EMPLOYEE_DATAFLEX DFEMP
...
LEFT OUTER JOIN DW_PROD.HAH.DIM_Branch AS brnch ON UPPER( TRIM(dataflex_gp.DBName)) = brnch.System_Code 
AND 3 = brnch.Source_System_ID AND dataflex_gp.OfficeNumber = brnch.Office_Number
LEFT OUTER JOIN DW_PROD.HAH.DIM_BRANCH AS crp_brnch on 1 =1 and crp_brnch.branch_name = ''CORPORATE'' and crp_brnch.Source_System_ID=5
...
LEFT OUTER JOIN DW_PROD.HAH.DIM_Branch AS brnch ON UPPER( TRIM(dataflex.DBName)) = brnch.System_Code 
...
`;
	...
	sql = `UPDATE DW_PROD.HAH.MDM_DIM_EMPLOYEE MDM 
...
HIRE_DATE,
TERMINATE_DATE
FROM DISC_PROD.DATA_VAULT.SAT_EMPLOYEE_GP
WHERE SSN IN (SELECT SSN FROM  DISC_PROD.DATA_VAULT.SAT_EMPLOYEE_GP WHERE DB = ''HAHIL'')
