CREATE OR REPLACE PROCEDURE DW_PROD.INTEGRATION.GET_MASTER_DIM_EMPLOYEE_MS_MERGED_DEDUPE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
(
	...
	CLUSTER_ID
	FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_MS_RL_OUTPUT
...
,NEW AS (
    SELECT * FROM (
        SELECT
            ...
            ,PA.CLUSTER_ID 
        FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_MS_RL_OUTPUT PA
        INNER JOIN LWD_NEW 
            ON LWD_NEW.ID = PA.ID
...
FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_MS_RL_OUTPUT R
LEFT JOIN OLD O 
...
FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_MS_RL_OUTPUT U
