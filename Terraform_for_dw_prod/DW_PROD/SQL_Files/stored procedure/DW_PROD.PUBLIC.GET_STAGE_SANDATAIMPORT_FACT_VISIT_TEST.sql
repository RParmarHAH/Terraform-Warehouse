CREATE OR REPLACE PROCEDURE DW_PROD.PUBLIC.GET_STAGE_SANDATAIMPORT_FACT_VISIT_TEST("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO PUBLIC.SANDATAIMPORT_FACT_VISIT_2 
...
,DERIVED_STATUS AS (
   ...
   FROM DISC_PROD.BI_REPOSITORY.SANDATAVISITS SV WHERE AGENCYID =''8485''
    ...
    SELECT DISTINCT MD5(SV.AGENCYID || ''-'' || SV.SCHEDULEID || ''-'' || ''SANDATAIMPORT'') AS VISIT_KEY,
	SV.SCHEDULEDATE AS REPORT_DATE,
	MD5(NVL(SV.AGENCYID, ''S'') || ''-'' || COALESCE(SV.LOCATIONID, '''') || ''-'' || ''SANDATAIMPORT'') AS BRANCH_KEY,
	MD5(NVL(SV.AGENCYID, ''S'') || ''-'' || SV.CLIENTID || ''-'' || ''SANDATAIMPORT'') AS CLIENT_KEY,
	MD5(NVL(SV.AGENCYID, ''S'') || ''-'' || SV.ADMISSIONTYPE || ''-'' || ''SANDATAIMPORT'') AS CONTRACT_KEY,
	...
	NVL(E.EMPLOYEE_KEY, MD5(NVL(SV.AGENCYID, ''S'') || ''-'' || UPPER(COALESCE(TRY_CAST(SV.STAFFAGENCYID AS INT)::STRING, TRIM(SV.STAFFAGENCYID), '''')) || ''-'' || ''1900-01-01'' || ''-'' || ''SANDATAIMPORT'')) AS EMPLOYEE_KEY,
	...
	COALESCE(TRY_CAST(SEMP.STAFFAGENCYID AS INT)::STRING, UPPER(TRIM(SV.STAFFAGENCYID))) AS SCHEDULED_EMPLOYEE_ID,								
	COALESCE(TRY_CAST(SV.STAFFAGENCYID AS INT)::STRING, UPPER(TRIM(SV.STAFFAGENCYID)), ''-1'') AS EMPLOYEE_ID,
...
LEFT JOIN HAH.DIM_VISIT_STATUS_MAPPING STATUS ON DSTATUS.VISIT_STATUS_KEY = STATUS.VISIT_STATUS_KEY
--LEFT JOIN HAH.DIM_VISIT_STATUS_MAPPING STATUS ON SV.AGENCYID = STATUS.system_code AND DSTATUS.DERIVED_STATUS = STATUS.DERIVED_DISC_STATUS AND STATUS.SOURCE_SYSTEM_ID=4
...
`;
	...
	sql = `
MERGE INTO PUBLIC.SANDATAIMPORT_FACT_VISIT_2 AS TARGET USING ( 
WITH clu as (
	    select
...
,DERIVED_STATUS AS (
	...
	SELECT DISTINCT
	    ...
	    md5(nvl(f.agencyID,''S'')|| ''-'' ||  coalesce(ClientAdmissions.CLIENTID,clu.CLIENTID,mc.CLIENTID) || ''-''  ||  ''SANDATAIMPORT'' ) AS CLIENT_KEY,
	    md5(nvl(f.agencyID,''S'') || ''-'' || f.ClientAdmitType || ''-''  ||  ''SANDATAIMPORT'') AS CONTRACT_KEY,
		md5(nvl(f.agencyID,''S'') || ''-'' || upper(TRIM(SEMP.STAFFAGENCYID)) || ''-'' || ''1900-01-01'' || ''-'' || ''SANDATAIMPORT'') AS SCHEDULED_EMPLOYEE_KEY,
	    nvl(e.EMPLOYEE_KEY, md5(nvl(f.agencyID,''S'') || ''-'' || upper(NVL(TRIM(f.STAFFAGENCYID), '''')) || ''-'' || ''1900-01-01'' || ''-'' || ''SANDATAIMPORT'')) AS EMPLOYEE_KEY,
	    4 AS SOURCE_SYSTEM_ID,
