CREATE OR REPLACE PROCEDURE DW_PROD.HAH.FACT_CLIENT_ACQUIRED_WEEKLY_POSTPROCESSING()
...
INSERT OVERWRITE INTO HAH.FACT_CLIENT_ACQUIRED_WEEKLY
...
--MATRIXCARE
SELECT DISTINCT 
	MD5(CLIENT_MERGED.CLIENT_KEY || VISIT.CONTRACT_KEY || BRANCH_MERGED.BRANCH_KEY || DD.HAH_CURRENT_WEEK_FIRST_DAY) AS CLIENT_ACQUIRED_KEY,
	CLIENT_MERGED.ORIGINAL_CLIENT_KEY,
	CLIENT_MERGED.CLIENT_KEY, CLIENT_MERGED.CLIENT_NUMBER, VISIT.SOURCE_SYSTEM_ID, VISIT.SYSTEM_CODE,BRANCH_MERGED.ORIGINAL_BRANCH_KEY, BRANCH_MERGED.BRANCH_KEY, CONTRACT_KEY, DD.HAH_CURRENT_WEEK_FIRST_DAY AS SERVICE_WEEK, ''ADAPTIVE'' AS AQUISITION_NAME,
...
--ADAPTIVE
SELECT DISTINCT 
	MD5(CLIENT_MERGED.CLIENT_KEY || VISIT.CONTRACT_KEY || BRANCH_MERGED.BRANCH_KEY || DATE_TRUNC(''MONTH'', REPORT_DATE)) AS CLIENT_ACQUIRED_KEY,
	CLIENT_MERGED.ORIGINAL_CLIENT_KEY,
	CLIENT_MERGED.CLIENT_KEY, CLIENT_MERGED.CLIENT_NUMBER, VISIT.SOURCE_SYSTEM_ID, VISIT.SYSTEM_CODE,BRANCH_MERGED.ORIGINAL_BRANCH_KEY, BRANCH_MERGED.BRANCH_KEY, CONTRACT_KEY, DATE_TRUNC(''MONTH'', REPORT_DATE) AS SERVICE_MONTH, ''ADAPTIVE'' AS AQUISITION_NAME,
	-1 AS ETL_TASK_KEY, -1 AS ETL_INSERTED_TASK_KEY, /*VISIT.ETL_INSERTED_DATE, VISIT.ETL_INSERTED_BY,*/ CONVERT_TIMEZONE(''UTC'', CURRENT_TIMESTAMP)::TIMESTAMP_NTZ AS ETL_LAST_UPDATED_DATE, CURRENT_USER AS ETL_LAST_UPDATED_BY, FALSE AS ETL_DELETED_FLAG
FROM HAH.FACT_VISIT VISIT
INNER JOIN HAH.DIM_BRANCH BRANCH
	ON VISIT.BRANCH_KEY = BRANCH.BRANCH_KEY
...
BRANCH_MERGED.ORIGINAL_BRANCH_KEY ,BRANCH_MERGED.BRANCH_KEY, CONTRACT_KEY, DD.HAH_CURRENT_WEEK_FIRST_DAY AS SERVICE_WEEK, ''CCSI'' AS AQUISITION_NAME,
	-1 AS ETL_TASK_KEY, -1 AS ETL_INSERTED_TASK_KEY, /*VISIT.ETL_INSERTED_DATE, VISIT.ETL_INSERTED_BY,*/ CONVERT_TIMEZONE(''UTC'', CURRENT_TIMESTAMP)::TIMESTAMP_NTZ AS ETL_LAST_UPDATED_DATE, CURRENT_USER AS ETL_LAST_UPDATED_BY, FALSE AS ETL_DELETED_FLAG
FROM HAH.FACT_VISIT VISIT
INNER JOIN HAH.DIM_BRANCH BRANCH
	ON VISIT.BRANCH_KEY = BRANCH.BRANCH_KEY
