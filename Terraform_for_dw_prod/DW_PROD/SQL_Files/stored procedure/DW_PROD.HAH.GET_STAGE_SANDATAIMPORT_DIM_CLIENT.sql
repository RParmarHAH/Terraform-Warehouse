CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_SANDATAIMPORT_DIM_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO DW_PROD.STAGE.SANDATAIMPORT_DIM_CLIENT
WITH VISIT AS (
    SELECT
...
-- F.STATUS_CODE IN (''02'', ''03'',''04'', ''05'')
	...
	--master query
		...
		FROM DISC_DEDUPE_PROD.SANDATAIMPORT.CLIENT_MASTER_LIST cl
		LEFT JOIN DISC_PROD.SANDATAIMPORT.SANDATA_CLIENTADMISSIONS cad ON 
		...
		LEFT JOIN DISC_PROD.SANDATAIMPORT.SANDATA_REFERRALSOURCES rf ON rf.REFERRALID =iff(a.adstatus=''02'',a.adref, a.refval)
		LEFT JOIN DW_PROD.HAH.DIM_CLIENT cc ON a.clientid = cc.CLIENT_NUMBER  AND cc.SYSTEM_CODE =''8485''
	)
SELECT 
