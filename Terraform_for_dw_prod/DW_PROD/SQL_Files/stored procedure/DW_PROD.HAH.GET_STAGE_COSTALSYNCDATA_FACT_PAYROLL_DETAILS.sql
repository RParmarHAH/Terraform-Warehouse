CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_COSTALSYNCDATA_FACT_PAYROLL_DETAILS("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INSERT OVERWRITE INTO DW_PROD.STAGE.COSTALSYNCDATA_FACT_PAYROLL_DETAILS
...
(
	SELECT DISTINCT 
		...
		CASE UPPER(TRIM(GP.DB)) WHEN ''ALTRU'' THEN ''SHC_ALTRUS'' WHEN ''SHC'' THEN ''SHC_SAVANNAH'' END  AS SYSTEM_CODE,
	...
	FROM (SELECT * FROM DISC_PROD.COSTALSYNCDATA.GPALLPAYCHECKDETAILSBASE WHERE AMOUNT <> 0) GP 
	LEFT JOIN DISC_PROD.COSTALSYNCDATA.GPALLPAYCHECKSBASE CB
...
LEFT JOIN DW_PROD.HAH.DIM_EMPLOYEE emp
...
--LEFT JOIN DW_PROD.HAH.DIM_PAYCODE_MAPPING BRANCH_MAPPING
...
LEFT JOIN DW_PROD.HAH.DIM_SOURCE_SYSTEM  SS 
	ON TRIM(CASE ss.SYSTEM_DESCRIPTION 
		...
		ELSE NULL END) = PD.SYSTEM_CODE
LEFT JOIN DISC_PROD.COSTALSYNCDATA.GPALLPAYCODESBASE CD
	ON TRIM(CD.PAY_CODE) = TRIM(PD.PAY_CODE)
	AND CD.DB = PD.DB
LEFT JOIN DISC_PROD.COSTALSYNCDATA.PAYCODE_CATEGORY PC
