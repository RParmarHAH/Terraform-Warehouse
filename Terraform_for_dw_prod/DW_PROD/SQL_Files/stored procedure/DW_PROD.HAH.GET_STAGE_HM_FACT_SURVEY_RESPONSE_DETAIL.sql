CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_HM_FACT_SURVEY_RESPONSE_DETAIL("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--*****************************************************************************************************************************
--
INSERT OVERWRITE INTO DW_PROD.STAGE.HM_FACT_SURVEY_RESPONSE_DETAIL
WITH HEADER AS 
	(SELECT  
...
,QUESTIONS_ANSWERS AS 
	...
	JOIN DW_PROD.STAGE.HM_FACT_SURVEY_QUESTION FSQ ON R.QUESTION_ID=FSQ.QUESTION_ID)
,LABLES AS 
	(SELECT
	HEADER.RESPONSE_ID,
	REPLACE(f.KEY,''#'','''') AS QUESTION_ID, 
