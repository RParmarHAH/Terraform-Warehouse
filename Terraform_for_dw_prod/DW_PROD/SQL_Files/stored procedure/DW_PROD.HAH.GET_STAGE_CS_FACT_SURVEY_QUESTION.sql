CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_CS_FACT_SURVEY_QUESTION("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
-- DATE        AUTHOR             	NOTES:
-- --------    -------------------  ---------------------------------------------------------------------------
-- 01/11/2022  Paras Bhavnani	    Initial DEVelopment
--*****************************************************************************************************************************
--
INSERT OVERWRITE INTO STAGE.CS_FACT_SURVEY_QUESTIONS 
WITH MAIN AS 
	...
	*
	FROM DISC_PROD.QUALTRICS_SURVEYS.QUALTRICS_SURVEY_QUESTION, table(flatten(SURVEY_QUESTION_JSON:result:questions )) f
...
,FINAL_CLEANED AS (
    ...
    FROM FINAL Q
    LEFT JOIN DISC_PROD.QUALTRICS_SURVEYS.QUESTION_MAPPING_EXTRACT_TO_API M ON M.OLD_SQID = REPLACE(Q.QUESTION_ID,''#'','''')
)
SELECT 
