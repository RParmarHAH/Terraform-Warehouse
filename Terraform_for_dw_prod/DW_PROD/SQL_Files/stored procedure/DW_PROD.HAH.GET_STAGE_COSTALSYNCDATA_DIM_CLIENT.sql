CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_COSTALSYNCDATA_DIM_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
(
	...
	FROM DISC_PROD.COSTALSYNCDATA.CV_SCHEDULES AS SCHEDULES
    ...
    FROM "DISC_PROD"."COSTALSYNCDATA"."CV_ADMISSIONS" A
    JOIN CLIENT_DATA AS CLIENTS
...
--      NULL AS CLIENT_ETHNICITY,
  ...
  FROM DISC_DEDUPE_PROD.COSTALSYNCDATA.CLIENT_MASTER_LIST F 
  LEFT JOIN FIRST_SERVICE_DATES AS FSD
    ON TRIM(FSD.DB) = TRIM(F.DB) AND TRIM(FSD.PATIENT_NUMBER) = TRIM(F.PATIENT_NUMBER)
  LEFT JOIN "DISC_PROD"."COSTALSYNCDATA"."CV_LOCATIONS" L ON TRIM(F.DB) = TRIM(L.DB) AND TRIM(F.LOCATION_CODE) = TRIM(L.LOCATION_CODE)
  LEFT JOIN HAH.DIM_STATE ST ON ST.STATE_ISO_CODE = NULLIF(TRIM(L.STATE_CODE), '''')
  LEFT JOIN "DISC_PROD"."COSTALSYNCDATA"."CV_DEPARTMENTS" D ON TRIM(F.DB) = TRIM(D.DB) AND TRIM(F.DEPARTMENT__CODE) = TRIM(D.DEPARTMENT__CODE)
