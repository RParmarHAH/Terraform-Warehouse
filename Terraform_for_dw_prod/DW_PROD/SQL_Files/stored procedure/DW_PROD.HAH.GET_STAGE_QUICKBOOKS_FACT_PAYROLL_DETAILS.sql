CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_QUICKBOOKS_FACT_PAYROLL_DETAILS("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
FROM DISC_PROD.ADP.QB_PAYROLL_DETAILS PAY
LEFT JOIN DW_PROD.STAGE.QUICKBOOKS_DIM_PAYCODE_MAPPING PCM
...
SELECT DISTINCT 
	...
	REPLACE(AMOUNT, '','') AS AMOUNT,
		...
		current_user() AS ETL_LAST_UPDATED_BY,
		FALSE ETL_DELETED_FLAG
FROM DISC_PROD.ADP.QB_PAYROLL_DETAILS_MIDWAY_2020 PAY
LEFT JOIN DW_PROD.STAGE.QUICKBOOKS_DIM_PAYCODE_MAPPING PCM
	ON ''QB - '' || PAY.COMPANY = PCM.SYSTEM_CODE
	AND REPLACE(PAY.PAYCODES, ''_'', '' '')  = PCM.PAY_CODE
LEFT JOIN DISC_PROD.ADP.QB_SSN_MAPPING QB_MAPPING
