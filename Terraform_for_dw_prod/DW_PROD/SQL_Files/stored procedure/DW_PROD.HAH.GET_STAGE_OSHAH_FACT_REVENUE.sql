CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_OSHAH_FACT_REVENUE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
(
	...
	(
		...
		FROM DISC_DEDUPE_PROD.HHAEXCHANGEOSHAH.EMPLOYEE_MASTER_LIST
	)
	UNION
	SELECT * FROM
	(
		SELECT DISTINCT CAREGIVERID, MASTER_ID
		FROM DISC_DEDUPE_PROD.HHAEXCHANGEOSHAH.EMPLOYEE_MATCH_LIST
...
FROM DISC_PROD.HHAEXCHANGEOSHAH.TBLINVOICEDETAILS_REPL DET
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.BILLING_PAIDVISITS_REPL BV ON BV.VISITID = DET.VISITID
AND BV.PATIENTID = DET.PATIENTID
-- PAYMENTS Join change on 28/04/23 for RCM by Pinkal
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.PAYMENTS PT 
...
, CONTRACTS AS (
	...
	FROM DISC_PROD.HHAEXCHANGEOSHAH.SERVICECODES SC
		LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.PAYER_REPL PR ON IFNULL(PR.CONTRACTID,PR.PAYERID) = SC.ContractID
		LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.PAYEROFFICES PO ON PR.PAYERID= PO.PAYERID --OR P.PAYERID = C.LINKEDCONTRACTPAYERID --ADDED
...
FROM DISC_PROD.HHAEXCHANGEOSHAH.TBLINVOICEDETAILS_REPL INV_DET
INNER JOIN DISC_PROD.HHAEXCHANGEOSHAH.TBLINVOICEHEADER_REPL INV_HED ON INV_DET.INVOICEHEADERID = INV_HED.INVOICEHEADERID
INNER JOIN CLIENT CLI ON INV_HED.PATIENTID = CLI.PATIENTID
INNER JOIN EMPLOYEE EMP ON INV_DET.CAREGIVERID = EMP.CAREGIVERID
INNER JOIN DISC_PROD.HHAEXCHANGEOSHAH.OFFICE_OFFICES_REPL OFFICE ON INV_HED.OFFICEID = OFFICE.OFFICEID
LEFT JOIN BILLING BNG ON INV_DET.INVOICEDETAILID = BNG.INVOICEDETAILID
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.TBLVISITS_REPL VISIT ON INV_DET.VISITID = VISIT.VISITID
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.TBLCOORDINATOR_REPL SUP ON VISIT.COORDINATORID = SUP.COORDINATORID
