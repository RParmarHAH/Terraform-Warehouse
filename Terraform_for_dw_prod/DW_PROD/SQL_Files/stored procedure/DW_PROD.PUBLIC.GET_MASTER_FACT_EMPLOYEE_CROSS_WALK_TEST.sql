CREATE OR REPLACE PROCEDURE DW_PROD.PUBLIC.GET_MASTER_FACT_EMPLOYEE_CROSS_WALK_TEST("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
BEGIN
--*****************************************************************************************************************************
-- NAME:  GET_MASTER_FACT_EMPLOYEE_CROSS_WALK_TEST
-- 
...
UPDATE DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST CW
...
CREATE OR REPLACE TABLE DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST 
CLONE DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_TEST;
...
UPDATE DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST CW_D
SET CW_D.AMS_EMPLOYEE_KEY=EM.MASTER_EMPLOYEE_KEY
FROM  DISC_DEDUPE_PROD.PUBLIC.VW_EMPLOYEE_MATCH_LIST EM 
...
AND EM.MASTER_EMPLOYEE_KEY NOT IN 
	...
	FROM DW_PROD.INTEGRATION.FACT_AMS_PAYROLL_MAPPING 
...
FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_AMS_MERGED DM
...
FROM  DISC_DEDUPE_PROD."PUBLIC".VW_PAYROLL_EMPLOYEE_MATCH_LIST PM 
...
FROM DW_PROD.INTEGRATION.DIM_EMPLOYEE_PAYROLL_MERGE_DEDUPE PM 
...
UPDATE DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST CW
SET DUP_FLAG = TRUE ,DUP_FLAG_REASON = ''MAPPING_INTERCHANGED''
FROM (SELECT  AMS_EMPLOYEE_KEY||''-''||PAYROLL_EMPLOYEE_KEY A_P
	FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST D
		WHERE EXISTS  
			(SELECT DISTINCT 1 FROM DW_PROD.INTEGRATION.FACT_AMS_PAYROLL_MAPPING M 
	...
	FROM DW_PROD.INTEGRATION.FACT_AMS_PAYROLL_MAPPING D
		WHERE EXISTS  
			(SELECT DISTINCT 1 FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST M 
			WHERE M.AMS_EMPLOYEE_KEY = D.AMS_EMPLOYEE_KEY )
		AND EXISTS  
			(SELECT DISTINCT 1 FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST M 
			WHERE M.PAYROLL_EMPLOYEE_KEY = D.PAYROLL_EMPLOYEE_KEY )
...
CREATE OR REPLACE TABLE DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TBL_MASTER_TEST 
AS SELECT * FROM DW_PROD.INTEGRATION.VW_EMPLOYEE_CROSS_WALK;

INSERT INTO DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_CHANGE_LOG_TEST
-----------------------------------
...
WITH KEYS_CHANGED AS 
	...
	FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST CW
	JOIN DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST  D1
	...
	LEFT JOIN DISC_DEDUPE_PROD."PUBLIC".VW_EMPLOYEE_MATCH_LIST VW 
	...
	JOIN DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST  D1
		...
		AND CW.INACTIVE <> TRUE 
		AND CW.DUP_FLAG <> TRUE
	LEFT JOIN DISC_DEDUPE_PROD."PUBLIC".VW_PAYROLL_EMPLOYEE_MATCH_LIST VW 
...
--		FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_CHANGE_LOG WHERE CHANGE_TYPE = ''KEY_CHANGE'')
	)
----------------------------------
...
,AMS_MAPP AS
	...
	FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST CW 
	JOIN DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TEST VW
...
--		FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_CHANGE_LOG)
)
...
UPDATE DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST TEMP_C
...
FROM
	...
	FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TEST VW
	LEFT JOIN DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_CHANGE_LOG_TEST CL -- JOINING ON CHANGE LOG TABLE TO GET THE LATEST KEYS
		ON COALESCE(CL.LATEST_INTEGRATION_LAYER_KEY,CL.LATEST_DISC_LAYER_KEY,CL."AMS / PAYROLL_KEY") = VW.AMS_EMPLOYEE_KEY
		AND CL.ETL_INSERTED_DATE = (SELECT MAX(ETL_INSERTED_DATE) FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_CHANGE_LOG_TEST)
	) SRC
...
--SELECT * FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_MERGED;

--CREATE OR REPLACE TABLE DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_MERGED
--CLONE DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_MERGED;


INSERT INTO DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_MERGED_TEST
-- Logic
-------------------------------------------
-- ************* CASE 1 ********************
...
WITH CROSSWALK AS
	...
	FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST
...
FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST CWD
JOIN DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST CWM -- NEED TO JOIN TO GET OLDER KEY 
	ON CWD.EMPLOYEE_ENTERPRISE_ID = CWM.EMPLOYEE_ENTERPRISE_ID 
WHERE CWD.AMS_EMPLOYEE_KEY IN 
	(
	SELECT AMS_EMPLOYEE_KEY
	FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST 
	WHERE INACTIVE <> TRUE AND DUP_FLAG <> TRUE 
	-- FILTERING THOSE KEYS WHICH 
	AND AMS_EMPLOYEE_KEY IN 
		(SELECT COALESCE(LATEST_INTEGRATION_LAYER_KEY,LATEST_DISC_LAYER_KEY)
		FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_CHANGE_LOG_TEST
		WHERE CHANGE_TYPE =''KEY_CHANGE''
		AND KEY_TYPE= ''AMS'' 
		AND ETL_INSERTED_DATE = (SELECT MAX(ETL_INSERTED_DATE) FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_CHANGE_LOG_TEST)
		)
...
,CROSSWALK_NOT_MAPPED_KEYS  AS 
	...
	FROM CROSSWALK CW
	LEFT JOIN DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_CHANGE_LOG_TEST CL -- JOINING ON CHANGE LOG TABLE TO GET THE LATEST KEYS WHICH GOT CHANGED IN PREVIOUS STEP
	...
	FROM CROSSWALK CW
	LEFT JOIN DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_CHANGE_LOG_TEST CL 
...
,RECENTLY_MAPPED AS 
	(
	SELECT *
	FROM DW_PROD.INTEGRATION.FACT_AMS_PAYROLL_MAPPING --table contains only mapped keys 
	WHERE (AMS_EMPLOYEE_KEY NOT IN (SELECT AMS_EMPLOYEE_KEY FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
    AND PAYROLL_EMPLOYEE_KEY NOT IN (SELECT PAYROLL_EMPLOYEE_KEY FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)) AND
    (PAYROLL_EMPLOYEE_KEY IN (SELECT * FROM CROSSWALK_NOT_MAPPED_KEYS)
...
-----------------------------------
SELECT COALESCE((SELECT MAX(ID) FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_MERGED_TEST),0)+ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS ID,
...
--  FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_MERGED)
...
UPDATE DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST 
SET INACTIVE = TRUE 
WHERE EMPLOYEE_ENTERPRISE_ID IN 
	(SELECT OLD_ENTERPRISE_ID FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_MERGED_TEST
	WHERE INSERTED_DATE = (SELECT MAX(INSERTED_DATE) FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_MERGED_TEST)
	);
...
--UPDATE DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS 
...
--	FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS 
...
FROM DW_PROD.INTEGRATION.FACT_AMS_PAYROLL_MAPPING FM 
WHERE (
		FM.AMS_EMPLOYEE_KEY = CW_D.AMS_EMPLOYEE_KEY 
		OR FM.PAYROLL_EMPLOYEE_KEY = CW_D.PAYROLL_EMPLOYEE_KEY
	  )
AND FM.AMS_EMPLOYEE_KEY NOT IN (SELECT AMS_EMPLOYEE_KEY FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
AND FM.PAYROLL_EMPLOYEE_KEY NOT IN (SELECT PAYROLL_EMPLOYEE_KEY FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
AND (CW_D.INACTIVE <> TRUE AND CW_D.DUP_FLAG <> TRUE)
...
UPDATE DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST M
SET	 M.DUP_FLAG = T.DUP_FLAG,
	M.DUP_FLAG_REASON = T.DUP_FLAG_REASON,
	M.INACTIVE = T.INACTIVE
FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST T 
...
CREATE OR REPLACE TABLE DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TBL_MASTER_TEST 
AS SELECT * FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TEST;


-- Inserting records which are not present currently in the table
INSERT INTO DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST
(
...
SELECT -- Newly added
    COALESCE((SELECT MAX(EMPLOYEE_ENTERPRISE_ID) FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST WHERE EMPLOYEE_ENTERPRISE_ID < 50000000),10000000)+ROW_NUMBER() OVER(ORDER BY (SELECT 1)), 
    -- Old
--  SRC.EMPLOYEE_ENTERPRISE_ID,
    ...
    SRC.EMPLOYEE_SSN,
    SRC.BROADSPIRE_ID
FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TBL_MASTER_TEST SRC
...
UPDATE DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST  AS cw
SET
...
FROM 
	...
	SELECT * FROM DW_PROD.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TBL_MASTER_TEST
		WHERE NVL(AMS_EMPLOYEE_KEY,'''') NOT IN (SELECT * FROM DUP_FLAG_TRUE_KEYS)
		 AND NVL(PAYROLL_EMPLOYEE_KEY,'''') NOT IN (SELECT * FROM DUP_FLAG_TRUE_KEYS) 
	 ) 	 
--	DW_PROD.INTEGRATION.VW_EMPLOYEE_CROSS_WALK
	 as src
...
FROM 
	...
	SELECT * FROM DW_PROD.INTEGRATION.VW_EMPLOYEE_CROSS_WALK_TBL_MASTER_TEST
		WHERE NVL(AMS_EMPLOYEE_KEY,'''') NOT IN (SELECT * FROM DUP_FLAG_TRUE_KEYS)
		 AND NVL(PAYROLL_EMPLOYEE_KEY,'''') NOT IN (SELECT * FROM DUP_FLAG_TRUE_KEYS) 
	 ) 	 
--	DW_PROD.INTEGRATION.VW_EMPLOYEE_CROSS_WALK
...
UPDATE DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK AS cw
SET
	CW.FIRST_NAME = SRC.FIRST_NAME,
	CW.LAST_NAME = SRC.LAST_NAME,
	CW.BRANCH_STATE_CODE = SRC.BRANCH_STATE_CODE,
...
----UPDATE ALAYACRE 
UPDATE DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST T1
...
FROM (
	...
	FROM
	DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST CW
	JOIN 
	DISC_DEDUPE_PROD.ALAYACARE.EMPLOYEE_MASTER_LIST EM 
	ON TRIM(CW.AMS_EMPLOYEE_ID) =TRIM(EM.EMPLOYEE_ID)
	JOIN DW_PROD.INTEGRATION.DIM_EMPLOYEE_AMS_MERGED DM 
...
FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST CW   
JOIN DISC_DEDUPE_PROD.HHAEXCHANGEOPENSYSTEMS.EMPLOYEE_MASTER_LIST EMP ON
    CW.AMS_EMPLOYEE_ID  = EMP.CAREGIVERID
JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.OFFICE_OFFICES_REPL BR ON
...
SELECT
    ...
    TRIM(EMP.CAREGIVERCODE) AS PUBLIC_ID
FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST CW   
JOIN DISC_DEDUPE_PROD.HHAEXCHANGEOSHAH.EMPLOYEE_MASTER_LIST EMP ON
    CW.AMS_EMPLOYEE_ID  = EMP.CAREGIVERID
JOIN DISC_PROD.HHAEXCHANGEOSHAH.OFFICE_OFFICES_REPL BR ON
    EMP.OFFICEID = BR.OFFICEID ::VARCHAR AND EMP.AGENCYID  = BR.AGENCYID
...
--UPDATE DATAFLEX
UPDATE DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST SRC
...
FROM (
	SELECT DISTINCT CW.EMPLOYEE_ENTERPRISE_ID,
	       ...
	       END AS PUBLIC_ID
	FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST CW
	LEFT JOIN DISC_DEDUPE_PROD.DATAFLEXSYNCDATA.EMPLOYEE_MASTER_LIST M
...
FROM (
    ...
    FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST CW
    LEFT JOIN DISC_DEDUPE_PROD.SANDATAIMPORT.EMPLOYEE_MASTER_LIST ML 
...
FROM DW_PROD.INTEGRATION.VW_WORKDAY_PARADOX_APPLICANT_MAPPING VW 
...
FROM
	(WITH ALL_REC AS
		...
		FROM DW_PROD.INTEGRATION.APPLICANT_CROSSWALK_MAPPING WCM 
		WHERE ON_HOLD <> TRUE
		QUALIFY ROW_NUMBER() OVER(PARTITION BY CROSSWALK_EMPLOYEE_ENTERPRISE_ID ORDER BY ETL_INSERTED_DATE DESC) = 1
		)
	, UNQ_REC AS 
	...
	FROM UNQ_REC UR 
	LEFT JOIN DW_PROD.HAH.DIM_EMPLOYEE EM 
		ON EM.EMPLOYEE_ID = UR.WORKDAY_ID 
	LEFT JOIN DW_PROD.HAH.DIM_APPLICANT APP 
...
INSERT INTO DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_HISTORICAL_TEST
...
FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST AT (OFFSET => -60*60*24)
EXCEPT 
SELECT * EXCLUDE (ETL_INSERTED_DATE, ETL_TASK_KEY, ETL_INSERTED_TASK_KEY, ETL_INSERTED_BY, ETL_LAST_UPDATED_DATE, ETL_LAST_UPDATED_BY, ETL_DELETED_FLAG, AMS_PUBLIC_ID, AMS_PAYROLL_ID, EMPLOYEE_SSN),
		CURRENT_TIMESTAMP, CURRENT_USER, AMS_PUBLIC_ID , AMS_PAYROLL_ID , EMPLOYEE_SSN
FROM DW_PROD.PUBLIC.EMPLOYEE_CROSS_WALK_TEST;
