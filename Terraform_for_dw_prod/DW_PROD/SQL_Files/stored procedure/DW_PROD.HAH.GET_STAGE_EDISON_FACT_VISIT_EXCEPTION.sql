CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_EDISON_FACT_VISIT_EXCEPTION("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO DW_PROD.STAGE.EDISON_FACT_VISIT_EXCEPTION
WITH CALLS AS (
	SELECT 
		V.VISITID,
	...
	FROM 
		DISC_PROD.HHAEXCHANGEEDISON.INREVIEWVISITS CM
	INNER JOIN 
		(SELECT DISTINCT AGENCYID,VISITID ,VISITDATE FROM DISC_PROD.HHAEXCHANGEEDISON.VISITS) V
		ON CM.AGENCYID = V.AGENCYID 
			AND CM.VISITID = V.VISITID	
	INNER JOIN 
		DISC_PROD.HHAEXCHANGEEDISON.TBLVISITS_REPL CRN 
		ON CRN.AGENCYID = V.AGENCYID
			AND CRN.VISITID = V.VISITID
	LEFT JOIN
		DISC_PROD.HHAEXCHANGEEDISON.REASONS_REPL RS
