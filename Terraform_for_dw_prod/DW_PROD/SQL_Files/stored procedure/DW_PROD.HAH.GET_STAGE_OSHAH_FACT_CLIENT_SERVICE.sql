CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_OSHAH_FACT_CLIENT_SERVICE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INSERT OVERWRITE INTO DW_PROD."STAGE".OSHAH_FACT_CLIENT_SERVICE
...
SELECT * FROM
    (SELECT DISTINCT CML.PATIENTID, MASTER_ID, CML.AGENCYID, CML.LASTNAME, CML.FIRSTNAME, CML.OFFICEID
	 ...
	 WHERE CML.PATIENTID NOT IN (SELECT PATIENTID FROM DISC_DEDUPE_PROD.HHAEXCHANGEOSHAH.CLIENT_MASTER_LIST))
),
...
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.PATIENTS P ON P.PATIENTID = CLIENT.PATIENTID
...
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.SERVICECODES SC ON SC.CONTRACTID = PA.CONTRACTID
...
WHERE O.STATE IN (SELECT STATE FROM DISC_PROD.HHAEXCHANGEOSHAH.CONFIGURATION WHERE CONFIG = TRUE)AND NOT (AUTHORIZATIONNUMBER ILIKE ''%TEMP%'' AND  MAXUNITSFORPERIOD <900 );
