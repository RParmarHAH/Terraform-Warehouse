CREATE OR REPLACE PROCEDURE DW_PROD.REPORT.GET_REPORT_FACT_APPLICANT_DETAILS("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INSERT OVERWRITE INTO DW_PROD.REPORT.FACT_APPLICANT_DETAILS
(
WITH VISITS_DATA
AS
(
	SELECT 
        ...
        LEFT JOIN DW_PROD.INTEGRATION.FACT_VISIT_MERGED FV
...
(
	SELECT 	WORKDAY_INTERNAL_ID AS WORKDAY_ID,
			...
			--cw.AMS_EMPLOYEE_KEY
	FROM  	DW_PROD.HAH.FACT_APPLICANT_STATUS se           --para
			LEFT JOIN DW_PROD.HAH.FACT_APPLICANT_STATUS de --wday
			ON se."CANDIDATE_ID" = de."LINKED_ID"  AND de."LINKED_ID" IS NOT NULL 
			LEFT JOIN DW_PROD."INTEGRATION"."EMPLOYEE_CROSS_WALK" cw ON cw."APPLICANT_EMPLOYEE_KEY" = se."APPLICANT_KEY" 
...
(
	SELECT  WORKDAY_INTERNAL_ID AS WORKDAY_ID,
			...
			CURRENT_USER AS ETL_LAST_UPDATED_BY
			--cw.AMS_EMPLOYEE_KEY
	FROM 	DW_PROD.HAH.FACT_APPLICANT_STATUS de--wday
			LEFT JOIN DW_PROD."INTEGRATION"."EMPLOYEE_CROSS_WALK" cw ON cw."APPLICANT_EMPLOYEE_KEY" = de."APPLICANT_KEY" AND INACTIVE <>TRUE AND DUP_FLAG <> TRUE 
			LEFT JOIN VISITS_DATA A ON A.EMPLOYEE_KEY = CW."AMS_EMPLOYEE_KEY"
