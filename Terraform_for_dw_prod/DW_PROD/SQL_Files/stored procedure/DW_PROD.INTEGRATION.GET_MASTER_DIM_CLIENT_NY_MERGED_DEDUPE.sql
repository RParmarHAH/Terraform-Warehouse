CREATE OR REPLACE PROCEDURE DW_PROD.INTEGRATION.GET_MASTER_DIM_CLIENT_NY_MERGED_DEDUPE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
WITH LWD_NEW AS (
    ...
    CLUSTER_ID
    FROM INTEGRATION.DIM_CLIENT_NY_RL_OUTPUT
)
, NEW AS (
    SELECT * FROM (
        SELECT 
            -- CASE WHEN SYSTEM_CODE=''PREFERRED'' THEN 1 ELSE 2 END as SRC_PRI,
            row_number() over(partition by NYO.CLUSTER_ID order by SRC_PRI) min_record_id,
            NYO.ID,
            ...
            MIN(REFERRAL_DATE::date) OVER(PARTITION BY NYO.CLUSTER_ID) AS DERIVED_REFERRAL_DATE,
            FIRST_SERVICE_DATE::DATE - MIN(REFERRAL_DATE::date) OVER(PARTITION BY NYO.CLUSTER_ID) AS DERIVED_DAYS_TO_SERVICE,
            CONTRACT_BEGIN_DATE,
            CONTRACT_END_DATE,
            MIN(FIRST_SERVICE_DATE::date) OVER(PARTITION BY NYO.CLUSTER_ID) AS DERIVED_FIRST_SERVICE_DATE,
            MAX(LAST_SERVICE_DATE::date) OVER(PARTITION BY NYO.CLUSTER_ID) AS DERIVED_LAST_SERVICE_DATE,
            MIN(BEGIN_DATE::date) OVER(PARTITION BY NYO.CLUSTER_ID) AS DERIVED_BEGIN_DATE,DAYS_TO_SERVICE,
            ...
            ETL_DELETED_FLAG,ETL_INFERRED_MEMBER_FLAG, NYO.CLUSTER_ID
        FROM INTEGRATION.DIM_CLIENT_NY_RL_OUTPUT NYO
        INNER JOIN LWD_NEW ON LWD_NEW.ID = NYO.ID
...
, OLD AS (
    SELECT * FROM (
        SELECT 
            row_number() over(partition by NYO.CLUSTER_ID order by SRC_PRI) min_record_id,
            NYO.ID,SYSTEM_CODE,SOURCE_SYSTEM_ID,CLIENT_NUMBER,CLIENT_PID,NYO.CLUSTER_ID,AGENCY
        FROM INTEGRATION.DIM_CLIENT_NY_RL_OUTPUT NYO
        INNER JOIN LWD_OLD ON LWD_OLD.ID = NYO.ID
        WHERE NYO.CLUSTER_ID IS NOT NULL
        )-- WHERE min_record_id = 1
...
FROM INTEGRATION.DIM_CLIENT_NY_RL_OUTPUT R
...
FROM INTEGRATION.DIM_CLIENT_NY_RL_OUTPUT U
