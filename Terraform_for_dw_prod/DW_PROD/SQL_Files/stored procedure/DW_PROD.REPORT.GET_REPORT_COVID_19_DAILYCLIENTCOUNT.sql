CREATE OR REPLACE PROCEDURE DW_PROD.REPORT.GET_REPORT_COVID_19_DAILYCLIENTCOUNT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
BEGIN
    ...
    INSERT OVERWRITE INTO REPORT.COVID_19_DAILYCLIENTCOUNT
with SD_Visits as
(
    SELECT
...
(
  SELECT DISTINCT CALENDAR_DATE,PreviousWeekDate,WEEK_OF_YEAR,DAY_OF_WEEK,DAY_NAME,
    STATE,BRANCH_NAME,CLIENTNUMBER,EVENTSTATUS,BRANCH_KEY
    FROM
    (
    ...
    CROSS JOIN
    (
	    SELECT DISTINCT STATE,BRANCH_NAME,CLientnumber,EVENTSTATUS,
        BRANCH_KEY
	    FROM SD_Visits
    )allstateandbranch
)
SELECT
 ...
 CURRENTWEEK.clientnumber as CurrentWeekclients,
 PreviousWeek.clientnumber as PreviousWeekclients,
...
and currentweek.CLientnumber = AllmasterDAta.CLientnumber
...
and AllmasterDAta.BRANCH_KEY = PreviousWeek.BRANCH_KEY
and PreviousWeek.STATE = AllmasterDAta.STATE
and PreviousWeek.CLientnumber = AllmasterDAta.CLientnumber
