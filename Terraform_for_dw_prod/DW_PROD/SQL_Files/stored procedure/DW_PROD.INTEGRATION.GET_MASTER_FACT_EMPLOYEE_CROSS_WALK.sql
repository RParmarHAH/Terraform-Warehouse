CREATE OR REPLACE PROCEDURE DW_PROD.INTEGRATION.GET_MASTER_FACT_EMPLOYEE_CROSS_WALK("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
CREATE OR REPLACE TABLE DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS 
CLONE DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK;
...
UPDATE DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS CW_D
...
UPDATE DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS CW
SET DUP_FLAG = TRUE ,DUP_FLAG_REASON = ''MAPPING_INTERCHANGED''
FROM (SELECT  AMS_EMPLOYEE_KEY||''-''||PAYROLL_EMPLOYEE_KEY A_P
	FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS D
		WHERE EXISTS  
	...
	FROM DW_PROD.INTEGRATION.FACT_AMS_PAYROLL_MAPPING D
		WHERE EXISTS  
			(SELECT DISTINCT 1 FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS M 
			WHERE M.AMS_EMPLOYEE_KEY = D.AMS_EMPLOYEE_KEY )
		AND EXISTS  
			(SELECT DISTINCT 1 FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS M 
			WHERE M.PAYROLL_EMPLOYEE_KEY = D.PAYROLL_EMPLOYEE_KEY )
...
INSERT INTO DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_CHANGE_LOG
-----------------------------------
...
WITH KEYS_CHANGED AS 
	...
	JOIN DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS  D1
...
,AMS_MAPP AS
	...
	FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS CW 
	JOIN DW_PROD.INTEGRATION.VW_EMPLOYEE_CROSS_WALK VW
...
UPDATE DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK TEMP_C
...
FROM
	...
	FROM DW_PROD.INTEGRATION.VW_EMPLOYEE_CROSS_WALK VW
	LEFT JOIN DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_CHANGE_LOG CL -- JOINING ON CHANGE LOG TABLE TO GET THE LATEST KEYS
		ON COALESCE(CL.LATEST_INTEGRATION_LAYER_KEY,CL.LATEST_DISC_LAYER_KEY,CL."AMS / PAYROLL_KEY") = VW.AMS_EMPLOYEE_KEY
		AND CL.ETL_INSERTED_DATE = (SELECT MAX(ETL_INSERTED_DATE) FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_CHANGE_LOG)
	) SRC
...
INSERT INTO DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_MERGED
-- Logic
-------------------------------------------
-- ************* CASE 1 ********************
...
FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS CWD
JOIN DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK CWM -- NEED TO JOIN TO GET OLDER KEY 
...
WHERE CWD.AMS_EMPLOYEE_KEY IN 
	...
	AND AMS_EMPLOYEE_KEY IN 
		...
		AND KEY_TYPE= ''AMS'' 
		AND ETL_INSERTED_DATE = (SELECT MAX(ETL_INSERTED_DATE) FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_CHANGE_LOG)
		)
...
,CROSSWALK_NOT_MAPPED_KEYS  AS 
	...
	FROM CROSSWALK CW
	LEFT JOIN DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_CHANGE_LOG CL -- JOINING ON CHANGE LOG TABLE TO GET THE LATEST KEYS WHICH GOT CHANGED IN PREVIOUS STEP
	...
	FROM CROSSWALK CW
	LEFT JOIN DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_CHANGE_LOG CL 
...
-----------------------------------
SELECT COALESCE((SELECT MAX(ID) FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_MERGED),0)+ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS ID,
...
WHERE EMPLOYEE_ENTERPRISE_ID IN 
	(SELECT OLD_ENTERPRISE_ID FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_MERGED
	WHERE INSERTED_DATE = (SELECT MAX(INSERTED_DATE) FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_MERGED)
	);
...
UPDATE DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK M
SET	 M.DUP_FLAG = T.DUP_FLAG,
	M.DUP_FLAG_REASON = T.DUP_FLAG_REASON,
	M.INACTIVE = T.INACTIVE
FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS T 
...
SELECT -- Newly added
    COALESCE((SELECT MAX(EMPLOYEE_ENTERPRISE_ID) FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK WHERE EMPLOYEE_ENTERPRISE_ID < 50000000),10000000)+ROW_NUMBER() OVER(ORDER BY (SELECT 1)), 
    -- Old
--  SRC.EMPLOYEE_ENTERPRISE_ID,
    ...
    SRC.EMPLOYEE_SSN,
    SRC.BROADSPIRE_ID
FROM DW_PROD.INTEGRATION.VW_EMPLOYEE_CROSS_WALK_TBL_MASTER SRC
...
FROM 
	...
	SELECT * FROM DW_PROD.INTEGRATION.VW_EMPLOYEE_CROSS_WALK_TBL_MASTER
		WHERE NVL(AMS_EMPLOYEE_KEY,'''') NOT IN (SELECT * FROM DUP_FLAG_TRUE_KEYS)
		 AND NVL(PAYROLL_EMPLOYEE_KEY,'''') NOT IN (SELECT * FROM DUP_FLAG_TRUE_KEYS) 
	 ) 	 
...
----UPDATE ALAYACRE 
UPDATE DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK T1
...
--UPDATE DATAFLEX
UPDATE DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK SRC
...
INSERT INTO DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK_HISTORICAL
...
FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK AT (OFFSET => -60*60*24)
EXCEPT 
SELECT * EXCLUDE (ETL_INSERTED_DATE, ETL_TASK_KEY, ETL_INSERTED_TASK_KEY, ETL_INSERTED_BY, ETL_LAST_UPDATED_DATE, ETL_LAST_UPDATED_BY, ETL_DELETED_FLAG, AMS_PUBLIC_ID, AMS_PAYROLL_ID, EMPLOYEE_SSN),
		CURRENT_TIMESTAMP, CURRENT_USER, AMS_PUBLIC_ID , AMS_PAYROLL_ID , EMPLOYEE_SSN
FROM DW_PROD.INTEGRATION.EMPLOYEE_CROSS_WALK;
