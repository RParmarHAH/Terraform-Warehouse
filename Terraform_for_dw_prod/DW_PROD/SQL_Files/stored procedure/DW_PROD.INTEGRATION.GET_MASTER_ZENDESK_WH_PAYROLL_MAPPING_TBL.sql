CREATE OR REPLACE PROCEDURE DW_PROD.INTEGRATION.GET_MASTER_ZENDESK_WH_PAYROLL_MAPPING_TBL("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO DW_PROD.INTEGRATION.ZENDESK_WH_PAYROLL_MAPPING_TBL
with zendesk_clusters as (
	select * from DW_PROD.INTEGRATION.ZENDESK_PAYROLL_MAPPING_OUTPUT
	where cluster_id in (select distinct cluster_id from DW_PROD.INTEGRATION.ZENDESK_PAYROLL_MAPPING_OUTPUT where source_system_id = 999 and cluster_id IS NOT NULL)
)
