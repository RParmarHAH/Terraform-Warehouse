CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_PREFERRED_DIM_VISIT_STATUS_MAPPING("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
BEGIN
    ...
    --*****************************************************************************************************************************
-- NAME:  GET_STAGE_PREFERRED_DIM_VISIT_STATUS_MAPPING
--
...
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
-- 14/09/22     Shraddha Sejpal      Initial development
-- 18/08/23    Harshala Kadam        Modified DERIVED_DISC_STATUS 
-- 22/11/23    Harshala Kadam		 Updated In review status  
-- 01/19/24	   Shubham Khunare	     schedule/visit Status name correction
--*****************************************************************************************************************************
INSERT OVERWRITE INTO STAGE.PREFERRED_DIM_VISIT_STATUS_MAPPING
WITH RESCHEDULED AS 
( 
...
--LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.VISITUNBILLEDDETAILS IV
...
STATUS.*,
    CASE WHEN DERIVED_DISC_STATUS IN (''ReScheduled Completed-Billed'',''ReScheduled Completed-Notbillable'',''Scheduled Completed Not billable'',''Completed Billed'',''ReScheduled Completed'',''ReScheduled Billed'',''Scheduled Completed UnBilled'',''Scheduled Completed Billed'',''ReScheduled Completed-Unbilled'')
	 THEN ''YES''
	 WHEN DERIVED_DISC_STATUS IN (''Scheduled Future Notbillable'',''ReScheduled In Review Notbillable'',''Scheduled In Review billed'',''Scheduled In Review Notbillable'',''ReScheduled In Review Unbilled'',''Scheduled In Review Billed'',''Scheduled In Review Unbilled'',''Scheduled Future Unbilled'',''Cancelled Not billable'',''Cancelled Billed'',''ReScheduled In Review-Billed'',''Rescheduled Future Unbilled'',''Future Cancelled Unbilled'',''Scheduled In Review Billed'',''Scheduled Billed'',''Scheduled Future NotBilled'',''Scheduled NotBilled In Review'',''Scheduled NotBilled'',''Scheduled Future'',''ReScheduled Notbillable In Review'',''ReScheduled Notbillable'',''Cancelled In Review Unbilled'',''Cancelled Unbilled'',''Cancelled'') 
	 THEN ''NO'' 
