CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_QUICKBOOKS_DIM_BRANCH("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
WITH BRANCHES AS (
	SELECT DISTINCT COMPANY, UPPER(TRIM(CLASS)) AS CLASS FROM DISC_PROD.ADP.QB_EMPLOYEES WHERE CLASS IS NOT NULL
)
, ALL_BRANCHES AS (
SELECT DISTINCT 
