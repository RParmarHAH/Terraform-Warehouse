CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_PREFERRED_FACT_VISIT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INSERT OVERWRITE INTO DW_PROD.STAGE.PREFERRED_FACT_VISIT
WITH EMPLOYEE AS 
(
	SELECT * FROM
	(
...
FROM DISC_PROD.HHAEXCHANGEPREFERRED.VISITAUTHORIZATIONS_REPL VAR
JOIN DISC_PROD.HHAEXCHANGEPREFERRED.STAGE_PATIENTAUTHORIZATION PA ON PA.AUTHORIZATIONID = VAR.AUTHORIZATIONID 
...
FROM DISC_PROD.HHAEXCHANGEPREFERRED.PAYMENTS
)
,AUTO_CONFIRMED_VISITS AS (
SELECT UV.VISITID
--FROM DISC_PROD.HHAEXCHANGEPREFERRED.UNTOUCHEDVISITS UV -- GETTING ONLY LATEST 3 MONTHS DATA FROM SOURCE
FROM DISC_PROD.HHAEXCHANGEPREFERRED.UNTOUCHEDVISITS_WITH_HISTORY UV -- IDDOX-490: CAPTURING THE HISTORY ALONG WITH THE LASTEST 3 MONTHS DATA
...
(
	...
	LEFT JOIN DISC_PROD.HHAEXCHANGEPREFERRED.STAGE_PATIENTS P ON P.PATIENTID = C.PATIENTID
