CREATE OR REPLACE PROCEDURE DW_PROD.HAH.FACT_CLIENT_ACQUIRED_POSTPROCESSING()
...
INSERT OVERWRITE INTO HAH.FACT_CLIENT_ACQUIRED
...
BRANCH_MERGED.ORIGINAL_BRANCH_KEY ,BRANCH_MERGED.BRANCH_KEY, CONTRACT_KEY, DATE_TRUNC(''MONTH'', REPORT_DATE) AS SERVICE_MONTH, ''CCSI'' AS AQUISITION_NAME,
	-1 AS ETL_TASK_KEY, -1 AS ETL_INSERTED_TASK_KEY, /*VISIT.ETL_INSERTED_DATE, VISIT.ETL_INSERTED_BY,*/ CONVERT_TIMEZONE(''UTC'', CURRENT_TIMESTAMP)::TIMESTAMP_NTZ AS ETL_LAST_UPDATED_DATE, CURRENT_USER AS ETL_LAST_UPDATED_BY, FALSE AS ETL_DELETED_FLAG
FROM HAH.FACT_VISIT VISIT
INNER JOIN HAH.DIM_BRANCH BRANCH
	ON VISIT.BRANCH_KEY = BRANCH.BRANCH_KEY
...
-- ALAYACARE 
SELECT DISTINCT 
MD5(CLIENT_MERGED.CLIENT_KEY || VISIT.CONTRACT_KEY || BRANCH_MERGED.BRANCH_KEY || DATE_TRUNC(''MONTH'', VISIT.REPORT_DATE)) AS CLIENT_ACQUIRED_KEY,
CLIENT_MERGED.ORIGINAL_CLIENT_KEY,
