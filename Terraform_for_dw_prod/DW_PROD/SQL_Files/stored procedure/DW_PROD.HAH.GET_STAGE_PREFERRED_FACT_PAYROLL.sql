CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_PREFERRED_FACT_PAYROLL("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
	...
	SELECT * FROM
		(
			...
			WHERE CAREGIVERID NOT IN (SELECT CAREGIVERID FROM DISC_DEDUPE_PROD.HHAEXCHANGEPREFERRED.EMPLOYEE_MASTER_LIST) 
                      ...
                      from "DISC_PROD"."HHAEXCHANGEPREFERRED"."PAYROLL_DETAILS_REPL"
...
, vacation_dates AS (SELECT CAREGIVERID,CAREGIVERABSENCETYPESID,STARTDATE,ENDDATE
                        FROM (SELECT * FROM "DISC_PROD"."HHAEXCHANGEPREFERRED"."CAREGIVERVACATIONS_REPL")
                        UNION  ALL
                        SELECT CAREGIVERID
...
, filtered_vacations AS (              
                      ...
                      LEFT JOIN DW_PROD.HAH.DIM_DATE d on d.CALENDAR_DATE = v.STARTDATE
...
, vacation_payroll_weeks AS (
                      ...
                      LEFT JOIN (SELECT DISTINCT FROMDATE,TODATE FROM "DISC_PROD"."HHAEXCHANGEPREFERRED"."PAYROLL_WEEKS_REPL") w
...
, vacation_week_category AS (
                      ...
                      LEFT JOIN "DW_PROD"."STAGE"."PREFERRED_FACT_PAYROLL_ABSENCE_GROUP_MAPPING" agm on agm.CAREGIVERABSENCETYPESID = vpw.CAREGIVERABSENCETYPESID
                      GROUP BY 1,2,3,4)  
...
from "DISC_PROD"."HHAEXCHANGEPREFERRED"."PAYROLL_HEADER_REPL" as prh
...
left join DISC_PROD.hhaexchangepreferred.caregivers caregivers
