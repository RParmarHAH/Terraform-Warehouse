CREATE OR REPLACE PROCEDURE DW_PROD.HAH.DIM_DATE_BACKUP_20220204("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
RETURNS VARCHAR(16777216)
LANGUAGE JAVASCRIPT
EXECUTE AS OWNER
AS '
        var sql = `
    INSERT OVERWRITE INTO DW_PROD.HAH.DIM_DATE
           ...
           sql = `UPDATE HAH.DIM_DATE SET Holiday_Flag = CAST( ''TRUE'' AS BOOLEAN)
            ...
            sql = `UPDATE HAH.DIM_DATE
                  ...
                  --  OR (MONTH = 2 AND Day_Name = ''Mon'' AND Day_of_Month BETWEEN 15 AND 21)  -- Presidents day
                    where (MONTH = 5 AND Day_Name = ''Mon'' AND Day_of_Month BETWEEN 25 AND 31)  -- Memorial day
              ...
              sql = `UPDATE HAH.DIM_DATE SET ALL_STATE_PAID = TRUE
                     WHERE CALENDAR_DATE <= (SELECT MIN(CHECK_DATE) PAYROLL_END_DATE FROM
                      (
                         ...
                         from HAH.FACT_BRANCH_PAYROLL_PERIODS
