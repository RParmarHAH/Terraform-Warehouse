CREATE OR REPLACE PROCEDURE DW_PROD.REPORT.GET_REPORT_TOP_OFFENDERS_EXCEPTION_DETAILS_BY_WEEK("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
INSERT OVERWRITE INTO DW_PROD.REPORT.TOP_OFFENDERS_EXCEPTION_DETAILS_BY_WEEK
WITH FILTERED_VISITS AS (
	SELECT
		...
		EXCEPTION_CODE
	FROM
		DW_PROD.HAH.FACT_VISIT_EXCEPTION FVE
	LEFT JOIN DW_PROD.HAH.DIM_DATE DT 
...
, VISITS AS (
	SELECT 
		...
		EMPLOYEE_ID
	FROM 
		DW_PROD.INTEGRATION.FACT_VISIT_MERGED
)
, BRANCH AS (
	SELECT
		ORIGINAL_BRANCH_KEY,
		BRANCH_NAME
	FROM 
		DW_PROD.INTEGRATION.DIM_BRANCH_MERGED
)
, EXCEPTIONS AS (
	SELECT
		FVE.SERVICE_WEEK,
	...
	FROM 
		FILTERED_VISITS FVE
	LEFT JOIN DW_PROD.HAH.DIM_VISIT_EXCEPTION DVE 
	...
	LEFT JOIN DW_PROD.HAH.DIM_VISIT_EXCEPTION_MAPPING_LEVELS DVEML 
