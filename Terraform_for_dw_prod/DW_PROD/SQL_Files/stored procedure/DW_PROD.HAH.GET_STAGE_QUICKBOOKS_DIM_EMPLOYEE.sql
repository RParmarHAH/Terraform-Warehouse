CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_QUICKBOOKS_DIM_EMPLOYEE("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
WITH PAYROLL AS (
	...
	FROM DISC_PROD.ADP.QB_PAYROLL PAY
	GROUP BY 1, 2, 3
	UNION
	SELECT DISTINCT 
		COMPANY,
		...
		MIN(PAY.CHK_DATE)::DATE AS FIRST_CHECK_DATE,
		MAX(PAY.CHK_DATE)::DATE AS LAST_CHECK_DATE
	FROM DISC_PROD.ADP.QB_PAYROLL_DATES PAY
	GROUP BY 1, 2, 3
	UNION 
	SELECT DISTINCT 
	COMPANY,
	...
	FROM DISC_PROD.ADP.QB_PAYROLL_MIDWAY_20_GROSS PAY
	GROUP BY 1, 2, 3
)
SELECT DISTINCT
