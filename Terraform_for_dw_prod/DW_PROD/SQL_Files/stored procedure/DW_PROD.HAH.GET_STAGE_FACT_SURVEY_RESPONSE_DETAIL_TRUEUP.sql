CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_FACT_SURVEY_RESPONSE_DETAIL_TRUEUP("SURVEY_ID" VARCHAR(16777216))
...
WITH HEADER AS 
	...
	f.value AS value_a
	FROM DISC_PROD.QUALTRICS_SURVEYS.QUALTRICS_SURVEY_RESPONSE_TRUEUP SR, table(flatten(SURVEY_JSON)) f
...
,QUESTIONS_ANSWERS AS 
	...
	JOIN DW_PROD.STAGE.FACT_SURVEY_QUESTION FSQ ON R.QUESTION_ID=FSQ.QUESTION_ID AND IS_ACTIVE AND FSQ.SURVEY_ID=''''''||:SURVEY_ID||''''''
	)
,LABLES AS 
	(SELECT
	HEADER.RESPONSE_ID,
