CREATE OR REPLACE PROCEDURE DW_PROD.HAH.GET_STAGE_SANDATAIMPORT_DIM_BRANCH("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
--
-- PURPOSE: Creates one row per branch according to SandataImport
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
-- 01/04/21		Mir Ali				 Changed Branch_Key to be made up of Office_Code not Office_Name
-- 11/02/21		Mir Ali				 Updated to select single branch when duplicates are present 
-- 05/29/23		Shikhar Saxena		 Changed the logic to add SOURCE_SYSTEM_TYPE field
--*****************************************************************************************************************************
--
INSERT OVERWRITE INTO STAGE.SANDATAIMPORT_DIM_BRANCH
(
...
WITH RISKCONNECT_HIERARCHY AS (
	...
	FROM (
		...
		FROM DISC_PROD.SANDATAIMPORT.SANDATA_LOCATIONS AS SANDATA_LOCATIONS 
		LEFT JOIN (
			SELECT CASE DBNAME 
					WHEN ''IL'' THEN ''8380'' 
					WHEN ''IN'' THEN ''38004''
			...
			FROM DISC_PROD.BI_REPOSITORY.EXTERNALIDS 
			WHERE TYP = ''L''
		) AS MAPPING
	...
	LEFT JOIN HAH.DIM_SOURCE_SYSTEM AS SOURCE_SYSTEM ON OFFICES.SOURCE_SYSTEM_ID = SOURCE_SYSTEM.SOURCE_SYSTEM_ID
	WHERE OFFICES.ROW_NUMBER = 1
--	UNION ALL
--    SELECT * FROM DW_PROD.HAH.DIM_ORPHAN_BRANCH 
--    WHERE SOURCE_SYSTEM_ID = 4 --ADDED A ORPHAN BRANCH AS PER IDDOX - 506
      ;
