resource "snowflake_procedure" "DW_REPORT_GET_REPORT_COVID_19_NEW_REFERRALS" {
	name ="GET_REPORT_COVID_19_NEW_REFERRALS"
...
WITH DATES AS (
	...
	FROM DISC_${var.SF_ENVIRONMENT}.BI_REPOSITORY.SANDATAVISITS EVV_VISITS
	JOIN STATES STATES
		ON STATES.AGENCYID = EVV_VISITS.AGENCYID
	JOIN FIRST_AND_LAST_DATES DATES
		ON EVV_VISITS.SCHEDULEDATE >= DATES.FIRST_DATE
	JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_CLIENTS EVV_CLIENTS
