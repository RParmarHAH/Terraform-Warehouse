resource "snowflake_procedure" "DW_REPORT_GET_REPORT_CAREGIVER_LINEAR_RETENTION_MONTHLY_INNER" {
	name ="GET_REPORT_CAREGIVER_LINEAR_RETENTION_MONTHLY_INNER"
	...
	statement = <<-EOT
BEGIN
DELETE FROM REPORT.CAREGIVER_LINEAR_RETENTION_MONTHLY WHERE SERVICE_MONTH = date_trunc(month,:MONTH_DATE);

INSERT INTO REPORT.CAREGIVER_LINEAR_RETENTION_MONTHLY
WITH ALL_DATA AS (
WITH PERIOD AS
(
...
--    ,fv.BRANCH_KEY
    ...
    , MAX(FV.SERVICE_MONTH = DATEADD(MONTH, -1, PERIOD.MONTH_PERIOD)) AS "30_Days_Base"
    ,CASE WHEN Is_Active = TRUE AND "30_Days_Base" = TRUE THEN TRUE ELSE FALSE END AS "30_Days_Active"
    , MAX(FV.SERVICE_MONTH = DATEADD(MONTH, -3, PERIOD.MONTH_PERIOD)) AS "90_Days_Base"
    ,CASE WHEN Is_Active = TRUE AND "90_Days_Base"=TRUE THEN TRUE ELSE FALSE END AS "90_Days_Active"
    , MAX(FV.SERVICE_MONTH = DATEADD(MONTH, -12, PERIOD.MONTH_PERIOD)) AS "12_Months_Base"
...
SELECT DISTINCT SERVICE_MONTH_NAME,SERVICE_MONTH,EMPLOYEE_KEY,BRANCH_KEY,"30_Days_Active","30_Days_Base","90_Days_Active",
