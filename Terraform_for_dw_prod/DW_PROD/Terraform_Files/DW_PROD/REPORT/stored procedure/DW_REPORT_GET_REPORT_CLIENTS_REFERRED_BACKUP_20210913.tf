resource "snowflake_procedure" "DW_REPORT_GET_REPORT_CLIENTS_REFERRED_BACKUP_20210913" {
	name ="GET_REPORT_CLIENTS_REFERRED_BACKUP_20210913"
...
WITH CLIENT_INFO AS (
SELECT DISTINCT
FIRST_VALUE(I.CONTRACT_KEY) OVER (PARTITION BY C.CLIENT_KEY ORDER BY I.REPORT_DATE) FIRST_CONTRACT_KEY,
FIRST_VALUE(I.CONTRACT_CODE) OVER (PARTITION BY C.CLIENT_KEY ORDER BY I.REPORT_DATE) FIRST_CONTRACT_CODE,
FIRST_VALUE(I.CONTRACT_KEY) OVER (PARTITION BY C.CLIENT_KEY ORDER BY I.REPORT_DATE DESC) LATEST_CONTRACT_KEY,
FIRST_VALUE(I.CONTRACT_CODE) OVER (PARTITION BY C.CLIENT_KEY ORDER BY I.REPORT_DATE DESC) LATEST_CONTRACT_CODE,
FIRST_VALUE(I.HOURS_AUTHORIZED_NON_ADJUSTED) OVER (PARTITION BY C.CLIENT_KEY ORDER BY I.REPORT_DATE) FIRST_NON_ADJUSTED_AUTH_HOURS,
FIRST_VALUE(I.HOURS_AUTHORIZED) OVER (PARTITION BY C.CLIENT_KEY ORDER BY I.REPORT_DATE) FIRST_ADJUSTED_AUTH_HOURS,
FIRST_VALUE(I.HOURS_AUTHORIZED_NON_ADJUSTED) OVER (PARTITION BY C.CLIENT_KEY ORDER BY I.REPORT_DATE DESC) LATEST_NON_ADJUSTED_AUTH_HOURS,
FIRST_VALUE(I.HOURS_AUTHORIZED_NON_ADJUSTED) OVER (PARTITION BY C.CLIENT_KEY ORDER BY I.REPORT_DATE DESC) LATEST_ADJUSTED_AUTH_HOURS,
...
THEN 
	CASE WHEN FIRST_VISIT_DATE IS NULL AND LATEST_VISIT_DATE IS NULL AND C.END_DATE>=CURRENT_DATE THEN 1 ELSE 0 END
ELSE 
	CASE WHEN FIRST_VISIT_DATE IS NULL AND LATEST_VISIT_DATE IS NULL THEN 1 ELSE 0 END
...
MAX(MAX(I.REAUTHORIZED_DATE)) OVER (PARTITION BY C.CLIENT_KEY) LATEST_CONTRACT_BEGIN_DATE
FROM INTEGRATION.DIM_CLIENT_MERGED C
LEFT JOIN HAH.DIM_CLIENT C2 ON C2.CLIENT_KEY = C.ORIGINAL_CLIENT_KEY
LEFT JOIN INTEGRATION.FACT_VISIT_MERGED V ON C.ORIGINAL_CLIENT_KEY=V.ORIGINAL_CLIENT_KEY AND NVL(V.STATUS_CODE, ''02'') IN (''02'', ''03'', ''04'', ''05'')
LEFT JOIN INTEGRATION.FACT_INTAKE_MERGED I ON C.ORIGINAL_CLIENT_KEY=I.ORIGINAL_CLIENT_KEY
LEFT JOIN (SELECT CLIENT_KEY,MIN(REPORT_DATE) AS NEXT_SCHEDULED_VISIT_DATE FROM INTEGRATION.FACT_VISIT_MERGED WHERE REPORT_DATE>CURRENT_DATE GROUP BY CLIENT_KEY) FV 
ON FV.CLIENT_KEY=C.CLIENT_KEY
LEFT JOIN (SELECT CLIENT_KEY,MIN(REPORT_DATE) AS NEXT_SCHEDULED_VISIT_DATE FROM HAH.FACT_EVV_VISIT WHERE REPORT_DATE>CURRENT_DATE GROUP BY CLIENT_KEY) FEV 
...
(
	...
	FROM CLIENT_INFO CI
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFCLIENTCONTRACTS CC ON CI.DF_SYSTEM_CODE=CC.DBNAME AND CI.DF_CLIENT_NUMBER=CC.CLIENTNUMBER AND CI.LATEST_CONTRACT_CODE=CC.CONTRACTCODE
)
,OLD_VIEW AS 
(
SELECT DISTINCT
...
FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFCLIENTS DFC
RIGHT JOIN DF_CLIENT_CONTRACTS C ON DFC.DBNAME=C.SYSTEM_CODE AND DFC.NUMBER=C.CLIENT_NUMBER
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFSERVICES S1 ON S1.DBNAME = C.SYSTEM_CODE AND S1.AREA = C.SERVICEAREA
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFSERVICES S2 ON S2.DBNAME = DFC.DBNAME AND S2.AREA = DFC.SERVICEAREA
)
,NEW_VIEW AS 
(
SELECT DISTINCT 
...
SELECT DISTINCT FIRST_CONTRACT_KEY, FIRST_CONTRACT_CODE, LATEST_CONTRACT_KEY, LATEST_CONTRACT_CODE, FIRST_ADJUSTED_AUTH_HOURS,
FIRST_NON_ADJUSTED_AUTH_HOURS, LATEST_ADJUSTED_AUTH_HOURS, LATEST_NON_ADJUSTED_AUTH_HOURS, NEXT_SCHEDULED_VISIT_DATE, 
CLIENT_NUMBER, ORIGINAL_CLIENT_NUMBER, CLIENT_KEY, ORIGINAL_CLIENT_KEY, SYSTEM_CODE, SOURCE_SYSTEM_ID,ORIGINAL_SOURCE_SYSTEM_ID,
CLIENT_NAME, CLIENT_HOME_PHONE, CLIENT_WORK_PHONE, SUPERVISOR_KEY, BRANCH_KEY, ORIGINAL_BRANCH_KEY, FIRST_VISIT_DATE, 
LATEST_VISIT_DATE, REFERRED_DATE, DAYS_TO_SERVICE, OPEN_REFERRAL, REFERRAL_AGE, FIRST_CONTRACT_BEGIN_DATE, 
LATEST_CONTRACT_BEGIN_DATE, END_DATE, OFFICE_NUMBER, SERVICE_AREA, CCU_AGREE_NUMBER, CONTRACT_NUMBER, GAUR_NAME,
NAME, ETL_TASK_KEY, ETL_INSERTED_TASK_KEY, ETL_INSERTED_DATE, ETL_INSERTED_BY, ETL_LAST_UPDATED_DATE, ETL_LAST_UPDATED_BY,
ETL_DELETED_FLAG
FROM NEW_VIEW
