resource "snowflake_procedure" "DW_REPORT_GET_REPORT_EMPLOYEE_DERIVED_METRICS" {
	name ="GET_REPORT_EMPLOYEE_DERIVED_METRICS"
...
--
-- PURPOSE: Creates derived metrics for each employee based on payroll and visit/service data
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------        
-- 11/16/20    Frank Noordover       Initial development
-- 09/21/21	   Shraddha Sejpal		 Added CCSI source system
-- 09/02/22    Vaidehi Shah    		 Added Alayacare source system
-- 06/08/22    Mirisha Jarecha       Added RAM source system
-- 07/13/2022  Moin Saiyed			 Added other category changes
-- 07/26/22	   Abhishek Sunil		 Added ASR source system
-- 10/05/22	   SAM HUFF     		 Added CLEARCARE source system
-- 11/08/22    Meet Hariyani         Added Applicant fields
-- 11/11/22	   Shraddha Sejpal		 Replaced Status_description to confirmed_flag in FACT_VISIT filter condition
-- 01/03/23	   Mohit Vaghadiya		 Added Alliance Source System
-- 01/02/23	   Sanket Jain			 Added Axxess source System
-- 01/05/23    Meet Hariyani         Added applicant_key and NEXT_SCHEDULED_SERVICE_DATE.
-- 05/12/23    Deepen Gajjar         Added Phone 1, 2 & 3 fields
-- 05/22/23    Meet Hariyani         Added Application_job_key
-- 05/26/23    Deepen Gajjar         Updated EMPLOYEE_LAST_WORKED_DATE logic 
--*****************************************************************************************************************************
INSERT OVERWRITE INTO Report.Employee_Derived_Metrics
--
...
SELECT *
, NVL(PHONE1_AMS,PHONE1_PAYROLL) AS PHONE1
, NVL(PHONE2_AMS,PHONE2_PAYROLL) AS PHONE2
, NVL(PHONE3_AMS,PHONE3_PAYROLL) AS PHONE3
FROM (
	SELECT 
	e.EMPLOYEE_KEY ,
	IFF( TRIM( E.Employee_Home_Phone)='''', NULL, E.Employee_Home_Phone) AS Phone1_AMS,
			       IFF( TRIM( E.Employee_Cell_Phone)='''', NULL, E.Employee_Cell_Phone) AS Phone2_AMS,
			       IFF( TRIM( E.Employee_Work_Phone)='''', NULL, E.Employee_Work_Phone) AS Phone3_AMS,
			       IFF( TRIM( PRL.Employee_Home_Phone)='''', NULL, PRL.Employee_Home_Phone) AS Phone1_PAYROLL,
			       IFF( TRIM( PRL.Employee_Cell_Phone)='''', NULL, PRL.Employee_Cell_Phone) AS Phone2_PAYROLL,
			       IFF( TRIM( PRL.Employee_Work_Phone)='''', NULL, PRL.Employee_Work_Phone) AS Phone3_PAYROLL
	FROM INTEGRATION.DIM_EMPLOYEE_MERGED e 
	LEFT JOIN INTEGRATION.FACT_AMS_PAYROLL_MAPPING p ON e.EMPLOYEE_KEY = p.AMS_EMPLOYEE_KEY 
	LEFT JOIN INTEGRATION.DIM_EMPLOYEE_MERGED prl ON p.PAYROLL_EMPLOYEE_KEY =prl.EMPLOYEE_KEY 
	)
),
--
-- Capture all employees across all source systems
...
FROM HAH.DIM_Employee AS emp
WHERE emp.Source_System_ID IN (1,2,3,4,7,8,9,13,14,16,17,19)
...
AND NVL( emp2.Employee_ID, ''999'') NOT IN (SELECT NVL( emp3.Payroll_ID, ''111'')
                                          ...
                                          WHERE emp3.Source_System_ID IN (1,2,3,4,7,8,9,13,14,16,17,19))
),
--
...
FROM HAH.DIM_Employee AS emp
INNER JOIN HAH.Fact_Visit AS vis ON vis.Employee_Key = emp.Employee_Key AND vis.Source_System_ID IN (1,2,3,4,7,8,9,13,14,16,17,19) AND vis.CONFIRMED_FLAG = ''YES''
...
INNER JOIN DW_${var.SF_ENVIRONMENT}.HAH.Fact_Payroll AS pyrl ON pyrl.Employee_Key = emp.Employee_Key AND pyrl.Source_System_ID IN (17)
...
SELECT emp.Employee_Key, emp.Employee_ID, emp.System_Code, emp.Source_System_ID, emp.Employee_PID, emp.Employee_Number,
    ...
    --    DATEDIFF( ''DAY'', Derived_Last_Check_Date, CURRENT_DATE) AS Days_Since_Last_Paycheck
FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_Employee AS emp
INNER JOIN DW_${var.SF_ENVIRONMENT}.HAH.Fact_Payroll AS pyrl ON pyrl.Employee_ID = emp.PAYROLL_ID  AND pyrl.Source_System_ID IN (6)
WHERE emp.Source_System_ID IN (13)
AND emp.Employee_Key NOT IN ( SELECT Employee_Key FROM Bill_Only_Employees)
GROUP BY emp.Employee_Key, emp.Employee_ID, emp.System_Code, emp.Source_System_ID, emp.Employee_PID, emp.Employee_Number
--
--Axxess Trustpoint data
UNION
SELECT emp.Employee_Key, emp.Employee_ID, emp.System_Code, emp.Source_System_ID, emp.Employee_PID, emp.Employee_Number,
       MIN( pyrl.Payroll_Date) AS Derived_First_Check_Date,
       MAX( pyrl.Payroll_Date) AS Derived_Last_Check_Date
...
FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_Employee AS emp
INNER JOIN DW_${var.SF_ENVIRONMENT}.HAH.Fact_Payroll AS pyrl ON pyrl.Employee_ID = emp.PAYROLL_ID  AND pyrl.Source_System_ID IN (6)
WHERE emp.Source_System_ID IN (14)
AND emp.Employee_Key NOT IN ( SELECT Employee_Key FROM Bill_Only_Employees)
GROUP BY emp.Employee_Key, emp.Employee_ID, emp.System_Code, emp.Source_System_ID, emp.Employee_PID, emp.Employee_Number
),
...
SELECT emp.Employee_Key, 
       ...
       NVL( NVL( NVL( ews.Derived_Work_State, emp.Primary_Branch_State), emp.SUTA_State), emp.Employee_State_Code) AS Derived_Work_State
FROM HAH.DIM_Employee AS emp
INNER JOIN List_Of_Employees AS loe ON emp.Employee_Key = loe.Employee_Key AND emp.Source_System_Id = loe.Source_System_ID AND loe.Source_System_ID IN (9)
LEFT OUTER JOIN Emp_Service_Dates AS esd ON emp.Source_System_ID = esd.Source_System_ID AND emp.Employee_Key = esd.Employee_Key
LEFT OUTER JOIN Emp_Work_State AS ews  ON emp.Source_System_ID = ews.Source_System_ID AND emp.Employee_Key = ews.Employee_Key
LEFT OUTER JOIN Emp_Payroll_Dates AS epd ON emp.Employee_Key = epd.Employee_Key--emp.Payroll_ID = epd.Employee_ID AND epd.Employee_PID = emp.Employee_PID AND emp.Source_System_ID IN (1,2) --
WHERE emp.Employee_Key NOT IN ( SELECT Employee_Key FROM Bill_Only_Employees)
...
LEFT OUTER JOIN Emp_Service_Dates AS esd ON emp.Source_System_ID = esd.Source_System_ID AND emp.Employee_Key = esd.Employee_Key
LEFT OUTER JOIN Emp_Work_State AS ews ON emp.Source_System_ID = ews.Source_System_ID AND emp.Employee_Key = ews.Employee_Key
LEFT OUTER JOIN Emp_Payroll_Dates AS epd ON emp.Employee_ID = epd.Employee_ID AND emp.Source_System_Id = epd.Source_System_ID AND /* epd.Employee_PID = emp.Employee_PID AND */ epd.Source_System_ID IN (17)
WHERE emp.Employee_Key NOT IN ( SELECT Employee_Key FROM Bill_Only_Employees)
)
,ASR_Employees AS
(
SELECT emp.Employee_Key, 
       ...
       NVL( NVL( NVL( ews.Derived_Work_State, emp.Primary_Branch_State), emp.SUTA_State), emp.Employee_State_Code) AS Derived_Work_State
FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_Employee AS emp
INNER JOIN List_Of_Employees AS loe ON emp.Employee_Key = loe.Employee_Key AND emp.Source_System_Id = loe.Source_System_ID AND loe.Source_System_ID IN (13) 
LEFT OUTER JOIN Emp_Service_Dates AS esd ON emp.Source_System_ID = esd.Source_System_ID AND emp.Employee_Key = esd.Employee_Key
LEFT OUTER JOIN Emp_Work_State AS ews ON emp.Source_System_ID = ews.Source_System_ID AND emp.Employee_Key = ews.Employee_Key
LEFT OUTER JOIN Emp_Payroll_Dates AS epd ON emp.Employee_ID = epd.Employee_ID AND emp.Source_System_Id = epd.Source_System_ID AND /* epd.Employee_PID = emp.Employee_PID AND */ epd.Source_System_ID IN (13)
WHERE emp.Employee_Key NOT IN ( SELECT Employee_Key FROM Bill_Only_Employees)
)
,Axxess_Employees AS
(
SELECT emp.Employee_Key, 
       ...
       NVL( NVL( NVL( ews.Derived_Work_State, emp.Primary_Branch_State), emp.SUTA_State), emp.Employee_State_Code) AS Derived_Work_State
FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_Employee AS emp
INNER JOIN List_Of_Employees AS loe ON emp.Employee_Key = loe.Employee_Key AND emp.Source_System_Id = loe.Source_System_ID AND loe.Source_System_ID IN (14) 
LEFT OUTER JOIN Emp_Service_Dates AS esd ON emp.Source_System_ID = esd.Source_System_ID AND emp.Employee_Key = esd.Employee_Key
LEFT OUTER JOIN Emp_Work_State AS ews ON emp.Source_System_ID = ews.Source_System_ID AND emp.Employee_Key = ews.Employee_Key
LEFT OUTER JOIN Emp_Payroll_Dates AS epd ON emp.Employee_ID = epd.Employee_ID AND emp.Source_System_Id = epd.Source_System_ID AND /* epd.Employee_PID = emp.Employee_PID AND */ epd.Source_System_ID IN (14)
WHERE emp.Employee_Key NOT IN ( SELECT Employee_Key FROM Bill_Only_Employees)
)
...
LEFT OUTER JOIN Emp_Service_Dates AS esd ON emp.Source_System_ID = esd.Source_System_ID AND emp.Employee_Key = esd.Employee_Key
LEFT OUTER JOIN Emp_Work_State AS ews ON emp.Source_System_ID = ews.Source_System_ID AND emp.Employee_Key = ews.Employee_Key
LEFT OUTER JOIN Emp_Payroll_Dates AS epd ON emp.Employee_ID = epd.Employee_ID AND emp.Source_System_Id = epd.Source_System_ID AND /* epd.Employee_PID = emp.Employee_PID AND */ epd.Source_System_ID IN (19)
WHERE emp.Employee_Key NOT IN ( SELECT Employee_Key FROM Bill_Only_Employees)
)
,
...
SELECT DISTINCT merged_emp.Employee_Key, 
       ...
       IFF(
		LENGTH(REGEXP_REPLACE(phn.phone1, ''\\\\D'', '''')) = 10,
		REGEXP_REPLACE(phn.phone1, ''\\\\D'', ''''),
		NULL) AS PHONE1,
		IFF(
		LENGTH(REGEXP_REPLACE(phn.phone2, ''\\\\D'', '''')) = 10,
		REGEXP_REPLACE(phn.phone2, ''\\\\D'', ''''),
		NULL) AS PHONE2,
		IFF(
		LENGTH(REGEXP_REPLACE(phn.phone3, ''\\\\D'', '''')) = 10,
		REGEXP_REPLACE(phn.phone3, ''\\\\D'', ''''),
		NULL) AS PHONE3
  ...
  LEFT JOIN employee_phone_number phn ON phn.employee_key = emp.EMPLOYEE_KEY 
),
Employee_Service_Line_List AS
(
SELECT DISTINCT vis.Employee_Key
FROM INTEGRATION.FACT_VISIT_MERGED AS vis
INNER JOIN HAH.DIM_Contract AS cntrct ON vis.Contract_Key = cntrct.Contract_Key AND NVL( cntrct.Mileage_Flag, ''FALSE'') = ''FALSE'' AND UPPER( cntrct.Revenue_Subcategory_Name ) NOT LIKE ''%MILE%'' AND (NVL( vis.Bill_Rate, 1) >= 1 OR vis.Bill_Rate = 0) AND cntrct.Revenue_Category IN (''CLS'',''HC'',''HH'',''IS'',''NS'',''OTHER'') 
WHERE vis.CONFIRMED_FLAG = ''YES''
AND vis.Employee_Key NOT IN ( SELECT Employee_Key FROM Bill_Only_Employees)
),
CLS_Employees AS
...
FROM INTEGRATION.FACT_VISIT_MERGED AS vis
INNER JOIN HAH.DIM_Contract AS cntrct ON vis.Contract_Key = cntrct.Contract_Key AND NVL( cntrct.Mileage_Flag, ''FALSE'') = ''FALSE'' AND UPPER( cntrct.Revenue_Subcategory_Name ) NOT LIKE ''%MILE%'' AND (NVL( vis.Bill_Rate, 1) >= 1 OR vis.Bill_Rate = 0) AND cntrct.Revenue_Category IN (''CLS'') AND cntrct.Revenue_SubCategory_Code IN (''RES'') 
...
FROM INTEGRATION.FACT_VISIT_MERGED AS vis
INNER JOIN HAH.DIM_Contract AS cntrct ON vis.Contract_Key = cntrct.Contract_Key AND NVL( cntrct.Mileage_Flag, ''FALSE'') = ''FALSE'' AND UPPER( cntrct.Revenue_Subcategory_Name ) NOT LIKE ''%MILE%'' AND (NVL( vis.Bill_Rate, 1) >= 1 OR vis.Bill_Rate = 0) AND cntrct.Revenue_Category IN (''HC'') 
INNER JOIN Employee_Service_Line_List AS esll ON vis.Employee_Key = esll.Employee_Key
WHERE vis.CONFIRMED_FLAG = ''YES''
AND vis.Employee_Key NOT IN ( SELECT Employee_Key FROM Bill_Only_Employees)
),
...
FROM INTEGRATION.FACT_VISIT_MERGED AS vis
INNER JOIN HAH.DIM_Contract AS cntrct ON vis.Contract_Key = cntrct.Contract_Key AND NVL( cntrct.Mileage_Flag, ''FALSE'') = ''FALSE'' AND UPPER( cntrct.Revenue_Subcategory_Name ) NOT LIKE ''%MILE%'' AND (NVL( vis.Bill_Rate, 1) >= 1 OR vis.Bill_Rate = 0) AND cntrct.Revenue_Category IN (''HH'') 
INNER JOIN Employee_Service_Line_List AS esll ON vis.Employee_Key = esll.Employee_Key
WHERE vis.CONFIRMED_FLAG = ''YES''
AND vis.Employee_Key NOT IN ( SELECT Employee_Key FROM Bill_Only_Employees)
),
...
INNER JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_Contract AS cntrct ON vis.Contract_Key = cntrct.Contract_Key AND NVL( cntrct.Mileage_Flag, ''FALSE'') = ''FALSE'' AND UPPER( cntrct.Revenue_Subcategory_Name ) NOT LIKE ''%MILE%'' AND (NVL( vis.Bill_Rate, 1) >= 1 OR vis.Bill_Rate = 0) AND cntrct.Revenue_Category IN (''OTHER'') 
INNER JOIN Employee_Service_Line_List AS esll ON vis.Employee_Key = esll.Employee_Key
WHERE vis.CONFIRMED_FLAG = ''YES''
AND vis.Employee_Key NOT IN ( SELECT Employee_Key FROM Bill_Only_Employees)
),
...
SELECT esll.Employee_Key,
	   ...
	   CAST( NVL( hoemp.OTHER_Revenue_Category, ''FALSE'') AS BOOLEAN) AS Delivered_OTHER_Services_Flag
...
LEFT OUTER JOIN OTHER_Employees AS hoemp ON esll.Employee_Key = hoemp.Employee_Key
WHERE emp.Employee_Key NOT IN ( SELECT Employee_Key FROM Bill_Only_Employees)
),
Union_Employees AS (
...
FROM DISC_${var.SF_ENVIRONMENT}.GpSyncData.PayStub_Master AS m
INNER JOIN DISC_${var.SF_ENVIRONMENT}.GpSyncData.PayStub_DetailLine AS dtl ON m.CheckID = dtl.CheckID AND ( dtl.PayCode LIKE ANY (''%DUE%'', ''%UNION%'') OR dtl.IsUnionDue = ''TRUE'' OR dtl.Paycode IN ( ''DUEADJ'', ''DUESM'', ''DUESO'', ''INDUES'', ''UNION'', ''UNA'', ''UNA2''))  
...
,APPLICANT_DATA AS (
	SELECT 
		DISTINCT 
			AEM.EMPLOYEE_KEY AS Employee_Key,
            AEM.APPLICANT_KEY,
			AEM.APPLICANT_ID,
			AEM.APPLICANT_SOURCE_SYSTEM_ID,
            FAS.JOB_KEY,
			MAX(FAS.HIRED_DATETIME) AS APPLICANT_HIRED_DATE,
			MIN(FAS.DATE_APPLIED) AS DATE_APPLIED,
			DATEDIFF(DAYS,MIN(FAS.DATE_APPLIED),MAX(FAS.HIRED_DATETIME)) AS APPLICATION_TO_HIRE_DAYS,
		    row_number() OVER (PARTITION BY Employee_Key ORDER BY iff(AEM.APPLICANT_SOURCE_SYSTEM_ID = 12, 1, 2)) r
	FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.FACT_APPLICANT_EMPLOYEE_MAPPING AEM
	INNER JOIN DW_${var.SF_ENVIRONMENT}.HAH.FACT_APPLICANT_STATUS FAS
	ON AEM.APPLICANT_KEY = FAS.APPLICANT_KEY 
	GROUP BY 1,2,3,4,5
QUALIFY r = 1
),
SERVICE_AFTER_REHIRE AS (
	SELECT 
	ETD.EMPLOYEE_KEY,
	MIN(FV.SERVICE_DATE) AS FIRST_SERVICE_AFTER_REHIRE 
	FROM EMPLOYEE_TENURE_DATES ETD
	LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.FACT_VISIT FV ON FV.EMPLOYEE_KEY = ETD.EMPLOYEE_KEY  
	WHERE (FV.SERVICE_DATE > ETD.DERIVED_LATEST_REHIRE_DATE) AND (ETD.DERIVED_REHIRE_FLAG = ''TRUE'') 
	GROUP BY 1
),
HIRE_SERVICE_DAY AS (
	SELECT 
	ETD.EMPLOYEE_KEY,
	CASE 
		WHEN ETD.Derived_Rehire_Flag = ''TRUE'' THEN DATEDIFF(DAYS,ETD.Derived_Latest_Rehire_Date,SAR.FIRST_SERVICE_AFTER_REHIRE)
		WHEN ETD.Derived_Rehire_Flag = ''FALSE'' AND (DATEDIFF(DAYS,AD.APPLICANT_HIRED_DATE,ETD.Derived_First_Service_Date) > 0) THEN DATEDIFF(DAYS,AD.APPLICANT_HIRED_DATE,ETD.Derived_First_Service_Date)
		WHEN ETD.Derived_Rehire_Flag = ''FALSE'' AND (DATEDIFF(DAYS,AD.APPLICANT_HIRED_DATE,ETD.Derived_First_Service_Date) < 0) THEN DATEDIFF(DAYS,ETD.DERIVED_HIRE_DATE,ETD.Derived_First_Service_Date)
	END AS HIRE_TO_FIRST_SERVICE_DAYS
	FROM Employee_Tenure_Dates ETD
	LEFT JOIN SERVICE_AFTER_REHIRE SAR ON ETD.EMPLOYEE_KEY = SAR.EMPLOYEE_KEY
	LEFT JOIN APPLICANT_DATA AD ON ETD.EMPLOYEE_KEY = AD.EMPLOYEE_KEY
),
SERVICE_DATE_DESC AS(SELECT 
DISTINCT vis.EMPLOYEE_KEY,
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.FACT_VISIT_MERGED vis
)
SELECT DISTINCT etd.Employee_Key, etd.Employee_ID, etd.System_Code, 
       ...
       upper(etd.Employee_Category),
 	   upper(etd.Derived_Employee_Category),
	   etd.Employee_Type,
       ...
       etd.Employee_Last_Check_Date, 
	   CASE WHEN (nvl(etd.Employee_Terminate_Date,''1900-01-01'') <> ''1900-01-01'' OR nvl(etd.Derived_Latest_Terminate_Date,''1900-01-01'') <> ''1900-01-01'' ) AND nvl(etd.Derived_First_Service_Date,''1900-01-01'') = ''1900-01-01'' THEN NULL ELSE  etd.Employee_Last_Worked_Date END AS Employee_Last_Worked_Date, 
	   etd.Employee_Terminate_Date,
       ...
       AD.APPLICANT_ID,
       AD.APPLICANT_KEY,
       AD.APPLICANT_SOURCE_SYSTEM_ID,
       AD.APPLICANT_HIRED_DATE,
       AD.DATE_APPLIED,
       AD.APPLICATION_TO_HIRE_DAYS,
       SAR.FIRST_SERVICE_AFTER_REHIRE,
       HSD.HIRE_TO_FIRST_SERVICE_DAYS,
       SD.NEXT_SCHEDULED_SERVICE_DATE,
       AD.JOB_KEY AS APPLICATION_JOB_KEY,
       etd.phone1,
	   etd.phone2,
	   etd.phone3
...
LEFT OUTER JOIN APPLICANT_DATA AD ON AD.Employee_Key = etd.Employee_Key
LEFT OUTER JOIN SERVICE_AFTER_REHIRE SAR ON SAR.EMPLOYEE_KEY = etd.Employee_Key
LEFT OUTER JOIN HIRE_SERVICE_DAY HSD ON HSD.EMPLOYEE_KEY = etd.Employee_Key
LEFT OUTER JOIN SERVICE_DATE_DESC SD ON etd.Employee_Key = SD.EMPLOYEE_KEY;
