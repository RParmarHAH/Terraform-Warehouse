resource "snowflake_procedure" "DW_REPORT_GET_REPORT_RPT_INVOICE_METRICS" {
	name ="GET_REPORT_RPT_INVOICE_METRICS"
...
DECLARE
	...
	LAST_UPDATED_DATE DATE DEFAULT (SELECT MAX(ETL_LAST_UPDATED_DATE) FROM DW_${var.SF_ENVIRONMENT}.REPORT.RPT_INVOICE_METRICS_MONTHLY);	--Added on 05/10/23
BEGIN
--********************************************************************************************
