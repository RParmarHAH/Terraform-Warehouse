resource "snowflake_procedure" "DW_REPORT_GET_REPORT_CLIENTS_REFERRED" {
	name ="GET_REPORT_CLIENTS_REFERRED"
...
(
	...
	FROM CLIENT_INFO CI
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFCLIENTCONTRACTS CC 
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFSERVICES S1 
	ON S1.DBNAME = C.SYSTEM_CODE AND S1.AREA = C.SERVICEAREA
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFSERVICES S2 
