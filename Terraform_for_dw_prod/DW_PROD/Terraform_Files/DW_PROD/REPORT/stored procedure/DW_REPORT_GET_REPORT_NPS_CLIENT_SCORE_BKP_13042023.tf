resource "snowflake_procedure" "DW_REPORT_GET_REPORT_NPS_CLIENT_SCORE_BKP_13042023" {
	name ="GET_REPORT_NPS_CLIENT_SCORE_BKP_13042023"
...
--*****************************************************************************************************************************
CREATE OR REPLACE TABLE DW_${var.SF_ENVIRONMENT}.REPORT.NPS_CLIENT_SCORE AS
WITH WH_CLIENTS AS(
  ...
  )--SELECT DISTINCT SURVEY_ID FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_SURVEY_RESPONSE_HEADER WHERE CLIENT_KEY IN (SELECT DISTINCT wh_client_key FROM BATCH);
  , SURVEY_MAPPING AS (
    ...
    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.FACT_SURVEY_QUESTION_MAPPING FSQM
    INNER JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.FACT_SURVEY_QUESTION_RESPONSE_MAPPING SQR 
