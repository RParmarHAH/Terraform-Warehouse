resource "snowflake_procedure" "DW_REPORT_GET_REPORT_CLIENT_LINEAR_RETENTION_FORWARD_MONTHLY_INNER" {
	name ="GET_REPORT_CLIENT_LINEAR_RETENTION_FORWARD_MONTHLY_INNER"
	...
	statement = <<-EOT
BEGIN
DELETE FROM REPORT.CLIENT_LINEAR_RETENTION_FORWARD_MONTHLY WHERE SERVICE_MONTH = date_trunc(month,:MONTH_DATE);

INSERT INTO REPORT.CLIENT_LINEAR_RETENTION_FORWARD_MONTHLY
WITH ALL_DATA AS (
WITH PERIOD AS
(
...
,FINAL AS (
SELECT TO_VARCHAR(PERIOD.MONTH_PERIOD, ''MMMM yyyy'') AS SERVICE_MONTH_NAME
...
--        , BRANCH_KEY 
        ...
        , MAX(HOURS_AND_CENSUS.PERIOD_BEGIN_DATE = DATEADD(MONTH, +1, PERIOD.MONTH_PERIOD)) AS "1_month_active"
        , MAX(HOURS_AND_CENSUS.PERIOD_BEGIN_DATE = DATEADD(MONTH, +3, PERIOD.MONTH_PERIOD)) AS "3_month_active"
        , MAX(HOURS_AND_CENSUS.PERIOD_BEGIN_DATE = DATEADD(MONTH, +12, PERIOD.MONTH_PERIOD)) AS "12_month_active"
...
SELECT SERVICE_MONTH_NAME,SERVICE_MONTH, CLIENT_KEY,BRANCH_KEY,"1_month_active","3_month_active",
