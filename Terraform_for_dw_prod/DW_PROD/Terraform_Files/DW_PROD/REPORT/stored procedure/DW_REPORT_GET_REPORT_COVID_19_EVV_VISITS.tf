resource "snowflake_procedure" "DW_REPORT_GET_REPORT_COVID_19_EVV_VISITS" {
	name ="GET_REPORT_COVID_19_EVV_VISITS"
...
SELECT ServiceDates.STATE,
    ...
    JOIN DISC_${var.SF_ENVIRONMENT}.BI_REPOSITORY.SANDATAVISITS SandataVisits
        ON SandataVisits.AGENCYID = ServiceDates.AGENCYID
           AND SandataVisits.SCHEDULEDATE = ServiceDates.SERVICEDATE
    JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_CLIENTS Clients
        ON Clients.AGENCYID = SandataVisits.AGENCYID 
           AND Clients.CLIENTID = SandataVisits.CLIENTID 
    JOIN DISC_${var.SF_ENVIRONMENT}.BI_REPOSITORY.EXTERNALIDS EL
