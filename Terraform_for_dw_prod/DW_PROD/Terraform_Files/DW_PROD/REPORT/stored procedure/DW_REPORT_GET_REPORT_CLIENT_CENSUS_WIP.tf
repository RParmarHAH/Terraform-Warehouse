resource "snowflake_procedure" "DW_REPORT_GET_REPORT_CLIENT_CENSUS_WIP" {
	name ="GET_REPORT_CLIENT_CENSUS_WIP"
...
BEGIN
    INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.REPORT.CLIENT_CENSUS_WIP
...
FROM INTEGRATION.FACT_VISIT_MERGED  V
--JOIN REPORT.VW_DASHBOARD_CONTRACTS C ON V.CONTRACT_KEY=C.CONTRACT_KEY
JOIN DW_${var.SF_ENVIRONMENT}.HAH.FACT_PARTNER_CONTRACT_SERVICE CS ON V.PARTNER_CONTRACT_SERVICE_KEY = CS.PARTNER_CONTRACT_SERVICE_KEY
JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_SERVICES DS ON DS.SERVICE_KEY = CS.SERVICE_KEY
JOIN INTEGRATION.DIM_CLIENT_MERGED CT ON V.CLIENT_KEY=CT.CLIENT_KEY
JOIN INTEGRATION.DIM_CLIENT_MERGED CT2 ON V.ORIGINAL_CLIENT_KEY=CT2.ORIGINAL_CLIENT_KEY
WHERE V.CONFIRMED_FLAG = ''YES'' 
