resource "snowflake_procedure" "DW_REPORT_GET_REPORT_CLIENT_LINEAR_RETENTION_MONTHLY" {
	name ="GET_REPORT_CLIENT_LINEAR_RETENTION_MONTHLY"
...
BEGIN
    ...
    IF (DAYOFMONTH(:MONTH_DATE) = 1) THEN
       ...
       CALL DW_${var.SF_ENVIRONMENT}.REPORT.GET_REPORT_CLIENT_LINEAR_RETENTION_MONTHLY_INNER(
    ...
    ELSEIF (DAYOFMONTH(:MONTH_DATE) > 1 AND DAYOFMONTH(:MONTH_DATE) <= 15) THEN
        ...
        CALL DW_${var.SF_ENVIRONMENT}.REPORT.GET_REPORT_CLIENT_LINEAR_RETENTION_MONTHLY_INNER(ADD_MONTHS(:MONTH_DATE, -1), :STR_ETL_TASK_KEY);
        CALL DW_${var.SF_ENVIRONMENT}.REPORT.GET_REPORT_CLIENT_LINEAR_RETENTION_MONTHLY_INNER(:MONTH_DATE, :STR_ETL_TASK_KEY);
    ELSE
