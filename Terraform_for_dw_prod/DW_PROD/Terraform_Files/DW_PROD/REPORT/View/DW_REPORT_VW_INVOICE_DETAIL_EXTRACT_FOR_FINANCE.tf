resource "snowflake_view" "DW_REPORT_VW_INVOICE_DETAIL_EXTRACT_FOR_FINANCE" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "REPORT"
	name = "VW_INVOICE_DETAIL_EXTRACT_FOR_FINANCE"
	statement = <<-SQL
...
FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFINVOICES AS inv
...
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFContracts AS cntrct ON cntrct.DBNAME = inv.DBNAME AND cntrct.ContractCode = inv.ContractCode
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFOffices AS off ON off.DBNAME = inv.DBNAME AND off.OfficeNumber = inv.Office
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFClients AS cl ON cl.DBName = inv.DBName AND cl.Number = inv.ClientNumber
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFServices AS srvc ON cl.DBName = srvc.DBName AND cl.ServiceArea = srvc.Area
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFEmployees AS emp ON invdtl.DBName = emp.DBName AND NULLIFZERO( invdtl.EmployeeNo1) = emp.Number
...
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFClients AS cl ON cl.DBName = inv.DBName AND cl.Number = inv.ClientNumber
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFServices AS srvc ON cl.DBName = srvc.DBName AND cl.ServiceArea = srvc.Area
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFEmployees AS emp ON invdtl.DBName = emp.DBName AND NULLIFZERO( invdtl.EmployeeNo2) = emp.Number
WHERE inv.INVOICECREATEDDATE BETWEEN CAST( '2017-01-01' AS DATE) AND CAST( CURRENT_DATE AS DATE) 
--AND inv.DBName = 'TN'
...
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFClients AS cl ON cl.DBName = inv.DBName AND cl.Number = inv.ClientNumber
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFServices AS srvc ON cl.DBName = srvc.DBName AND cl.ServiceArea = srvc.Area
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFEmployees AS emp ON invdtl.DBName = emp.DBName AND NULLIFZERO( invdtl.EmployeeNo3) = emp.Number
WHERE inv.INVOICECREATEDDATE BETWEEN CAST( '2017-01-01' AS DATE) AND CAST( CURRENT_DATE AS DATE) 
--AND inv.DBName = 'TN'
