resource "snowflake_view" "DW_REPORT_VW_EXECUTIVE_OPERATIONS_EXCELLENCE_UTILIZATION_FOR_HH_VALIDATION_FJN" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "REPORT"
	name = "VW_EXECUTIVE_OPERATIONS_EXCELLENCE_UTILIZATION_FOR_HH_VALIDATION_FJN"
	statement = <<-SQL
	 	WITH DATES AS (
		...
		FROM DW_${var.SF_ENVIRONMENT}.REPORT.VW_DASHBOARD_CONTRACTS
...
--			NVL(CASE WHEN CONTRACT.INCLUDE_FOR_HOURS = 1 AND NOT (NVL(DATA.HOURS_AUTHORIZED, 0) = 0 OR DATA.HOURS_AUTHORIZED > 900) THEN DATA.HOURS_AUTHORIZED END, 0) HOURS_AUTHORIZED_WHERE_AUTH_EXISTS,
		...
		FROM DW_${var.SF_ENVIRONMENT}.REPORT.UTILIZATION AS DATA
        ...
        JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_CLIENT CLIENT
