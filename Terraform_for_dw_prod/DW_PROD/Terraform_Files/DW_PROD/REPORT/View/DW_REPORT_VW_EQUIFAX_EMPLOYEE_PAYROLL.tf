resource "snowflake_view" "DW_REPORT_VW_EQUIFAX_EMPLOYEE_PAYROLL" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "REPORT"
	name = "VW_EQUIFAX_EMPLOYEE_PAYROLL"
	statement = <<-SQL
...
SELECT DISTINCT 
	...
	CASE WHEN UPPER(COALESCE (tp.field_worker_flag, sd.fieldstaff)) = 'FALSE' THEN 'ADMIN'
		...
		AS "JobTitle"
FROM DW_${var.SF_ENVIRONMENT}.HAH.FACT_PAYROLL AS pay
INNER JOIN (SELECT * FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_EMPLOYEE
...
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.TRUSTPOINTDATA.EMPLOYEES AS tp 
	ON tp.employee_number = pay.employee_id
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_STAFFS AS sd 
	ON sd.STAFFID = pay.employee_ID
LEFT OUTER JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_COMPANY_STATE fein
...
(
	...
	LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.GPSYNCDATA.PAYSTUB_MASTER m
		ON trim(emp.employee_id) = trim(m.EMPLOYEEID)
		AND trim(emp.db) = trim(m.DB)
	LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.GPSYNCDATA.PAYSTUB_DETAILLINE dl 
...
(
	...
	FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.GPALLPAYCHECKSBASE emp 
	LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.GPALLPAYCHECKDETAILSBASE pcd 
...
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.PAYCODE_CATEGORY pc 
ON pc.DB = pcd.db
...
p.employee_pid AS "EmployeeUniqueIdentifier", 
	...
	CASE WHEN emp.EMPLOYEE_CATEGORY = 'Field' THEN 'FIELD'
		...
		AS "JobTitle"
FROM pid p
LEFT OUTER JOIN (SELECT * FROM DW_${var.SF_ENVIRONMENT}.HAH.dim_employee WHERE source_system_id IN (1,2,5)) AS emp 
	ON MD5( HEX_DECODE_STRING( emp.employee_pid)) = p.employee_pid 
LEFT OUTER JOIN (SELECT * FROM DW_${var.SF_ENVIRONMENT}.HAH.FACT_PAYROLL WHERE payroll_date >= '2016-10-01'::DATE) AS pay
	ON pay.EMPLOYEE_KEY = emp.employee_key
...
WHERE "EmployeeUniqueIdentifier" IN (SELECT "EmployeeUniqueIdentifier" FROM DW_${var.SF_ENVIRONMENT}.REPORT.VW_EQUIFAX_EMPLOYEE)
