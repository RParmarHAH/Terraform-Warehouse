resource "snowflake_view" "DW_REPORT_VW_DAYFALL_CALC" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "REPORT"
	name = "VW_DAYFALL_CALC"
	statement = <<-SQL
	 WITH PAYROLLPERIODS
AS
(
...
) -- SELECT * FROM ACTUALHOURS_DAY WHERE STATE = 'GA' ORDER BY 1 DESC;
/*    SELECT dd.DAY_NAME, SUM( ahd.HOURS) AS VISIT_HOURS, COUNT( DISTINCT dd.CALENDAR_DATE) AS NUM_DAY_NAME, VISIT_HOURS/NUM_DAY_NAME
	FROM ACTUALHOURS_DAY ahd INNER JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_DATE dd ON ahd.SERVICE_DATE = dd.CALENDAR_DATE
	WHERE ahd.STATE = 'GA' AND YEAR( ahd.SERVICE_DATE) = 2020 AND MONTH( ahd.SERVICE_DATE) IN (1)
	GROUP BY 1;*/
,
...
/*INNER JOIN (SELECT DISTINCT SOURCE_SYSTEM_ID, STATE_CODE, SERVICE_LINE_CODE, OFFICE_NUMBER, BRANCH_KEY
...
SELECT --dm.SOURCE_SYSTEM_ID,
	dm.STATE, dm.REVENUE_CATEGORY, /*dm.OFFICE_NUMBER, dm.BRANCH_KEY,*/ dm.MONTH
...
) SELECT * FROM DAYFALL_BYDAY --WHERE /*DAYFALL_AMT > 1 AND*/ STATE = 'MO' AND MONTH = '2021-01-01' 
...
AS
(
SELECT SOURCE_SYSTEM_ID, STATE, OFFICE_NUMBER, BRANCH_KEY, REVENUE_CATEGORY,
	MONTH, CALENDAR_DATE, MONTH_DAYFALL, TOTALDAYSOFMONTHSOFAR, PERIOD_END_DATE, MONTHBUDGET_HOURS, MONTHBUDGET_REVENUE, MONTHDAYS,
	PERCENT_SATURDAY, PERCENT_SUNDAY, PERCENT_HOLIDAY
FROM (SELECT dfbd.SOURCE_SYSTEM_ID, dfbd.STATE, dfbd.OFFICE_NUMBER, dfbd.BRANCH_KEY, dfbd.REVENUE_CATEGORY, dfbd.MONTH, dfbd.CALENDAR_DATE
		,dfbd.MONTH_DAYFALL, dfbd.TOTALDAYSOFMONTHSOFAR,dfbd.PERIOD_END_DATE, dfbd.MONTHBUDGET_HOURS, dfbd.MONTHBUDGET_REVENUE,
         dfbd.MonthDays, dfbd.PERCENT_SATURDAY, dfbd.PERCENT_SUNDAY, dfbd.PERCENT_HOLIDAY
...
WHERE rownum = 1
) --SELECT * FROM DAYFALL_MONTHRECORD WHERE OFFICE_NUMBER = 743 ORDER BY MONTH
,
...
AS
(
SELECT STATE, BRANCH_KEY, REVENUE_CATEGORY,
...
AS
(
SELECT BRANCH.BRANCH_KEY, BRANCH.REGION_NAME, BRANCH.DETAILED_OFFICE_NAME, BRANCH.OFFICE_STATE_CODE, dmr.REVENUE_CATEGORY AS SERVICELINE, dmr.OFFICE_NUMBER
	,dmr.MONTH
	,ahm.ACTUALHOURS
    ,ahm.ACTUALREVENUE
	,dmr.MONTH_DAYFALL
    ...
    ,DMR.PERIOD_END_DATE
    ,dmr.PERCENT_SATURDAY
    ,dmr.PERCENT_SUNDAY
    ,dmr.PERCENT_HOLIDAY
FROM DAYFALL_MONTHRECORD dmr
INNER JOIN (select DISTINCT SOURCE_SYSTEM_ID,BRANCH_KEY,DETAILED_OFFICE_NAME,OFFICE_STATE_CODE,
		    CASE WHEN SYSTEM_CODE = '8485' THEN 'PA' ELSE SYSTEM_CODE END as SYSTEM_CODE, OFFICE_NUMBER,
		    REGION_NAME
		    from HAH.DIM_BRANCH) BRANCH
ON dmr.BRANCH_KEY = BRANCH.BRANCH_KEY
LEFT JOIN ACTUALHOURS_MONTH ahm
ON dmr.BRANCH_KEY = ahm.BRANCH_KEY
