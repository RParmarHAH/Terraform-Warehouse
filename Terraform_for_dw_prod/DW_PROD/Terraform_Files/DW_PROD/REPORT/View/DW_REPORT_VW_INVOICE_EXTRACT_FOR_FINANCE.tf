resource "snowflake_view" "DW_REPORT_VW_INVOICE_EXTRACT_FOR_FINANCE" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "REPORT"
	name = "VW_INVOICE_EXTRACT_FOR_FINANCE"
	statement = <<-SQL
...
FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFINVOICES inv 
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DataFlexSyncData.DFInvoicePayments AS pay ON pay.DBNAME = inv.DBNAME AND pay.INVOICENO = inv.INVOICENO AND pay.PaidAmount <> 0
WHERE inv.INVOICECREATEDDATE BETWEEN CAST( '2017-01-01' AS DATE) AND CAST( CURRENT_DATE AS DATE)
--AND TRIM( inv.DBName) = 'TN'
...
FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFINVOICES inv 
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DataFlexSyncData.DFInvoicePayments AS pay ON pay.DBNAME = inv.DBNAME AND pay.INVOICENO = inv.INVOICENO AND pay.WriteOffAmount <> 0
WHERE inv.INVOICECREATEDDATE BETWEEN CAST( '2017-01-01' AS DATE) AND CAST( CURRENT_DATE AS DATE)
--AND TRIM( inv.DBName) = 'TN'
--AND inv.invoiceno = 53320
GROUP BY 1, 2
