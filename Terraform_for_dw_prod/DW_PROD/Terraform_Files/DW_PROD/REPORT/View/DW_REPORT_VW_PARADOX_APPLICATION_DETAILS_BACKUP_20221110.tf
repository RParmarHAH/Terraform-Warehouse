resource "snowflake_view" "DW_REPORT_VW_PARADOX_APPLICATION_DETAILS_BACKUP_20221110" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "REPORT"
	name = "VW_PARADOX_APPLICATION_DETAILS_BACKUP_20221110"
	statement = <<-SQL
...
FROM DW_${var.SF_ENVIRONMENT}.HAH.FACT_APPLICANT_STATUS S
    ...
    --LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.TBL_COST_CENTRES_TIER_4 CCL ON TRIM(UPPER(J.CITY)) = TRIM(UPPER(CCL.PROPERTIES_DESCRIPTION)) 
    --LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.GROUPS GL ON TRIM(UPPER(J.CITY)) = TRIM(UPPER(GL.PROPERTIES_DESCRIPTION))
...
WHERE
    ...
    and S.APPLICANT_STATUS IN ('Hire: Hired', 'Orientation/ Hired: Hired', 'Hired: Hired')
