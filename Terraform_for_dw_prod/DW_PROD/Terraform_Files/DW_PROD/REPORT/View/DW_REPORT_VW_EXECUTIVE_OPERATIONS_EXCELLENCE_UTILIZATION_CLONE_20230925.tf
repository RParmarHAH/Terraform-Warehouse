resource "snowflake_view" "DW_REPORT_VW_EXECUTIVE_OPERATIONS_EXCELLENCE_UTILIZATION_CLONE_20230925" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "REPORT"
	name = "VW_EXECUTIVE_OPERATIONS_EXCELLENCE_UTILIZATION_CLONE_20230925"
	statement = <<-SQL
	 WITH DATES AS 
(
...
--			NULL AS PARTNER_CODE,
       ...
       CASE WHEN COALESCE(CCSI_BRANCH.CLIENT_KEY,CCSI_ACQUIRED_CLIENT.CLIENT_KEY) IS NOT NULL THEN 'CCSI' 
		...
		left join dw_prod.public.ms_con mc on mc.client_key = data.client_key and mc.contract_key=data.contract_key and data.period_begin_date='2023-08-01' 																																																																
...
--										   WHEN PCS.SOURCE_SYSTEM_ID =4 AND PCS.SYSTEM_CODE ='8485' THEN 'HC' END)
	...
	--DATA.VISITS_NEED_MAINTENANCE,
       ...
       CASE WHEN COALESCE(CCSI_BRANCH.CLIENT_KEY,CCSI_ACQUIRED_CLIENT.CLIENT_KEY) IS NOT NULL THEN 'CCSI' 
        ...
        left join dw_prod.public.ms_con mc on mc.client_key =data.client_key and data.partner_contract_service_key = mc.hha_con and data.period_begin_date='2023-08-01'																																									   
