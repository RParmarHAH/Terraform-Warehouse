resource "snowflake_view" "DW_REPORT_VW_CLIENTS_REFERRED_BY_REFERRAL_DATE_OPD_MAPPING_BACKUP" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "REPORT"
	name = "VW_CLIENTS_REFERRED_BY_REFERRAL_DATE_OPD_MAPPING_BACKUP"
	statement = <<-SQL
...
WHERE DC.INCLUDE_FOR_OPS_PERF_CLIENTS = 1 AND R.REFERRED_DATE BETWEEN DATEADD('YEAR',-2,DATE_TRUNC('YEAR',CURRENT_DATE)) AND CURRENT_DATE;
SQL
	or_replace = true 
	is_secure = false 
}
