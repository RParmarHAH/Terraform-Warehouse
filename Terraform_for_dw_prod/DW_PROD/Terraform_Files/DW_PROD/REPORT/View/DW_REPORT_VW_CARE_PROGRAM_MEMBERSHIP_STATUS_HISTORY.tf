resource "snowflake_view" "DW_REPORT_VW_CARE_PROGRAM_MEMBERSHIP_STATUS_HISTORY" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "REPORT"
	name = "VW_CARE_PROGRAM_MEMBERSHIP_STATUS_HISTORY"
	statement = <<-SQL
...
WITH LCM AS (
    ...
    FROM APP_DB_${var.SF_ENVIRONMENT}.CARE_COORDINATION.CLIENT_PROGRAM_MEMBERSHIP_STATUS_HISTORY CPM
    ...
    LEFT JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.ROSTER_AMS_MAPPING_HISTORY RAM 
        ON CPM.CLIENT_KEY = RAM.AMS_CLIENT_KEY AND CPC.ROSTER_NAME = RAM.ROSTER_NAME --AND RAM.AMS_CLIENT_KEY <> RAM.PARTNER_CLIENT_KEY 
    LEFT JOIN DW_${var.SF_ENVIRONMENT}.REPORT.VW_GATOR_CLIENT_MAPPING CML 
        ON CPM.CLIENT_KEY = CML.CLIENT_KEY AND  CML.ORIGINAL_SOURCE_SYSTEM = 'DW'
    LEFT JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.ROSTER_AMS_MAPPING_HISTORY RAM2 
        ON CML.ORIGINAL_CLIENT_KEY = RAM2.AMS_CLIENT_KEY AND CPC.ROSTER_NAME = RAM2.ROSTER_NAME
    LEFT JOIN DW_${var.SF_ENVIRONMENT}.REPORT.VW_GATOR_CLIENT_MAPPING CML2 
        ON CPM.CLIENT_KEY = CML2.CLIENT_KEY AND  CML2.ORIGINAL_SOURCE_SYSTEM = 'DW'
    LEFT JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.ROSTER_AMS_MAPPING_HISTORY RAM3 
