resource "snowflake_view" "DW_REPORT_VW_HOURSDATA" {
...
(
  ...
  --DIMDATE
  LEFT JOIN
  "DISC_${var.SF_ENVIRONMENT}"."BI_REPOSITORY"."KMOFFICEBUDGETS" OFFICEBUDGET
  ON CAST(OFFICEBUDGET.MONTH AS DATE) = CAST(FIRST_DAY_OF_MONTH AS DATE)
  AND TRIM(OFFICEBUDGET.DBNAME) = DIMDATE.STATE
  INNER JOIN
  "DW_${var.SF_ENVIRONMENT}"."HAH"."DIM_SOURCE_SYSTEM" SOURCESYSTEM
