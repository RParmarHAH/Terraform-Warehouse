resource "snowflake_view" "DW_REPORT_VW_RISKONNECT_EXTRACT" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "REPORT"
	name = "VW_RISKONNECT_EXTRACT"
	statement = <<-SQL
	 SELECT DISTINCT
...
--	SELECT CLIENT_SSN, COUNT(*) FROM (
	...
	FIRST_VALUE( cl.CLIENT_KEY IGNORE NULLS) OVER (PARTITION BY CLIENT_SSN ORDER BY cl.ACTIVE_CLIENT_FLAG DESC, cl.LAST_SERVICE_DATE DESC NULLS LAST, cl.REFERRAL_DATE DESC NULLS LAST) AS CLIENT_KEY
	 FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_CLIENT cl
	 ...
	 LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.CV_PATIENTS AS cvp ON cl.SOURCE_SYSTEM_ID IN (1,2) AND TRIM( cl.SYSTEM_CODE) = TRIM( cvp.DB) AND cl.CLIENT_NUMBER::VARCHAR = TRIM( cvp.PATIENT_NUMBER)::VARCHAR
...
FROM (SELECT DISTINCT DB, CLIENTNUMBER FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.CV_MONTHLYHOURS
	  WHERE NVL( HoursAuthorized,0) <> 0 AND ETL_LAST_UPDATED_DATE >= '1900-01-01') mh
LEFT OUTER JOIN ( SELECT DISTINCT DB, CLIENTNUMBER, LOCATION_CODE, FIRSTDATEOFSERVICE, REFERRALDATE FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.CV_MONTHLYHOURS
...
FROM (SELECT * FROM DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_CLIENTADMISSIONS WHERE AGENCYID = '8485')
)
...
LEFT OUTER JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_CLIENT AS cl ON uc.CLIENT_KEY = cl.CLIENT_KEY
...
LEFT OUTER JOIN (SELECT * FROM DW_${var.SF_ENVIRONMENT}.REPORT.VW_RISKONNECT_NODE_HIERARCHY WHERE UPPER( HIERARCHY_NODE) NOT LIKE ('%INACTIVE%')) AS ro
	ON cl.SOURCE_SYSTEM_ID IN (3,4)
				AND ro.State = co.Office_State_Code AND TO_VARCHAR( ro.OfficeNo) = TO_VARCHAR( co.Office_Number)
LEFT OUTER JOIN (SELECT * FROM DW_${var.SF_ENVIRONMENT}.REPORT.VW_RISKONNECT_NODE_HIERARCHY WHERE UPPER( HIERARCHY_NODE) NOT LIKE ('%INACTIVE%')) AS ro2
...
--LEFT OUTER JOIN @DISC_${var.SF_ENVIRONMENT}.Stage.AZStage/CSV_Files/RK_PAclients_fromNicole_20200104 (file_format => DISC_${var.SF_ENVIRONMENT}.Stage.CSV_Format) pa ON cl.SOURCE_SYSTEM_ID = 4 AND cl.CLIENT_NUMBER = TRIM( pa.$3)
	...
	(
		SELECT DISTINCT MD5( HEX_DECODE_STRING( EMPLOYEE_PID)) AS EMPLOYEE_PID,
			FIRST_VALUE( EMPLOYEE_KEY) IGNORE NULLS 
				...
				AS EMPLOYEE_KEY
		FROM (
			  (SELECT * FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_EMPLOYEE WHERE SOURCE_SYSTEM_ID IN (1,2)
			  ) UNION
			  (SELECT * FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_EMPLOYEE WHERE SOURCE_SYSTEM_ID IN (4) AND SYSTEM_CODE = '8485'
			  ) UNION
			  (SELECT * FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_EMPLOYEE WHERE SOURCE_SYSTEM_ID IN (5) AND UPPER( WORK_STATE) NOT IN ('GA','SC')
			 )) e
...
GP_COASTAL AS
	(
		...
		FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.GPALLEMPLOYEESBASE
	),
CV_EMP AS
	(
		SELECT DISTINCT TRIM( DB) AS DB,
			...
			FIRST_VALUE( TRIM( FACILITY_CODE) IGNORE NULLS)
				OVER (PARTITION BY TRIM( DB), TRIM( NURSE_ID) ORDER BY ETL_LAST_UPDATED_DATE DESC, ETL_INSERTED_DATE DESC, USER_CODE)
				AS FACILITY_CODE
		FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.CV_EMPLOYEES
	)
...
LEFT OUTER JOIN (SELECT * FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_Employee WHERE SOURCE_SYSTEM_ID IN (1,2)) AS emp
...
LEFT OUTER JOIN (SELECT * FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_Branch WHERE BRANCH_KEY <> '115cd45a7a09b6c402f4191bf6e21b62') AS brnch
	...
	(
		SELECT DISTINCT TRIM( SSN) AS SSN,
			...
			FIRST_VALUE( PAYROLLID) IGNORE NULLS 
				...
				ORDER BY NVL( LASTCHECKDATE, '1900-01-01') DESC, NVL( TERMINATEDATE, '1900-01-01') DESC, NVL( HIREDATE, '1900-01-01'), NUMBER)
				AS PAYROLLID
		FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFEMPLOYEES
...
--       ,COALESCE( NULLIF( brnch.OFFICE_STATE_CODE,''), NULLIF( emp.EMPLOYEE_STATE_CODE,''), NULLIF( emp.WORK_STATE,''), NULLIF( emp.SUTA_STATE,'')) AS MATCH_STATE
	   ...
	   ,upid.EMPLOYEE_KEY
FROM UNIQUE_PID upid
LEFT OUTER JOIN (SELECT * FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_Employee
...
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFEMPLOYEES AS dfe
	ON TRIM( udf.DBNAME) = TRIM( dfe.DBNAME) AND udf.NUMBER = dfe.NUMBER
LEFT OUTER JOIN (SELECT * FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_Branch WHERE BRANCH_KEY <> '115cd45a7a09b6c402f4191bf6e21b62') AS brnch ON brnch.Branch_Key = emp.Primary_Branch_Key
LEFT OUTER JOIN (SELECT * FROM DW_${var.SF_ENVIRONMENT}.REPORT.VW_RISKONNECT_NODE_HIERARCHY WHERE UPPER( HIERARCHY_NODE) NOT LIKE ('%INACTIVE%')) AS ro
...
--       ,COALESCE( NULLIF( brnch.OFFICE_STATE_CODE,''), NULLIF( emp.EMPLOYEE_STATE_CODE,''), NULLIF( emp.WORK_STATE,''), NULLIF( emp.SUTA_STATE,'')) AS MATCH_STATE
	   ...
	   ,upid.EMPLOYEE_KEY
FROM UNIQUE_PID upid
LEFT OUTER JOIN (SELECT * FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_Employee WHERE SOURCE_SYSTEM_ID = 4 AND TRIM( SYSTEM_CODE) = '8485') AS emp
	ON upid.EMPLOYEE_KEY = emp.EMPLOYEE_KEY
...
LEFT OUTER JOIN (SELECT * FROM DISC_${var.SF_ENVIRONMENT}.TRUSTPOINTDATA.EMPLOYEES WHERE TRIM( COMPANY) = 'EXCEL') tp
...
/*LEFT OUTER JOIN @DISC_${var.SF_ENVIRONMENT}.Stage.AZStage/CSV_Files/RISKONNECT/Riskonnect_contactList_20210111_145PM (file_format => DISC_${var.SF_ENVIRONMENT}.Stage.CSV_Format) rkSSN
ON TRIM( RISK.SSN) = RIGHT( '000000000'||REPLACE( TRIM( rkSSN.$4),'-',''),9)
LEFT OUTER JOIN @DISC_${var.SF_ENVIRONMENT}.Stage.AZStage/CSV_Files/RISKONNECT/Riskonnect_contactList_20210111_145PM (file_format => DISC_${var.SF_ENVIRONMENT}.Stage.CSV_Format) rkPAY
ON TRIM( RISK.PAYROLL_ID)::VARCHAR = TRIM( rkPAY.$8)::VARCHAR*/
--INNER JOIN @DW_${var.SF_ENVIRONMENT}.Stage.AZStage/CSV_Files/RISKONNECT/RK_lostSouls_20210126 (file_format => DW_${var.SF_ENVIRONMENT}.PUBLIC.CSV_Format) rkLS
