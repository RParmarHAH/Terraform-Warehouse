resource "snowflake_view" "DW_REPORT_VW_EMPLOYEE_DERIVED_MONTHLY_VISIT_METRICS" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "REPORT"
	name = "VW_EMPLOYEE_DERIVED_MONTHLY_VISIT_METRICS"
	statement = <<-SQL
...
FROM REPORT.EMPLOYEE_DERIVED_MONTHLY_VISIT_METRICS 
WHERE SERVICE_MONTH >= DATE_TRUNC('YEAR',DATEADD('YEAR',-2,CURRENT_DATE)) --AND CURRENT_DATE;
;
SQL
	or_replace = true 
	is_secure = false 
