resource "snowflake_view" "DW_REPORT_CLIENT_METRICS_FOR_SLT_FJN_W_BILLABLE" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "REPORT"
	name = "CLIENT_METRICS_FOR_SLT_FJN_W_BILLABLE"
	statement = <<-SQL
	 WITH candidate_clients_2018 AS (
SELECT DISTINCT cdmv.Client_Key, DATE_TRUNC( 'YEAR', cdmv.Service_Month) AS Service_Year
--FROM DW_${var.SF_ENVIRONMENT}.Report.Client_Derived_Monthly_Visit_Metrics_FJN AS cdmv
FROM DW_${var.SF_ENVIRONMENT}.Report.Client_Derived_Monthly_Visit_Metrics_CLS_w_Billable AS cdmv
...
WHERE cdmv.Service_Month BETWEEN CAST( '2018-01-01' AS DATE) AND CAST( '2020-12-31' AS DATE)
AND NVL( HC_Service_Line_Provided, 0) <> 0
AND  cdmv.Number_of_Visits IS NOT NULL
),
...
--FROM DW_${var.SF_ENVIRONMENT}.Report.Client_Derived_Monthly_Visit_Metrics_FJN AS cdmv
FROM DW_${var.SF_ENVIRONMENT}.Report.Client_Derived_Monthly_Visit_Metrics_CLS_w_Billable AS cdmv
INNER JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_Client AS cl ON cdmv.Client_Key = cl.Client_Key
