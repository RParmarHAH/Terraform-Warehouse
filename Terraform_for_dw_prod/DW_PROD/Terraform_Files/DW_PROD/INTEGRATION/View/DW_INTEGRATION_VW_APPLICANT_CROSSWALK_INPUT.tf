resource "snowflake_view" "DW_INTEGRATION_VW_APPLICANT_CROSSWALK_INPUT" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "INTEGRATION"
	name = "VW_APPLICANT_CROSSWALK_INPUT"
	statement = <<-SQL
...
from DW_${var.SF_ENVIRONMENT}.stage.workday_dim_applicant 
  where APPLICANT_ID not in (select COALESCE(WORKDAY_INTERNAL_ID,RANDOM()) from DW_${var.SF_ENVIRONMENT}.INTEGRATION.EMPLOYEE_CROSS_WALK WHERE WORKDAY_INTERNAL_ID IS NOT NULL AND INACTIVE = FALSE and DUP_FLAG = FALSE)
  ) 
,CROSS_WALK_DATA AS (
