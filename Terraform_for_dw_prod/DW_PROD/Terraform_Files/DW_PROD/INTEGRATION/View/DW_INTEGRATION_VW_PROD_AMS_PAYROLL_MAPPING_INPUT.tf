resource "snowflake_view" "DW_INTEGRATION_VW_PROD_AMS_PAYROLL_MAPPING_INPUT" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "INTEGRATION"
	name = "VW_PROD_AMS_PAYROLL_MAPPING_INPUT"
	statement = <<-SQL
...
--AMS
 SELECT DISTINCT EMPLOYEE_KEY AS ID,
  ...
  REGEXP_REPLACE(COALESCE(DW_${var.SF_ENVIRONMENT}.PUBLIC.REPLACE_NON_UTF8_CHARS(EMPLOYEE_HOME_PHONE),EMPLOYEE_CELL_PHONE,EMPLOYEE_WORK_PHONE), '[^0-9]','') PHONE,
  REGEXP_REPLACE(TRIM(DW_${var.SF_ENVIRONMENT}.PUBLIC.REPLACE_NON_UTF8_CHARS(EMPLOYEE_HOME_PHONE)), '[^0-9]','') EMPLOYEE_HOME_PHONE,
  ...
  UPPER(COALESCE(DW_${var.SF_ENVIRONMENT}.PUBLIC.REPLACE_NON_UTF8_CHARS(EMPLOYEE_ADDRESS1),EMPLOYEE_ADDRESS2)) ADDRESS,UPPER(TRIM(EMPLOYEE_CITY)) AS CITY,
  ...
  FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_AMS_MERGED 
  UNION 
  --PAYROLL
  SELECT DISTINCT EMPLOYEE_KEY AS ID,
  CASE 
  ...
  UPPER(REGEXP_REPLACE(EMPLOYEE_FIRST_NAME, '[^A-Za-z]', '')) AS EMPLOYEE_FIRST_NAME,UPPER(REGEXP_REPLACE(EMPLOYEE_LAST_NAME, '[^A-Za-z]', '')) AS EMPLOYEE_LAST_NAME,
  TRIM(HEX_DECODE_STRING(EMPLOYEE_PID)) AS EMPLOYEE_PID,TRIM(EMPLOYEE_DOB) AS EMPLOYEE_DOB,
  REGEXP_REPLACE(TRIM(COALESCE(DW_${var.SF_ENVIRONMENT}.PUBLIC.REPLACE_NON_UTF8_CHARS(EMPLOYEE_HOME_PHONE),
  EMPLOYEE_CELL_PHONE,EMPLOYEE_WORK_PHONE)), '[^0-9]','') PHONE,
  REGEXP_REPLACE(TRIM(DW_${var.SF_ENVIRONMENT}.PUBLIC.REPLACE_NON_UTF8_CHARS(EMPLOYEE_HOME_PHONE)), '[^0-9]','') EMPLOYEE_HOME_PHONE,
  REGEXP_REPLACE(TRIM(EMPLOYEE_CELL_PHONE), '[^0-9]','') EMPLOYEE_CELL_PHONE,UPPER(TRIM(COALESCE(EMPLOYEE_PERSONAL_EMAIL,EMPLOYEE_WORK_EMAIL))) EMAIL,
  UPPER(TRIM(COALESCE(DW_${var.SF_ENVIRONMENT}.PUBLIC.REPLACE_NON_UTF8_CHARS(EMPLOYEE_ADDRESS1),EMPLOYEE_ADDRESS2))) ADDRESS,UPPER(TRIM(EMPLOYEE_CITY)) AS CITY,
  ...
  COALESCE(COALESCE(WORK_STATE,PRIMARY_BRANCH_STATE), S.DEFAULT_STATE) STATE_CODE,'PAYROLL' AS SOURCE_SYSTEM FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_PAYROLL_MERGE_DEDUPE P 
  LEFT JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.DEFAULT_SYSTEM_STATE S ON P.SOURCE_SYSTEM_ID = S.SOURCE_SYSTEM_ID AND P.SYSTEM_CODE = S.SYSTEM_CODE 
