resource "snowflake_view" "DW_INTEGRATION_VW_DIM_CLIENT_GA" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "INTEGRATION"
	name = "VW_DIM_CLIENT_GA"
	statement = <<-SQL
  ...
  FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_CLIENT WHERE (SOURCE_SYSTEM_ID =9 AND UPPER(SYSTEM_CODE) ='GEORGIA (1002)' AND (LINKED_ID NOT LIKE '%CLIENT%'  OR  LINKED_ID IS NULL) )
UNION SELECT SYSTEM_CODE AS LINKED_ID_SOURCE, CLIENT_NUMBER::VARCHAR AS LINKED_ID_MATCH,* FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_CLIENT WHERE (SOURCE_SYSTEM_ID =3 AND SYSTEM_CODE ='GA') OR (SOURCE_SYSTEM_ID IN (1,2) ));
SQL
	or_replace = true 
	is_secure = false 
}
