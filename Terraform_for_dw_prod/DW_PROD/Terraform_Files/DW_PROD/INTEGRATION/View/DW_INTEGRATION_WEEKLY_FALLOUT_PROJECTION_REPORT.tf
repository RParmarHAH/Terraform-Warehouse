resource "snowflake_view" "DW_INTEGRATION_WEEKLY_FALLOUT_PROJECTION_REPORT" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "INTEGRATION"
	name = "WEEKLY_FALLOUT_PROJECTION_REPORT"
	statement = <<-SQL
...
WITH VIEW_DATA AS (
    WITH WORKDAY_DATA AS 
            ...
            FROM DISC_${var.SF_ENVIRONMENT}.WORKDAY.WORKER_STATUS WS 
            LEFT JOIN DISC_${var.SF_ENVIRONMENT}.WORKDAY.WORKER_ADDITIONAL_INFORMATION ADD_INFO 
                ON ADD_INFO.EMPLOYEE_ID = WS.EMPLOYEE_ID 
            LEFT JOIN DISC_${var.SF_ENVIRONMENT}.WORKDAY.WORKER_POSITION_DATA W_POS 
    ...
    --          END AS REMARK2,
            ...
            FV.SERVICE_DATE ,
            FV.BRANCH_NAME
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.FACT_VISIT_MERGED FV
        ...
        ----
        LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_EMPLOYEE E 
    ...
    --      )
    ----
    --LEFT JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.EMPLOYEE_CROSS_WALK CW2
...
, VISIT_DATA AS (
    ...
    FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.STVHC_T_SCHEDULES
    )
    ,ALAYACARE AS
    (
    ...
    FROM DISC_${var.SF_ENVIRONMENT}.ALAYACARE.VISIT VISITS LEFT JOIN
    DISC_${var.SF_ENVIRONMENT}.ALAYACARE.BRANCH B ON VISITS.BRANCH_ID = B.BRANCH_ID
    )
    ,AXXESS AS
    (
       ...
       DISC_${var.SF_ENVIRONMENT}.AXXESS.VW_AXXESS_HC_TASKDETAILS
    )
    ,OSHAH AS
    (
