resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_CLIENT_PA_MERGED_DEDUPE" {
	name ="GET_MASTER_DIM_CLIENT_PA_MERGED_DEDUPE"
...
WITH LWD_NEW AS (
    ...
    CLUSTER_ID
    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_PA_RL_OUTPUT
)
, NEW AS (
    SELECT * FROM (
        SELECT 
        ...
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_PA_RL_OUTPUT PA
...
, LWD_OLD AS (
    SELECT ID,
    ROW_NUMBER() OVER (PARTITION BY CLUSTER_ID ORDER BY LAST_SERVICE_DATE ASC NULLS LAST, FIRST_SERVICE_DATE ASC NULLS LAST, BEGIN_DATE ASC NULLS LAST,SYSTEM_CODE desc) AS SRC_PRI,
    CLUSTER_ID
    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_PA_RL_OUTPUT
) 
, OLD AS (
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_PA_RL_OUTPUT R
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_PA_RL_OUTPUT U
