resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_CLIENT_OH_MERGED_DEDUPE" {
	name ="GET_MASTER_DIM_CLIENT_OH_MERGED_DEDUPE"
...
WITH LWD_NEW AS (
    ...
    CLUSTER_ID
    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_OH_RL_OUTPUT
)
, NEW AS (
    SELECT * FROM (
        SELECT 
        ...
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_OH_RL_OUTPUT OHO
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_OH_RL_OUTPUT R
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_OH_RL_OUTPUT U
