resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_EMPLOYEE_OH_MERGED_DEDUPE" {
	name ="GET_MASTER_DIM_EMPLOYEE_OH_MERGED_DEDUPE"
...
(
	...
	CLUSTER_ID
	FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_OH_RL_OUTPUT
...
,NEW AS (
	SELECT * FROM (
		SELECT
			SRC_PRI,
			row_number() over(partition by OHO.CLUSTER_ID order by SRC_PRI ASC) min_record_id,
			OHO.ID,EMPLOYEE_ID,SYSTEM_CODE,SOURCE_SYSTEM_ID,EMPLOYEE_PID,EMPLOYEE_NUMBER,EMPLOYEE_DOB,
			EMPLOYEE_DATE_OF_DEATH,
			MIN(EMPLOYEE_HIRE_DATE::date) OVER(PARTITION BY OHO.CLUSTER_ID) AS DERIVED_EMPLOYEE_HIRE_DATE,
			MAX(EMPLOYEE_REHIRE_DATE::date) OVER(PARTITION BY OHO.CLUSTER_ID) AS DERIVED_EMPLOYEE_REHIRE_DATE,
			MIN(EMPLOYEE_BENEFIT_START_DATE::date) OVER(PARTITION BY OHO.CLUSTER_ID) AS DERIVED_EMPLOYEE_BENEFIT_START_DATE,
			MIN(EMPLOYEE_FIRST_CHECK_DATE::date) OVER(PARTITION BY OHO.CLUSTER_ID) AS DERIVED_EMPLOYEE_FIRST_CHECK_DATE,
			MAX(EMPLOYEE_LAST_CHECK_DATE::date) OVER(PARTITION BY OHO.CLUSTER_ID) AS DERIVED_EMPLOYEE_LAST_CHECK_DATE,
			MAX(EMPLOYEE_LAST_WORKED_DATE::date) OVER(PARTITION BY OHO.CLUSTER_ID) AS DERIVED_EMPLOYEE_LAST_WORKED_DATE,
			...
			,OHO.CLUSTER_ID 
		FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_OH_RL_OUTPUT OHO
		INNER JOIN LWD_NEW 
			ON LWD_NEW.ID = OHO.ID
...
SELECT * FROM (
	SELECT 
		SRC_PRI,
		row_number() over(partition by OHO.CLUSTER_ID order by SRC_PRI DESC) min_record_id,
		OHO.ID,SOURCE_SYSTEM_ID,SYSTEM_CODE,EMPLOYEE_NUMBER,EMPLOYEE_PID,OHO.CLUSTER_ID
		FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_OH_RL_OUTPUT OHO
		INNER JOIN LWD_OLD 
			ON LWD_OLD.ID = OHO.ID
		WHERE OHO.CLUSTER_ID IS NOT NULL
	) -- WHERE min_record_id = 1
) --SELECT * FROM OLD;
SELECT distinct 
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_OH_RL_OUTPUT R
LEFT JOIN OLD O 
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_OH_RL_OUTPUT U
