resource "snowflake_procedure" "DW_INTEGRATION_SP_WORKDAY_X_ATTRIBUTE_COMPARISION_TABLE_REFRESH" {
	name ="SP_WORKDAY_X_ATTRIBUTE_COMPARISION_TABLE_REFRESH"
...
def main(session: snowpark.Session): 
    ...
    query = f"SELECT  COLUMN_NAME  FROM DW_${var.SF_ENVIRONMENT}.INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = ''DIM_EMPLOYEE_MERGED'' and TABLE_SCHEMA = ''INTEGRATION''  and COLUMN_NAME not in (''SYSTEM_CODE'',''PUBLIC_ID'',''ETL_INFERRED_MEMBER_FLAG'',''ETL_DELETED_FLAG'',''ETL_LAST_UPDATED_BY'',''ETL_LAST_UPDATED_DATE'',''ETL_INSERTED_BY'',''ETL_INSERTED_DATE'',''ETL_INSERTED_TASK_KEY'',''ETL_TASK_KEY'',''EFFECTIVE_TO_DATE'',''EFFECTIVE_FROM_DATE'')"
...
def get_datatype_sql(session,column_name):
    query = f"SELECT  upper(DATA_TYPE) AS  DATA_TYPE FROM DW_${var.SF_ENVIRONMENT}.INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = ''DIM_EMPLOYEE_MERGED'' and TABLE_SCHEMA = ''INTEGRATION''  and COLUMN_NAME  in (''" + column_name + "'')"
    # print(query)
    df_sql = session.sql(query)
...
def generate_comparison_sql(session, base_columns, source_table):
    ...
    for column in base_columns:
        ...
        comparison_template = """
        CREATE OR REPLACE TABLE  DW_${var.SF_ENVIRONMENT}.INTEGRATION.WORKDAY_X_ATTRIBUTE_COMPARISION as
        SELECT 
        ...
        FROM 
            ...
            FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.{source_table} SOURCE
            JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.EMPLOYEE_CROSS_WALK C ON SOURCE.EMPLOYEE_ID = C.WORKDAY_INTERNAL_ID
            JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_SOURCE_SYSTEM SS ON SS.SOURCE_SYSTEM_ID = SOURCE.SOURCE_SYSTEM_ID
        ...
        LEFT JOIN 
            ...
            FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.{source_table} SOURCE
            JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.EMPLOYEE_CROSS_WALK C ON SOURCE.EMPLOYEE_KEY = C.AMS_EMPLOYEE_KEY
            JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_SOURCE_SYSTEM SS ON SS.SOURCE_SYSTEM_ID = SOURCE.SOURCE_SYSTEM_ID
        ...
        LEFT JOIN 
            ...
            FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.{source_table} SOURCE
            JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.EMPLOYEE_CROSS_WALK C ON SOURCE.EMPLOYEE_KEY = C.PAYROLL_EMPLOYEE_KEY
            JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_SOURCE_SYSTEM SS ON SS.SOURCE_SYSTEM_ID = SOURCE.SOURCE_SYSTEM_ID
