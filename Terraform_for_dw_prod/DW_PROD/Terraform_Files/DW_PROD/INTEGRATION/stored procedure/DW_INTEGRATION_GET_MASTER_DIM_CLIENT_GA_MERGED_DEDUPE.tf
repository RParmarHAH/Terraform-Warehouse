resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_CLIENT_GA_MERGED_DEDUPE" {
	name ="GET_MASTER_DIM_CLIENT_GA_MERGED_DEDUPE"
...
INSERT INTO DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_MERGED_DEDUPE
WITH NEW AS (
SELECT * FROM (
...
CLUSTER_ID FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_GA_RL_OUTPUT WHERE CLUSTER_ID IS NOT NULL
)
...
ID,SYSTEM_CODE,SOURCE_SYSTEM_ID,CLIENT_NUMBER,CLIENT_PID,CLUSTER_ID,AGENCY FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_GA_RL_OUTPUT WHERE CLUSTER_ID IS NOT NULL
)-- WHERE min_record_id = 1
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_GA_RL_OUTPUT R
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_GA_RL_OUTPUT U
