resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_APPLICANT_WORKDAY_EMPLOYEE_MAPPING" {
	name ="GET_MASTER_APPLICANT_WORKDAY_EMPLOYEE_MAPPING"
...
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.integration.APPLICANT_WORKDAY_EMPLOYEE_MAPPING
WITH MAPPING_DATA AS (  -- PULL THE LIST OF DUPLICATES
    SELECT * FROM   DW_${var.SF_ENVIRONMENT}.INTEGRATION.APPLICANT_EMPLOYEE_DEDUPE_OUTPUT_NEW  WHERE CLUSTER_ID IS NOT NULL
) --select * from MAPPING_DATA;
,EMPLOYEE AS (
select M.* from MAPPING_DATA M 
  LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_EMPLOYEE E ON M.EMPLOYEE_KEY = E.EMPLOYEE_KEY
  WHERE SOURCE_SYSTEM = ''EMPLOYEE''
QUALIFY ROW_NUMBER() OVER (PARTITION BY CLUSTER_ID ORDER BY   
                                 case when  M.SYSTEM_CODE in (SELECT SYSTEM_CODE FROM DW_${var.SF_ENVIRONMENT}.HAH.SOURCE_SYSTEM_MIGRATION where STATUS = ''LEGACY'') then ''1900-01-01''::DATE ELSE current_Date END  DESC
