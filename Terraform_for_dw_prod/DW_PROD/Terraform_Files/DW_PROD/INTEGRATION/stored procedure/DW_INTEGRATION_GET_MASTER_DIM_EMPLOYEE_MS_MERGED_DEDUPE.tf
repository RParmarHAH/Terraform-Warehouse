resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_EMPLOYEE_MS_MERGED_DEDUPE" {
	name ="GET_MASTER_DIM_EMPLOYEE_MS_MERGED_DEDUPE"
...
(
    SELECT ID,
	ROW_number() OVER (PARTITION BY CLUSTER_ID ORDER BY CASE WHEN SOURCE_SYSTEM_ID =17 THEN 1 WHEN SOURCE_SYSTEM_ID =3 THEN 2 end ) AS SRC_PRI,
	CLUSTER_ID
	FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_MS_RL_OUTPUT
...
,NEW AS (
    SELECT * FROM (
        SELECT
            ...
            ,PA.CLUSTER_ID 
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_MS_RL_OUTPUT PA
        INNER JOIN LWD_NEW 
            ON LWD_NEW.ID = PA.ID
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_MS_RL_OUTPUT R
LEFT JOIN OLD O 
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_MS_RL_OUTPUT U
