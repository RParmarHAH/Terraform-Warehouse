resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_EMPLOYEE_GA_MERGED_DEDUPE" {
	name ="GET_MASTER_DIM_EMPLOYEE_GA_MERGED_DEDUPE"
...
ID,EMPLOYEE_ID,SYSTEM_CODE,SOURCE_SYSTEM_ID,EMPLOYEE_PID,EMPLOYEE_NUMBER,EMPLOYEE_DOB,
...
,CLUSTER_ID FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_GA_RL_OUTPUT WHERE CLUSTER_ID IS NOT NULL
)
...
ID,SOURCE_SYSTEM_ID,SYSTEM_CODE,EMPLOYEE_NUMBER,EMPLOYEE_PID,CLUSTER_ID FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_GA_RL_OUTPUT WHERE CLUSTER_ID IS NOT NULL
) --WHERE min_record_id = 1
) --SELECT * FROM OLD;
SELECT distinct O.ID ORIGINAL_EMPLOYEE_KEY,O.SOURCE_SYSTEM_ID ORIGINAL_SOURCE_SYSTEM_ID ,O.SYSTEM_CODE ORIGINAL_SYSTEM_CODE,O.EMPLOYEE_NUMBER ORIGINAL_EMPLOYEE_NUMBER,O.EMPLOYEE_PID ORIGINAL_EMPLOYEE_PID,
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_GA_RL_OUTPUT R
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_GA_RL_OUTPUT U
