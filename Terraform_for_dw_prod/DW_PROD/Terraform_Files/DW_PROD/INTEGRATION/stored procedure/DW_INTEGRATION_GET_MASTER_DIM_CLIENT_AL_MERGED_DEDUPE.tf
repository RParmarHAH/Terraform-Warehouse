resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_CLIENT_AL_MERGED_DEDUPE" {
	name ="GET_MASTER_DIM_CLIENT_AL_MERGED_DEDUPE"
...
SELECT (CASE  WHEN source_system_id=9 THEN 1
			  WHEN source_system_id=3 THEN 2
		end) as SRC_PRI,
...
CLUSTER_ID FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_AL_RL_OUTPUT WHERE CLUSTER_ID IS NOT NULL
)
...
ID,SYSTEM_CODE,SOURCE_SYSTEM_ID,CLIENT_NUMBER,CLIENT_PID,CLUSTER_ID,AGENCY FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_AL_RL_OUTPUT WHERE CLUSTER_ID IS NOT NULL
)-- WHERE min_record_id = 1
...
FROM  DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_AL_RL_OUTPUT R
...
FROM  DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_AL_RL_OUTPUT U
