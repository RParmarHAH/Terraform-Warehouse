resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_FACT_APPLICANT_EMPLOYEE_MAPPING_BKP" {
	name ="GET_MASTER_FACT_APPLICANT_EMPLOYEE_MAPPING_BKP"
...
var sql = `
  ...
  WITH APPLICANT_CLUSTERS AS (
    SELECT * 
    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.APPLICANT_EMPLOYEE_DEDUPE_OUTPUT 
    WHERE CLUSTER_ID IN (
      SELECT cluster_id FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.APPLICANT_EMPLOYEE_DEDUPE_OUTPUT 
