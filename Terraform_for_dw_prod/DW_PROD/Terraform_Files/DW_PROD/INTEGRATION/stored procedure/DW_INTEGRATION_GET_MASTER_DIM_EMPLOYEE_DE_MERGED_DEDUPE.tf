resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_EMPLOYEE_DE_MERGED_DEDUPE" {
	name ="GET_MASTER_DIM_EMPLOYEE_DE_MERGED_DEDUPE"
...
(
    SELECT ID,
    ROW_NUMBER() OVER (PARTITION BY CLUSTER_ID ORDER BY CASE WHEN SYSTEM_CODE=''OSHAH - DE'' THEN getdate() ELSE nvl(EMPLOYEE_LAST_CHECK_DATE,''1900-01-01'') end desc) AS SRC_PRI,
    CLUSTER_ID
    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_DE_RL_OUTPUT
    WHERE CLUSTER_ID IS NOT NULL
),
LWD_OLD AS
(
    SELECT ID,
    ROW_NUMBER() OVER (PARTITION BY CLUSTER_ID ORDER BY CASE WHEN SYSTEM_CODE=''OSHAH - DE'' THEN getdate() ELSE nvl(EMPLOYEE_LAST_CHECK_DATE,''1900-01-01'') end asc) AS SRC_PRI,
    CLUSTER_ID
    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_DE_RL_OUTPUT
    WHERE CLUSTER_ID IS NOT NULL
)
,NEW AS (
    SELECT * FROM (
        SELECT
            SRC_PRI,
            row_number() over(partition by PA.CLUSTER_ID order by SRC_PRI ASC) min_record_id,
            ...
            ,PA.CLUSTER_ID 
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_DE_RL_OUTPUT PA
        INNER JOIN LWD_NEW 
            ON LWD_NEW.ID = PA.ID
...
SELECT * FROM (
    SELECT 
        SRC_PRI,
        row_number() over(partition by PA.CLUSTER_ID order by SRC_PRI DESC) min_record_id,
        PA.ID,SOURCE_SYSTEM_ID,SYSTEM_CODE,EMPLOYEE_NUMBER,EMPLOYEE_PID,PA.CLUSTER_ID
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_DE_RL_OUTPUT PA
        INNER JOIN LWD_OLD 
            ON LWD_OLD.ID = PA.ID
        WHERE PA.CLUSTER_ID IS NOT NULL
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_DE_RL_OUTPUT R
LEFT JOIN OLD O 
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_DE_RL_OUTPUT U
