resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_FACT_AMS_PAYROLL_OH_MAPPING" {
	name ="GET_MASTER_FACT_AMS_PAYROLL_OH_MAPPING"
...
BEGIN
    ...
    WITH MAPPING_DATA AS (
      SELECT * FROM (
        ...
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.AMS_PAYROLL_OH_DEDUPE_OUTPUT) 
