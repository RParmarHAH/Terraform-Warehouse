resource "snowflake_procedure" "DW_INTEGRATION_TEMP_PROC_APPLICANT_EMPLOYEE_MAPPING" {
	name ="TEMP_PROC_APPLICANT_EMPLOYEE_MAPPING"
  ...
  var sql_delete_fact_applicant_status = `DELETE FROM DW_${var.SF_ENVIRONMENT}.HAH.FACT_APPLICANT_STATUS WHERE SYSTEM_CODE = ''PARADOX_${var.SF_ENVIRONMENT}'';`
  var sql_delete_dim_applicant = `DELETE FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_APPLICANT WHERE SYSTEM_CODE = ''PARADOX_${var.SF_ENVIRONMENT}'';`
  var sql_delete_dim_job = `DELETE FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_JOB WHERE SYSTEM_CODE = ''PARADOX_${var.SF_ENVIRONMENT}'';`
  
  var sql_insert_dim_applicant = `
    INSERT INTO DW_${var.SF_ENVIRONMENT}.HAH.DIM_APPLICANT
    SELECT  
    APPLICANT_KEY,SOURCE_SYSTEM_ID,''PARADOX_${var.SF_ENVIRONMENT}'',LAST_NAME,FIRST_NAME,MIDDLE_NAME,APPLICANT_ID,SSN,EMAIL,MOBILE_PHONE,HOME_PHONE,DATE_OF_BIRTH,ADDRESS,ADDRESS_LINE_2
  ...
  var sql_insert_dim_job = `
    INSERT INTO DW_${var.SF_ENVIRONMENT}.HAH.DIM_JOB
    SELECT 
    JOB_KEY,SOURCE_SYSTEM_ID,''PARADOX_${var.SF_ENVIRONMENT}'',JOB_ID,STATE,CITY,JOB_POSTING_DATE,JOB_CLOSING_DATE,JOB_CREATED_DATE,JOB_TITLE,JOB_TITLE_CODE
    ...
    FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_JOB
    WHERE (SOURCE_SYSTEM_ID = 12 AND SYSTEM_CODE IN (''PARADOX''));
  `
    
  var sql_insert_fact_applicant_status = `
    INSERT INTO DW_${var.SF_ENVIRONMENT}.HAH.FACT_APPLICANT_STATUS
    SELECT 
    APPLICANT_STATUS_KEY,SOURCE_SYSTEM_ID,''PARADOX_${var.SF_ENVIRONMENT}'',APPLICANT_KEY,JOB_KEY,JOB_FAMILY,CANDIDATE_ID,DATE_APPLIED,CONVERSATION_INPROGRESS_DATETIME
