resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_ZENDESK_WH_AMS_MAPPING_TBL" {
	name ="GET_MASTER_ZENDESK_WH_AMS_MAPPING_TBL"
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.INTEGRATION.ZENDESK_WH_AMS_MAPPING_TBL
with zendesk_clusters as (
	select * from DW_${var.SF_ENVIRONMENT}.INTEGRATION.ZENDESK_AMS_MAPPING_OUTPUT
	where cluster_id in (select distinct cluster_id from DW_${var.SF_ENVIRONMENT}.INTEGRATION.ZENDESK_AMS_MAPPING_OUTPUT where source_system_id = 999 and cluster_id IS NOT NULL)
)
