resource "snowflake_procedure" "DW_INTEGRATION_GET_APPLICANT_CROSSWALK_MAPPING" {
	name ="GET_APPLICANT_CROSSWALK_MAPPING"
...
UPDATE DW_${var.SF_ENVIRONMENT}.INTEGRATION.APPLICANT_CROSSWALK_MAPPING 
SET CURRENT_RECORDS = FALSE ;

INSERT INTO DW_${var.SF_ENVIRONMENT}.INTEGRATION.APPLICANT_CROSSWALK_MAPPING
WITH MAPPING_DATA AS (  -- PULL THE LIST OF DUPLICATES
    SELECT * FROM  DW_${var.SF_ENVIRONMENT}.INTEGRATION.APPLICANT_CROSSWALK_DEDUPE_OUTPUT  WHERE CLUSTER_ID IS NOT NULL
...
update DW_${var.SF_ENVIRONMENT}.INTEGRATION.APPLICANT_CROSSWALK_MAPPING M
 SET ON_HOLD = TRUE
  from (
with DUPS AS (    
select WORKDAY_ID,ETL_INSERTED_DATE,COUNT(*) from DW_${var.SF_ENVIRONMENT}.INTEGRATION.APPLICANT_CROSSWALK_MAPPING 
