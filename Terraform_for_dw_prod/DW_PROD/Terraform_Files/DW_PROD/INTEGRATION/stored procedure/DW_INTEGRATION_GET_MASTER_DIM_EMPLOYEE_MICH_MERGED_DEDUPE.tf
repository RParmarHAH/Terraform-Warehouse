resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_EMPLOYEE_MICH_MERGED_DEDUPE" {
	name ="GET_MASTER_DIM_EMPLOYEE_MICH_MERGED_DEDUPE"
...
(
    ...
    CLUSTER_ID
    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_MICH_RL_OUTPUT
    WHERE CLUSTER_ID IS NOT NULL
),
LWD_OLD AS
(
    SELECT ID,
    ROW_number() OVER (PARTITION BY CLUSTER_ID ORDER BY CASE WHEN SYSTEM_CODE=''MATRIXCARE'' THEN getdate() ELSE nvl(EMPLOYEE_LAST_CHECK_DATE,''1900-01-01'') end ) AS SRC_PRI,
    CLUSTER_ID
    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_MICH_RL_OUTPUT
    WHERE CLUSTER_ID IS NOT NULL
)
,NEW AS (
    SELECT * FROM (
        SELECT
            ...
            ,NYO.CLUSTER_ID 
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_MICH_RL_OUTPUT NYO
        INNER JOIN LWD_NEW 
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_MICH_RL_OUTPUT R
LEFT JOIN OLD O 
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_MICH_RL_OUTPUT U
