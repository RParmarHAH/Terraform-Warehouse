resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_CLIENT_MS_MERGED_DEDUPE" {
	name ="GET_MASTER_DIM_CLIENT_MS_MERGED_DEDUPE"
...
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
-- 08/24/23    jigar prajapati       Initial Development
--

INSERT INTO INTEGRATION.DIM_CLIENT_MERGED_DEDUPE
WITH LWD_NEW AS (
    SELECT ID,
    ROW_NUMBER() OVER (PARTITION BY CLUSTER_ID ORDER BY LAST_SERVICE_DATE DESC NULLS LAST, FIRST_SERVICE_DATE DESC NULLS LAST, BEGIN_DATE DESC NULLS LAST,SYSTEM_CODE ASC) AS SRC_PRI,
    CLUSTER_ID
    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_MS_RL_OUTPUT
)
, NEW AS (
    SELECT * FROM (
        SELECT 
            -- CASE WHEN SYSTEM_CODE=''PREFERRED'' THEN 1 ELSE 2 END as SRC_PRI,
            row_number() over(partition by PA.CLUSTER_ID order by SRC_PRI) min_record_id,
            PA.ID,
            ...
            MIN(REFERRAL_DATE::date) OVER(PARTITION BY PA.CLUSTER_ID) AS DERIVED_REFERRAL_DATE,
            FIRST_SERVICE_DATE::DATE - MIN(REFERRAL_DATE::date) OVER(PARTITION BY PA.CLUSTER_ID) AS DERIVED_DAYS_TO_SERVICE,
            CONTRACT_BEGIN_DATE,
            CONTRACT_END_DATE,
            MIN(FIRST_SERVICE_DATE::date) OVER(PARTITION BY PA.CLUSTER_ID) AS DERIVED_FIRST_SERVICE_DATE,
            MAX(LAST_SERVICE_DATE::date) OVER(PARTITION BY PA.CLUSTER_ID) AS DERIVED_LAST_SERVICE_DATE,
            MIN(BEGIN_DATE::date) OVER(PARTITION BY PA.CLUSTER_ID) AS DERIVED_BEGIN_DATE,DAYS_TO_SERVICE,
            ...
            ETL_DELETED_FLAG,ETL_INFERRED_MEMBER_FLAG, PA.CLUSTER_ID
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_MS_RL_OUTPUT PA
        INNER JOIN LWD_NEW ON LWD_NEW.ID = PA.ID
...
, LWD_OLD AS (
    SELECT ID,
    ROW_NUMBER() OVER (PARTITION BY CLUSTER_ID ORDER BY LAST_SERVICE_DATE ASC NULLS LAST, FIRST_SERVICE_DATE ASC NULLS LAST, BEGIN_DATE ASC NULLS LAST,SYSTEM_CODE ASC) AS SRC_PRI,
    CLUSTER_ID
    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_MS_RL_OUTPUT
) 
, OLD AS (
    SELECT * FROM (
        SELECT 
            row_number() over(partition by PA.CLUSTER_ID order by SRC_PRI) min_record_id,
            PA.ID,SYSTEM_CODE,SOURCE_SYSTEM_ID,CLIENT_NUMBER,CLIENT_PID,PA.CLUSTER_ID,AGENCY
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_MS_RL_OUTPUT PA
        INNER JOIN LWD_OLD ON LWD_OLD.ID = PA.ID
        WHERE PA.CLUSTER_ID IS NOT NULL
        )-- WHERE min_record_id = 1
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_MS_RL_OUTPUT R
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_MS_RL_OUTPUT U
