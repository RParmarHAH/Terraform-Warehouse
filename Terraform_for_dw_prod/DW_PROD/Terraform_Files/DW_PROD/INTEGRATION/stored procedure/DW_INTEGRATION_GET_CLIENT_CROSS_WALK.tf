resource "snowflake_procedure" "DW_INTEGRATION_GET_CLIENT_CROSS_WALK" {
	name ="GET_CLIENT_CROSS_WALK"
...
***********************************************************************************************************/
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.integration.client_cross_walk
...
(SELECT CLIENT_KEY
	...
	(
		...
		WHERE FV.CONFIRMED_FLAG =''YES'' 
				...
				AND FV.BRANCH_KEY IN (SELECT BRANCH_KEY FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_BRANCH 
...
CENTENE_CLIENTS AS (
	...
	JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_CENTENE_RL_OUTPUT CENTENE ON AMS.CLUSTER_ID=CENTENE.CLUSTER_ID 
...
OSH_CLIENTS AS(
	...
	JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_MERGED AS OSH 
...
MOLINA_CLIENTS AS
	...
	JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_MOLINA_RL_OUTPUT MOLINA ON AMS.CLUSTER_ID=MOLINA.CLUSTER_ID 
...
(
	...
	INNER JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_AMERIHEALTH_RL_OUTPUT AMERIHEALTH 
