resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_EMPLOYEE_AL_MERGED_DEDUPE" {
	name ="GET_MASTER_DIM_EMPLOYEE_AL_MERGED_DEDUPE"
...
,CLUSTER_ID FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_AL_RL_OUTPUT WHERE CLUSTER_ID IS NOT NULL
)
...
ID,SOURCE_SYSTEM_ID,SYSTEM_CODE,EMPLOYEE_NUMBER,EMPLOYEE_PID,CLUSTER_ID FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_AL_RL_OUTPUT WHERE CLUSTER_ID IS NOT NULL
) --WHERE min_record_id = 1
) --SELECT * FROM OLD;
...
FROM  DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_AL_RL_OUTPUT R
...
FROM  DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_AL_RL_OUTPUT U
