resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_EMPLOYEE_IN_MERGED_DEDUPE" {
	name ="GET_MASTER_DIM_EMPLOYEE_IN_MERGED_DEDUPE"
...
(
    ...
    CLUSTER_ID
    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_IN_RL_OUTPUT
...
,NEW AS (
    SELECT * FROM (
        SELECT
            ...
            ,NYO.CLUSTER_ID 
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_IN_RL_OUTPUT NYO
        INNER JOIN LWD_NEW 
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_IN_RL_OUTPUT R
LEFT JOIN OLD O 
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_IN_RL_OUTPUT U
