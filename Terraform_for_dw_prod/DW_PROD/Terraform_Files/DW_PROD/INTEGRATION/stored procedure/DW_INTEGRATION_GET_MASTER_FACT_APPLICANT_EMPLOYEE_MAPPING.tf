resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_FACT_APPLICANT_EMPLOYEE_MAPPING" {
	name ="GET_MASTER_FACT_APPLICANT_EMPLOYEE_MAPPING"
...
var sql = `
  ...
  WITH MAPPING_DATA AS (
      SELECT * FROM (
        ...
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.APPLICANT_EMPLOYEE_DEDUPE_OUTPUT WHERE CLUSTER_ID IS NOT NULL)
