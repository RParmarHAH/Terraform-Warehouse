resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_CLIENT_IN_MERGED_DEDUPE" {
	name ="GET_MASTER_DIM_CLIENT_IN_MERGED_DEDUPE"
...
WITH LWD_NEW AS (
    ...
    CLUSTER_ID
    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_IN_RL_OUTPUT
)
, NEW AS (
    SELECT * FROM (
        SELECT 
            --CASE WHEN SYSTEM_CODE=''MATRIXCARE'' THEN 1 ELSE 2 END as SRC_PRI,
            row_number() over(partition by INCLI.CLUSTER_ID order by SRC_PRI) min_record_id,
            INCLI.ID,
            ...
            MIN(REFERRAL_DATE::date) OVER(PARTITION BY INCLI.CLUSTER_ID) AS DERIVED_REFERRAL_DATE,
            FIRST_SERVICE_DATE::DATE - MIN(REFERRAL_DATE::date) OVER(PARTITION BY INCLI.CLUSTER_ID) AS DERIVED_DAYS_TO_SERVICE,
            CONTRACT_BEGIN_DATE,
            CONTRACT_END_DATE,
            MIN(FIRST_SERVICE_DATE::date) OVER(PARTITION BY INCLI.CLUSTER_ID) AS DERIVED_FIRST_SERVICE_DATE,
            MAX(LAST_SERVICE_DATE::date) OVER(PARTITION BY INCLI.CLUSTER_ID) AS DERIVED_LAST_SERVICE_DATE,
            MIN(BEGIN_DATE::date) OVER(PARTITION BY INCLI.CLUSTER_ID) AS DERIVED_BEGIN_DATE,DAYS_TO_SERVICE,
            ...
            ETL_DELETED_FLAG,ETL_INFERRED_MEMBER_FLAG, INCLI.CLUSTER_ID
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_IN_RL_OUTPUT INCLI
        INNER JOIN LWD_NEW ON LWD_NEW.ID = INCLI.ID
        WHERE INCLI.CLUSTER_ID IS NOT NULL
    )
...
, OLD AS (
    SELECT * FROM (
        SELECT 
            row_number() over(partition by INCLI.CLUSTER_ID order by SRC_PRI) min_record_id,
            INCLI.ID,SYSTEM_CODE,SOURCE_SYSTEM_ID,CLIENT_NUMBER,CLIENT_PID,INCLI.CLUSTER_ID,AGENCY
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_IN_RL_OUTPUT INCLI
        INNER JOIN LWD_OLD ON LWD_OLD.ID = INCLI.ID
        WHERE INCLI.CLUSTER_ID IS NOT NULL
        )-- WHERE min_record_id = 1
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_IN_RL_OUTPUT R
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_CLIENT_IN_RL_OUTPUT U
