resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_EMPLOYEE_PA_MERGED_DEDUPE" {
	name ="GET_MASTER_DIM_EMPLOYEE_PA_MERGED_DEDUPE"
...
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
-- 01/24/23    Mirisha Jarecha        Initial Development
--
...
(
	...
	CLUSTER_ID
	FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_PA_RL_OUTPUT
...
,NEW AS (
    SELECT * FROM (
        SELECT
            ...
            ,PA.CLUSTER_ID 
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_PA_RL_OUTPUT PA
        INNER JOIN LWD_NEW 
            ON LWD_NEW.ID = PA.ID
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_PA_RL_OUTPUT R
LEFT JOIN OLD O 
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_PA_RL_OUTPUT U
