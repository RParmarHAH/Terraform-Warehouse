resource "snowflake_procedure" "DW_INTEGRATION_GET_MASTER_DIM_EMPLOYEE_PAYROLL_MERGE_DEDUPE" {
	name ="GET_MASTER_DIM_EMPLOYEE_PAYROLL_MERGE_DEDUPE"
...
BEGIN
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_PAYROLL_MERGE_DEDUPE
SELECT 
*
from(
  WITH LWD_NEW AS
(
SELECT ID,DERIVED_PAYROLL_FIRST_CHECK_DATE,DERIVED_PAYROLL_LAST_CHECK_DATE,
    MAX (R.EMPLOYEE_LAST_CHECK_DATE) OVER (PARTITION BY CLUSTER_ID) AS DERIVED_EMPLOYEE_LAST_CHECK_DATE,
    MAX (R.EMPLOYEE_LAST_WORKED_DATE)OVER (PARTITION BY CLUSTER_ID) AS DERIVED_EMPLOYEE_LAST_WORKED_DATE,
    MAX (R.EMPLOYEE_TERMINATE_DATE)OVER (PARTITION BY CLUSTER_ID) AS DERIVED_EMPLOYEE_TERMINATE_DATE,
    MAX (R.EMPLOYEE_FIRST_CHECK_DATE)OVER (PARTITION BY CLUSTER_ID) AS DERIVED_EMPLOYEE_FIRST_CHECK_DATE,
    MAX (R.EMPLOYEE_HIRE_DATE)OVER (PARTITION BY CLUSTER_ID) AS DERIVED_EMPLOYEE_HIRE_DATE
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_PAYROLL_RL_OUTPUT R
left join DW_${var.SF_ENVIRONMENT}.HAH.DIM_EMPLOYEE E ON R.ID = E.EMPLOYEE_KEY
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_PAYROLL_RL_OUTPUT
WHERE CLUSTER_ID IS NOT NULL
),
NEW AS (
SELECT *
FROM(
    SELECT 
    GREATEST( COALESCE(DERIVED_PAYROLL_FIRST_CHECK_DATE,''1999-01-01''),
                     COALESCE(DERIVED_PAYROLL_LAST_CHECK_DATE,''1999-01-01''),
    ...
    row_number() over(partition by PA.CLUSTER_ID order by 
     case when  SOURCE_SYSTEM_ID in (6,10,22,34,36) then CURRENT_DATE ELSE ''1900-01-01''::DATE END  DESC ,               
         CASE WHEN SOURCE_SYSTEM_ID in  (36) THEN  ''1900-01-01''::DATE ELSE  LATEST_DATE END DESC,  
EMPLOYEE_HIRE_DATE desc,
    EMPLOYEE_ID desc
    ) min_record_id,
        PA.ID,        EMPLOYEE_ID,        SYSTEM_CODE,        SOURCE_SYSTEM_ID,        EMPLOYEE_PID,        EMPLOYEE_NUMBER,        EMPLOYEE_DOB,
        EMPLOYEE_DATE_OF_DEATH,
        MIN(EMPLOYEE_HIRE_DATE::date) OVER(PARTITION BY PA.CLUSTER_ID) AS DERIVED_EMPLOYEE_HIRE_DATE,
        MAX(EMPLOYEE_REHIRE_DATE::date) OVER(PARTITION BY PA.CLUSTER_ID) AS DERIVED_EMPLOYEE_REHIRE_DATE,
        MIN(EMPLOYEE_BENEFIT_START_DATE::date) OVER(PARTITION BY PA.CLUSTER_ID) AS DERIVED_EMPLOYEE_BENEFIT_START_DATE,
        MIN(EMPLOYEE_FIRST_CHECK_DATE::date) OVER(PARTITION BY PA.CLUSTER_ID) AS DERIVED_EMPLOYEE_FIRST_CHECK_DATE,
        MAX(EMPLOYEE_LAST_CHECK_DATE::date) OVER(PARTITION BY PA.CLUSTER_ID) AS DERIVED_EMPLOYEE_LAST_CHECK_DATE,
        MAX(EMPLOYEE_LAST_WORKED_DATE::date) OVER(PARTITION BY PA.CLUSTER_ID) AS DERIVED_EMPLOYEE_LAST_WORKED_DATE,
        EMPLOYEE_TERMINATE_DATE,
        EMPLOYEE_HIRE_DATE::date AS EMPLOYEE_HIRE_DATE,
        EMPLOYEE_REHIRE_DATE::date AS EMPLOYEE_REHIRE_DATE,    EMPLOYEE_BENEFIT_START_DATE::date AS EMPLOYEE_BENEFIT_START_DATE,    EMPLOYEE_FIRST_CHECK_DATE::date AS EMPLOYEE_FIRST_CHECK_DATE,    EMPLOYEE_LAST_CHECK_DATE::date AS EMPLOYEE_LAST_CHECK_DATE,    EMPLOYEE_LAST_WORKED_DATE::date AS EMPLOYEE_LAST_WORKED_DATE,    REASON_TO_TERMINATE,    ACTIVE_EMPLOYEE_FLAG,    ABLE_TO_REHIRE_FLAG,    EMPLOYEE_PREFIX,    EMPLOYEE_FIRST_NAME,    EMPLOYEE_MIDDLE_NAME,    EMPLOYEE_LAST_NAME,    EMPLOYEE_SUFFIX,    EMPLOYEE_ADDRESS1,    EMPLOYEE_ADDRESS2,    EMPLOYEE_CITY,    EMPLOYEE_STATE_CODE,    EMPLOYEE_ZIP,    EMPLOYEE_HOME_PHONE,    EMPLOYEE_CELL_PHONE,    EMPLOYEE_WORK_PHONE,    EMPLOYEE_PERSONAL_EMAIL,    EMPLOYEE_WORK_EMAIL,    LINKED_IN_PROFILE_URL,    EMPLOYEE_GENDER,    EMPLOYEE_ETHNICITY,    EMPLOYEE_LANGUAGE,    EMPLOYEE_MARITAL_STATUS,    EMPLOYEE_TYPE,    EMPLOYEE_CATEGORY,    EXEMPT_FLAG,    JOB_TITLE,    JOB_DESCRIPTION,    CLASS_ID,    WORKERS_COMP,    PAYROLL_ID,    PERCENT_401K_DEDUCTION,    AMOUNT_401K_DEDUCTION,    JAZZHR_APPLICANT_ID,    JAZZHR_USER_ID,    JAZZHR_CONTACT_ID,    WORK_STATE,    SUTA_STATE,    EMPLOYEE_DEPARTMENT,    EMPLOYEE_OFFICE_CODE,    PRIMARY_BRANCH_KEY,    PRIMARY_BRANCH_STATE,    PRIMARY_BRANCH_NAME,    PRIMARY_BRANCH_SYSTEM_CODE,    CASE_MANAGER_FLAG,    SUPERVISOR_FLAG,    COORDINATOR_FLAG,    LINKED_ID,    SALARY,    PAY_RATE,    EFFECTIVE_FROM_DATE,    EFFECTIVE_TO_DATE,    MDM_DIM_EMPLOYEE_KEY,    ETL_TASK_KEY,    ETL_INSERTED_TASK_KEY,    ETL_INSERTED_DATE,    ETL_INSERTED_BY,    ETL_LAST_UPDATED_DATE,    ETL_LAST_UPDATED_BY,    ETL_DELETED_FLAG,
        ETL_INFERRED_MEMBER_FLAG,
        PA.CLUSTER_ID
    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_PAYROLL_RL_OUTPUT PA
    LEFT JOIN LWD_NEW 
            ON LWD_NEW.ID = PA.ID
...
SELECT  *
FROM(
    SELECT  ROW_NUMBER() OVER(PARTITION BY PA.CLUSTER_ID
    ORDER BY 
         case when  SOURCE_SYSTEM_ID in (5,6,10,22,34) then CURRENT_DATE ELSE ''1900-01-01''::DATE END ASC,                    
        coalesce(EMPLOYEE_LAST_CHECK_DATE,DERIVED_EMPLOYEE_LAST_CHECK_DATE) asc, 
        coalesce(EMPLOYEE_LAST_WORKED_DATE,DERIVED_EMPLOYEE_LAST_WORKED_DATE) asc, 
        coalesce(EMPLOYEE_TERMINATE_DATE,DERIVED_EMPLOYEE_TERMINATE_DATE) asc, 
        coalesce(EMPLOYEE_FIRST_CHECK_DATE,DERIVED_EMPLOYEE_FIRST_CHECK_DATE) asc,
        coalesce(EMPLOYEE_HIRE_DATE,DERIVED_EMPLOYEE_HIRE_DATE) asc ) min_record_id,
        PA.ID,
        SOURCE_SYSTEM_ID,
        SYSTEM_CODE,
        EMPLOYEE_NUMBER,
...
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_EMPLOYEE e on N.id = e.employee_key
...
FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_PAYROLL_RL_OUTPUT U
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_EMPLOYEE e on U.id = e.employee_key
