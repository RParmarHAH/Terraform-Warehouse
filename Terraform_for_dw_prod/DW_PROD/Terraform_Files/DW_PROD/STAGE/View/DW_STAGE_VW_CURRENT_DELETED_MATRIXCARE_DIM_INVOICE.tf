resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_MATRIXCARE_DIM_INVOICE" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_MATRIXCARE_DIM_INVOICE"
	statement = <<-SQL
...
(
	...
	LEFT JOIN INVOICE_DETAILS AS INVOICE_DETAILS
		...
		AND CAST(INVOICE_HEADER.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.HIST_STVHC_T_INVOICEHEADER)
)
,ALL_DATA AS 
(
