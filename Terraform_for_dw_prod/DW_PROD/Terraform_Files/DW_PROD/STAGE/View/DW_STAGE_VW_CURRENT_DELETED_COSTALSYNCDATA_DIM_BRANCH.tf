resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_COSTALSYNCDATA_DIM_BRANCH" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_COSTALSYNCDATA_DIM_BRANCH"
	statement = <<-SQL
...
--		CAST(NULL AS VARCHAR(10)) AS SECONDARY_BRANCH_MANAGER_DERIVED_LAST_NAME,
	...
	WHERE OFFICES.STATE IN ('GA', 'SC')
		...
		AND CAST(OFFICES.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.HAHUSERS.LOGIN_OFFICE)
),
...
OFFICE_LIST AS (
	...
	FROM DISC_${var.SF_ENVIRONMENT}.CostalSyncData.HIST_CV_Locations 
    WHERE ETL_DELETED_FLAG = TRUE
        AND CAST(ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.CostalSyncData.HIST_CV_Locations)
--	UNION ALL
