resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_SANDATAIMPORT_FACT_INTAKE" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_SANDATAIMPORT_FACT_INTAKE"
	statement = <<-SQL
	 WITH 
CLIENT AS
(
...
AD AS
    ...
    SELECT DISTINCT AGENCYID, ADMISSIONID, PAYORID, PERIODID, SERVICEID, EVENTID, DATEBEGIN
...
FROM (
	...
	FROM DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.HIST_SANDATA_AUTHORIZATIONS A
	...
	AND A.ETL_DELETED_FLAG = TRUE
    AND CAST(A.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.HIST_SANDATA_AUTHORIZATIONS)
) LIST
...
THEN 
  md5(NVL('CC_'||AUTHS.SOURCE_SYSTEM_CODE, '') || '-' || NVL(AUTHS.REPORT_DATE::string,'') || '-' || AUTHS.BRANCH_NAME || '-' || NVL(AUTHS.CLIENT_CODE,-1) || '-' || NVL(AUTHS.CONTRACT_CODE,'') || '-'|| NVL(AUTHS.AUTHID,-1)
