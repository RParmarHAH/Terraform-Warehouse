resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_COSTALSYNCDATA_DIM_CONTRACT" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_COSTALSYNCDATA_DIM_CONTRACT"
	statement = <<-SQL
    ...
    LEFT JOIN DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.CV_DEPARTMENTS SUPERVISORS
    	ON SUPERVISORS.DB = ADMISSIONS.DB AND SUPERVISORS.DEPARTMENT__CODE = ADMISSIONS.DEPARTMENT__CODE
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.CV_LOCATIONS LOCATIONS
...
FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_PLANS AS PLANS
...
WHERE PLANS.ETL_DELETED_FLAG = TRUE
	AND CAST(PLANS.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_PLANS);
SQL
	or_replace = true 
	is_secure = false 
}
