resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_COSTALSYNCDATA_FACT_PAYROLL" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_COSTALSYNCDATA_FACT_PAYROLL"
	statement = <<-SQL
	 WITH ServicePayCode
AS 
(
...
(
	...
	FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_GPALLPAYCHECKSBASE pcb
	...
	WHERE pcb.ETL_DELETED_FLAG = TRUE
        AND CAST(pcb.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_GPALLPAYCHECKSBASE)
)
