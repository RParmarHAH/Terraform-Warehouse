resource "snowflake_view" "DW_STAGE_SANDATAIMPORT_FACT_REVENUE_TEMP_VIEW" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "SANDATAIMPORT_FACT_REVENUE_TEMP_VIEW"
	statement = <<-SQL
	 WITH VISITS AS (
	...
	FROM (
		...
		LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_BRANCH B ON B.BRANCH_KEY = V.BRANCH_KEY
...
--	 OVER (PARTITION BY H.INVOICENUMBER ORDER BY H.INVOICENUMBER) AS NETAMOUNT -- ADDED ON 0401
	...
	LEFT JOIN (SELECT INVOICEDETAILID, SUM(PAYMENTAMOUNT) AS PAYMENTAMOUNT FROM DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_PAYMENTDETAILS GROUP BY INVOICEDETAILID)	PM 
...
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_DATE dte
