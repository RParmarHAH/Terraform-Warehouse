resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_DATAFLEXSYNCDATA_FACT_REVENUE" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_DATAFLEXSYNCDATA_FACT_REVENUE"
	statement = <<-SQL
	 WITH paid_off as (
    SELECT
           inv.DBNAME,
	...
	FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFARINVOICES I
	WHERE I.ETL_LAST_UPDATED_DATE >= '1900-01-01'
		AND I.ETL_DELETED_FLAG = TRUE
		AND CAST(I.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFARINVOICES)
), AR_DIM_INVOICE AS (
	...
	JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFARINVOICES INV
	...
	WHERE INV.ETL_DELETED_FLAG = TRUE
		AND CAST(INV.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFARINVOICES)
), CLIENT_DATA AS
(
	SELECT * FROM
	(
...
FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFINVOICEDETAILS det
LEFT JOIN paid_off po
   ON det.INVOICENO = po.INVOICENO
       and det.DBNAME = po.DBNAME
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFINVOICES inv
   ON det.DBNAME = inv.DBNAME
   AND det.INVOICENO = inv.INVOICENO
   AND inv.ETL_DELETED_FLAG = FALSE
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFCONTRACTS con ON det.DBNAME = con.DBNAME AND det.CONTRACTCODE = con.CONTRACTCODE
   AND con.ETL_DELETED_FLAG = FALSE
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFCLIENTSUPERVISORS spr ON det.DBNAME = spr.DBNAME AND det.CLIENTNUMBER = spr.CLIENTNUMBER
   AND con.PAYORCODE = spr.BUSINESSLINE AND spr.ETL_DELETED_FLAG = FALSE
--LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFCLIENTS cli ON det.DBNAME = cli.DBNAME AND det.CLIENTNUMBER = cli.NUMBER
LEFT JOIN CLIENT_DATA cli ON det.DBNAME = cli.DBNAME AND det.CLIENTNUMBER = cli.Client_number
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFOFFICES oc ON cli.DBNAME = oc.DBNAME AND cli.OFFICE_NUMBER = oc.OFFICENUMBER
...
LEFT JOIN (SELECT DISTINCT year(hr.SERVICEDATE) || RIGHT('0' || month(hr.SERVICEDATE),2) as period, hr.SERVICEDATE::date AS service_date
   ...
   FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.ALL_TIMESHEETSHOURS  hr
   JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.ALL_DFTIMESHEETS  ts ON hr.DBNAME = ts.DBNAME AND hr.REFERENCENO = ts.REFERENCENO
...
WHERE det.ETL_DELETED_FLAG = TRUE and det.DATEFROM <= dte.CALENDAR_DATE AND NVL(det.DATETO, det.DATEFROM) >= dte.CALENDAR_DATE
	...
	AND CAST(det.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFINVOICEDETAILS)
