resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_MATRIXCARE_DIM_CONTRACT" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_MATRIXCARE_DIM_CONTRACT"
	statement = <<-SQL
	...
	FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.HIST_STVHC_T_PAYERS AS PAYERS
	...
	WHERE (PAYERS.ETL_LAST_UPDATED_DATE >= '1900-01-01'
		...
		AND CAST(PAYERS.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.HIST_STVHC_T_PAYERS)
)
