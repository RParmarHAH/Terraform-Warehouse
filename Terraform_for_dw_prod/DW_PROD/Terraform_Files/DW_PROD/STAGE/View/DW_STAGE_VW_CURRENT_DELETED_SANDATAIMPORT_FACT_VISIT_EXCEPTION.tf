resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_SANDATAIMPORT_FACT_VISIT_EXCEPTION" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_SANDATAIMPORT_FACT_VISIT_EXCEPTION"
	statement = <<-SQL
...
(
	...
	INNER JOIN
		DISC_${var.SF_ENVIRONMENT}.BI_REPOSITORY.HIST_SANDATAVISITS SV
...
(
	...
	FROM
		VISITS_WITH_EXCEPTIONS SCH
	INNER JOIN
		DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.HIST_SANDATA_VISITS SV
		ON SV.AGENCYID = SCH.AGENCYID
			AND SV.SCHEDULEID = SCH.SCHEDULEID
	LEFT JOIN 
		DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_EXTERNALREASONCODES ERC 
	...
	WHERE SV.AGENCYID = '8485' --AND SV.ADJUSTEDTIMEOUT IS NOT NULL
		...
		AND SV.ETL_DELETED_FLAG = TRUE
		AND CAST(SV.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.HIST_SANDATA_VISITS)
)
SELECT
