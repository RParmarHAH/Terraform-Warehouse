resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_MATRIXCARE_FACT_INTAKE" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_MATRIXCARE_FACT_INTAKE"
	statement = <<-SQL
...
--    ,MAX(AUTH.AUTH_ENDDATE ) OVER (Partition by AUTH.AUTH_ADMISSIONID,AUTH.AUTH_PAYERID ) AS LATEST_AUTH_END_DATE
    ...
    WHERE (AUTH.ETL_LAST_UPDATED_DATE IN (SELECT MAX(ETL_LAST_UPDATED_DATE) FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.HIST_STVHC_T_AUTHORIZATION) AND AUTH.ETL_DELETED_FLAG=TRUE) OR 
(AUTHDTL.ETL_LAST_UPDATED_DATE IN (SELECT MAX(ETL_LAST_UPDATED_DATE) FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.HIST_STVHC_T_AUTHORIZATIONDETAIL) AND AUTH.ETL_DELETED_FLAG=TRUE) 
),
DATES AS 
(
    SELECT 
