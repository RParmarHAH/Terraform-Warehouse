resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_COSTALSYNCDATA_FACT_REVENUE" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_COSTALSYNCDATA_FACT_REVENUE"
	statement = <<-SQL
...
--		) AS SERVICE_HOURS,
	...
	FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_BILLINGTRANSACTIONS AS TRANSACTIONS
	JOIN DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_CHARGES AS CHARGES
...
--			AND TRIM(UPPER(SCHEDULES.SCHEDULE_STATUS_CODE)) IN ('B', 'BP', 'P') 
	...
	WHERE ((TRANSACTIONS.ETL_DELETED_FLAG = TRUE AND CAST(TRANSACTIONS.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_BILLINGTRANSACTIONS))
            OR (CHARGES.ETL_DELETED_FLAG = TRUE AND CAST(CHARGES.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_CHARGES)))
	...
	FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_BILLINGDATA DATA
	WHERE DATA.SERVICE_DATE >= '2010-01-01'
	AND DATA.ETL_DELETED_FLAG = TRUE
    AND CAST(DATA.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_BILLINGDATA)
