resource "snowflake_view" "DW_STAGE_VW_CURRENT_MATRIXCARE_DIM_CLIENT" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_MATRIXCARE_DIM_CLIENT"
	statement = <<-SQL
...
( 
	...
	(
		...
		ROW_NUMBER() OVER (PARTITION BY B.MASTER_CLIENT_KEY ORDER BY B.CLIENT_NUMBER DESC  ) 
			ELSE 1  END AS RN 
		FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.STVHC_T_CLIENT A
