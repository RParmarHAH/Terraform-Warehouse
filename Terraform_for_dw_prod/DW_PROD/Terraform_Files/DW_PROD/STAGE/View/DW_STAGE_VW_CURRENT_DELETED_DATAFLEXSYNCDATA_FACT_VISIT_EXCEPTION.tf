resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_DATAFLEXSYNCDATA_FACT_VISIT_EXCEPTION" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_DATAFLEXSYNCDATA_FACT_VISIT_EXCEPTION"
	statement = <<-SQL
...
--				AND MERC.CODE = SCH.RESOLUTION_CODE
	...
	LEFT JOIN 
		DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFTIMESHEETS TS
		ON TS.TRACKINGID = ALL_EXC.TRACKINGID 
			AND TS.WEEKENDDATE = ALL_EXC.WEEKENDDATE 
			AND TS.DBNAME = ALL_EXC.STATE
	INNER JOIN
		DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFTIMESHEETHOURS F
