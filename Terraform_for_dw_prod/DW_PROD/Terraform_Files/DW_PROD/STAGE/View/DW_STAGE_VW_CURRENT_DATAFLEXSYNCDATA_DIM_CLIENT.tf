resource "snowflake_view" "DW_STAGE_VW_CURRENT_DATAFLEXSYNCDATA_DIM_CLIENT" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DATAFLEXSYNCDATA_DIM_CLIENT"
	statement = <<-SQL
	 WITH TIMESHEET_0 AS(
SELECT
...
FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFCLIENTS F
