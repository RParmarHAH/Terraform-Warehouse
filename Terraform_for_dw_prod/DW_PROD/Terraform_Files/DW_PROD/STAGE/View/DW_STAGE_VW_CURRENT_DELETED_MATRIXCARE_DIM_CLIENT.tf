resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_MATRIXCARE_DIM_CLIENT" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_MATRIXCARE_DIM_CLIENT"
	statement = <<-SQL
...
( 
	...
	(
		...
		ROW_NUMBER() OVER (PARTITION BY B.MASTER_CLIENT_KEY ORDER BY B.CLIENT_NUMBER DESC  ) 
			ELSE 1  END AS RN 
		FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.HIST_STVHC_T_CLIENT A
		...
		WHERE A.ETL_DELETED_FLAG = TRUE
			AND CAST(A.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.HIST_STVHC_T_CLIENT)
