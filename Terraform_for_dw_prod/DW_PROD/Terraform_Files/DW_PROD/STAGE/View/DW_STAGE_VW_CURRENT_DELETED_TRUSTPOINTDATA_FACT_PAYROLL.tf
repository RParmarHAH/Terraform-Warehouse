resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_TRUSTPOINTDATA_FACT_PAYROLL" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_TRUSTPOINTDATA_FACT_PAYROLL"
	statement = <<-SQL
	 WITH PAYROLL AS (
	...
	FROM DISC_${var.SF_ENVIRONMENT}.TRUSTPOINTDATA.HIST_PAYROLL AS PAYROLL
	...
	WHERE PAYROLL.COMPANY = 'EXCEL'
		...
		AND CAST(PAYROLL.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.TRUSTPOINTDATA.HIST_PAYROLL)
)
