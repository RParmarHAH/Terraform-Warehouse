resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_COSTALSYNCDATA_FACT_CLIENT_SERVICE" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_COSTALSYNCDATA_FACT_CLIENT_SERVICE"
	statement = <<-SQL
	 WITH CHANGED_DATA AS (
SELECT DB, ADMISSION_NUMBER 
FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_ADMISSIONS T
WHERE T.ETL_LAST_UPDATED_DATE >= '1900-01-01'
		AND T.ETL_DELETED_FLAG = TRUE
		AND CAST(T.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_ADMISSIONS)
UNION
	SELECT DB, ADMISSION_NUMBER 
	FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_DISCIPLINES T
	WHERE T.ETL_LAST_UPDATED_DATE >= '1900-01-01'
		AND T.ETL_DELETED_FLAG = TRUE
		AND CAST(T.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_DISCIPLINES)
UNION
	SELECT DB, ADMISSION_NUMBER 
	FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_DISCIPLINEFREQUENCIES T
	WHERE T.ETL_LAST_UPDATED_DATE >= '1900-01-01'
		AND T.ETL_DELETED_FLAG = TRUE
		AND CAST(T.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_DISCIPLINEFREQUENCIES)
	)
,ADMISSIONS
AS 
(
...
FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_ADMISSIONS ADMISSIONS
...
WHERE TRIM(NVL(ADMISSIONS.CURRENT_PLAN_CODE, '')) <> ''
	AND ADMISSIONS.ETL_DELETED_FLAG = TRUE
	AND CAST(ADMISSIONS.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_ADMISSIONS)
) 
, AUTHORIZATION_BY_DAY AS (
	...
	FROM (
		...
		JOIN DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_DISCIPLINES DISCIPLINES
		...
		JOIN DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_DISCIPLINEFREQUENCIES FREQUENCIES
		...
		WHERE NULLIF(DISCIPLINES.STATRT_DATE, '1900-01-01') IS NOT NULL AND NULLIF(DISCIPLINES.END__DATE, '1900-01-01') IS NOT NULL
			...
			AND (
				(DISCIPLINES.ETL_DELETED_FLAG = TRUE AND CAST(DISCIPLINES.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_DISCIPLINES))
				OR (FREQUENCIES.ETL_DELETED_FLAG = TRUE AND CAST(FREQUENCIES.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.HIST_CV_DISCIPLINEFREQUENCIES)))
	) DATA
) 
