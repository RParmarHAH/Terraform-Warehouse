resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_MATRIXCARE_FACT_VISIT_EXCEPTION" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_MATRIXCARE_FACT_VISIT_EXCEPTION"
	statement = <<-SQL
	 WITH CALLS AS (
	SELECT 
		DISTINCT 
...
INNER JOIN -- TO ONLY GET exceptions OF the visits that ARE IN FACT_VISIT AND EXCLUDE ORPHANS
	DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.HIST_STVHC_T_SCHEDULES VISITS
	ON CALLS.TCL_SCHEDULEID = VISITS.SCH_ID
WHERE 
	VISITS.ETL_DELETED_FLAG = TRUE AND VISITS.ETL_LAST_UPDATED_DATE::DATE IN (SELECT MAX(ETL_LAST_UPDATED_DATE)::DATE FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.HIST_STVHC_T_SCHEDULES )
)
SELECT
