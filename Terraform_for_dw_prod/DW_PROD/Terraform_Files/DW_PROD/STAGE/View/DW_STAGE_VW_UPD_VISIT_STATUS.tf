resource "snowflake_view" "DW_STAGE_VW_UPD_VISIT_STATUS" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_UPD_VISIT_STATUS"
	statement = <<-SQL
	 SELECT
VISIT_KEY,
SOURCE_SYSTEM_ID,
...
FROM 
DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.CV_Schedules WHERE Schedule_Date >= current_date-365 )
GROUP BY 1,2,3,4
...
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_VISIT_STATUS_MAPPING STATUS_MAPPING ON 
...
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_VISIT_STATUS_MAPPING SM 
...
FROM 
DISC_${var.SF_ENVIRONMENT}.BI_REPOSITORY.SANDATAVISITS SV WHERE agencyId ='8485' AND SCHEDULEDATE >= current_date-365
UNION ALL 
SELECT DISTINCT CASE WHEN SV.SERVICEID IN ('CARECO','VBPCG')	
	...
	4 AS SOURCE_SYSTEM_ID 
FROM 
DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_VISITS SV WHERE  agencyID ='8485' AND  SV.DATE >= current_date-365 )
