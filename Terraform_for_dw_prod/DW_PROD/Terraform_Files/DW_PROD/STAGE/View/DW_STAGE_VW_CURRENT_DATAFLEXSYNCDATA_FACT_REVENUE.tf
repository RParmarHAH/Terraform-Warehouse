resource "snowflake_view" "DW_STAGE_VW_CURRENT_DATAFLEXSYNCDATA_FACT_REVENUE" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DATAFLEXSYNCDATA_FACT_REVENUE"
	statement = <<-SQL
	 WITH paid_off as (
    SELECT
           inv.DBNAME,
...
WHERE det.ETL_DELETED_FLAG = FALSE and det.DATEFROM <= dte.CALENDAR_DATE AND NVL(det.DATETO, det.DATEFROM) >= dte.CALENDAR_DATE
