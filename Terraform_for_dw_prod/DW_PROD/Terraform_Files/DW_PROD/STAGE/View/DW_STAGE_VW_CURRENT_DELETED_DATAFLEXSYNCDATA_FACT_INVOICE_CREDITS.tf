resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_DATAFLEXSYNCDATA_FACT_INVOICE_CREDITS" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_DATAFLEXSYNCDATA_FACT_INVOICE_CREDITS"
	statement = <<-SQL
...
--		INVOICE_PAYMENTS.SUPPLEMENTNO AS PAYMENT_NUMBER,
	...
	FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFINVOICEPAYMENTS AS INVOICE_PAYMENTS
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFINVOICES AS INVOICES ON INVOICES.DBNAME = INVOICE_PAYMENTS.DBNAME AND INVOICES.INVOICENO = INVOICE_PAYMENTS.INVOICENO 
    WHERE INVOICE_PAYMENTS.ETL_DELETED_FLAG='TRUE'
),
STATE_COPAY_PAYMENTS AS (
	...
	FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFINVOICEPAYMENTS AS INVOICE_PAYMENTS
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFINVOICES AS INVOICES ON INVOICES.DBNAME = INVOICE_PAYMENTS.DBNAME AND INVOICES.INVOICENO = INVOICE_PAYMENTS.INVOICENO 
	WHERE INVOICE_PAYMENTS.COPAY <> 0 AND INVOICE_PAYMENTS.ETL_DELETED_FLAG='TRUE'
),
CLIENT_PAYMENTS AS (
...
--		PAYMENTS.PAYMENTNO AS PAYMENT_NUMBER,
	...
	FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFPAYMENTS AS PAYMENTS
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFPAYMENTDETAILS AS PAYMENT_DETAILS
...
LUMP_SUM_ARPAYMENTS AS (
	...
	FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFARPAYMENTS AS PAYMENTS
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFARINVOICES AS INVOICES
	...
	WHERE TRIM(NVL(PAYMENTS.PERIOD, '')) <> '' AND  PAYMENTS.ETL_DELETED_FLAG='TRUE'
),
...
--		INVOICES.SUPPLEMENTNO AS PAYMENT_NUMBER,
	...
	FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFARINVOICES AS INVOICES
	...
	WHERE PAYMENTS.SYSTEM_CODE IS NULL AND INVOICES.ETL_DELETED_FLAG='TRUE'
),
RESULTS AS (
