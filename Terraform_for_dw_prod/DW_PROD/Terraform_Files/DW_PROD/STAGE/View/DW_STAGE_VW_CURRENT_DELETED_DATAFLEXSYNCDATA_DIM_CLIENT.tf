resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_DATAFLEXSYNCDATA_DIM_CLIENT" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_DATAFLEXSYNCDATA_DIM_CLIENT"
	statement = <<-SQL
	 WITH TIMESHEET_0 AS(
SELECT
...
FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFTIMESHEETHOURS H
JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFTIMESHEETS T ON T.REFERENCENO = H.REFERENCENO AND T.DBNAME = H.DBNAME AND T.WEEKENDDATE = H.WEEKENDDATE
...
FROM (
	...
	FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFCLIENTS AS C
) AS SPV_LIST
...
FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFCLIENTS F
...
OR S2.ETL_LAST_UPDATED_DATE >= '1900-01-01')
	...
	AND CAST(F.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFCLIENTS)
