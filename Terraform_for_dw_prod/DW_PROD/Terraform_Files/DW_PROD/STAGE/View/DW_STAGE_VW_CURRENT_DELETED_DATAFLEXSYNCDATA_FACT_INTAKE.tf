resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_DATAFLEXSYNCDATA_FACT_INTAKE" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_DATAFLEXSYNCDATA_FACT_INTAKE"
	statement = <<-SQL
	 WITH MAXREAUTH AS (
...
,TIMESHEETS AS
 ...
 CHANGED_CLIENTS AS (
 	...
 	FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFCLIENTS C
 	WHERE C.ETL_LAST_UPDATED_DATE >= '1900-01-01'
 	AND C.ETL_DELETED_FLAG = TRUE 
 	AND CAST(C.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFCLIENTS)
  	--UNION 
...
-- 	FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFCLIENTCONTRACTAUTHORIZATIONS A
-- 	WHERE A.ETL_LAST_UPDATED_DATE >= '1900-01-01'
-- 		AND A.ETL_DELETED_FLAG = TRUE
-- 		AND CAST(A.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFCLIENTCONTRACTAUTHORIZATIONS)
-- 	UNION 
 	SELECT A.DBNAME , A.CLIENTNUMBER , A.CONTRACTCODE 
 	FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFCLIENTCONTRACTS A
 	WHERE A.ETL_LAST_UPDATED_DATE >= '1900-01-01'
 	AND A.ETL_DELETED_FLAG = TRUE
 	AND CAST(A.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFCLIENTCONTRACTS)
 ),
  REAUTH AS(
     SELECT CASE
...
--          , MIRA.MIN_REAUTH                                                                             AS MIN_REAUTH
     ...
     FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFCLIENTCONTRACTS F
              LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFCLIENTS C ON C.DBNAME = F.DBNAME AND C.NUMBER = F.CLIENTNUMBER
...
--                                          MIRA.CONTRACTCODE = F.CONTRACTCODE
	...
	WHERE (F.ETL_DELETED_FLAG = TRUE AND CAST(F.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFCLIENTCONTRACTS))
		OR (C.ETL_DELETED_FLAG = TRUE AND CAST(C.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFCLIENTS))
