resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_DATAFLEXSYNCDATA_DIM_BRANCH" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_DATAFLEXSYNCDATA_DIM_BRANCH"
	statement = <<-SQL
	 WITH DEDUPE_EMP as
(
...
from DISC_DEDUPE_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.EMPLOYEE_MATCH_LIST M
) S
...
from DISC_DEDUPE_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.EMPLOYEE_MASTER_LIST D
) L
...
,RISKCONNECT_HIERARCHY AS (
	...
	WHERE TRY_CAST(OFFICES.OFFICENUMBER AS INT) IS NOT NULL
		...
		AND OFFICES.ETL_DELETED_FLAG = TRUE AND CAST(OFFICES.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.HAHUSERS.LOGIN_OFFICE)
	...
	FROM DISC_${var.SF_ENVIRONMENT}.HAH_REPORTING.KEYMETOFFICES AS KEY_MET_OFFICES
	WHERE TRY_CAST(KEY_MET_OFFICES.OFFICENUMBER AS INT) IS NOT NULL
	AND KEY_MET_OFFICES.ETL_DELETED_FLAG = TRUE 
		AND CAST(KEY_MET_OFFICES.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.HAH_REPORTING.KEYMETOFFICES)
), OFFICE_LIST AS (
	...
	FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFOFFICES
	 WHERE ETL_DELETED_FLAG = TRUE
        AND CAST(ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DFOFFICES)
	UNION ALL
	...
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFOFFICES AS DFOFFICES
