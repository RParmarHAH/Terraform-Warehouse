resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_SANDATAIMPORT_DIM_CLIENT" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_SANDATAIMPORT_DIM_CLIENT"
	statement = <<-SQL
	 WITH VISIT AS (
    SELECT
...
AND (C.ETL_LAST_UPDATED_DATE >= '1900-01-01' OR hold_status.ETL_LAST_UPDATED_DATE >= '1900-01-01' OR VISIT.ETL_LAST_UPDATED_DATE >= '1900-01-01')
	...
	AND CAST(C.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.HIST_SANDATA_CLIENTS);
SQL
	or_replace = true 
	is_secure = false 
}
