resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_SANDATAIMPORT_DIM_BRANCH" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_SANDATAIMPORT_DIM_BRANCH"
	statement = <<-SQL
	...
	FROM (
		...
		FROM DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.HIST_SANDATA_LOCATIONS AS SANDATA_LOCATIONS 
		LEFT JOIN (
			SELECT CASE DBNAME 
					WHEN 'IL' THEN '8380' 
					WHEN 'IN' THEN '38004'
		...
		WHERE SANDATA_LOCATIONS.AGENCYID = '8485'
			AND SANDATA_LOCATIONS.ETL_DELETED_FLAG = TRUE
			AND CAST(SANDATA_LOCATIONS.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.HIST_SANDATA_LOCATIONS)
	) AS LIST
