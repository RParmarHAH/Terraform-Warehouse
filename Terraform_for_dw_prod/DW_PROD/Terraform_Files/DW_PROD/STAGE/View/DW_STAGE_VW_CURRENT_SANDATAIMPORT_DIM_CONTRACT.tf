resource "snowflake_view" "DW_STAGE_VW_CURRENT_SANDATAIMPORT_DIM_CONTRACT" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_SANDATAIMPORT_DIM_CONTRACT"
	statement = <<-SQL
	 WITH MAPPING AS (
    SELECT DISTINCT
	...
	WHERE CA.AGENCYID = '8485'
		AND (CA.ETL_LAST_UPDATED_DATE >= '1900-01-01' 
			OR M.ETL_LAST_UPDATED_DATE >= '1900-01-01')
)
SELECT
