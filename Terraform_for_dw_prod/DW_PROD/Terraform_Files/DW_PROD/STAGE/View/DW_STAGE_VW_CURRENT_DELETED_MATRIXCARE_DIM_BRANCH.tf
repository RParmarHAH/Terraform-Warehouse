resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_MATRIXCARE_DIM_BRANCH" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_MATRIXCARE_DIM_BRANCH"
	statement = <<-SQL
	...
	FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.HIST_STVHC_T_BRANCHES AS BRANCHES
	...
	WHERE BRANCHES.ETL_DELETED_FLAG = TRUE
		AND CAST(BRANCHES.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.HIST_STVHC_T_BRANCHES)
), OFFICES AS (
	SELECT *
	FROM (
