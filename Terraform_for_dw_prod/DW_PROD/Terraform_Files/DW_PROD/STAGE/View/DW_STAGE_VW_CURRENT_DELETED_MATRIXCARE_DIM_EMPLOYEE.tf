resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_MATRIXCARE_DIM_EMPLOYEE" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_MATRIXCARE_DIM_EMPLOYEE"
	statement = <<-SQL
	 WITH CAREGIVER AS 
(
...
FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.HIST_STVHC_T_CAREGIVER CG 
...
AND CAST(cg.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.HIST_STVHC_T_CAREGIVER)
) WHERE RN=1
),
SUPERVISOR_FLAG AS (
