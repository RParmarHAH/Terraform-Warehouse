resource "snowflake_view" "DW_STAGE_VW_CURRENT_DELETED_DATAFLEXSYNCDATA_DIM_EMPLOYEE" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "STAGE"
	name = "VW_CURRENT_DELETED_DATAFLEXSYNCDATA_DIM_EMPLOYEE"
	statement = <<-SQL
...
INNER JOIN DISC_${var.SF_ENVIRONMENT}.DataFlexSyncData.DFTimeSheetHours AS tsh ON ts.DBName = tsh.DBName AND ts.ReferenceNo = tsh.ReferenceNo
...
FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DfEmployees e
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.GPSyncData.GPAllEmployeesBase AS gp ON TRIM( e.PayrollID) = TRIM( gp.Employee_ID) AND TRIM( e.DBName) = TRIM( gp.Office_State) AND TRIM( gp.DB) <> 'HAHIL'
...
--AND e.ETL_LAST_UPDATED_DATE >= ( SELECT Date_to_Start_Processing FROM Find_Latest_Changes)
    ...
    AND CAST(e.ETL_LAST_UPDATED_DATE AS DATE) IN (SELECT CAST(MAX(ETL_LAST_UPDATED_DATE) AS DATE) FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.HIST_DfEmployees)
