resource "snowflake_procedure" "DW_PUBLIC_GET_MASTER_FACT_EMPLOYEE_CROSS_WALK_TEST" {
...
UPDATE DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST CW
...
CREATE OR REPLACE TABLE DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST 
CLONE DW_${var.SF_ENVIRONMENT}.INTEGRATION.EMPLOYEE_CROSS_WALK_TEST;
...
UPDATE DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST CW_D
...
UPDATE DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST CW
SET DUP_FLAG = TRUE ,DUP_FLAG_REASON = ''MAPPING_INTERCHANGED''
FROM (SELECT  AMS_EMPLOYEE_KEY||''-''||PAYROLL_EMPLOYEE_KEY A_P
	FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST D
		WHERE EXISTS  
	...
	FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.FACT_AMS_PAYROLL_MAPPING D
		WHERE EXISTS  
			(SELECT DISTINCT 1 FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST M 
			WHERE M.AMS_EMPLOYEE_KEY = D.AMS_EMPLOYEE_KEY )
		AND EXISTS  
			(SELECT DISTINCT 1 FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST M 
			WHERE M.PAYROLL_EMPLOYEE_KEY = D.PAYROLL_EMPLOYEE_KEY )
...
CREATE OR REPLACE TABLE DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TBL_MASTER_TEST 
AS SELECT * FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.VW_EMPLOYEE_CROSS_WALK;

INSERT INTO DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_CHANGE_LOG_TEST
-----------------------------------
...
WITH KEYS_CHANGED AS 
	...
	FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST CW
	JOIN DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST  D1
...
,AMS_MAPP AS
	...
	FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST CW 
	JOIN DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TEST VW
...
UPDATE DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST TEMP_C
...
FROM
	...
	FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TEST VW
	LEFT JOIN DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_CHANGE_LOG_TEST CL -- JOINING ON CHANGE LOG TABLE TO GET THE LATEST KEYS
		ON COALESCE(CL.LATEST_INTEGRATION_LAYER_KEY,CL.LATEST_DISC_LAYER_KEY,CL."AMS / PAYROLL_KEY") = VW.AMS_EMPLOYEE_KEY
		AND CL.ETL_INSERTED_DATE = (SELECT MAX(ETL_INSERTED_DATE) FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_CHANGE_LOG_TEST)
	) SRC
...
INSERT INTO DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_MERGED_TEST
-- Logic
-------------------------------------------
-- ************* CASE 1 ********************
...
WITH CROSSWALK AS
	...
	FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST
...
FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST CWD
JOIN DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST CWM -- NEED TO JOIN TO GET OLDER KEY 
	ON CWD.EMPLOYEE_ENTERPRISE_ID = CWM.EMPLOYEE_ENTERPRISE_ID 
WHERE CWD.AMS_EMPLOYEE_KEY IN 
	(
	SELECT AMS_EMPLOYEE_KEY
	FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST 
	WHERE INACTIVE <> TRUE AND DUP_FLAG <> TRUE 
	-- FILTERING THOSE KEYS WHICH 
	AND AMS_EMPLOYEE_KEY IN 
		(SELECT COALESCE(LATEST_INTEGRATION_LAYER_KEY,LATEST_DISC_LAYER_KEY)
		FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_CHANGE_LOG_TEST
		WHERE CHANGE_TYPE =''KEY_CHANGE''
		AND KEY_TYPE= ''AMS'' 
		AND ETL_INSERTED_DATE = (SELECT MAX(ETL_INSERTED_DATE) FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_CHANGE_LOG_TEST)
		)
...
,CROSSWALK_NOT_MAPPED_KEYS  AS 
	...
	FROM CROSSWALK CW
	LEFT JOIN DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_CHANGE_LOG_TEST CL -- JOINING ON CHANGE LOG TABLE TO GET THE LATEST KEYS WHICH GOT CHANGED IN PREVIOUS STEP
	...
	FROM CROSSWALK CW
	LEFT JOIN DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_CHANGE_LOG_TEST CL 
...
,RECENTLY_MAPPED AS 
	...
	WHERE (AMS_EMPLOYEE_KEY NOT IN (SELECT AMS_EMPLOYEE_KEY FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
    AND PAYROLL_EMPLOYEE_KEY NOT IN (SELECT PAYROLL_EMPLOYEE_KEY FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)) AND
...
-----------------------------------
SELECT COALESCE((SELECT MAX(ID) FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_MERGED_TEST),0)+ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS ID,
...
UPDATE DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST 
SET INACTIVE = TRUE 
WHERE EMPLOYEE_ENTERPRISE_ID IN 
	(SELECT OLD_ENTERPRISE_ID FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_MERGED_TEST
	WHERE INSERTED_DATE = (SELECT MAX(INSERTED_DATE) FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_MERGED_TEST)
	);
...
AND FM.AMS_EMPLOYEE_KEY NOT IN (SELECT AMS_EMPLOYEE_KEY FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
AND FM.PAYROLL_EMPLOYEE_KEY NOT IN (SELECT PAYROLL_EMPLOYEE_KEY FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
AND (CW_D.INACTIVE <> TRUE AND CW_D.DUP_FLAG <> TRUE)
...
UPDATE DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST M
SET	 M.DUP_FLAG = T.DUP_FLAG,
	M.DUP_FLAG_REASON = T.DUP_FLAG_REASON,
	M.INACTIVE = T.INACTIVE
FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEMP_FOR_DUPS_TEST T 
...
CREATE OR REPLACE TABLE DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TBL_MASTER_TEST 
AS SELECT * FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TEST;


-- Inserting records which are not present currently in the table
INSERT INTO DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST
(
...
SELECT -- Newly added
    COALESCE((SELECT MAX(EMPLOYEE_ENTERPRISE_ID) FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST WHERE EMPLOYEE_ENTERPRISE_ID < 50000000),10000000)+ROW_NUMBER() OVER(ORDER BY (SELECT 1)), 
    -- Old
--  SRC.EMPLOYEE_ENTERPRISE_ID,
    ...
    SRC.EMPLOYEE_SSN,
    SRC.BROADSPIRE_ID
FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TBL_MASTER_TEST SRC
...
UPDATE DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST  AS cw
SET
...
FROM 
	...
	SELECT * FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_TBL_MASTER_TEST
		WHERE NVL(AMS_EMPLOYEE_KEY,'''') NOT IN (SELECT * FROM DUP_FLAG_TRUE_KEYS)
		 AND NVL(PAYROLL_EMPLOYEE_KEY,'''') NOT IN (SELECT * FROM DUP_FLAG_TRUE_KEYS) 
	 ) 	 
...
FROM 
	...
	SELECT * FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.VW_EMPLOYEE_CROSS_WALK_TBL_MASTER_TEST
		WHERE NVL(AMS_EMPLOYEE_KEY,'''') NOT IN (SELECT * FROM DUP_FLAG_TRUE_KEYS)
		 AND NVL(PAYROLL_EMPLOYEE_KEY,'''') NOT IN (SELECT * FROM DUP_FLAG_TRUE_KEYS) 
	 ) 	 
--	DW_${var.SF_ENVIRONMENT}.INTEGRATION.VW_EMPLOYEE_CROSS_WALK
...
----UPDATE ALAYACRE 
UPDATE DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST T1
...
FROM (
	...
	FROM
	DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST CW
	JOIN 
...
--UPDATE DATAFLEX
UPDATE DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST SRC
...
UPDATE DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST TGT
...
INSERT INTO DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_HISTORICAL_TEST
...
FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST AT (OFFSET => -60*60*24)
EXCEPT 
SELECT * EXCLUDE (ETL_INSERTED_DATE, ETL_TASK_KEY, ETL_INSERTED_TASK_KEY, ETL_INSERTED_BY, ETL_LAST_UPDATED_DATE, ETL_LAST_UPDATED_BY, ETL_DELETED_FLAG, AMS_PUBLIC_ID, AMS_PAYROLL_ID, EMPLOYEE_SSN),
		CURRENT_TIMESTAMP, CURRENT_USER, AMS_PUBLIC_ID , AMS_PAYROLL_ID , EMPLOYEE_SSN
FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.EMPLOYEE_CROSS_WALK_TEST;
