resource "snowflake_view" "DW_PUBLIC_VW_EXECUTIVE_OPERATIONS_EXCELLENCE_UTILIZATION_TEST_DG" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "PUBLIC"
	name = "VW_EXECUTIVE_OPERATIONS_EXCELLENCE_UTILIZATION_TEST_DG"
	statement = <<-SQL
	 (WITH DATES AS 
(
...
--										   WHEN PCS.SOURCE_SYSTEM_ID =4 AND PCS.SYSTEM_CODE ='8485' THEN 'HC' END)
			...
			DATA.etl_updated_date AS ETL_LAST_UPDATED_DATE ,
			DATA.VISITS_ALL,
			DATA.VISITS_CLEAN_SHIFTS,
			DATA.VISITS_NEED_MAINTENANCE,
       ...
       CASE WHEN COALESCE(CCSI_BRANCH.CLIENT_KEY,CCSI_ACQUIRED_CLIENT.CLIENT_KEY) IS NOT NULL THEN 'CCSI' 
			...
			DATA.FUTURE_HOLD_VISITS,
			DATA.CLIENT_SERVED_FLAG
		FROM public.UTI_UPDATED AS DATA
