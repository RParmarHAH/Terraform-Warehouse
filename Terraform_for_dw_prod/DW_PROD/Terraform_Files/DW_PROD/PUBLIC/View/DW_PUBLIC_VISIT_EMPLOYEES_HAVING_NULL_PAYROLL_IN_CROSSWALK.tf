resource "snowflake_view" "DW_PUBLIC_VISIT_EMPLOYEES_HAVING_NULL_PAYROLL_IN_CROSSWALK" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "PUBLIC"
	name = "VISIT_EMPLOYEES_HAVING_NULL_PAYROLL_IN_CROSSWALK"
	statement = <<-SQL
	 (
WITH  EMPLOYEES AS (
	 WITH FIRST_SERVICE_DATE AS (
		...
		FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.FACT_VISIT_MERGED AS VIS
		INNER JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_AMS_MERGED  AS EMP ON EMP.ORIGINAL_EMPLOYEE_KEY = VIS.ORIGINAL_EMPLOYEE_KEY AND IFNULL( EMP.EMPLOYEE_CATEGORY, 'Caregiver') <> 'ADMIN'
		LEFT OUTER JOIN DW_${var.SF_ENVIRONMENT}.HAH.VW_DIM_SOURCE_SYSTEM_TO_MARKET AS STM ON VIS.SOURCE_SYSTEM_ID = STM.SOURCE_SYSTEM_ID AND VIS.SYSTEM_CODE = STM.SYSTEM_CODE
		LEFT OUTER JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_BRANCH_MERGED AS BR ON BR.ORIGINAL_BRANCH_KEY = VIS.ORIGINAL_BRANCH_KEY
		...
		LEFT OUTER JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.EMPLOYEE_CROSS_WALK AS ECW 
...
, FINAL_EMP AS (
	...
	FROM EMPLOYEES EMP
	LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_EMPLOYEE AMS_EMP
	...
	LEFT JOIN (
		    ...
		    FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_EMPLOYEE EMP
			JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_SOURCE_SYSTEM SS 
		...
		ON (UPPER(AMS_EMP.EMPLOYEE_FIRST_NAME) = UPPER(PAY_EMP.EMPLOYEE_FIRST_NAME)
			...
			OR  (UPPER(AMS_EMP.EMPLOYEE_LAST_NAME) = UPPER(PAY_EMP.EMPLOYEE_LAST_NAME)
				AND UPPER(AMS_EMP.EMPLOYEE_PID) = UPPER(PAY_EMP.EMPLOYEE_PID))
	LEFT JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_PAYROLL_MERGE_DEDUPE P 
		ON P.ORIGINAL_EMPLOYEE_KEY = PAY_EMP.EMPLOYEE_KEY
	LEFT JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_AMS_MERGED AMS
		ON AMS.ORIGINAL_EMPLOYEE_KEY = EMP.AMS_MERGED_EMPLOYEE_KEY
	LEFT JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.AMS_PAYROLL_MAPPING_OUTPUT AMS_AP
		ON EMP.AMS_MERGED_EMPLOYEE_KEY = AMS_AP.ID
	LEFT JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.AMS_PAYROLL_MAPPING_OUTPUT PAYROLL_AP
		ON P.EMPLOYEE_KEY = PAYROLL_AP.ID
	LEFT JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.FACT_AMS_PAYROLL_MAPPING APM 
