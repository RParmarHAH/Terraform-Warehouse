resource "snowflake_view" "DW_PUBLIC_VW_EMPLOYEE_CROSS_WALK_TEST" {
	database = "DW_${var.SF_ENVIRONMENT}"
	schema = "PUBLIC"
	name = "VW_EMPLOYEE_CROSS_WALK_TEST"
	statement = <<-SQL
...
--          COALESCE(BR_PAYROLL.Office_State_Code,EMP.PRIMARY_BRANCH_STATE ,BR_AMS.Office_State_Code) IN ('IN','PA','IL','MS','MO','AL','MI','GA','OH','DE','KY','TN','FL','SC')
          AND TRIM(COALESCE(BR_PAYROLL.Office_State_Code,EMP.WORK_STATE, EMP.PRIMARY_BRANCH_STATE ,BR_AMS.Office_State_Code, '')) NOT IN ('NY', 'DC','FL') AND M.AMS_EMPLOYEE_KEY NOT IN (SELECT AMS_EMPLOYEE_KEY FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING) AND M.PAYROLL_EMPLOYEE_KEY NOT IN (SELECT PAYROLL_EMPLOYEE_KEY FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
     ---------------
     UNION
     ---------------
        SELECT DISTINCT 
...
--         -- Added on 22th nov to exclude across dups which were not identified at integration layer but falls in same cluster
            ...
            AND EMP.EMPLOYEE_KEY NOT IN (SELECT PAYROLL_EMPLOYEE_KEY FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
	-------------------
    UNION
    -------------------
        SELECT DISTINCT 
...
--        WHERE BR.Office_State_Code IN ('IN','PA','IL','MS','MO','AL','MI','GA','OH','DE','KY','TN','FL','SC') 
	...
	-- Added on 22th nov to exclude across dups which were not identified at integration layer but falls in same cluster
        ...
        AND EM.EMPLOYEE_KEY NOT IN (SELECT AMS_EMPLOYEE_KEY FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
        AND APP.APPLICANT_KEY NOT IN (SELECT APPLICANT_EMPLOYEE_KEY FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
    UNION
        SELECT DISTINCT 
        ...
        WHERE
            ...
            AND STATUS.APPLICANT_KEY NOT IN (SELECT APPLICANT_EMPLOYEE_KEY FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING)
            UNION
SELECT DISTINCT 
        	EMP.EMPLOYEE_FIRST_NAME FIRST_NAME, 
            EMP.EMPLOYEE_LAST_NAME LAST_NAME, 
...
--                    AND NULL_BR.EMPLOYEE_KEY IS NULL  --to make exception flag true for null branch_state employees
            ...
            EM.PAYROLL_ID AS AMS_PAYROLL_ID,
            COALESCE(EMP.EMPLOYEE_PID, EM.EMPLOYEE_PID) AS EMPLOYEE_SSN
        FROM DW_${var.SF_ENVIRONMENT}.PUBLIC.VW_EMPLOYEE_CROSS_WALK_AMS_PAYROLL_APPLICANT_MAPPING VECAM
        LEFT JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_AMS_MERGED EM
        ...
        LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_APPLICANT APPLICANT 
            ON APPLICANT.APPLICANT_KEY = VECAM.APPLICANT_EMPLOYEE_KEY
        LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.FACT_APPLICANT_STATUS STATUS 
            ON STATUS.APPLICANT_KEY = APPLICANT.APPLICANT_KEY
        LEFT OUTER JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_Branch_Merged AS BR_AMS
            ON COALESCE(EM.PRIMARY_BRANCH_KEY,FP.BRANCH_KEY) = BR_AMS.ORIGINAL_Branch_Key 
        LEFT OUTER JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_Branch_Merged AS BR_PAYROLL
