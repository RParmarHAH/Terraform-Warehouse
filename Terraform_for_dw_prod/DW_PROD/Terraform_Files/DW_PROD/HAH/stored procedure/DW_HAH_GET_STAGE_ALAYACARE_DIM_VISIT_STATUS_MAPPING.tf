resource "snowflake_procedure" "DW_HAH_GET_STAGE_ALAYACARE_DIM_VISIT_STATUS_MAPPING" {
	name ="GET_STAGE_ALAYACARE_DIM_VISIT_STATUS_MAPPING"
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.WORK_SESSION_TIER_4 WORK_SESSION ON WORK_SESSION.VISIT_ID = VISITS.VISIT_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.OFFER_RESPONSE OFFER ON OFFER.OFFER_GUID_TO = VISITS.GUID)
