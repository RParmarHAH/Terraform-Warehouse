resource "snowflake_procedure" "DW_HAH_FACT_EMPLOYEE_ACQUIRED_POSTPROCESSING" {
	name ="FACT_EMPLOYEE_ACQUIRED_POSTPROCESSING"
...
INSERT OVERWRITE INTO HAH.FACT_EMPLOYEE_ACQUIRED
...
preferred_employee AS (
	...
	FROM
		(
			...
			FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.EMPLOYEE_MASTER_LIST a
			JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.AIDECUSTOMFIELDS B ON A.CAREGIVERID = B.AIDEID
		)
	UNION
	SELECT MASTER_ID,CAREGIVERID,MD5(''EDISON'' || ''-'' || MASTER_ID  || ''-'' || ''EDISON'') AS EMPLOYEE_KEY,
	CASE WHEN Acquisition = ''Recco'' THEN UPPER(Acquisition) END AS ACQ_NAME
	FROM
		(
			...
			FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.EMPLOYEE_MATCH_LIST A
			JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.AIDECUSTOMFIELDS B ON A.CAREGIVERID = B.AIDEID
...
WHERE EMPLOYEE_key in (
  				select employee_key from hah.dim_employee where payroll_id in
 				(select EMPLOYEEID from DISC_${var.SF_ENVIRONMENT}.EMPLOYEESETUP.ES_ENTRIESDATA where referral = ''Acquisition - Longevity''))
  				and SOURCE_SYSTEM_ID = 3
),
...
FROM DISC_${var.SF_ENVIRONMENT}.SandataImport.Sandata_Staffs  
	...
	WITH DATA as(
		...
		JOIN
			(	SELECT CA.ID || Ca.AGENCY_ID AS EMPLOYEE_NUMBER , PPT.PROFILETAG_ID 
				FROM DISC_${var.SF_ENVIRONMENT}.CLEARCARE.CAREGIVER_CAREGIVER  CA
				LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.PROFILE_PROFILE PP ON CA.ID=PP.CAREGIVER_ID 
				LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.PROFILE_PROFILE_TAGS PPT ON PPT.PROFILE_ID =PP.ID
...
FROM
	(
		...
		FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.EMPLOYEE_MASTER_LIST a
        JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.AIDECUSTOMFIELDS B ON A.CAREGIVERID = B.AIDEID
...
FROM
	(
		...
		FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.EMPLOYEE_MATCH_LIST A
		JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.AIDECUSTOMFIELDS B ON A.CAREGIVERID = B.AIDEID
...
FROM  DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.STVHC_T_CAREGIVERVALUES  a 
INNER JOIN  DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.STVHC_T_CAREGIVERFIELDS b ON a.CARV_FIELDID  = b.CARF_ID 
LEFT JOIN   DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.STVHC_T_CAREGIVERFIELDLISTITEMS c ON a.CARV_VALUE= c.CARDD_VALUE::STRING AND a.CARV_FIELDID =c.CARDD_FIELDID  
...
FROM dat A 
JOIN DISC_DEDUPE_${var.SF_ENVIRONMENT}.MATRIXCARE.EMPLOYEE_MASTER_LIST CL ON A.CARV_RECORDID = CL.CAR_ID
WHERE A.ACQ_NAME in (''Alliance'',''MYCARE'')
UNION 
SELECT DISTINCT  CARV_RECORDID, ACQ_NAME, TRY_TO_DATE(ACQ_DATE,''YYYY.MM.DD'') AS ACQ_DATE
,MASTER_ID,MD5(''MATRIXCARE'' || ''-'' || CL.MASTER_ID::STRING || ''-'' || ''MATRIXCARE'') AS EMPLOYEE_KEY
FROM dat A 
JOIN DISC_DEDUPE_${var.SF_ENVIRONMENT}.MATRIXCARE.EMPLOYEE_MATCH_LIST CL ON A.CARV_RECORDID = CL.CAR_ID
WHERE A.ACQ_NAME in (''Alliance'',''MYCARE'')
AND CL.CAR_ID NOT IN (SELECT L.CAR_ID FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.MATRIXCARE.EMPLOYEE_MASTER_LIST L)
),
...
SELECT DISTINCT 
	MD5(EMPLOYEE_MERGED.EMPLOYEE_KEY  || VISIT.CONTRACT_KEY || BRANCH_MERGED.BRANCH_KEY || DATE_TRUNC(''MONTH'', REPORT_DATE)) AS EMPLOYEE_ACQUIRED_KEY,
	EMPLOYEE_MERGED.ORIGINAL_EMPLOYEE_KEY ,
	EMPLOYEE_MERGED.EMPLOYEE_KEY, EMPLOYEE_MERGED.EMPLOYEE_ID, VISIT.SOURCE_SYSTEM_ID, VISIT.SYSTEM_CODE,BRANCH_MERGED.ORIGINAL_BRANCH_KEY  ,BRANCH_MERGED.BRANCH_KEY, CONTRACT_KEY, DATE_TRUNC(''MONTH'', REPORT_DATE) AS SERVICE_MONTH, ''ADAPTIVE'' AS AQUISITION_NAME,
...
SELECT DISTINCT 
	MD5(EMPLOYEE_MERGED.EMPLOYEE_KEY  || VISIT.CONTRACT_KEY || BRANCH_MERGED.BRANCH_KEY || DATE_TRUNC(''MONTH'', REPORT_DATE)) AS EMPLOYEE_ACQUIRED_KEY,
	EMPLOYEE_MERGED.ORIGINAL_EMPLOYEE_KEY ,
	EMPLOYEE_MERGED.EMPLOYEE_KEY, EMPLOYEE_MERGED.EMPLOYEE_ID, VISIT.SOURCE_SYSTEM_ID, VISIT.SYSTEM_CODE, BRANCH_MERGED.ORIGINAL_BRANCH_KEY ,BRANCH_MERGED.BRANCH_KEY, CONTRACT_KEY, DATE_TRUNC(''MONTH'', REPORT_DATE) AS SERVICE_MONTH, ''CCSI'' AS AQUISITION_NAME,
	-1 AS ETL_TASK_KEY, -1 AS ETL_INSERTED_TASK_KEY, /*VISIT.ETL_INSERTED_DATE, VISIT.ETL_INSERTED_BY,*/ CONVERT_TIMEZONE(''UTC'', CURRENT_TIMESTAMP)::TIMESTAMP_NTZ AS ETL_LAST_UPDATED_DATE, CURRENT_USER AS ETL_LAST_UPDATED_BY, FALSE AS ETL_DELETED_FLAG
FROM HAH.FACT_VISIT VISIT
INNER JOIN HAH.DIM_BRANCH BRANCH
	ON VISIT.BRANCH_KEY = BRANCH.BRANCH_KEY
...
SELECT DISTINCT 
	...
	BRANCH_MERGED.ORIGINAL_BRANCH_KEY ,BRANCH_MERGED.BRANCH_KEY, CONTRACT_KEY, DATE_TRUNC(''MONTH'', REPORT_DATE) AS SERVICE_MONTH, 
	A.AQUISITION_NAME,
...
SELECT DISTINCT 
	...
	INNER JOIN DW_${var.SF_ENVIRONMENT}.HAH.FACT_VISIT VISIT ON VISIT.EMPLOYEE_KEY = A.EMPLOYEE_KEY
	...
	INNER JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_MERGED EMPLOYEE_MERGED ON EMPLOYEE_MERGED.ORIGINAL_EMPLOYEE_KEY  = VISIT.EMPLOYEE_KEY
UNION
SELECT DISTINCT 
	MD5(EMPLOYEE_MERGED.EMPLOYEE_KEY  || VISIT.CONTRACT_KEY || BRANCH_MERGED.BRANCH_KEY || DATE_TRUNC(''MONTH'', REPORT_DATE)) AS EMPLOYEE_ACQUIRED_KEY,
	EMPLOYEE_MERGED.ORIGINAL_EMPLOYEE_KEY ,
