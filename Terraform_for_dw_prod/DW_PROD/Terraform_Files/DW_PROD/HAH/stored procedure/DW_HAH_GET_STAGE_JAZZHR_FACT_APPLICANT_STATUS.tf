resource "snowflake_procedure" "DW_HAH_GET_STAGE_JAZZHR_FACT_APPLICANT_STATUS" {
	name ="GET_STAGE_JAZZHR_FACT_APPLICANT_STATUS"
...
*
FROM
DISC_${var.SF_ENVIRONMENT}.JAZZHR.APPLICANTS_DETAILS_ACTIVITIES)
...
,J.JOB_ID AS FS
FROM
DISC_${var.SF_ENVIRONMENT}.JAZZHR.APPLICANTS_DETAILS_JOBS J
...
AND J.JOB_TITLE = A.JOB
AND J.APPLICANT_PROGRESS = A.T
INNER JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_JAZZHR_PARADOX_MAPPING PM
...
(
	SELECT
		...
		MASTER_ID
	FROM
		DISC_DEDUPE_${var.SF_ENVIRONMENT}.JAZZHR.EMPLOYEE_MASTER_LIST
	UNION ALL
		SELECT
		ID,
		MASTER_IDD
	FROM
		DISC_DEDUPE_${var.SF_ENVIRONMENT}.JAZZHR.EMPLOYEE_MATCH_LIST
	WHERE
		ID NOT IN (
					SELECT
						ID
...
A.HIRED_DATE AS HIRED_DATE,
 ...
 ,A.JOB_ID
 FROM
 DISC_${var.SF_ENVIRONMENT}.JAZZHR.HIRES A 
...
FROM DISC_${var.SF_ENVIRONMENT}.JAZZHR.APPLICANTS_DETAILS_JOBS J
EXCEPT
...
FROM
DISC_${var.SF_ENVIRONMENT}.JAZZHR.APPLICANTS_DETAILS_JOBS J
 JOIN DISC_${var.SF_ENVIRONMENT}.JAZZHR.JOBS AJ
ON J.JOB_ID = AJ.ID
--LEFT JOIN DISC_${var.SF_ENVIRONMENT}.JAZZHR.APPLICANT_DETAILS AD
--ON J.APPLICANT_ID = AD.ID
-- JOIN DISC_${var.SF_ENVIRONMENT}.JAZZHR.VW_APPLICANT_DETAILS AD
-- ON J.APPLICANT_ID = AD.ID
...
FROM FD RS
INNER JOIN DISC_${var.SF_ENVIRONMENT}.JAZZHR.APPLICANTS_DETAILS_JOBS J
