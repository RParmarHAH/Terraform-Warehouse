resource "snowflake_procedure" "DW_HAH_GET_STAGE_EDISON_FACT_ADMISSION" {
	name ="GET_STAGE_EDISON_FACT_ADMISSION"
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}."STAGE".EDISON_FACT_ADMISSION
...
REFERRAL AS (
	SELECT * FROM (select ROW_NUMBER() OVER (PARTITION BY UPPER(FIRSTNAME),UPPER(LASTNAME) ORDER BY REF.REFERRALID DESC) AS ROW_NUM,
            REF.*from DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.REFERRAL REF WHERE REFERRALSTATUS != ''Lost'') WHERE  ROW_NUM=1
     ),	
ADMISSIONS_DATE AS(
	SELECT * FROM (SELECT ROW_NUMBER () OVER(PARTITION BY PAT.PATIENTID  ORDER BY PAT.SERVICESTARTDATE  DESC) AS RN,
			* FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.PATIENTS PAT) WHERE RN=1
	),
COORDINATORS AS(
SELECT * FROM (SELECT ROW_NUMBER () OVER(PARTITION BY TC.COORDINATORNAME ORDER BY TC.COORDINATORID  DESC) AS RN,
			* FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.TBLCOORDINATOR_REPL TC) WHERE RN=1
),
VISITS AS (
    SELECT * FROM ( SELECT V.PATIENTID,VI.PRIMARYSERVICECODEID AS SERVICECODEID FROM  DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.VISITS V 
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.VISITINFO_REPL VI ON VI.VISITID = V.VISITID
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.TBLVISITS_REPL TBLV ON TBLV.VISITID = VI.VISITID
	UNION ALL 
	SELECT PA.PATIENTID,SERVICECODEID FROM  DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.PATIENTAUTHORIZATION  PA)
 GROUP BY 1,2
    )
SELECT DISTINCT 
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.SERVICECODES S ON S.SERVICECODEID = V.SERVICECODEID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.VW_EDISON_REVENUE_SEG REV ON REV.SERVICECODEID  = S.SERVICECODEID
...
ON  S.SERVICECODEID  = CR.SERVICECODEID 
	...
	AND CR.FROMDATE :: DATE <= GETDATE()
         LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.PATIENTS PAT ON PAT.PATIENTID = CLIENT.PATIENTID
         LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.PATIENTCONTRACTS_REPL PCR
...
ON IFNULL(PR.CONTRACTID,PR.PAYERID) = S.CONTRACTID  
                   LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.PATIENTCONTRACTCHANGES PCC
    ...
    //REFERRALS
         ...
         LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.REFERRALSOURCE REFS ON REFS.NAME = REF.REFERRALSOURCE
         LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.REFERRALSUBMISSIONS RFS ON RFS.REFERRALID = REF.REFERRALID
         LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.REFERRALPHYSICIANS REFPHY ON REFPHY.REFERRALID = REF.REFERRALID
         LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.REFERRALDIAGNOSIS REFD ON REFD.REFERRALID = REF.REFERRALID
         LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.REFERRAL  R ON PAT.ADMISSIONID = R.ADMISSIONID 
