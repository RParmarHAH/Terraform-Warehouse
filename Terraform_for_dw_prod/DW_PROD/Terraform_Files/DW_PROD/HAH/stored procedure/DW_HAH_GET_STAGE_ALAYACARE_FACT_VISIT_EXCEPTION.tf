resource "snowflake_procedure" "DW_HAH_GET_STAGE_ALAYACARE_FACT_VISIT_EXCEPTION" {
	name ="GET_STAGE_ALAYACARE_FACT_VISIT_EXCEPTION"
...
, EXCEPTIONS AS (
	...
	INNER JOIN -- TO ONLY GET exceptions OF the visits that ARE IN FACT_VISIT AND EXCLUDE ORPHANS
		DISC_${var.SF_ENVIRONMENT}.ALAYACARE.VISIT V
...
INNER JOIN 
	DISC_${var.SF_ENVIRONMENT}.ALAYACARE.CONFIGURATION CONFIG 
