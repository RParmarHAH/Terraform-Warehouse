resource "snowflake_procedure" "DW_HAH_GET_STAGE_PREFERRED_DIM_EMPLOYEE" {
	name ="GET_STAGE_PREFERRED_DIM_EMPLOYEE"
...
FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.EMPLOYEE_MASTER_LIST EM
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.LANGUAGES L ON L.LANGUAGEID = EM.LANGUAGE1
-- LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.OFFICE_OFFICES_REPL  O ON EM.OFFICEID = O.OFFICEID AND EM.AGENCYID = O.AGENCYID 
LEFT  JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.PAYROLL_DETAILS_REPL PD ON PD.CAREGIVERID = EM.CAREGIVERID 
