resource "snowflake_procedure" "DW_HAH_GET_STAGE_WORKDAY_FACT_APPLICANT_STATUS" {
	name ="GET_STAGE_WORKDAY_FACT_APPLICANT_STATUS"
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.STAGE.WORKDAY_FACT_APPLICANT_STATUS
WITH HIST_APPLICANTS AS (
	SELECT
		DISTINCT EMPLOYEE_ID 
	FROM
		DISC_${var.SF_ENVIRONMENT}.WORKDAY.HIST_WORKER_ADDITIONAL_INFORMATION ADINFO
...
(SELECT 
	...
	FROM
		DISC_${var.SF_ENVIRONMENT}.WORKDAY.HIST_WORKER_POSITION_DATA CJ
	LEFT JOIN
		DISC_${var.SF_ENVIRONMENT}.WORKDAY.WORKER_POSITION_DATA CJ_C
...
FROM DISC_${var.SF_ENVIRONMENT}.WORKDAY.WORKER_SUMMARY EM
     LEFT JOIN DISC_${var.SF_ENVIRONMENT}.WORKDAY.WORKER_ADDITIONAL_INFORMATION ADINFO
	...
	-- LEFT JOIN DISC_${var.SF_ENVIRONMENT}.WORKDAY.WORKER_POSITION_DATA POS
	 LEFT JOIN HIST_PROFILES POS
	 ON EM.EMPLOYEE_ID = POS.EMPLOYEE_ID
	 LEFT JOIN DISC_${var.SF_ENVIRONMENT}.WORKDAY.REMOTE_LOCATIONS re
	 ON POS.BUSINESS_SITE_NAME = RE.BUSINESS_SITE_NAME
	 LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_STATE DS 
	 ON UPPER(POS.BUSINESS_SITE_REGION) = UPPER(DS.STATE_NAME)
	 LEFT JOIN DISC_${var.SF_ENVIRONMENT}.PARADOX.CANDIDATE_JOURNEY CJ
