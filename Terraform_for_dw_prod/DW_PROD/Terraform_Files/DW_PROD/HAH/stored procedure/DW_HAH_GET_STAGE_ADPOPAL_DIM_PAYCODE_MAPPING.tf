resource "snowflake_procedure" "DW_HAH_GET_STAGE_ADPOPAL_DIM_PAYCODE_MAPPING" {
	name ="GET_STAGE_ADPOPAL_DIM_PAYCODE_MAPPING"
...
WITH PAYCODE_USAGE_DATES AS (
	...
	FROM DISC_${var.SF_ENVIRONMENT}.ADP.OPALPAYROLLDETAIL
	GROUP BY 1, 2, 3
)
SELECT DISTINCT 
...
FROM DISC_${var.SF_ENVIRONMENT}.ADP.OPALPAYROLLDETAIL DETAIL
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ADP.OPALPAYCODES PAYCODE
	ON PAYCODE.PAY_CODE 
		=
		CASE WHEN PAYCODES LIKE ''ADDITIONAL_EARNING%'' 
		THEN SPLIT_PART(PAYCODES, ''_'', 4)
