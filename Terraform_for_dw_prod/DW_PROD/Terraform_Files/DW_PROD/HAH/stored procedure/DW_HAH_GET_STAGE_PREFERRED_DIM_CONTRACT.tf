resource "snowflake_procedure" "DW_HAH_GET_STAGE_PREFERRED_DIM_CONTRACT" {
	name ="GET_STAGE_PREFERRED_DIM_CONTRACT"
	...
	statement = <<-EOT
                ...
                BEGIN
  --*****************************************************************************************************************************
-- NAME:  PREFERREDDB_DIM_CONTRACT
--
-- PURPOSE: Creates one row per CONTRACT according to PREFERREDDB
--
-- DEVELOPMENT LOG:
-- DATE        	AUTHOR                	NOTES:
-- --------    	-------------------   	-----------------------------------------------------------------------------------------------
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO STAGE.PREFERRED_DIM_CONTRACT
WITH CTE AS 
(
	SELECT DISTINCT (S.CONTRACTID) AS CONTRACTID
	    			...
	    			, IFF(SC.CATEGORY = ''Home Health'' OR S.SERVICECODE LIKE ''%HHA%'',''HH'',''HC'') AS R_CATEGORY
	...
	FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.VisitInfo_REPL VI
	JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.SERVICETYPE_REPL ST ON ST.SERVICETYPEID = VI.SERVICETYPEID
	    	AND ST.AGENCYID = VI.AGENCYID
	JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.SERVICECATEGORY_REPL SC ON SC.SERVICECATEGORYID = VI.SERVICECATEGORYID
	        AND ST.SERVICECATEGORYID
	INNER JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.AGENCY_MAPPING AGENCY_CONFIG
		ON AGENCY_CONFIG.AGENCYID = VI.AGENCYID
			AND AGENCY_CONFIG.ISACTIVE = TRUE
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.SERVICECODES S ON S.SERVICECODEID = VI.PRIMARYSERVICECODEID
	        AND S.AGENCYID = VI.AGENCYID
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.CONTRACTS C ON S.CONTRACTID = C.CONTRACTID
	        AND C.AGENCYID = VI.AGENCYID
	JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.CONTRACTRATES CR ON CR.CONTRACTID = C.CONTRACTID
	    	AND CR.AGENCYID = C.AGENCYID
	        AND CR.SERVICECODEID = S.SERVICECODEID
	        AND CR.TODATE :: DATE > GETDATE()
	        AND CR.FROMDATE :: DATE <= GETDATE()
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.TBLVISITS_REPL TV ON TV.VISITID = VI.VISITID
	        AND TV.AGENCYID = VI.AGENCYID
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.VISITCONTRACTS_REPL VC ON VC.VISITID = TV.VISITID
	        AND TV.AGENCYID = VC.AGENCYID
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.STAGE_PATIENTCONTRACTS_REPL PC ON PC.PATIENTCONTRACTID = VC.CONTRACTID
...
SELECT DISTINCT MD5(SC.AGENCYID || ''-'' || SC.CONTRACTID || ''-'' || NVL(CR.CONTRACTRATEID, -1) || 
    	...
    	, MD5(''PREFERRED'' || ''-'' || SC.AGENCYID || nvl(SC.SERVICECODEID, -1) || ''-'' || ''PREFERRED'') AS SERVICE_KEY
    	, NULL AS DEFAULT_BILL_CODE 
		, NULL AS PAYROLL_CODE
		, NVL(PS.REVENUE_CATEGORY, ''UNKNOWN'') AS REVENUE_CATEGORY
		, NVL(PS.REVENUE_SUBCATEGORY_CODE, ''UNKNOWN'') AS REVENUE_SUBCATEGORY_CODE
		, NVL(PS.REVENUE_SUBCATEGORY_NAME, ''UNKNOWN'') AS REVENUE_SUBCATEGORY_NAME
    	, NULL AS PAYOR_CODE
...
FROM  DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.SERVICECODES SC
INNER JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.AGENCY_MAPPING AGENCY_CONFIG
	ON AGENCY_CONFIG.AGENCYID = SC.AGENCYID
		AND AGENCY_CONFIG.ISACTIVE = TRUE
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.CONTRACTRATES CR ON CR.CONTRACTID = SC.CONTRACTID
