resource "snowflake_procedure" "DW_HAH_GET_STAGE_OSHAH_FACT_PARTNER_CONTRACT_SERVICE" {
	name ="GET_STAGE_OSHAH_FACT_PARTNER_CONTRACT_SERVICE"
...
WITH CLIENT AS(
	...
	SELECT * FROM (	SELECT DISTINCT PATIENTID,MASTER_ID,AGENCYID,LASTNAME,FIRSTNAME,OFFICEID--,ADMISSIONID --,OFFICEID
					FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.CLIENT_MATCH_LIST	WHERE PATIENTID NOT IN (
					SELECT PATIENTID FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.CLIENT_MASTER_LIST))
	)
,  STATES AS (
...
/*LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.PATIENTAUTHORIZATION PA 
    ON S.CONTRACTID = PA.CONTRACTID
	AND S.SERVICECODEID = PA.SERVICECODEID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.CONTRACTS C 
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.VisitInfo_REPL VI
 		ON S.SERVICECODEID = VI.PRIMARYSERVICECODEID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.CONTRACTRATES CR ON
