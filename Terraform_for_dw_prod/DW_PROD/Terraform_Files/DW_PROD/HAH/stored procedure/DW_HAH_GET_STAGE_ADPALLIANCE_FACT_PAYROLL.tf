resource "snowflake_procedure" "DW_HAH_GET_STAGE_ADPALLIANCE_FACT_PAYROLL" {
	name ="GET_STAGE_ADPALLIANCE_FACT_PAYROLL"
...
--		SUM(/*NVL("ADDITIONAL HOURS   CV1-OSHA ETS", 0) + */NVL("ADDITIONAL HOURS   HBO-HHA HOURLY B", 0))
	...
	FROM DISC_${var.SF_ENVIRONMENT}.ADP.ALLIANCE_PAYROLL
	GROUP BY 1, 2, 3
)
SELECT DISTINCT 
...
FROM DISC_${var.SF_ENVIRONMENT}.ADP.ALLIANCE_PAYROLL PAY
LEFT JOIN SALARY
...
LEFT JOIN DISC_DEDUPE_${var.SF_ENVIRONMENT}.GENERATIONSALLIANCE.EMPLOYEE_MASTER_LIST GEN_EMP
