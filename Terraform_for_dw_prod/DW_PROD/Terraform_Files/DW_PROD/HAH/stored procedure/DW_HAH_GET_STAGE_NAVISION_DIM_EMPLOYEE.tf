resource "snowflake_procedure" "DW_HAH_GET_STAGE_NAVISION_DIM_EMPLOYEE" {
	name ="GET_STAGE_NAVISION_DIM_EMPLOYEE"
...
WITH EMPLOYEE_CHECK_DATES AS (
	...
	FROM DISC_${var.SF_ENVIRONMENT}.NAVISION.CHECK_LEDGER_ENTRY CLE
	LEFT JOIN disc_${var.SF_ENVIRONMENT}.NAVISION.PAYROLL_LEDGER_ENTRY PLE 
...
--	REPLACE(PHONE_NO_, ''-'') AS EMPLOYEE_WORK_PHONE,
	DW_${var.SF_ENVIRONMENT}."STAGE".PHONE_CLEANUP(HOME_PHONE_NO_) AS EMPLOYEE_HOME_PHONE,
	DW_${var.SF_ENVIRONMENT}."STAGE".PHONE_CLEANUP(MOBILE_PHONE_NO_) AS EMPLOYEE_CELL_PHONE,
	DW_${var.SF_ENVIRONMENT}."STAGE".PHONE_CLEANUP(PHONE_NO_) AS EMPLOYEE_WORK_PHONE,
...
FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.NAVISION.EMPLOYEE_MASTER_LIST EMP
LEFT JOIN EMPLOYEE_CHECK_DATES CD
	ON EMP.MASTER_ID = CD.EMPLOYEE_NO_
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.NAVISION.BRANCH_MAPPING BM
