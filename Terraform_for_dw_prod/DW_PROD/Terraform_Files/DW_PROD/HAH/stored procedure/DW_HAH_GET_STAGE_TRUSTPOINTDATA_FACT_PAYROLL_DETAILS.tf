resource "snowflake_procedure" "DW_HAH_GET_STAGE_TRUSTPOINTDATA_FACT_PAYROLL_DETAILS" {
	name ="GET_STAGE_TRUSTPOINTDATA_FACT_PAYROLL_DETAILS"
...
(
    ...
    FROM DISC_${var.SF_ENVIRONMENT}.TRUSTPOINTDATA.PAYROLL_DETAILS
    WHERE PAY_AMOUNT <> 0
    GROUP BY 1,2,3
),
UNIT_TYPE AS
