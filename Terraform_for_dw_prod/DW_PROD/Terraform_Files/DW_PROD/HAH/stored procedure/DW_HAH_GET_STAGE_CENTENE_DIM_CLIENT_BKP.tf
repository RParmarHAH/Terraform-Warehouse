resource "snowflake_procedure" "DW_HAH_GET_STAGE_CENTENE_DIM_CLIENT_BKP" {
	name ="GET_STAGE_CENTENE_DIM_CLIENT_BKP"
...
SELECT
     MD5(''CENTENE'' || TRIM(MEMBER_CURR_CK) || ''-'' || TRIM(FIRST_NAME) || ''-'' || TRIM(LAST_NAME) || ''-'' || ''CENTENE'') AS CLIENT_KEY
	,TRIM(MEMBER_CURR_CK) AS CLIENT_NUMBER 
    ...
    ,TRIM(TO_CHAR(LAST_NAME)) || '','' || TRIM(TO_CHAR(FIRST_NAME))  AS CLIENT_NAME
	...
	,NULL NOTES
 	,NULL AGENCY 
 	,MEDICAID_NBR
    ,TO_DATE(ELIG_START_DT) AS EFFECTIVE_FROM_DATE
