resource "snowflake_procedure" "DW_HAH_FACT_CLIENT_ACQUIRED_WEEKLY_POSTPROCESSING" {
	name ="FACT_CLIENT_ACQUIRED_WEEKLY_POSTPROCESSING"
...
,CLEARCARE_CLIENT AS (
	...
	JOIN ( SELECT PA.ID || pa.AGENCY_ID AS CLIENT_NUMBER, PPT.PROFILETAG_ID 
			FROM DISC_${var.SF_ENVIRONMENT}.CLEARCARE.PATIENT_PATIENT PA
			LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.PROFILE_PROFILE PP ON PA.ID=PP.PATIENT_ID
