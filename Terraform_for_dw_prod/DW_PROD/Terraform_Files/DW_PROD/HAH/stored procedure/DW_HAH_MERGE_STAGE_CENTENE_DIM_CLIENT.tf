resource "snowflake_procedure" "DW_HAH_MERGE_STAGE_CENTENE_DIM_CLIENT" {
	name ="MERGE_STAGE_CENTENE_DIM_CLIENT"
...
-- 11/08/2023    TRUSHALI RAMOLIYA     ADDED A COLUMN FOR HISPANIC AND LATINO AS PER IDDOX - 388
--*****************************************************************************************************************************

MERGE INTO HAH.DIM_CLIENT TGT 
USING STAGE.CENTENE_DIM_CLIENT STAGE 
...
WHERE SYSTEM_CODE = ''CENTENE''
        AND ACTIVE_CLIENT_FLAG -- Only active client
        AND CLIENT_KEY NOT IN (SELECT CLIENT_KEY FROM STAGE.CENTENE_DIM_CLIENT);
