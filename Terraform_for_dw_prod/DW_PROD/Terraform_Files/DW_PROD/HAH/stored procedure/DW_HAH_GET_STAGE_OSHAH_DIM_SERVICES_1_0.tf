resource "snowflake_procedure" "DW_HAH_GET_STAGE_OSHAH_DIM_SERVICES_1_0" {
	name ="GET_STAGE_OSHAH_DIM_SERVICES_1_0"
...
JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.PAYER_REPL PR
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.VW_OSHAH_REVENUE RP ON SC.SERVICECODEID = RP.SERVICECODEID 
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.Contracts c  on SC.ContractID = c.ContractID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.Payer_REPL P ON
