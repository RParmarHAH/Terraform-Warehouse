resource "snowflake_procedure" "DW_HAH_GET_STAGE_MATRIXCARE_DIM_BRANCH" {
	name ="GET_STAGE_MATRIXCARE_DIM_BRANCH"
...
WITH RISKCONNECT_HIERARCHY AS (
	...
	FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.STVHC_T_BRANCHES AS BRANCHES
	...
	FROM (
		...
		LEFT JOIN DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.MATRIXCARE_MASTER_BRANCH_MAPPING AS MATRIXCARE_BRANCH_MAPPING
...
:STR_CDC_START::timestamp_ntz
	...
	SELECT DISTINCT 
		...
		IFF(CONVERGED_OFFICES.BRANCH_KEY IS NOT NULL, CONVERGED_OFFICES.OFFICE_ADDRESS1, OFFICES.OFFICE_ADDRESS1) AS OFFICE_ADDRESS1,
		IFF(CONVERGED_OFFICES.BRANCH_KEY IS NOT NULL, CONVERGED_OFFICES.OFFICE_ADDRESS2, OFFICES.OFFICE_ADDRESS2) AS OFFICE_ADDRESS2,
