resource "snowflake_procedure" "DW_HAH_GET_STAGE_GPSYNCDATA_FACT_PAYROLL_DETAILS" {
	name ="GET_STAGE_GPSYNCDATA_FACT_PAYROLL_DETAILS"
...
(
	...
	FROM DISC_${var.SF_ENVIRONMENT}.GPSYNCDATA.PAYSTUB_DETAILLINE pd
	GROUP BY 1, 2
),
UNIT_TYPE AS 
(
...
(
	SELECT DISTINCT
		...
		SUM(PD.RATE) AS RATE
	FROM DISC_${var.SF_ENVIRONMENT}.GPSYNCDATA.PAYSTUB_MASTER PM
	LEFT JOIN (SELECT *	FROM DISC_${var.SF_ENVIRONMENT}.GPSYNCDATA.PAYSTUB_DETAILLINE
...
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_BRANCH BRANCH ON  
