resource "snowflake_procedure" "DW_HAH_GET_STAGE_CCSI_FACT_CLIENT_SERVICE" {
	name ="GET_STAGE_CCSI_FACT_CLIENT_SERVICE"
...
BEGIN
--*****************************************************************************************************************************
-- NAME:  CCSI FACT CLIENT SERVICE
--
-- PURPOSE: Populates Stage CCSI FACT CLIENT SERVICE
--
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
...
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.STAGE.CCSI_FACT_CLIENT_SERVICE
...
--SELECT *
FROM DISC_${var.SF_ENVIRONMENT}.CCSI.MASTERCLIENT
UNION 
SELECT  CLIENT_IDOA_NO,used_for_Area AS OFFICE  ,PHONE_NO,REPLACE (replace(REPLACE(replace(PHONE_NO,''('',''''),'')'',''''),''-'',''''),'' '','''') CALLER_ID
FROM --DISC_DEDUPE_${var.SF_ENVIRONMENT}.CCSI.CLIENT_MATCH_LIST_CCSI 
(
(SELECT * FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.CCSI.CLIENT_MATCH_LIST_CCSI M WHERE NOT EXISTS (SELECT * FROM
DISC_DEDUPE_${var.SF_ENVIRONMENT}.CCSI.CLIENT_MASTER_LIST_CCSI MA WHERE M.CLIENT_IDOA_NO= MA.CLIENT_IDOA_NO AND 
M.USED_FOR_AREA = MA.USED_FOR_AREA))
)
) mst 
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CCSI.CALLSFIELD  cl
...
MD5(NVL(R.CLIENT_ID,''E'')::STRING || ''-'' || NVL(COALESCE(O.AREA,O1.AREA),''E'')::STRING 
		...
		,NULL AS BILL_TYPE
		,NULL AS BILL_UOM
		,''Hourly'' SCHEDULE_TYPE
		,''Hourly'' SCHEDULE_UOM
		,s.SERVICE_CODE AUTHORIZATION_DISCIPLINE
		,TMP.START_DATE AS AUTHORIZATION_DATE
		,DC.INITIAL_SERVICE_DATE AS AUTHORIZATION_PERIOD_START_DATE                        
		,SRVC.LAST_PUNCH_TIME AS AUTHORIZATION_PERIOD_END_DATE
		,NULL AS EFFECTIVE_DAYS
		,''Hourly'' AS AUTHORIZATION_TYPE
		,''Monthly'' AS AUTHORIZATION_UOM
		,DATES.FIRST_DAY_OF_MONTH::VARIANT AS AUTH_INFO -- MONTH OF AUTH AS INTAKE
		,C.HOURS_AUTHORIZED AS AUTHORIZATION_MAX_UNITS_BY_UOM
		,C.HOURS_AUTHORIZED AS AUTHORIZATION_MAX_UNITS
		,C.HOURS_AUTHORIZED AS AUTHORIZATION_MAX_UNITS_ADJUSTED_BY_UOM
		,C.HOURS_AUTHORIZED AS AUTHORIZATION_MAX_UNITS_ADJUSTED
		,IFF(CURRENT_DATE() BETWEEN AUTHORIZATION_PERIOD_START_DATE AND AUTHORIZATION_PERIOD_END_DATE,TRUE,FALSE)ACTIVE_FLAG
	...
	LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_SERVICES S ON S.SERVICE_CODE = R.SERVICE_TYPE
	LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.FACT_PARTNER_CONTRACT_SERVICE PCS ON R.CONTRACT_NO = PCS.CONTRACT_CODE 
	                                        AND PCS.SERVICE_CODE = R.SERVICE_TYPE 
...
MD5(NVL(R.CLIENT_ID,''E'')::STRING || ''-'' || NVL(COALESCE(O.AREA,O1.AREA),''E'')::STRING 
	...
	LEFT JOIN ENDSRVCDATE AS SRVC  ON SRVC.CLIENT_ID=R.CLIENT_ID AND SRVC.OFFICE=COALESCE(O.AREA,O1.AREA) -- ADDED BY PJSHAH ON 27042022
		...
		SELECT *
		,ROW_NUMBER() OVER (PARTITION BY CLIENT_SERVICE_KEY ORDER BY CLIENT_SERVICE_KEY DESC) AS RN  
		FROM RESULT
		)		
WHERE RN = 1;
