resource "snowflake_procedure" "DW_HAH_MERGE_STAGE_SANDATAIMPORT_FACT_EVV_VISIT_BCP" {
	name ="MERGE_STAGE_SANDATAIMPORT_FACT_EVV_VISIT_BCP"
    ...
    try {
      var sqlCmd = `
    MERGE INTO HAH.FACT_EVV_VISIT_BCP TGT 
USING STAGE.SANDATAIMPORT_FACT_EVV_VISIT_BCP STAGE 
