resource "snowflake_procedure" "DW_HAH_GET_STAGE_ADPALLIANCE_DIM_EMPLOYEE" {
	name ="GET_STAGE_ADPALLIANCE_DIM_EMPLOYEE"
...
WITH SALARY AS (
	SELECT DISTINCT
		...
		CASE WHEN "RATE_TYPE" = ''Hourly''
			...
			ELSE RATE * 12
		END AS SALARY
	FROM DISC_${var.SF_ENVIRONMENT}.ADP.ALLIANCE_PAYROLL AP QUALIFY RANK = 1
),
PAYROLL AS (
	SELECT DISTINCT
	...
	FROM DISC_${var.SF_ENVIRONMENT}.ADP.ALLIANCE_PAYROLL AP
	GROUP BY 1, 2, 3
)
SELECT DISTINCT
...
FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.ADP.ALLIANCE_EMPLOYEE_MASTER_LIST AE
LEFT JOIN PAYROLL AS PAY
	ON PAY.FILE_NUMBER = AE.FILE_NUMBER
LEFT JOIN DISC_DEDUPE_${var.SF_ENVIRONMENT}.GENERATIONSALLIANCE.EMPLOYEE_MASTER_LIST EMP
