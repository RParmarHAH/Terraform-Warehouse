resource "snowflake_procedure" "DW_HAH_GET_STAGE_OSHAH_DIM_CLIENT" {
	name ="GET_STAGE_OSHAH_DIM_CLIENT"
...
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.STAGE.OSHAH_DIM_CLIENT
WITH CLIENT AS  
(
	SELECT * FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.CLIENT_MASTER_LIST  WHERE MASTER_ID IN 
	(
...
FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.PATIENTSTATUSCHANGEACTIVITY ) 
...
FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.PATIENTSTATUSCHANGEACTIVITY ST
...
SELECT
     ...
     ,ADM.CONTRACTID
FROM
	DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.ADMISSIONS ADM
...
FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.PATIENTCONTRACTCHANGES PC
...
SELECT
	...
	COORDINATORNAME
FROM
	DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.PROVIDERPATIENTCOORDINATORS 
QUALIFY ROW_NUMBER() OVER (PARTITION BY PATIENTID
ORDER BY
...
FROM
	DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.VISITS VI
LEFT JOIN CLIENT CL
ON
	TRIM(CL.PATIENTID)= TRIM(VI.PATIENTID)
JOIN  DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.TBLVISITS_REPL VIS
...
FROM CLIENT CLI
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.OFFICE_OFFICES_REPL OFFICE ON CLI.OFFICEID = OFFICE.OFFICEID
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.PATIENTCUSTOMFIELDS PC
		  ON TRIM(PC.PATIENTID) = TRIM(CLI.PATIENTID)
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.REFERRALSOURCE RS ON RS.REFERRALSOURCEID = CLI.REFERRALSOURCEID
