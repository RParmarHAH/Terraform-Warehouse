resource "snowflake_procedure" "DW_HAH_GET_STAGE_COASTALGP_DIM_PAYCODE_MAPPING" {
	name ="GET_STAGE_COASTALGP_DIM_PAYCODE_MAPPING"
...
FROM @DW_${var.SF_ENVIRONMENT}.Stage.AZStage/Jigar_test/paycode (file_format => DW_${var.SF_ENVIRONMENT}."PUBLIC".CSV_Format)wm
),
...
FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.GPALLPAYCHECKDETAILSBASE
...
FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.GPALLPAYCHECKDETAILSBASE D
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.GPALLPAYCODESBASE p ON
