resource "snowflake_procedure" "DW_HAH_GET_STAGE_ZENDESK_FACT_SLA" {
	name ="GET_STAGE_ZENDESK_FACT_SLA"
...
WITH POLICY_METRICS AS (
    ...
    FROM DISC_${var.SF_ENVIRONMENT}.ZENDESK.SLA_POLICIES T,
    LATERAL FLATTEN(input => T.POLICY_METRICS) f
    ORDER BY ID,PRIORITY,METRIC  ASC ),
    TICKET_METRICS_EVENT AS (
    SELECT * FROM DISC_${var.SF_ENVIRONMENT}.ZENDESK.TICKET_METRICS_EVENT WHERE TYPE = ''apply_sla'' AND INSTANCE_ID=''1''   -- taking instance id =1 to remove next reply time records and only take first reply time , all other metrics have instance id =1 , so it wont affect them
...
JOIN DISC_${var.SF_ENVIRONMENT}.ZENDESK.TICKET_METRICS TM 
	ON TM.TICKET_ID = TME.TICKET_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ZENDESK.TICKETS TKT
