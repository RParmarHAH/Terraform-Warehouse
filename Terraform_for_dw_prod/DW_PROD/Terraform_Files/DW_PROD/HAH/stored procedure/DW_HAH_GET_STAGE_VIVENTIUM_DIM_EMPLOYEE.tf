resource "snowflake_procedure" "DW_HAH_GET_STAGE_VIVENTIUM_DIM_EMPLOYEE" {
	name ="GET_STAGE_VIVENTIUM_DIM_EMPLOYEE"
...
from DISC_DEDUPE_${var.SF_ENVIRONMENT}.VIVENTIUM.EMPLOYEE_MASTER_LIST  EM left join checkdate CD on CD.EMPLOYEENUMBER = EM.EMPLOYEENUMBER and CD.employeekey = EM.employeekey and CD.companycode = EM.companycode
