resource "snowflake_procedure" "DW_HAH_GET_STAGE_DATAFLEXSYNCDATA_DIM_VISIT_STATUS_MAPPING" {
	name ="GET_STAGE_DATAFLEXSYNCDATA_DIM_VISIT_STATUS_MAPPING"
...
FULL OUTER JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_VISITS SD 
ON SCH.SCHEDULEID = SD.SCHEDULEID AND SCH.AGENCYID = SD.AGENCYID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.BI_REPOSITORY.SANDATAVISITS BI ON SCH.SCHEDULEID = BI.SCHEDULEID AND SCH.AGENCYID = BI.AGENCYID
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.ALL_DFTIMESHEETS TS 																-- This join will only be used to get fields, the previous TS1 join''s purpose is to just act as mapping between tables
