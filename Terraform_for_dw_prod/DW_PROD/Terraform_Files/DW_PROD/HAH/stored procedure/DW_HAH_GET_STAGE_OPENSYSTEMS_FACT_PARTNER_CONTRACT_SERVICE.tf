resource "snowflake_procedure" "DW_HAH_GET_STAGE_OPENSYSTEMS_FACT_PARTNER_CONTRACT_SERVICE" {
	name ="GET_STAGE_OPENSYSTEMS_FACT_PARTNER_CONTRACT_SERVICE"
...
UNION
	SELECT * FROM
		...
		WHERE PATIENTID NOT IN ( SELECT	PATIENTID FROM
				DISC_DEDUPE_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.CLIENT_MASTER_LIST) 
))
	SELECT
		DISTINCT MD5(
		''OPENSYSTEMS'' || ''-'' || 
	...
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.PATIENTAUTHORIZATION PA 
    	ON S.CONTRACTID = PA.CONTRACTID
		AND S.SERVICECODEID = PA.SERVICECODEID
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.CONTRACTS C ON
	...
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.OFFICE_OFFICES_REPL O ON VI.OFFICEID = O.OFFICEID
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.CONTRACTRATES CR ON
