resource "snowflake_procedure" "DW_HAH_GET_STAGE_QUICKBOOKS_DIM_PAYCODE_MAPPING" {
	name ="GET_STAGE_QUICKBOOKS_DIM_PAYCODE_MAPPING"
...
WITH USAGE_DATES AS (
	...
	FROM (
		SELECT DISTINCT COMPANY, PAYCODES, NAME, COALESCE(CHK_DATE, TYPE) AS PAYROLL_DATE FROM DISC_${var.SF_ENVIRONMENT}.ADP.QB_PAYROLL_DETAILS
		UNION
		SELECT DISTINCT COMPANY, PAYCODES, NAME, DATE AS PAYROLL_DATE FROM DISC_${var.SF_ENVIRONMENT}.ADP.QB_PAYROLL_DETAILS_MIDWAY_2020
		)
	GROUP BY 1, 2
)
SELECT DISTINCT
...
FROM (
	SELECT DISTINCT COMPANY, PAYCODES FROM DISC_${var.SF_ENVIRONMENT}.ADP.QB_PAYROLL_DETAILS
	UNION
	SELECT DISTINCT COMPANY, PAYCODES FROM DISC_${var.SF_ENVIRONMENT}.ADP.QB_PAYROLL_DETAILS_MIDWAY_2020
	) AS PAY
