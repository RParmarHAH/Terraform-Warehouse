resource "snowflake_procedure" "DW_HAH_GET_STAGE_DATAFLEXSYNCDATA_FACT_INTAKE_BKP_BFR_AUTH_08082022" {
	name ="GET_STAGE_DATAFLEXSYNCDATA_FACT_INTAKE_BKP_BFR_AUTH_08082022"
...
WITH MAXREAUTH AS (
 ...
 (
     SELECT MIN(PAYROLLDATE) AS TIMESHEET_DATE
          ...
          , DBNAME
          , CLIENTNUMBER
     FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFTIMESHEETS
