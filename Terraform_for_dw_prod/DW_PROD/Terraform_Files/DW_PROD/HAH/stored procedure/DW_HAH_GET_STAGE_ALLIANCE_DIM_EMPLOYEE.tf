resource "snowflake_procedure" "DW_HAH_GET_STAGE_ALLIANCE_DIM_EMPLOYEE" {
	name ="GET_STAGE_ALLIANCE_DIM_EMPLOYEE"
...
SELECT DISTINCT
    ...
    ,NULL AS LINKED_ID
 FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.GENERATIONSALLIANCE.EMPLOYEE_MASTER_LIST EMP
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.ADP.ALLIANCE_EMPLOYEE rhd ON CASE WHEN EMP.SOCIALSECURITYNUM <> rhd.EMPLOYEE_SSN THEN EMP."PAYCHEXID" = rhd."EMPLOYEE_NUMBER"::string ELSE EMP.SOCIALSECURITYNUM = rhd.EMPLOYEE_SSN END
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.GENERATIONSALLIANCE.CLASSIFICATION CLA ON EMP.CLASSIFICATIONID = CLA.CLASSIFICATIONID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.GENERATIONSALLIANCE.VW_BRANCH_MAPPING BM ON BM.LOCATION_ID = EMP.CLASSNAME;
RETURN ''SUCCESS'';
END;
