resource "snowflake_procedure" "DW_HAH_GET_STAGE_QUICKBOOKS_FACT_PAYROLL" {
	name ="GET_STAGE_QUICKBOOKS_FACT_PAYROLL"
...
FROM DISC_${var.SF_ENVIRONMENT}.ADP.QB_PAYROLL AS QB_PAYROLL
...
SELECT DISTINCT
	...
	REPLACE(NVL(DATES.REIMBURSEMENT, 0) + NVL(DATES.REIMBURSEMENT_CPR, 0) + NVL(DATES.MILEAGE_REIMBURSEMENT, 0), '','') AS REINBURSMENT_AMOUNT,
		...
		current_user() AS ETL_LAST_UPDATED_BY,
		FALSE ETL_DELETED_FLAG
FROM DISC_${var.SF_ENVIRONMENT}.ADP.QB_PAYROLL_DATES DATES
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ADP.QB_PAYROLL_HOURS HOURS
	ON DATES.COMPANY = HOURS.COMPANY AND DATES."NAME" = HOURS."NAME" AND dates.DATE = HOURS.DATE
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ADP.QB_PAYROLL_RATE RATE 
...
SELECT DISTINCT
	...
	REPLACE(GROSS.GROSS_PAY_REIMB, '','') AS REINBURSMENT_AMOUNT,
		...
		current_user() AS ETL_LAST_UPDATED_BY,
		FALSE ETL_DELETED_FLAG
FROM DISC_${var.SF_ENVIRONMENT}.ADP.QB_PAYROLL_MIDWAY_20_GROSS GROSS
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ADP.QB_PAYROLL_MIDWAY_20_HOURS HOURS
