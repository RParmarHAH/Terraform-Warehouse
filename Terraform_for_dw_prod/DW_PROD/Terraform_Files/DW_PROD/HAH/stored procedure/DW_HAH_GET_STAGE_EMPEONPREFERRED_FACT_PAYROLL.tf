resource "snowflake_procedure" "DW_HAH_GET_STAGE_EMPEONPREFERRED_FACT_PAYROLL" {
	name ="GET_STAGE_EMPEONPREFERRED_FACT_PAYROLL"
...
VACATION_HOURS AS (
	...
	FROM DISC_${var.SF_ENVIRONMENT}.EMPEONPREFERRED.EPAYHISTDETAIL 
...
SICK_HOURS AS (
	...
	FROM DISC_${var.SF_ENVIRONMENT}.EMPEONPREFERRED.EPAYHISTDETAIL 
	WHERE TRIM(UPPER(DETCODE)) IN (SELECT DISTINCT ECODE FROM DISC_${var.SF_ENVIRONMENT}.EMPEONPREFERRED.CEARN WHERE TRIM(UPPER(ECODE)) LIKE ''%SICK%'' OR TRIM(UPPER(DESCRIPTION)) LIKE ''%SICK%'')
	GROUP BY 1
),
NON_SERVICE_UNITS AS (
	...
	WHERE TRIM(UPPER(DETCODE)) IN (
		SELECT DISTINCT
			ECODE FROM DISC_${var.SF_ENVIRONMENT}.EMPEONPREFERRED.CEARN
	...
	AND TRIM(UPPER(DETCODE)) NOT IN (SELECT DISTINCT TCODE FROM DISC_${var.SF_ENVIRONMENT}.EMPEONPREFERRED.ETAX)
	GROUP BY 1
),
UNION_DUES AS (
	SELECT DISTINCT
...
FROM DISC_${var.SF_ENVIRONMENT}.EMPEONPREFERRED.EPAYHIST EP
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.EMPEONPREFERRED.EPAYHISTDETAIL EPD
