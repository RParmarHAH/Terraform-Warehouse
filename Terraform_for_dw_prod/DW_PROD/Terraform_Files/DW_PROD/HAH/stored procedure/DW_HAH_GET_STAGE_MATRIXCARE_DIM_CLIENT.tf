resource "snowflake_procedure" "DW_HAH_GET_STAGE_MATRIXCARE_DIM_CLIENT" {
	name ="GET_STAGE_MATRIXCARE_DIM_CLIENT"
...
--			ELSE 1  END AS RN 
		FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.MATRIXCARE.CLIENT_MASTER_LIST A
--		LEFT JOIN DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.MATRIXCARE_MASTER_CLIENT_MAPPING_ALL B ON A.CLI_ID= B.CLIENT_NUMBER 
	) 	--WHERE RN =1
),
...
--    LEFT JOIN DISC_DEDUPE_${var.SF_ENVIRONMENT}.MATRIXCARE.CLIENT_MASTER_LIST B On A.CLIENTID = B.MASTER_ID
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.STVHC_T_RACE RACE ON RACE.RCE_ID = CLI.CLI_RACEID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.STVHC_T_CLIENTSTATUS CS ON TRIM(CLI.CLI_STATUSID) = TRIM(CS.CLS_ID)
LEFT JOIN DISCHARGE_REASON DRN ON TRIM(DRN.ADM_CLIENTID) = TRIM(CLI.CLI_ID) AND TRIM(DRN.ADM_BRANCHID) = TRIM(CLI.CLI_BRANCHID)
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.STVHC_T_ADMISSIONSDISCHARGEREASONS ADMDISC ON TRIM(ADMDISC.ADMDCR_ID) = TRIM(DRN.ADM_DISCHARGEREASONID)																								 
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.STVHC_T_BRANCHES BR ON BR.BR_ID = COALESCE(BRANCH_MAPPING.CONVERGED_OFFICE_CODE, CLI.CLI_BRANCHID::STRING)
