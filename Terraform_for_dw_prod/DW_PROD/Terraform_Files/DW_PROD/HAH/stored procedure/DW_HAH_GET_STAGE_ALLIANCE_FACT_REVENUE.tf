resource "snowflake_procedure" "DW_HAH_GET_STAGE_ALLIANCE_FACT_REVENUE" {
	name ="GET_STAGE_ALLIANCE_FACT_REVENUE"
...
(
    ...
    (
        ...
        WHERE MASTER_ID  NOT IN (SELECT MASTER_ID  FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.GENERATIONSALLIANCE.EMPLOYEE_MASTER_LIST)
    )
), SUPERVISOR AS 
(
	SELECT * FROM
...
(
	...
	FROM DISC_${var.SF_ENVIRONMENT}.GENERATIONSALLIANCE.CLIENTPAYORS CPY
	...
	INNER JOIN DISC_${var.SF_ENVIRONMENT}.GENERATIONSALLIANCE.PAYOR P
