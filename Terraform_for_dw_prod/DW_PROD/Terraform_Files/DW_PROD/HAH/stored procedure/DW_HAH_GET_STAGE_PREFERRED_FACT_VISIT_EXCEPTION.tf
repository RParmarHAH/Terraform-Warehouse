resource "snowflake_procedure" "DW_HAH_GET_STAGE_PREFERRED_FACT_VISIT_EXCEPTION" {
	name ="GET_STAGE_PREFERRED_FACT_VISIT_EXCEPTION"
...
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.STAGE.PREFERRED_FACT_VISIT_EXCEPTION
WITH CALLS AS (
	SELECT 
		V.VISITID,
	...
	FROM 
		DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.INREVIEWVISITS CM
	INNER JOIN 
		DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.VISITINFO_REPL V
		ON CM.AGENCYID = V.AGENCYID 
			AND CM.VISITID = V.VISITID
	INNER JOIN 
		DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.TBLVISITS_REPL CRN 
		ON CRN.AGENCYID = V.AGENCYID
			AND CRN.VISITID = V.VISITID
	LEFT JOIN
		DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.REASONS_REPL RS
