resource "snowflake_procedure" "DW_HAH_GET_STAGE_SANDATAIMPORT_DIM_SERVICES" {
	name ="GET_STAGE_SANDATAIMPORT_DIM_SERVICES"
...
WITH Rev_cat AS (
  ...
  SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY SERVICEID ORDER BY CASE 
        ...
        ELSE 0 END) AS rn
  FROM DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_INVOICEDETAILS
)
SELECT SERVICEID,
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_SERVICES AS S
		ON S.agencyID = F.AgencyId
		AND S.SERVICECODE =F.SERVICEID
LEFT JOIN Rev_cat RC ON RC.SERVICEID = S.SERVICECODE 
LEFT JOIN  DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_INVOICEDETAILS R	
...
:STR_ETL_TASK_KEY AS ETL_INSERTED_TASK_KEY,
     ...
     CURRENT_USER as ETL_LAST_UPDATED_BY,
     0  as ETL_DELETED_FLAG         
FROM DISC_${var.SF_ENVIRONMENT}.BI_REPOSITORY.SANDATAVISITS F
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_SERVICES AS S
		ON S.agencyID = F.AgencyId
		AND S.SERVICECODE =F.SERVICEID 
		LEFT JOIN Rev_cat RC ON RC.SERVICEID = S.SERVICECODE
...
:STR_ETL_TASK_KEY AS ETL_INSERTED_TASK_KEY,
     ...
     CURRENT_USER as ETL_LAST_UPDATED_BY,
     0  as ETL_DELETED_FLAG
FROM DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_CLIENTADMISSIONS CADs
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_AUTHORIZATIONS AS AUTH
	    ON CADs.agencyID = AUTH.AgencyId
	    AND CADs.admissionID = AUTH.AdmissionId
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_SERVICES S 
...
:STR_ETL_TASK_KEY AS ETL_INSERTED_TASK_KEY,
     ...
     CURRENT_USER as ETL_LAST_UPDATED_BY,
     0  as ETL_DELETED_FLAG     
FROM DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_SERVICES AS S
  LEFT JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_ADMISSIONSERVICES ADM_S
                   ON S.AGENCYID = ADM_S.AGENCYID AND ADM_S.SERVICEID  = S.SERVICECODE 
  LEFT JOIN Rev_cat RC ON RC.SERVICEID = S.SERVICECODE
  LEFT JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_CLIENTADMISSIONS CADs
