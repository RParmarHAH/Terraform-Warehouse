resource "snowflake_procedure" "DW_HAH_GET_STAGE_ALAYACARE_DIM_CONTRACT" {
	name ="GET_STAGE_ALAYACARE_DIM_CONTRACT"
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.SERVICE_CODE S ON SBC.SERVICE_CODE_ID = S.SERVICE_CODE_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.BILL_CODE AS BCF ON CASE WHEN BCF.FUNDER_ID =21 THEN BCF.FUNDER_ID = F.FUNDER_ID END 
