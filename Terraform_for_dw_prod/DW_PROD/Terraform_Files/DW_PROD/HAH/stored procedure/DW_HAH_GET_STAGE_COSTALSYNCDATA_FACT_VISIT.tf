resource "snowflake_procedure" "DW_HAH_GET_STAGE_COSTALSYNCDATA_FACT_VISIT" {
	name ="GET_STAGE_COSTALSYNCDATA_FACT_VISIT"
...
, CLIENT_DATA AS
(
--	SELECT * FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.CLIENT_MASTER_LIST
SELECT * FROM
	(
		...
		FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.CLIENT_MASTER_LIST MASTER
	)
	UNION
	SELECT * FROM
	(
...
, DATA AS (
	...
	FROM Visit_Agg v
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.PAYOR_CONTRACT_UI.PAYOR_CONTRACT_MAPPING PCM ON v.DB=PCM.ORIGINAL_SYSTEM_CODE
