resource "snowflake_procedure" "DW_HAH_GET_STAGE_OSHAH_FACT_INTAKE" {
	name ="GET_STAGE_OSHAH_FACT_INTAKE"
...
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.STAGE.OSHAH_FACT_INTAKE
WITH CLIENT AS
(
	SELECT * FROM
	(
...
FROM 
DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.PATIENTAUTHORIZATION PA

INNER JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_DATE DD
	ON DD.CALENDAR_DATE BETWEEN PA.FROMDATE AND PA.TODATE 

INNER JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.SERVICECODES SC
...
SELECT CORD.PATIENTID, CORD.COORDINATORID, CORD.COORDINATORNAME
	...
	FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.PROVIDERPATIENTCOORDINATORS CORD
...
SELECT CONTRACTID,SERVICECODEID, REVENUE_CATEGORY, REVENUE_SUBCATEGORY_NAME, REVENUE_SUBCATEGORY_CODE 
	FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.VW_OSHAH_REVENUE
...
SELECT DISTINCT VR.PATIENTID AS PATIENTID
	...
	FROM 
	DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.VISITINFO_REPL VISITS
	...
	INNER JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.VISITS VS
...
INNER JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.PATIENTS P
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.OFFICE_OFFICES_REPL OFFICES
		ON OFFICES.OFFICEID = P.OFFICEID
WHERE OFFICES.STATE IN (SELECT STATE FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.CONFIGURATION WHERE CONFIG = TRUE)
)
SELECT INTAKE_KEY
		, REPORT_DATE
		, BRANCH_KEY
