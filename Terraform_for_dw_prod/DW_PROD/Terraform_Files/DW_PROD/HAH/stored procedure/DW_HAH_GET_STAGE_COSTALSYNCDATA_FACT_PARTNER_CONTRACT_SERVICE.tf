resource "snowflake_procedure" "DW_HAH_GET_STAGE_COSTALSYNCDATA_FACT_PARTNER_CONTRACT_SERVICE" {
	name ="GET_STAGE_COSTALSYNCDATA_FACT_PARTNER_CONTRACT_SERVICE"
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CostalSyncData.CV_Locations l ON UPPER( TRIM( l.LOCATION_CODE)) = UPPER( TRIM( s.LOCATION_CODE))
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.STAGE.COASTAL_OFFICE_MAPPING t On TRIM( t.DB) = TRIM( l.DB) AND TRIM( t.LOCATION_CODE) = TRIM( l.Location_Code)
LEFT JOIN ( SELECT * FROM DISC_${var.SF_ENVIRONMENT}.HAHUSERS.LOGIN_OFFICE WHERE STATE IN (''GA'', ''SC'')
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.PAYOR_CONTRACT_UI.PAYOR_CONTRACT_MAPPING PCM ON S.DB=PCM.ORIGINAL_SYSTEM_CODE 
...
CURRENT_USER AS ETL_LAST_UPDATED_BY,
0 AS ETL_DELETED_FLAG 
FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.CV_ADMISSIONS s
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CostalSyncData.CV_Locations l ON UPPER( TRIM( l.LOCATION_CODE)) = UPPER( TRIM( s.LOCATION_CODE))
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.STAGE.COASTAL_OFFICE_MAPPING t On TRIM( t.DB) = TRIM( l.DB) AND TRIM( t.LOCATION_CODE) = TRIM( l.Location_Code)
LEFT JOIN ( SELECT * FROM DISC_${var.SF_ENVIRONMENT}.HAHUSERS.LOGIN_OFFICE WHERE STATE IN (''GA'', ''SC'')
		AND TRY_CAST(OFFICENUMBER AS INT) IS NOT NULL
