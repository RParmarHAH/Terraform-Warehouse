resource "snowflake_procedure" "DW_HAH_MERGE_STAGE_OPENSYSTEMS_DIM_INVOICE" {
	name ="MERGE_STAGE_OPENSYSTEMS_DIM_INVOICE"
...
BEGIN
MERGE INTO DW_${var.SF_ENVIRONMENT}.HAH.DIM_INVOICE TGT 
USING DW_${var.SF_ENVIRONMENT}.STAGE.OPENSYSTEMS_DIM_INVOICE STAGE 
ON TGT.INVOICE_KEY = STAGE.INVOICE_KEY
WHEN MATCHED THEN 
UPDATE SET 
    TGT.CLIENT_KEY= STAGE.CLIENT_KEY
   ,TGT.INVOICE_NUMBER= STAGE.INVOICE_NUMBER
   ,TGT.ALTERNATE_INVOICE_NUMBER= STAGE.ALTERNATE_INVOICE_NUMBER
...
VALUES (
   ...
   ,STAGE.ALTERNATE_INVOICE_NUMBER
   ,STAGE.PERIOD
   ,STAGE.INVOICE_TYPE
