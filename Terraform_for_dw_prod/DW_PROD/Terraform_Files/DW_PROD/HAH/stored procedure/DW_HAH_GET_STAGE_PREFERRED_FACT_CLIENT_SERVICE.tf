resource "snowflake_procedure" "DW_HAH_GET_STAGE_PREFERRED_FACT_CLIENT_SERVICE" {
	name ="GET_STAGE_PREFERRED_FACT_CLIENT_SERVICE"
...
--*****************************************************************************************************************************
-- NAME:  PREFERRED FACT CLIENT SERVICE
--
-- PURPOSE: Populates Stage PREFERRED FACT CLIENT SERVICE
--
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
-- 03/09/23    SANKET JAIN              Initial version
-- 28/09/23    Nutan Jagnade            Changed service_key Logic
--*****************************************************************************************************************************

INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.STAGE.PREFERRED_FACT_CLIENT_SERVICE
...
(	SELECT * FROM
	...
	(
        ...
        MA.PATIENTID , MA.MASTER_ID, MA.AGENCYID, MA.LASTNAME, MA.FIRSTNAME, MA.OFFICEID
...
SELECT AUTHORIZATIONID
		...
		, IFF(SC.SERVICECODE ILIKE ANY(''%T1020%'', ''%S5126%'', ''%S5151%'') AND AUTH_HOURS_PER_DAY > 13
   	    ...
   	    , FALSE AS FLAG
FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.STAGE_PATIENTAUTHORIZATION  PA
INNER JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_DATE DD
	ON DD.CALENDAR_DATE BETWEEN PA.FROMDATE AND PA.TODATE
INNER JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.SERVICECODES SC
...
(
    ...
    INNER JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.STAGE_PATIENTAUTHORIZATION PA
...
SELECT DISTINCT MD5(''PREFERRED''
	...
	, MD5(''PREFERRED''
	 || ''-'' || CLIENT.PATIENTID
     || ''-'' || P.ADMISSIONID 
     || ''-'' ||NVL(RS.REVENUE_SUBCATEGORY_CODE,''-1'')
     || ''-'' || SC.SERVICECODEID
     || ''-'' || ''HHAEXCHANGE'') AS CLIENT_ADMISSION_KEY
	,DC.CLIENT_KEY AS CLIENT_KEY
	--,MD5(''PREFERRED'' || ''-'' || SC.SERVICECODEID || ''-'' || ''HHAEXCHANGE'') AS SERVICE_KEY
    , MD5(''PREFERRED'' || ''-'' || SC.SERVICECODEID || ''-'' ||NVL(RS.REVENUE_SUBCATEGORY_CODE,''-1'')|| ''-'' || ''HHAEXCHANGE'')  AS SERVICE_KEY
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.STAGE_PATIENTS P ON P.PATIENTID = CLIENT.PATIENTID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.OFFICE_OFFICES_REPL O ON CLIENT.OFFICEID = O.OFFICEID
...
FULL JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.REVENUE_MAPPING RS ON
	RS.SERVICECODEID = SC.SERVICECODEID
