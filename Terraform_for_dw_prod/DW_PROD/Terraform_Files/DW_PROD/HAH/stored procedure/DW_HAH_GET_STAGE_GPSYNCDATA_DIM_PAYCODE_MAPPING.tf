resource "snowflake_procedure" "DW_HAH_GET_STAGE_GPSYNCDATA_DIM_PAYCODE_MAPPING" {
	name ="GET_STAGE_GPSYNCDATA_DIM_PAYCODE_MAPPING"
...
FROM DISC_${var.SF_ENVIRONMENT}.GPSyncData.PayStub_Detailline AS dtl
INNER JOIN DISC_${var.SF_ENVIRONMENT}.GPSYNCDATA.PAYSTUB_Master AS m ON TRIM( dtl.CheckID) = TRIM( m.CheckID)
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.GPSyncData.Employer_Edge_Paycodes AS eep ON m.DB = eep.Company_Code AND dtl.PayCode = eep.Pay_Code
),
...
SELECT DISTINCT 5 AS Source_System_ID,
	...
	(CASE WHEN PAY_TYPE IN (''HOURLY'',''DOUBLETIME'',''HOLIDAY'',''OVERTIME'',''PIECEWORK'',''SICK'',''VACATION'') THEN ''TRUE'' ELSE ''FALSE'' END)::BOOLEAN
		AS HOURS_PAYCODE
FROM @DISC_${var.SF_ENVIRONMENT}.STAGE.AWSAZSTAGEPROD/Files/MERCER/Mercer_OE22_HAHpaycodes_20211006 (file_format => DISC_${var.SF_ENVIRONMENT}.STAGE.CSV_FORMAT) pc --RSUTHAR - Change Stage/File Format
