resource "snowflake_procedure" "DW_HAH_GET_STAGE_FACT_SURVEY_RESPONSE_DETAIL" {
	name ="GET_STAGE_FACT_SURVEY_RESPONSE_DETAIL"
...
,RESPONSES AS 
	...
	JOIN DW_${var.SF_ENVIRONMENT}.HAH.FACT_SURVEY_QUESTION FSQ ON R.QUESTION_ID=FSQ.QUESTION_ID AND FSQ.IS_ACTIVE AND FSQ.SURVEY_ID=''''''||:SURVEY_ID||'''''')
,LABLES AS 
	(SELECT
	HEADER.RESPONSE_ID,
	REPLACE(f.KEY,''''#'''','''''''') AS QUESTION_ID, 
