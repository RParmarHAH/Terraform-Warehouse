resource "snowflake_procedure" "DW_HAH_GET_STAGE_WORKDAY_DIM_EMPLOYEE" {
	name ="GET_STAGE_WORKDAY_DIM_EMPLOYEE"
...
LEFT JOIN
	DISC_${var.SF_ENVIRONMENT}.WORKDAY.WORKER_POSITION_DATA POS
	ON POS.EMPLOYEE_ID = EM.EMPLOYEE_ID
LEFT JOIN
	DW_${var.SF_ENVIRONMENT}.HAH.DIM_STATE WORK_ST
...
LEFT JOIN 
	DISC_${var.SF_ENVIRONMENT}.WORKDAY.WORKER_COMPENSATION COMP
