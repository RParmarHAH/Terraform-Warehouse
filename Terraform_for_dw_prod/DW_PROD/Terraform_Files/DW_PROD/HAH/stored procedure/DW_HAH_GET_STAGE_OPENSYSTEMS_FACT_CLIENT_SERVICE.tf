resource "snowflake_procedure" "DW_HAH_GET_STAGE_OPENSYSTEMS_FACT_CLIENT_SERVICE" {
	name ="GET_STAGE_OPENSYSTEMS_FACT_CLIENT_SERVICE"
...
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}."STAGE".OPENSYSTEMS_FACT_CLIENT_SERVICE
...
SELECT * FROM
    (SELECT DISTINCT CML.PATIENTID, MASTER_ID, CML.AGENCYID, CML.LASTNAME, CML.FIRSTNAME, CML.OFFICEID
	 ...
	 WHERE CML.PATIENTID NOT IN (SELECT PATIENTID FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.CLIENT_MASTER_LIST))
),
...
FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.PATIENTAUTHORIZATION  PA
...
(
    ...
    INNER JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.PATIENTAUTHORIZATION PA
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.PATIENTS P ON P.PATIENTID = CLIENT.PATIENTID
...
JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.SERVICECODES SC ON SC.CONTRACTID = PA.CONTRACTID
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.PAYER_REPL  PR
   ...
   WHERE NOT (AUTHORIZATIONNUMBER ILIKE ''%TEMP%'' AND  MAXUNITSFORPERIOD <900 );
