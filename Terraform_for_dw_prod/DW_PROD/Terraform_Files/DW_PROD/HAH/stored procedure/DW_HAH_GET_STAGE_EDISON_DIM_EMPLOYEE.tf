resource "snowflake_procedure" "DW_HAH_GET_STAGE_EDISON_DIM_EMPLOYEE" {
	name ="GET_STAGE_EDISON_DIM_EMPLOYEE"
...
-- 03/15/23	   Shikhar Saxena 		 Changes in Primary Branch key logic for Corporate branch
-- 09/25/2023  Trushali Ramoliya      updated the logic for PAYROLL_ID
...
--*****************************************************************************************************************************
INSERT OVERWRITE  INTO STAGE.EDISON_DIM_EMPLOYEE
WITH CHECK_DATE AS 
(
	SELECT CAREGIVERID , TO_DATE(MIN(VISITDATE)) AS FIRST_CHECK_DATE , TO_DATE(MAX(VISITDATE))
	AS  LAST_CHECK_DATE
	FROM  DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.PAYROLL_DETAILS_REPL
	GROUP BY 1
)
SELECT DISTINCT
MD5(''EDISON'' || ''-'' || EM.MASTER_ID  || ''-'' || ''EDISON'') AS EMPLOYEE_KEY,
NVL(EM.MASTER_ID, EM.CAREGIVERID)  AS EMPLOYEE_ID,
...
CASE WHEN UPPER(REGEXP_REPLACE(EM.STATE,''([ .]){1,}'','''')) = ''NEWYORK'' THEN ''NY''  
     WHEN LEN(UPPER(REGEXP_REPLACE(EM.STATE,''([ .]){1,}'',''''))) = 2 THEN UPPER(REGEXP_REPLACE(EM.STATE,''([ .]){1,}'','''')) 
     ELSE ''NA'' END AS EMPLOYEE_STATE_CODE,
...
IFF(UPPER(TRIM(EM.ETHNICITY)) ILIKE ANY(''HISPANIC%'',''LATINO%'',''HISPANIC OR LATINO%''), TRUE, FALSE) AS HISPANIC_OR_LATINO,
L."LANGUAGE"  AS EMPLOYEE_LANGUAGE,
CASE WHEN TRIM(UPPER(EM.MARITALSTATUS))=''S'' THEN ''SINGLE''
     	   WHEN TRIM(UPPER(EM.MARITALSTATUS))=''M'' THEN ''MARRIED'' ELSE NULL END AS EMPLOYEE_MARITAL_STATUS,
...
TRIM(EM.CAREGIVERCODE) AS PAYROLL_ID,
...
CASE WHEN UPPER(REGEXP_REPLACE(EM.STATE,''([ .]){1,}'','''')) = ''NEWYORK'' THEN ''NY''  
     WHEN LEN(UPPER(REGEXP_REPLACE(EM.STATE,''([ .]){1,}'',''''))) = 2 THEN UPPER(REGEXP_REPLACE(EM.STATE,''([ .]){1,}'','''')) 
     ELSE ''NA'' END AS WORK_STATE,         ---DO WE HAVE THIS? TAKE CAREGIVER STATE IF BLANK STATE THEN TAKE ZIP STATE
NULL AS SUTA_STATE,
...
''EDISON'' AS PRIMARY_BRANCH_SYSTEM_CODE,
...
FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.EMPLOYEE_MASTER_LIST EM
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.LANGUAGES L ON L.LANGUAGEID = EM.LANGUAGE1
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.OFFICE_OFFICES_REPL  O ON EM.OFFICEID = O.OFFICEID AND EM.AGENCYID = O.AGENCYID 
LEFT  JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.PAYROLL_DETAILS_REPL PD ON PD.CAREGIVERID = EM.CAREGIVERID 
LEFT JOIN CHECK_DATE CD ON CD.CAREGIVERID = EM.CAREGIVERID
WHERE EM.AGENCYID IN (155);
;
    RETURN ''SUCCESS'';
    END;
