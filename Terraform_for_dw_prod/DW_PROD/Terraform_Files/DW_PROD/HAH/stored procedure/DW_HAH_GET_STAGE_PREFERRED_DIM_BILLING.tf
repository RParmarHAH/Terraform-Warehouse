resource "snowflake_procedure" "DW_HAH_GET_STAGE_PREFERRED_DIM_BILLING" {
	name ="GET_STAGE_PREFERRED_DIM_BILLING"
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.STAGE_Payer_REPL PR ON  COALESCE(PR.ContractID,PR.PayerID) = SC.ContractID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.CONTRACTRATES CR ON CR.AGENCYID = SC.AGENCYID
...
LEFT JOIN (SELECT *FROM  DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.TBLINVOICEDETAILS_REPL
