resource "snowflake_procedure" "DW_HAH_MERGE_STAGE_MOLINA_DIM_CLIENT" {
	name ="MERGE_STAGE_MOLINA_DIM_CLIENT"
...
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
-- 02/08/2023    AKASH THAKKER         Added logic for the active flag update
-- 11/08/2023   TRUSHALI RAMOLIYA       ADDED A COLUMN FOR HISPANIC_OR_LATINO AS PER TICKET IDDOX - 388																																   
--*****************************************************************************************************************************
MERGE INTO HAH.DIM_CLIENT TGT 
USING STAGE.MOLINA_DIM_CLIENT STAGE 
...
-- Active Flag Update
-------------------------------------------
-- Inactivating clients who is not part of current roster, but were there in previous roster list
UPDATE HAH.DIM_CLIENT
SET ACTIVE_CLIENT_FLAG = FALSE
WHERE SYSTEM_CODE = ''MOLINA''
        AND ACTIVE_CLIENT_FLAG -- Only active client
        AND CLIENT_KEY NOT IN (SELECT CLIENT_KEY FROM STAGE.MOLINA_DIM_CLIENT);
