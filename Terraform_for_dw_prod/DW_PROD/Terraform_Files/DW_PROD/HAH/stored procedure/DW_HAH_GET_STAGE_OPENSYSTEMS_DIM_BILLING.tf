resource "snowflake_procedure" "DW_HAH_GET_STAGE_OPENSYSTEMS_DIM_BILLING" {
	name ="GET_STAGE_OPENSYSTEMS_DIM_BILLING"
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.STAGE_Payer_REPL PR ON  COALESCE(PR.ContractID,PR.PayerID) = SC.ContractID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.CONTRACTRATES CR ON CR.AGENCYID = SC.AGENCYID
...
LEFT JOIN (SELECT *FROM  DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.TBLINVOICEDETAILS_REPL
