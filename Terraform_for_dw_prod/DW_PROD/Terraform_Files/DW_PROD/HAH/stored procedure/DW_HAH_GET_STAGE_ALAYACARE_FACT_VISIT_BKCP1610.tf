resource "snowflake_procedure" "DW_HAH_GET_STAGE_ALAYACARE_FACT_VISIT_BKCP1610" {
	name ="GET_STAGE_ALAYACARE_FACT_VISIT_BKCP1610"
...
FROM DISC_${var.SF_ENVIRONMENT}.ALAYACARE.AUDIT_VISIT_EMPLOYEE_CHANGE AUDIT
...
FROM
DISC_${var.SF_ENVIRONMENT}.ALAYACARE.VISIT VISITS
LEFT JOIN  DISC_${var.SF_ENVIRONMENT}.ALAYACARE.INVOICE_ITEM I ON
...
EVV_VISIT_EXCEPTION AS (
  ...
  FROM DISC_${var.SF_ENVIRONMENT}.ALAYACARE.EVV_VISIT E,
    LATERAL FLATTEN( INPUT => EXCEPTIONS, OUTER => TRUE) J
  QUALIFY R = 1
),
DEDUPE AS (--to remove records who are duplicates
    SELECT S.SERVICE_CLIENT_ID,F.* 
    FROM DISC_${var.SF_ENVIRONMENT}.ALAYACARE.FUNDING_BLOCK_TIER_4 F
    INNER JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_DATE AS DATES
        ON DATES.CALENDAR_DATE BETWEEN F.FUNDING_BLOCK_START_DATE AND F.FUNDING_BLOCK_END_DATE--JOIN WITH dim_date 63k TO 18k
    LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.SERVICE S ON F.SERVICE_ID= S.SERVICE_ID
...
FROM
DISC_${var.SF_ENVIRONMENT}.ALAYACARE.VISIT AS VISITS
LEFT JOIN INVOICE_STATUS ON
INVOICE_STATUS.VISIT_ID = VISITS.VISIT_ID
LEFT JOIN (SELECT VISIT_ID, MIN(START_TIME) AS START_TIME, MAX(END_TIME) END_TIME FROM DISC_${var.SF_ENVIRONMENT}.ALAYACARE.WORK_SESSION_TIER_4 GROUP BY 1) WORK_SESSION ON 
WORK_SESSION.VISIT_ID = VISITS.VISIT_ID
LEFT JOIN EVV_VISIT_EXCEPTION EVV ON 
EVV.ALAYACARE_VISIT_ID = VISITS.VISIT_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.OFFER_RESPONSE OFFER ON 
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.SERVICE AS SERVICES ON
VISITS.SERVICE_ID = SERVICES.SERVICE_ID
AND VISITS.BRANCH_ID = SERVICES.BRANCH_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.TBL_GUID_COST_CENTRE_TIER_4 COST_CENTRE_MAPPING ON
COST_CENTRE_MAPPING.GUID_TO = C.GUID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.TBL_COST_CENTRES_TIER_4 AS COST_CENTRE ON
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.VISIT_CODE VC ON
...
INNER JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.CONFIGURATION CONFIG ON
