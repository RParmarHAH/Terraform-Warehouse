resource "snowflake_procedure" "DW_HAH_GET_STAGE_SANDATAIMPORT_FACT_ADMISSION" {
	name ="GET_STAGE_SANDATAIMPORT_FACT_ADMISSION"
...
SELECT * FROM (
    ...
    FROM DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_ADMISSIONDIAGNOSES 
 	WHERE AGENCYID = ''8485''
 	AND TRIM("RANK") = ''P''
) WHERE rn=1 ),
--,ADMISSION_PAYOR AS (
...
--		FROM DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_ADMISSIONSPAYORS
--		WHERE AGENCYID = ''8485'' 
--		--AND PAYORID <> 0
--) WHERE PAYOR_RN = 1 )
clu as (
...
ELSE NULL END AS STATUS -- Values taken from Sandata UI
	FROM CLIENT AS CLIENT
         ...
         LEFT JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_REFERRALSOURCES RS
        ...
        --LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_DIAGNOSIS_2_0 AS DIAGNOSIS 
        	--ON  COALESCE(ADM_D.ICD10CODE,regexp_replace(ADM_D.ICD9CODE,''\\\\.'','')) = DIAGNOSIS.DIAGNOSIS_CODE 
		--LEFT JOIN (Select distinct AGENCYID,ADMISSIONID,PAYORID from DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_ADMISSIONSPAYORS) AS PAYOR 
				--ON PAYOR.ADMISSIONID = CA.ADMISSIONID and PAYOR.AGENCYID = CA.AGENCYID  
        --LEFT JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_PAYORS AS PY 	
	    ...
	    --LEFT JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_SERVICES SS 
