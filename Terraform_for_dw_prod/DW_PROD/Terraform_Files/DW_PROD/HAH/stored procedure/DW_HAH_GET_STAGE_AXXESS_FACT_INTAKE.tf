resource "snowflake_procedure" "DW_HAH_GET_STAGE_AXXESS_FACT_INTAKE" {
	name ="GET_STAGE_AXXESS_FACT_INTAKE"
...
WITH DEDUP_LEVERAGE AS (
    ...
    (
        ...
        FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.AXXESS.CLIENT_MASTER_LIST--DISC_DEDUPE_${var.SF_ENVIRONMENT}.ASR.CLIENT_MASTER_LIST 
    )
    UNION
    SELECT * FROM 
    (
        SELECT MASTER_ID, PATIENT_ID, CONCAT(CASE_MANAGER_FIRST_NAME, '' ,'',CASE_MANAGER_LAST_NAME) CASE_MANAGER_NAME
        FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.AXXESS.CLIENT_MATCH_LIST  --DISC_DEDUPE_${var.SF_ENVIRONMENT}.ASR.CLIENT_MATCH_LIST 
...
CLEANED_CLI AS (
    ...
    FROM DEDUP_LEVERAGE --DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_EPISODESNAPSHOTS 
),
...
SELECT MIN(IFNULL(INTAKE.START_DATE, TO_DATE(''1900-01-01'', ''YYYY-MM-DD''))) AS MIN_START,
    ...
    FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_AUTHORIZATIONS INTAKE
...
LEFT JOIN  DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_AUTHORIZATIONS LATEST_AUTH_END
...
LEFT JOIN  DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_AUTHORIZATIONS AUTH
...
FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_PATIENTVISITS
...
--FROM
--    DISC_${var.SF_ENVIRONMENT}.AXXESS.VW_AXXESS_HC_VISITS_UPPER
--WHERE
