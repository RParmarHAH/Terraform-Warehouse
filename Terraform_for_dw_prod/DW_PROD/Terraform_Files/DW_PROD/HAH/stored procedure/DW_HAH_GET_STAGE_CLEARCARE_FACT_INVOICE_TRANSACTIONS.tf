resource "snowflake_procedure" "DW_HAH_GET_STAGE_CLEARCARE_FACT_INVOICE_TRANSACTIONS" {
	name ="GET_STAGE_CLEARCARE_FACT_INVOICE_TRANSACTIONS"
...
FROM DISC_${var.SF_ENVIRONMENT}.CLEARCARE.RECEIVABLES_PAYMENTDESIGNATION PD 
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.PAYMENT_TRANSACTION PT  ON PT.ID = PD.PAYMENT_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.INVOICE_TRANSACTION INV ON PD.INVOICE_ID = INV.ID 
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.RECEIVABLES_INVOICE_CARELOGS  IC ON IC.INVOICE_ID = PD.INVOICE_ID
