resource "snowflake_procedure" "DW_HAH_GET_STAGE_ALAYACARE_DIM_CLIENT" {
	name ="GET_STAGE_ALAYACARE_DIM_CLIENT"
...
(
	...
	-1)::number)
AS REFERRER_KEY
		FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.ALAYACARE.CLIENT_MASTER_LIST c
		LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.PROFILE p ON c.PROFILE_ID =p.PROFILE_ID 
...
SELECT  NVL(C.MASTER_ID::INT,VISITS.CLIENT_ID::INT) AS CLIENT_ID,NVL(C.BRANCH_ID::INT,VISITS.BRANCH_ID::INT ) AS BRANCH_ID ,
	...
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.SERVICE AS SERVICES ON VISITS.SERVICE_ID = SERVICES.SERVICE_ID AND VISITS.BRANCH_ID = SERVICES.BRANCH_ID
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.SERVICE_CODE_BILL_CODE SBC ON SERVICES.SERVICE_FUNDER_ID  = SBC.FUNDER_ID AND VISITS.SERVICE_CODE_ID = SBC.SERVICE_CODE_ID
...
SELECT * FROM DISC_${var.SF_ENVIRONMENT}.AlayaCare.Client_Status CS
JOIN (
SELECT CLIENT_ID AS CLIENT_IDL ,MAX(ID) AS ID,BRANCH_ID  AS BRANCH_IDL
FROM  DISC_${var.SF_ENVIRONMENT}.AlayaCare.Client_Status CS 
...
DISCHARGE_REASON AS (SELECT * FROM (
	...
	* FROM  DISC_${var.SF_ENVIRONMENT}.AlayaCare.Client_Status
)WHERE rn=1) 
,
CLIENT_ONHOLD AS (
SELECT * FROM DISC_${var.SF_ENVIRONMENT}.AlayaCare.Client_Status CS  
...
FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.ALAYACARE.CLIENT_MASTER_LIST AS C
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.CLIENT_TAGS AS CT ON C.CLIENT_ID =CT.CLIENT_ID
