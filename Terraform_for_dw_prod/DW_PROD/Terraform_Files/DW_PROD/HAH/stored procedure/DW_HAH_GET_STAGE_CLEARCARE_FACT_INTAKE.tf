resource "snowflake_procedure" "DW_HAH_GET_STAGE_CLEARCARE_FACT_INTAKE" {
	name ="GET_STAGE_CLEARCARE_FACT_INTAKE"
...
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------        
-- 08/15/2022    AYSHWARYA	           INITIAL DEVELOPMENT
-- 10/06/2023    NUTAN                 Changed logic for hours_authoized_non_adjusted 
--*****************************************************************************************************************************
--

INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.STAGE.CLEARCARE_FACT_INTAKE
WITH
CLIENTKEY 
AS
 (
...
CLIENT_ID,
BILL_RATE_ID
FROM DISC_${var.SF_ENVIRONMENT}.CLEARCARE.BILLING_CLIENTAUTHORIZATION
...
SELECT
	...
	start_date,
	end_date,
	hourspermonth as initial_hours,
	first_day_of_month,
	B.AUTHORIZATION_ID,
	BR.AMOUNT,
...
FROM
	hah.dim_date D
INNER JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.BILLING_CLIENTAUTHORIZATION B ON
	d.calendar_date BETWEEN START_DATE AND END_DATE
INNER JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.BILLING_BILLRATE BR ON
	BR.ID = B.BILL_RATE_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.PATIENT_PATIENT PA ON PA.ID = B.CLIENT_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.AGENCY_AGENCYLOCATION A ON A.ID = PA.LOCATION_ID
WHERE
	period = 1  -- Weekly
UNION
SELECT
...
INNER JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.BILLING_BILLRATE BR ON BR.ID = A.BILL_RATE_ID
INNER JOIN AUTHDATES AD ON AD.CLIENT_ID = A.CLIENT_ID AND AD.BILL_RATE_ID = A.BILL_RATE_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.PROFILE_PARENTPAYERSERVICE PP ON PP.id = A.SERVICE_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.BILLING_ITEM IT ON IT.AUTHORIZATION_ID = A.ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.PATIENT_PATIENT PA ON PA.ID = C.PATIENT_ID
