resource "snowflake_procedure" "DW_HAH_GET_STAGE_OSHAH_FACT_VISIT_EXCEPTION" {
	name ="GET_STAGE_OSHAH_FACT_VISIT_EXCEPTION"
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.STAGE.OSHAH_FACT_VISIT_EXCEPTION
WITH CALLS AS (
	SELECT 
		V.VISITID,
	...
	FROM 
		DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.INREVIEWVISITS CM
	INNER JOIN 
		DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.VISITS V
		ON CM.AGENCYID = V.AGENCYID 
			AND CM.VISITID = V.VISITID
	LEFT JOIN
		DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.OFFICE_OFFICES_REPL OFFICE ON CM.OFFICEID = OFFICE.OFFICEID
	LEFT JOIN 
		DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.TBLVISITS_REPL CRN 
		ON CRN.AGENCYID = V.AGENCYID
			AND CRN.VISITID = V.VISITID
	LEFT JOIN
		DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.REASONS_REPL RS
