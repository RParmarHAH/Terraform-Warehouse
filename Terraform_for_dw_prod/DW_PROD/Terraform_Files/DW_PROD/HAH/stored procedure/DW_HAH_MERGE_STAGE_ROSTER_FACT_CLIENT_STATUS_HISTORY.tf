resource "snowflake_procedure" "DW_HAH_MERGE_STAGE_ROSTER_FACT_CLIENT_STATUS_HISTORY" {
	name ="MERGE_STAGE_ROSTER_FACT_CLIENT_STATUS_HISTORY"
	...
	execute_as = "OWNER"
	statement = <<-EOT
DECLARE RETURN_RESULT VARCHAR;
BEGIN
-----------------------------------------------------------------------
-- 03/10/2023    Pooja Deokate		 Adding new Client Status History
-----------------------------------------------------------------------
MERGE INTO DW_${var.SF_ENVIRONMENT}.HAH.FACT_CLIENT_STATUS_HISTORY AS TGT
USING DW_${var.SF_ENVIRONMENT}.STAGE.ROSTER_FACT_CLIENT_STATUS_HISTORY STAGE
...
--------------------------------------------------------------------
UPDATE DW_${var.SF_ENVIRONMENT}.HAH.FACT_CLIENT_STATUS_HISTORY STATUS_HISTORY
...
FROM DW_${var.SF_ENVIRONMENT}.STAGE.ROSTER_FACT_CLIENT_STATUS_HISTORY ROSTER
WHERE STATUS_HISTORY.CLIENT_KEY NOT IN (SELECT CLIENT_KEY FROM DW_${var.SF_ENVIRONMENT}.STAGE.ROSTER_FACT_CLIENT_STATUS_HISTORY)
