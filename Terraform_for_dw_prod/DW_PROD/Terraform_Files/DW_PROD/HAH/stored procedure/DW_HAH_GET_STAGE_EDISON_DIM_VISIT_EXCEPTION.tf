resource "snowflake_procedure" "DW_HAH_GET_STAGE_EDISON_DIM_VISIT_EXCEPTION" {
	name ="GET_STAGE_EDISON_DIM_VISIT_EXCEPTION"
...
BEGIN
--*****************************************************************************************************************************
-- NAME:  EDISON_DIM_VISIT_EXCEPTION
--
-- PURPOSE: Creates one row per exception according to EDISON
--
-- DEVELOPMENT LOG:
-- DATE			AUTHOR							NOTES:
-- --------		-------------------				-------------------------------------------------------------------------------
...
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.STAGE.EDISON_DIM_VISIT_EXCEPTION
WITH EXCEPTIONS AS (
	SELECT 
		DISTINCT
		...
		ED."STATUS"
	FROM 
		DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.EXCEPTIONDETAILS ED
		)
SELECT
	MD5(SYSTEM_CODE || ''-'' || EXC.EXCEPTIONCODE || ''-'' || ''EDISON'') AS EXCEPTION_KEY,
