resource "snowflake_procedure" "DW_HAH_GET_STAGE_EMPEONEDISON_DIM_EMPLOYEE" {
	name ="GET_STAGE_EMPEONEDISON_DIM_EMPLOYEE"
...
WITH RATE AS (
		SELECT 
			...
			CASE WHEN RATE = 0 
				 ...
				 ELSE RATE 
			END AS PAY_RATE
		FROM DISC_${var.SF_ENVIRONMENT}.EMPEONEDISON.ERATE QUALIFY RANK = 1        
	),
	...
	(
		...
		FROM DISC_${var.SF_ENVIRONMENT}.EMPEONEDISON.EPAYENTRY
	...
	(
		...
		FROM DISC_${var.SF_ENVIRONMENT}.EMPEONEDISON.ESTATUS ES
		LEFT JOIN DISC_${var.SF_ENVIRONMENT}.EMPEONEDISON.CEMPSTATUS CES
...
IFF(UPPER(TRIM(c.DESCRIPTION)) ILIKE ANY(''HISPANIC%'',''LATINO%'',''HISPANIC OR LATINO%''), TRUE, FALSE) AS HISPANIC_OR_LATINO, 
	...
	FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.EMPEONEDISON.EMPLOYEE_MASTER_LIST AS emp_m
	LEFT JOIN EMPLOYEE_CHECK_DATE AS e_check 
		ON TRIM(emp_m.ID) = TRIM(e_check.EMPLOYEE_ID) AND TRIM(emp_m.CO) = TRIM(e_check.CO)
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.EMPEONEDISON.CDEPT1 AS cd
		ON TRIM(emp_m.CC1) = TRIM(cd.CC1) AND TRIM(emp_m.CO) = TRIM(cd.CO)
	LEFT JOIN DISC_${var.SF_ENVIRONMENT}.EMPEONEDISON.CETHNICITY AS c 
...
--	WHERE DW_${var.SF_ENVIRONMENT}.STAGE.SSN_CLEANUP(TRIM(REPLACE(SSN, ''-''))) IS NOT NULL;
