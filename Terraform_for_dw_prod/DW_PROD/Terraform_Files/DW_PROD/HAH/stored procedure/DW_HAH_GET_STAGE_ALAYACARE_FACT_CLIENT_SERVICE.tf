resource "snowflake_procedure" "DW_HAH_GET_STAGE_ALAYACARE_FACT_CLIENT_SERVICE" {
	name ="GET_STAGE_ALAYACARE_FACT_CLIENT_SERVICE"
...
--10/26/2023    Rajat Sapkal             Enhanced authorization_max_units and authorization_max_units_adjusted logic
--*****************************************************************************************************************************
INSERT OVERWRITE INTO STAGE.ALAYACARE_FACT_CLIENT_SERVICE
WITH
CLIENT AS  
(
SELECT * FROM
...
,IFF(CUTOVER.END_DATE < FUNDING_BLOCK_END_DATE, CUTOVER.END_DATE, FUNDING_BLOCK_END_DATE) AS AUTHORIZATION_PERIOD_END_DATE--,DERIVED_FUNDING_BLOCK_END_DATE
 ...
 ,CASE WHEN FUNDING_UNITS =''hours'' THEN ''Hourly'' 
    ...
    --,FUNDING_FREQUENCY AS authorization_uom
     ,CASE WHEN FUNDING_FREQUENCY=''weekly'' THEN ''Weekly''
     WHEN FUNDING_FREQUENCY=''monthly'' THEN ''Monthly''
...
--         ,auth_hrs_per_day* EFFECTIVE_DAYS AS AUTHORIZATION_MAX_UNITS_ADJUSTED
 ...
 ,CASE WHEN FUNDING_FREQUENCY=''weekly'' then (authorization_max_units_by_uom*(EFFECTIVE_DAYS/ 7)) 
    WHEN FUNDING_FREQUENCY=''monthly'' THEN (authorization_max_units_by_uom*(DATEDIFF(''MONTH'',authorization_period_start_date,AUTHORIZATION_PERIOD_END_DATE)+1))
    ...
    ,CASE WHEN FUNDING_FREQUENCY=''weekly'' then (authorization_max_units_by_uom*(EFFECTIVE_DAYS/ 7)) 
          WHEN FUNDING_FREQUENCY=''monthly'' THEN
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.SERVICE S ON AUTH.SERVICE_ID = S.SERVICE_ID
...
--LEFT JOIN (SELECT DISTINCT SERVICE_ID,SERVICE_CODE_ID,VISIT_COMPUTED_RATE_UNITS,BRANCH_ID FROM DISC_${var.SF_ENVIRONMENT}.ALAYACARE.VISIT) VISITS 
--ON VISITS.SERVICE_ID = S.SERVICE_ID AND VISITS.SERVICE_CODE_ID = S.SERVICE_CODE_ID AND VISITS.BRANCH_ID = NVL(S.BRANCH_ID,-1)
--LEFT JOIN (SELECT DISTINCT SERVICE_ID,SERVICE_CODE_ID,VISIT_COMPUTED_RATE_UNITS,BRANCH_ID FROM DISC_${var.SF_ENVIRONMENT}.ALAYACARE.VISIT WHERE SERVICE_ID=3888 
--QUALIFY ROW_NUMBER()OVER(PARTITION BY SERVICE_ID,SERVICE_CODE_ID,BRANCH_ID ORDER BY VISIT_COMPUTED_RATE_UNITS)=1) VISITS 
-- ON VISITS.SERVICE_CODE_ID = S.SERVICE_CODE_ID AND VISITS.BRANCH_ID = NVL(BC.BRANCH_ID,-1)
--LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.BILL_CODE BC ON s.SERVICE_BILL_CODE_ID = BC.BILL_CODE_ID 
--LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.VISIT VISITS ON VISITS.SERVICE_ID = S.SERVICE_ID AND VISITS.CLIENT_ID =s.SERVICE_CLIENT_ID  AND VISITS.START_AT BETWEEN 
