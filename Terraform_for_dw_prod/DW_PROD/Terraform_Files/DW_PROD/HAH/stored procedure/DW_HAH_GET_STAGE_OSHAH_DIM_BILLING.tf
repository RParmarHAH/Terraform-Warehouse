resource "snowflake_procedure" "DW_HAH_GET_STAGE_OSHAH_DIM_BILLING" {
	name ="GET_STAGE_OSHAH_DIM_BILLING"
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.STAGE_Payer_REPL PR ON  COALESCE(PR.ContractID,PR.PayerID) = SC.ContractID
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.CONTRACTRATES CR ON CR.AGENCYID = SC.AGENCYID
    	AND CR.SERVICECODEID = SC.SERVICECODEID
    	AND CR.TODATE :: DATE > GETDATE()
    	AND CR.FROMDATE :: DATE <= GETDATE()
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.VW_OSHAH_REVENUE  vor ON vor.SERVICECODEID = sc.SERVICECODEID
LEFT JOIN (SELECT *FROM  DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.TBLINVOICEDETAILS_REPL
...
WHERE OFF.STATE IN (SELECT STATE FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.CONFIGURATION WHERE CONFIG = TRUE)  --CHANGED PR.STATE TO OFF.STATE
