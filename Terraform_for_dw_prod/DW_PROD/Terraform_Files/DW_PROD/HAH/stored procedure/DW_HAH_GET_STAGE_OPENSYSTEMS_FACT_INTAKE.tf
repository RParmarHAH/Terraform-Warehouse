resource "snowflake_procedure" "DW_HAH_GET_STAGE_OPENSYSTEMS_FACT_INTAKE" {
	name ="GET_STAGE_OPENSYSTEMS_FACT_INTAKE"
...
--***************************************************************************************************************************
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.STAGE.OPENSYSTEMS_FACT_INTAKE
WITH CLIENT AS
(
    SELECT * FROM
    (
...
INNER JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.SERVICECODES SC
...
(
    ...
    FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.PROVIDERPATIENTCOORDINATORS CORD
    WHERE CORD.COORDINATORSRNO  = 1
), OPENSYSTEMS_REVENUE_SEG AS
(
    select * FROM  DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.VW_OPENSYSTEMS_REVENUE
), SERVICE_DATA AS
(
    ...
    INNER JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.VISITS VS
...
INNER JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.PATIENTS P
    ON P.PATIENTID = CLIENT.CLIENT_NUMBER
INNER JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOPENSYSTEMS.OFFICE_OFFICES_REPL O
