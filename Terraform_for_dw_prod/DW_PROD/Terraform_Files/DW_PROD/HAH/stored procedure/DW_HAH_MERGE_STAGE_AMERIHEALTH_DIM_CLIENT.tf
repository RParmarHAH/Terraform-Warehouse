resource "snowflake_procedure" "DW_HAH_MERGE_STAGE_AMERIHEALTH_DIM_CLIENT" {
	name ="MERGE_STAGE_AMERIHEALTH_DIM_CLIENT"
...
USING STAGE.AMERIHEALTH_DIM_CLIENT STAGE 
...
WHERE SYSTEM_CODE = ''AMERIHEALTH''
        AND ACTIVE_CLIENT_FLAG -- Only active client
        AND CLIENT_KEY NOT IN (SELECT CLIENT_KEY FROM STAGE.AMERIHEALTH_DIM_CLIENT);
