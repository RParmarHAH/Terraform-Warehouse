resource "snowflake_procedure" "DW_HAH_GET_STAGE_DATAFLEXSYNCDATA_FACT_INVOICE_TRANSACTIONS" {
	name ="GET_STAGE_DATAFLEXSYNCDATA_FACT_INVOICE_TRANSACTIONS"
    ...
    BEGIN
        ...
        WITH CLIENT_DATA AS (SELECT *
             ...
             INVPAY AS (SELECT REFERENCENO,
                        ...
                        FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFINVOICEPAYMENTS
                        WHERE PERIOD > ''202012''
                        UNION
                        ...
                        FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFPAYMENTS P
                                 JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFPAYMENTDETAILS PD ON P.CLIENTNUMBER = PD.CLIENTNUMBER
        ...
        FROM INVPAY --DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFINVOICEPAYMENTS INVPAY
                 LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.ALL_DFINVOICEDETAILS det ON det.DBNAME = INVPAY.DBNAME
            AND det.INVOICENO = INVPAY.INVOICENO
                 LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFINVOICES inv ON inv.DBNAME = det.DBNAME
            AND inv.INVOICENO = det.INVOICENO
            AND inv.ETL_DELETED_FLAG = FALSE
                 LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFCONTRACTS con ON con.DBNAME = inv.DBNAME
            ...
            AND det.CLIENTNUMBER = cli.Client_number
                 LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFOFFICES oc ON oc.DBNAME = cli.DBNAME
