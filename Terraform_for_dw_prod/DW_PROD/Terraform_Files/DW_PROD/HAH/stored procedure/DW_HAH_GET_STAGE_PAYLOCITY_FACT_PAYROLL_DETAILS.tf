resource "snowflake_procedure" "DW_HAH_GET_STAGE_PAYLOCITY_FACT_PAYROLL_DETAILS" {
	name ="GET_STAGE_PAYLOCITY_FACT_PAYROLL_DETAILS"
...
LEFT JOIN (SELECT * FROM DISC_${var.SF_ENVIRONMENT}.PAYLOCITY.PAY_STATEMENT_DETAILS WHERE AMOUNT <> 0) PSD 
