resource "snowflake_procedure" "DW_HAH_GET_STAGE_EDISON_DIM_BILLING" {
	name ="GET_STAGE_EDISON_DIM_BILLING"
...
FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.SERVICECODES SC
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.Contracts c  on SC.ContractID = c.ContractID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.Payer_REPL PR ON  COALESCE(PR.ContractID,PR.PayerID) = SC.ContractID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.CONTRACTRATES CR ON CR.AGENCYID = SC.AGENCYID
    	AND CR.SERVICECODEID = SC.SERVICECODEID
    	AND CR.TODATE :: DATE > GETDATE()
    	AND CR.FROMDATE :: DATE <= GETDATE()
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.VW_EDISON_REVENUE_SEG  vor ON vor.SERVICECODEID = sc.SERVICECODEID
LEFT JOIN (SELECT *FROM  DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEDISON.TBLINVOICEDETAILS_REPL
