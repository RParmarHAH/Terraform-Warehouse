resource "snowflake_procedure" "DW_HAH_GET_STAGE_GPSYNCDATA_DIM_EMPLOYEE" {
	name ="GET_STAGE_GPSYNCDATA_DIM_EMPLOYEE"
...
--FROM DISC_${var.SF_ENVIRONMENT}.GPSyncData.GPAllEmployeesBase AS emp2
...
(
	...
	FROM DISC_${var.SF_ENVIRONMENT}.DataFlexSyncData.DFTimeSheets AS ts
	INNER JOIN DISC_${var.SF_ENVIRONMENT}.DataFlexSyncData.DFTimeSheetHours AS tsh 
		ON ts.DBName = tsh.DBName AND ts.ReferenceNo = tsh.ReferenceNo
	LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DataFlexSyncData.DFEmployees AS emp 
...
RATE_BASE_PAY_RATE AS (
	...
	FROM DISC_${var.SF_ENVIRONMENT}.GPSYNCDATA.GPALLEMPLOYEERATESBASE 
...
ADMIN_PAY_RATE AS (
	SELECT EMPLOYEE_ID, GP_HOURLY_RATE, GP_ANNUAL_SALARY FROM (
        ...
        FROM DISC_${var.SF_ENVIRONMENT}.GPSYNCDATA.GPALLEMPLOYEERATESBASE erb
        INNER JOIN DISC_${var.SF_ENVIRONMENT}.GPSYNCDATA.GPALLEMPLOYEESBASE emp
...
--	END AS Employee_Home_Phone,
	CASE 
		WHEN DW_${var.SF_ENVIRONMENT}.STAGE.PHONE_CLEANUP(emp.Phone_Number) NOT IN (SELECT Phone_Number FROM Data_Management.Data_Quality.Invalid_Phone_Number) AND DW_${var.SF_ENVIRONMENT}.STAGE.PHONE_CLEANUP(emp.Phone_Number) NOT LIKE ''%0000000%'' AND DW_${var.SF_ENVIRONMENT}.STAGE.PHONE_CLEANUP(emp.Phone_Number) IS NOT NULL
			THEN DW_${var.SF_ENVIRONMENT}.STAGE.PHONE_CLEANUP(emp.Phone_Number)
		WHEN DW_${var.SF_ENVIRONMENT}.STAGE.PHONE_CLEANUP(dfe.PhoneNumber) NOT IN (SELECT Phone_Number FROM Data_Management.Data_Quality.Invalid_Phone_Number) AND DW_${var.SF_ENVIRONMENT}.STAGE.PHONE_CLEANUP(dfe.PhoneNumber) NOT LIKE ''%0000000%'' AND DW_${var.SF_ENVIRONMENT}.STAGE.PHONE_CLEANUP(dfe.PhoneNumber) IS NOT NULL
			THEN DW_${var.SF_ENVIRONMENT}.STAGE.PHONE_CLEANUP(dfe.PhoneNumber)
		ELSE NULL
	END AS Employee_Home_Phone,
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.GPSYNCDATA.GPSYNC_EMPLOYEES_EMAIL EMA
	ON TRIM(EMA.SSN) = TRIM(EMP.SSN)
LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DfEmployees AS dfe ON TRIM( dfe.PayrollID) = TRIM( emp.Employee_ID) AND TRIM( dfe.DBName) = TRIM( emp.Office_State) AND TRIM( emp.DB) <> ''HAHIL''
