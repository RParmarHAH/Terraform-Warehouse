resource "snowflake_procedure" "DW_HAH_GET_STAGE_ASR_FACT_INTAKE" {
	name ="GET_STAGE_ASR_FACT_INTAKE"
...
--*****************************************************************************************************************************
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.STAGE.ASR_FACT_INTAKE
...
, WEEKLY_INTAKES AS (
	SELECT
		...
		NULL AS PICKER
	FROM
		DISC_${var.SF_ENVIRONMENT}.ASR.ASR_PLANOFSERVICE PL
	LEFT JOIN
...
, CASE_WORKERS AS (
	...
	FROM
		DISC_DEDUPE_${var.SF_ENVIRONMENT}.ASR.CLIENT_MASTER_LIST CML
	LEFT JOIN
		DISC_${var.SF_ENVIRONMENT}.ASR.ASR_CASEWORKERS CW ON TRIM(CW.CASEWORKERID) = TRIM(CML.CASEWORKERID)
)
,SERVICE_DATES AS 
(
	SELECT 
	...
	FROM 
		CLIENT C
	LEFT JOIN 
		DISC_${var.SF_ENVIRONMENT}.ASR.ASR_VISITS V ON TRIM(C.CUSTOMERID) = TRIM(V.CUSTOMERID)
	LEFT JOIN 
		DISC_${var.SF_ENVIRONMENT}.ASR.BRANCH_MAPPING BR ON V.LOCATIONID = BR.OLD_LOCATION_ID
