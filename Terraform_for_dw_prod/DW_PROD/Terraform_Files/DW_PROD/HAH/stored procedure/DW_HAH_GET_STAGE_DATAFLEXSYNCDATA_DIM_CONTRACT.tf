resource "snowflake_procedure" "DW_HAH_GET_STAGE_DATAFLEXSYNCDATA_DIM_CONTRACT" {
	name ="GET_STAGE_DATAFLEXSYNCDATA_DIM_CONTRACT"
...
with sl as (
    ...
    from (
             ...
             FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFCONTRACTS F
         ) t
    group by 1,2
)
SELECT
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFREVENUESUBCATEGORIES AS DFREVSUBCAT
