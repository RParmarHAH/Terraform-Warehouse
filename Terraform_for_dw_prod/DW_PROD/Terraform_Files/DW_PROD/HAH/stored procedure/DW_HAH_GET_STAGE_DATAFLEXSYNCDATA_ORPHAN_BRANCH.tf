resource "snowflake_procedure" "DW_HAH_GET_STAGE_DATAFLEXSYNCDATA_ORPHAN_BRANCH" {
	name ="GET_STAGE_DATAFLEXSYNCDATA_ORPHAN_BRANCH"
...
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.STAGE.DATAFLEXSYNCDATA_ORPHAN_BRANCH

--There is null values for column - OFFICENO in DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.ALL_DFTIMESHEETS  for respective column-  DBNAME in DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.ALL_DFTIMESHEETS.
...
WHERE  f.DBNAME || ''-'' || COALESCE(ts.OffceNo, -1) NOT IN (SELECT DISTINCT OFFICES.DBNAME || ''-'' || OFFICES.OFFICENUMBER FROM DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFOFFICES offices)
UNION 
--NO RESPECTIVE VALUE FOR COLUMN  - INTID(NULL) FOR TABLE DISC_${var.SF_ENVIRONMENT}.PUBLIC.VW_DATAFLEX_SANDATA_PENDING_VISITS
SELECT 
