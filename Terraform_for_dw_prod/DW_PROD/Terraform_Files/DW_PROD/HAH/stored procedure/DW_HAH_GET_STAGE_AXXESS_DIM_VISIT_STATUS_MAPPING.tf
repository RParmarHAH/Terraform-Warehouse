resource "snowflake_procedure" "DW_HAH_GET_STAGE_AXXESS_DIM_VISIT_STATUS_MAPPING" {
	name ="GET_STAGE_AXXESS_DIM_VISIT_STATUS_MAPPING"
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_FINANCIALTRANSACTIONS FT ON TD.CLAIM_ID =FT.CLAIM_ID --AND FT.TRANSACTION_DATE =FIN.TRANSACTION_DATE
)T
),
DERIVED_SCH AS
(
...
FROM VISITS
--LEFT JOIN DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_INVOICE_STATUS_MAPPING INV ON VISITS.INVOICE_STATUS=UPPER(TRIM(INV.FINAL_OR_MANAGEDCARE_STATUS ))
)
