resource "snowflake_procedure" "DW_HAH_GET_STAGE_TRUSTPOINTDATA_DIM_BRANCH" {
	name ="GET_STAGE_TRUSTPOINTDATA_DIM_BRANCH"
...
INSERT OVERWRITE INTO DW_${var.SF_ENVIRONMENT}.STAGE.TRUSTPOINTDATA_DIM_BRANCH
...
SELECT
	...
	TO_DATE(''9999-12-31'', ''YYYY-MM-DD'') AS EFFECTIVE_TO_DATE,
		...
		TRY_CAST( ''FALSE'' AS BOOLEAN )  as ETL_DELETED_FLAG,
		TRY_CAST( ''FALSE'' AS BOOLEAN ) as ETL_INFERRED_MEMBER_FLAG
FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_BRANCH B WHERE SOURCE_SYSTEM_ID IN(4)
UNION
SELECT
...
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_BRANCH br
...
FROM ALL_BRANCHES 
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_SOURCE_SYSTEM AS SOURCE_SYSTEM ON ALL_BRANCHES.SOURCE_SYSTEM_ID = SOURCE_SYSTEM.SOURCE_SYSTEM_ID;
