resource "snowflake_procedure" "DW_HAH_GET_STAGE_ASR_DIM_INVOICE" {
...
LEFT JOIN (SELECT BILLINGTRANSACTIONID  FROM  DISC_${var.SF_ENVIRONMENT}.ASR.ASR_PAYMENTSRECEIVED
WHERE CLAIMSTATUS = 1 
...
LEFT JOIN 
    DISC_${var.SF_ENVIRONMENT}.ASR.ASR_PAYERSOURCES P ON UPPER(P.PAYERSOURCEID) = UPPER(RSB.PAYERSOURCEID)
LEFT JOIN 
   DISC_${var.SF_ENVIRONMENT}.ASR.ASR_SUPERVISORS S ON S.SUPERVISORID=CLI.SUPERVISORID
LEFT JOIN 
   INVOICE_STATUS_MAPPING ISM ON ISM.INVOICE_NO = RSB.INVOICENUMBER
LEFT JOIN 
   DW_${var.SF_ENVIRONMENT}.HAH.DIM_INVOICE_STATUS_MAPPING DISM ON DISM.INVOICE_STATUS_KEY = ISM.INVOICE_STATUS_KEY
