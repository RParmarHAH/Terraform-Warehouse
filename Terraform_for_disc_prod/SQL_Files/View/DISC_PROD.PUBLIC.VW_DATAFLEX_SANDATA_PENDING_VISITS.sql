create or replace view DISC_PROD.PUBLIC.VW_DATAFLEX_SANDATA_PENDING_VISITS(
	VISIT_KEY,
	AGENCYID,
	SCHEDULEID,
	SCHEDULEDATE,
...
, ETL_LAST_UPDATED_DATE 
FROM DISC_PROD.SANDATAIMPORT.SANDATA_SCHEDULES 
...
, IFF(COMMENTS = '', NULL, COMMENTS) AS COMMENTS
, ETL_LAST_UPDATED_DATE 
FROM DISC_PROD.SANDATAIMPORT.SANDATA_VISITS S 
...
LEFT JOIN (SELECT DISTINCT AGENCYID, ADMISSIONID, FIRST_VALUE(SERVICEID)OVER(PARTITION BY AGENCYID, ADMISSIONID ORDER BY ID DESC ) SERVICEID 
			FROM DISC_PROD.SANDATAIMPORT.SANDATA_ADMISSIONSERVICES ) ADS 
...
LEFT JOIN DISC_PROD.SANDATAIMPORT.SANDATA_STAFFS SS 
...
LEFT JOIN DISC_PROD.SANDATAIMPORT.SANDATA_EVENTCANCELS CN 
...
This is done in the following manner:
	...
	7. The Admissiontype/Clientadmittype and serviceid is checked in DISC_PROD.SANDATAEXCHANGE.OPENSAM_CLIENTADMISSIONTYPES table which has a relevant 
	   DF CONTRACTCODE & BILLCODE
	8. The LocationID from Sandata is checked in DISC_PROD.BI_REPOSITORY.EXTERNALIDS table which consists their respective DF OFFICENO. The locationid can directly 
