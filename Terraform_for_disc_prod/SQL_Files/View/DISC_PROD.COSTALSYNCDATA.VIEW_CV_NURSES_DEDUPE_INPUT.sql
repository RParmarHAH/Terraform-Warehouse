create or replace view DISC_PROD.COSTALSYNCDATA.VIEW_CV_NURSES_DEDUPE_INPUT(
	ID,
	DB,
	NURSE_ID,
	LAST_NAME,
...
FROM DISC_DEV.CostalSyncData.GPAllEmployeesBase AS gpe
WHERE TRIM( UPPER( gpe.DB)) || TRIM( UPPER( gpe.Employee_ID)) || TRIM( gpe.SSN) NOT IN (SELECT TRIM( UPPER( gpe2.DB)) || TRIM( UPPER( gpe2.Employee_ID)) || TRIM(gpe2.SSN) FROM DISC_DEV.CostalSyncData.GPAllEmployeesBase AS gpe2 WHERE TRIM( UPPER( gpe2.Reason_to_Term)) LIKE ANY ( '%DUPLICATE%PROFILE%', '%WRONG%ID%', '%CODE%IN%USE%', '%WRONG%CODE%'))
...
FROM DISC_DEV.CostalSyncData.GPAllEmployeesBase AS gpe
INNER JOIN DISC_DEV.Stage.Coastal_Office_Mapping_Payroll AS comp ON comp.DB = gpe.DB AND comp.Department = gpe.Department
...
AND TRIM(UPPER(gpe.DB)) || TRIM(UPPER(gpe.Employee_ID)) || TRIM(gpe.SSN) NOT IN (SELECT TRIM(UPPER(gpe2.DB)) || TRIM(UPPER(gpe2.Employee_ID)) || TRIM(gpe2.SSN) FROM DISC_DEV.CostalSyncData.GPAllEmployeesBase AS gpe2 WHERE TRIM(UPPER(gpe2.Reason_to_Term)) LIKE ANY ('%DUPLICATE%PROFILE%', '%WRONG%ID%', '%CODE%IN%USE%', '%WRONG%CODE%'))
)
...
FROM DISC_PROD.COSTALSYNCDATA.CV_NURSES cve
