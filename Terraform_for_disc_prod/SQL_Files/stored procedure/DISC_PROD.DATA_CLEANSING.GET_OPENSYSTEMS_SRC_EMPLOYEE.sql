CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.GET_OPENSYSTEMS_SRC_EMPLOYEE("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
FROM DISC_PROD.HHAEXCHANGEOpenSystems.Caregivers AS car
LEFT JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.OFFICE_OFFICES_REPL  O ON car.OFFICEID = O.OFFICEID AND car.AGENCYID = O.AGENCYID
LEFT JOIN DISC_PROD.HHAEXCHANGEOpenSystems.TBLVISITS_REPL AS VISIT ON car.CaregiverID = visit.CaregiverID
