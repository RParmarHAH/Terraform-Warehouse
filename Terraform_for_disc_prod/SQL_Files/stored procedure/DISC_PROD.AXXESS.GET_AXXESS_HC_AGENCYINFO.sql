CREATE OR REPLACE PROCEDURE DISC_PROD.AXXESS.GET_AXXESS_HC_AGENCYINFO("TASKKEY" VARCHAR(16777216))
...
--*****************************************************************************************************************************
-- NAME:  DISC_PROD.AXXESS.GET_AXXESS_HC_AGENCYINFO 
--
...
--*****************************************************************************************************************************
 ...
 MERGE INTO DISC_PROD.AXXESS.HIST_AXXESS_HC_AGENCYINFO TARGET
USING (
    SELECT 
        A.$1 AS Agency_Id, 
    ...
    FROM @DISC_PROD.stage.AWSAZSTAGEPROD/Axxess/SFTP_files/AXXESS_HC_AGENCYINFO(file_format => DISC_PROD.STAGE.PSV_FORMAT) A) SOURCE
...
WHEN NOT MATCHED THEN
 ...
 UPDATE DISC_PROD.AXXESS.HIST_AXXESS_HC_AGENCYINFO
 SET ETL_DELETED_FLAG = TRUE, ETL_LAST_UPDATED_DATE = CURRENT_TIMESTAMP(),ETL_LAST_UPDATED_BY = CURRENT_USER()
 WHERE Agency_Id NOT IN ( SELECT A.$1 FROM @DISC_PROD.stage.AWSAZSTAGEPROD/Axxess/SFTP_files/AXXESS_HC_AGENCYINFO(file_format => DISC_PROD.STAGE.PSV_ZIPCODES) A);

 CREATE OR REPLACE TABLE DISC_PROD.AXXESS.AXXESS_HC_AGENCYINFO AS SELECT * FROM DISC_PROD.AXXESS.HIST_AXXESS_HC_AGENCYINFO WHERE 
 ETL_DELETED_FLAG=FALSE AND (Agency_Id, ETL_INSERTED_DATE) IN (SELECT Agency_Id, MAX(ETL_INSERTED_DATE)FROM 
 DISC_PROD.AXXESS.HIST_AXXESS_HC_AGENCYINFO GROUP BY Agency_Id);
