CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_VAULT.MDM_DIM_EMPLOYEE_TP("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
BEGIN       
    INSERT INTO DW_PROD.HAH.MDM_DIM_EMPLOYEE
with employee_TP as 
(SELECT *
...
FROM DISC_PROD.DATA_VAULT.VW_SAT_EMPLOYEE_TRUSTPOINT emp),
...
(
    ...
    FROM DISC_PROD.DATA_VAULT.VW_SAT_EMPLOYEE_SANDATA SNEMP
    INNER JOIN DISC_PROD.DATA_VAULT.LINK_EMPLOYEE LNKEMP
...
(
    ...
    WHERE HASH_EMPLOYEE_KEY NOT IN (
      SELECT HASH_EMPLOYEE_KEY FROM DISC_PROD.DATA_VAULT.LINK_EMPLOYEE LNKEMP
...
FROM DISC_PROD.DATA_VAULT.HUB_EMPLOYEE EMP_HUB
...
and stf.rnk_ssn = 1
LEFT OUTER JOIN DW_PROD.HAH.DIM_Branch AS brnch ON brnch.SOURCE_SYSTEM_ID = 6 AND brnch.OFFICE_NUMBER = tp.EMPLOYEE_OFFICE_NUMBER
LEFT OUTER JOIN DW_PROD.HAH.DIM_BRANCH AS BRANCH ON BRANCH.SYSTEM_CODE = stf.AGENCYID AND BRANCH.OFFICE_CODE = stf.HOMELOCATION
LEFT OUTER JOIN DW_PROD.HAH.DIM_State AS st ON UPPER(st.State_ISO_Code) = IFF( TRIM(UPPER(tp.Employee_State_Code))='''', UPPER(stf.State), TRIM( UPPER( tp.Employee_State_Code)))
LEFT OUTER JOIN DW_PROD.HAH.DIM_Zip_Code AS zip ON UPPER( zip.Zip_Code) = IFF( TRIM( UPPER(tp.Employee_Zip))='''', UPPER(stf.Zip), TRIM( UPPER(tp.Employee_Zip)))
...
and stf.rnk_ssn = 1
LEFT OUTER JOIN DW_PROD.HAH.DIM_BRANCH AS BRANCH ON BRANCH.SYSTEM_CODE = stf.AGENCYID AND BRANCH.OFFICE_CODE = stf.HOMELOCATION
LEFT OUTER JOIN DISC_PROD.TrustPointData.Employees AS tp ON stf.AGENCYID = ''8485'' AND tp.EMPLOYEE_NUMBER = TRY_CAST(stf.STAFFAGENCYID AS INTEGER) 
LEFT OUTER JOIN DW_PROD.HAH.DIM_State AS st ON st.State_ISO_Code = IFF( TRIM( UPPER( stf.State))='''', UPPER( tp.Employee_State_Code), TRIM( UPPER( stf.State)))
LEFT OUTER JOIN DW_PROD.HAH.DIM_Zip_Code AS zip ON UPPER( zip.Zip_Code) = IFF( TRIM( UPPER( stf.Zip))='''', UPPER( tp.Employee_Zip), TRIM( UPPER( stf.Zip)))
