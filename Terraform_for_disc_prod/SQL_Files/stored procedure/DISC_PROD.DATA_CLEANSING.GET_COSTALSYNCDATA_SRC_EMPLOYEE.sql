CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.GET_COSTALSYNCDATA_SRC_EMPLOYEE("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
FROM DISC_PROD.COSTALSYNCDATA.CV_NURSES AS  CVE
LEFT JOIN DISC_PROD.COSTALSYNCDATA.GPALLEMPLOYEESBASE AS  GPE 
ON TRIM(CVE.DB) = TRIM(GPE.DB) AND trim(gpe.EMPLOYEE_ID) = trim(cve.NURSE_ID)
LEFT JOIN DISC_PROD.COSTALSYNCDATA.CV_SCHEDULES AS  S 
