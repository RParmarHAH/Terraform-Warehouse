CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.GET_ALAYACARE_SRC_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
FROM DISC_PROD.Alayacare.Client AS c
LEFT OUTER JOIN DW_PROD.HAH.DIM_Zip_Code AS z ON z.Zip_Code = c.Profile_Zip
LEFT JOIN DISC_PROD.Alayacare.Visit AS v ON v.Client_ID = c.Client_ID
LEFT JOIN DISC_PROD.ALAYACARE.TBL_GUID_COST_CENTRE_TIER_4 COST_CENTRE_MAPPING ON COST_CENTRE_MAPPING.GUID_TO = C.GUID
LEFT JOIN DISC_PROD.ALAYACARE.TBL_COST_CENTRES_TIER_4 AS COST_CENTRE ON COST_CENTRE.ID = COST_CENTRE_MAPPING.COST_CENTRE_ID
LEFT JOIN DISC_PROD.ALAYACARE.GROUPS AS G ON G.BRANCH_ID = COST_CENTRE.PROPERTIES_BRANCH_ID
LEFT JOIN DISC_PROD.ALAYACARE.BRANCH B ON B.BRANCH_ID = COALESCE(C.BRANCH_ID, G.BRANCH_ID)
