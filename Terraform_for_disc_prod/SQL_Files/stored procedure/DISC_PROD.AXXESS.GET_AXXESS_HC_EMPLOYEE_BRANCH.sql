CREATE OR REPLACE PROCEDURE DISC_PROD.AXXESS.GET_AXXESS_HC_EMPLOYEE_BRANCH("TASKKEY" VARCHAR(16777216))
...
--*****************************************************************************************************************************
-- NAME:  DISC_PROD.AXXESS.GET_AXXESS_HC_EMPLOYEE_BRANCH 
--
...
MERGE INTO DISC_PROD.AXXESS.HIST_AXXESS_HC_EMPLOYEE_BRANCH TARGET 
USING (
    SELECT 
        A.$1 AS Account_Id, 
        A.$2 AS Account_Name, 
    ...
    FROM @DISC_PROD.stage.AWSAZSTAGEPROD/Axxess/SFTP_files/AXXESS_HC_EMPLOYEE_BRANCH(file_format => DISC_PROD.STAGE.PSV_FORMAT) A
) SOURCE
...
WHEN NOT MATCHED THEN
 ...
 UPDATE DISC_PROD.AXXESS.HIST_AXXESS_HC_EMPLOYEE_BRANCH
 SET ETL_DELETED_FLAG = TRUE, ETL_LAST_UPDATED_DATE = CURRENT_TIMESTAMP(),ETL_LAST_UPDATED_BY = CURRENT_USER()
 WHERE Account_Id NOT IN ( SELECT A.$1 FROM @DISC_PROD.stage.AWSAZSTAGEPROD/Axxess/SFTP_files/AXXESS_HC_EMPLOYEE_BRANCH(file_format => DISC_PROD.STAGE.PSV_ZIPCODES) A);


 CREATE OR REPLACE TABLE DISC_PROD.AXXESS.AXXESS_HC_EMPLOYEE_BRANCH AS SELECT * FROM DISC_PROD.AXXESS.HIST_AXXESS_HC_EMPLOYEE_BRANCH WHERE 
 ETL_DELETED_FLAG=FALSE AND (Account_Id, ETL_INSERTED_DATE) IN (SELECT Account_Id, MAX(ETL_INSERTED_DATE)FROM 
 DISC_PROD.AXXESS.HIST_AXXESS_HC_EMPLOYEE_BRANCH GROUP BY Account_Id);
