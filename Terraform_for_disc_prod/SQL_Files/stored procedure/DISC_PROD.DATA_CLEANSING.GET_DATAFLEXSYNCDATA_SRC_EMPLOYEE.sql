CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.GET_DATAFLEXSYNCDATA_SRC_EMPLOYEE("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  DATAFLEXSYNCDATA_SRC_Employee
--
-- PURPOSE: Creates script for data cleanse for DATAFLEXSYNCDATA source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
,TRIM( CAST( e.DBNAME AS VARCHAR)) AS SYSTEM_CODE,
TRIM( CAST( e.Number AS VARCHAR)) AS Employee_ID, UPPER( TRIM( e.FirstName)) AS Employee_First_Name,
UPPER( TRIM( e.LastName)) AS Employee_Last_Name,
UPPER( TRIM( IFNULL( TRIM( e.ADDRESS1) , '''') ||'' ''|| IFNULL( TRIM( e.ADDRESS2), ''''))) AS Src_Employee_Address
,UPPER( TRIM( e.CITY)) AS Src_Employee_City
,CASE WHEN ST.STATE_ISO_CODE IS NOT NULL AND UPPER( TRIM( e.STATE)) REGEXP ''[A-Z][A-Z]'' THEN ST.STATE_ISO_CODE ELSE e.DBNAME END AS Src_Employee_STATE_CODE
...
,UPPER( TRIM( e.PhoneNumber)) AS Src_Employee_Home_Phone
...
,MIN( IFNULL( tsh.ServiceDate, CAST( ''2040-12-31'' AS DATE))) AS Src_Employee_Min_Activity_Date,
MAX( IFNULL( tsh.ServiceDate, CAST( ''1900-01-01'' AS DATE))) AS Src_Employee_Max_Activity_Date
...
LEFT JOIN DW_PROD.HAH.DIM_STATE ST ON ST.STATE_ISO_CODE = e.STATE
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFTimeSheets AS TS ON e.DBName = ts.DBName AND e.Number = ts.EmployeeNumber
LEFT JOIN DISC_PROD.DataFlexSyncData.DfTimeSheetHours AS tsh ON tsh.DBName = ts.DBName AND tsh.ReferenceNo = ts.ReferenceNo
