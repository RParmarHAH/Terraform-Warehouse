CREATE OR REPLACE PROCEDURE DISC_PROD.AXXESS.GET_AXXESS_HH_BRANCHINFO("TASKKEY" VARCHAR(16777216))
RETURNS VARCHAR(16777216)
LANGUAGE SQL
EXECUTE AS OWNER
AS '
--*****************************************************************************************************************************
-- NAME:  DISC_PROD.AXXESS.GET_AXXESS_HH_BranchInfo 
--
...
WHEN NOT MATCHED THEN
	...
	UPDATE DISC_PROD.AXXESS.HIST_AXXESS_HH_BRANCHINFO SET
	ETL_DELETED_FLAG=TRUE, ETL_LAST_UPDATED_DATE=CURRENT_TIMESTAMP(),ETL_LAST_UPDATED_BY=CURRENT_USER()
	WHERE Branch_Id NOT IN (SELECT A.$1 FROM @DISC_PROD.stage.AWSAZSTAGEPROD/Axxess/SFTP_files/AXXESS_HH_BRANCHINFO(file_format => DISC_PROD.STAGE.PSV_FORMAT) A);
	
	CREATE OR REPLACE TABLE DISC_PROD.AXXESS.AXXESS_HH_BRANCHINFO AS SELECT * FROM DISC_PROD.AXXESS.HIST_AXXESS_HH_BRANCHINFO WHERE 
   ETL_DELETED_FLAG=FALSE AND (Branch_Id,ETL_INSERTED_DATE) IN (SELECT Branch_Id,MAX(ETL_INSERTED_DATE)FROM 
   DISC_PROD.AXXESS.HIST_AXXESS_HH_BRANCHINFO GROUP BY Branch_Id);
