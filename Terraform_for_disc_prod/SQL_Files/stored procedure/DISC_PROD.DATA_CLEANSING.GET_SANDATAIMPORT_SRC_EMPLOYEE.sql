CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.GET_SANDATAIMPORT_SRC_EMPLOYEE("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
FROM DISC_PROD.SandataImport.Sandata_Staffs AS c
LEFT JOIN DW_PROD.HAH.DIM_STATE S ON S.STATE_ISO_CODE=UPPER(TRIM(C.STATE))
LEFT JOIN DISC_PROD.SandataImport.SANDATA_SchedulesStaffs AS SS ON ss.AGENCYID = C.AGENCYID AND ss.StaffID = C.StaffID
LEFT JOIN DISC_PROD.SANDATAIMPORT.SANDATA_SCHEDULES AS sch ON ss.AGENCYID = sch.AGENCYID AND ss.SCHEDULEID = sch.SCHEDULEID
