CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.GET_DATAFLEXSYNCDATA_SRC_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
LEFT JOIN DW_PROD.HAH.DIM_STATE ST ON ST.STATE_ISO_CODE = c.STATE
LEFT JOIN DISC_PROD.DATAFLEXSYNCDATA.DFTimeSheets AS TS ON c.DBName = ts.DBName AND c.Number = ts.ClientNumber
