CREATE OR REPLACE PROCEDURE DISC_PROD.SANDATAEXCHANGE.GET_DFIMPORT_DATAFLEXTIMESHEETVISITS("TASKKEY" VARCHAR(16777216))
RETURNS VARCHAR(16777216)
LANGUAGE SQL
EXECUTE AS OWNER
AS ' 
--*****************************************************************************************************************************
-- NAME:  DISC_PROD.SanDataExchange.GET_DfImport_DataFlexTimeSheetVisits 
--
...
BEGIN 
    --TargetSQL
    COPY INTO DISC_PROD.SanDataExchange.HIST_DfImport_DataFlexTimeSheetVisits FROM (SELECT t.$1 AS Id, t.$2 AS TimesheetId, t.$3 AS ServiceDate, t.$4 AS DateAbbreviation, t.$5 AS DeltaHours, t.$6 AS SYS_CHANGE_VERSION, t.$7 AS SYS_CHANGE_OPERATION, :TaskKey AS ETL_TASK_KEY, :TaskKey AS ETL_INSERTED_TASK_KEY, current_timestamp::TIMESTAMP_NTZ(9) AS ETL_INSERTED_DATE, current_user() AS ETL_INSERTED_BY, current_timestamp::TIMESTAMP_NTZ(9) AS ETL_LAST_UPDATED_DATE, current_user() AS ETL_LAST_UPDATED_BY, DECODE(t.$7,''D'', True, False) AS ETL_DELETED_FLAG FROM @DISC_PROD.STAGE.AWSAZSTAGEPROD/To_Be_Processed/SANDATAEXCHANGE/ (PATTERN => ''.*SanDataExchange_DfImport_DataFlexTimeSheetVisits.*[.]csv.gz'' , FILE_FORMAT=>DISC_PROD.STAGE.MY_CSV_FORMAT) t);
