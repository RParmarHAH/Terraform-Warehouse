CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.GET_OSHAH_SRC_EMPLOYEE("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
FROM DISC_PROD.HHAEXCHANGEOSHAH.Caregivers AS car
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.OFFICE_OFFICES_REPL  O ON car.OFFICEID = O.OFFICEID AND car.AGENCYID = O.AGENCYID 
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.TBLVISITS_REPL AS VISIT ON car.CaregiverID = visit.CaregiverID
LEFT JOIN DISC_PROD.HHAEXCHANGEOSHAH.VISITS VI ON VI.MISSEDVISIT = ''N'' AND VI.VISITID = VISIT.VISITID
