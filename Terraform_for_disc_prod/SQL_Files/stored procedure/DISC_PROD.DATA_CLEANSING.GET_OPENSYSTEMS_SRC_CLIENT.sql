CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.GET_OPENSYSTEMS_SRC_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  OPENSYSTEMS_SRC_CLIENT
--
-- PURPOSE: Creates script for data cleanse for OPENSYSTEMS source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
FROM DISC_PROD.HHAEXCHANGEOpenSystems.Stage_Patients AS pt
INNER JOIN DISC_PROD.HHAEXCHANGEOPENSYSTEMS.OFFICE_OFFICES_REPL OFFICE ON pt.OFFICEID = OFFICE.OFFICEID
LEFT JOIN DISC_PROD.HHAEXCHANGEOpenSystems.TBLVISITS_REPL AS VISIT ON pt.PatientID = visit.PatientID
LEFT JOIN DISC_PROD.HHAEXCHANGEOpenSystems.VISITS VI ON VI.MISSEDVISIT = ''N'' AND VI.VISITID = VISIT.VISITID
