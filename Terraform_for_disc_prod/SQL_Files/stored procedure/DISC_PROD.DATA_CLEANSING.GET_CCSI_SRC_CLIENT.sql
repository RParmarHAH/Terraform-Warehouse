CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.GET_CCSI_SRC_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
FROM  DISC_PROD.CCSI.MASTERCLIENT_LATEST M
LEFT JOIN DW_PROD.HAH.DIM_STATE ST1 ON ST1.STATE_ISO_CODE = ''IL'' 
LEFT JOIN DW_PROD.HAH.DIM_STATE ST2 ON ST1.STATE_ISO_CODE IS NULL AND UPPER(ST2.STATE_NAME) = ''IL''
LEFT JOIN DW_PROD.HAH.DIM_ZIP_CODE Z ON ST1.STATE_ISO_CODE IS NULL AND ST2.STATE_ISO_CODE IS NULL AND Z.ZIP_CODE = LEFT(TRIM(M.ZIP), 5)
LEFT JOIN DISC_PROD.CCSI.RAWVRFP R ON M.CLIENT_IDOA_NO = R.CLIENT_ID  AND TRIM(M.RECORD_NUMBER) = TRIM(R.RECORD_NUMBER)
