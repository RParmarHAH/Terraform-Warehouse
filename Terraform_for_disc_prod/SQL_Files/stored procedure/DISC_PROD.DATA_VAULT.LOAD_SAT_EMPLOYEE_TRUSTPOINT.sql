CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_VAULT.LOAD_SAT_EMPLOYEE_TRUSTPOINT("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
BEGIN
	    INSERT INTO DATA_VAULT.HUB_EMPLOYEE 
					...
					FROM DISC_PROD.TRUSTPOINTDATA.EMPLOYEES emp left join 
...
WHERE emp.ETL_LAST_UPDATED_DATE >= ''1900-01-01''  and (EMPLOYEE_ssn not like ''000000%'') and (EMPLOYEE_ssn is not NULL) and (EMPLOYEE_ssn<>'''')
					...
					FROM DISC_PROD.TRUSTPOINTDATA.EMPLOYEES em
	               ...
	               CREATE OR REPLACE TABLE DATA_VAULT.SAT_EMPLOYEE_TRUSTPOINT AS 
					SELECT
						...
						, tp.*,P.PAYROLL_DATE
					FROM DISC_PROD.TRUSTPOINTDATA.EMPLOYEES tp 
					 left join 
					 (SELECT TRY_CAST(EMPLOYEE_ID as BIGINT) EMPLOYEE_ID,max(PAYROLL_DATE) PAYROLL_DATE
						FROM DISC_PROD.TRUSTPOINTDATA.PAYROLL
						GROUP BY 1
	            ...
	            CREATE OR REPLACE TABLE DATA_VAULT.VW_SAT_EMPLOYEE_TRUSTPOINT AS
					WITH latest as (SELECT HASH_EMPLOYEE_KEY, ''8485'' DB,MAX(ETL_LAST_UPDATED_DATE) max_date
						FROM DISC_PROD.DATA_VAULT.SAT_EMPLOYEE_TRUSTPOINT GROUP BY 1,2)
					SELECT sat.*
					FROM DISC_PROD.DATA_VAULT.SAT_EMPLOYEE_TRUSTPOINT sat
	               ...
	               INSERT INTO DATA_VAULT.LINK_EMPLOYEE
					WITH new_rec AS ( select 
						...
						FROM DISC_PROD.TRUSTPOINTDATA.EMPLOYEES T LEFT OUTER JOIN DISC_PROD.SandataImport.Sandata_Staffs AS stf 
