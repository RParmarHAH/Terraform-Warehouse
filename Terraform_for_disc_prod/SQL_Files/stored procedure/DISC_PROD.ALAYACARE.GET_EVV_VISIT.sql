CREATE OR REPLACE PROCEDURE DISC_PROD.ALAYACARE.GET_EVV_VISIT()
...
//    Insert Records in Hist table
    COPY INTO DISC_PROD.ALAYACARE.HIST_EVV_VISIT FROM (
      SELECT
        DISTINCT
      ...
      FROM @DISC_PROD.STAGE.AWSAZSTAGEPROD/Alayacare/EVV_Visit/alayacare_evv_visit (file_format => DISC_PROD.STAGE.MY_JSON_FORMAT) j
    );
              
//     Merge records in current view table
    MERGE INTO DISC_PROD.ALAYACARE.EVV_VISIT TGT
    USING ( 
            SELECT *, 
                ROW_NUMBER() OVER (PARTITION BY ALAYACARE_VISIT_ID ORDER BY IFF(APIRESPONSE:claim_status::varchar = ''void'', 0, 1) DESC) R  
                FROM DISC_PROD.ALAYACARE.HIST_EVV_VISIT
            WHERE ETL_LAST_UPDATED_DATE = (SELECT MAX(ETL_LAST_UPDATED_DATE) FROM DISC_PROD.ALAYACARE.HIST_EVV_VISIT)
            QUALIFY R = 1
          ) STAGE
