CREATE OR REPLACE PROCEDURE DISC_PROD.AXXESS.GET_AXXESS_HH_AUTHORIZATION("TASKKEY" VARCHAR(16777216))
RETURNS VARCHAR(16777216)
LANGUAGE SQL
EXECUTE AS OWNER
AS ' 
--*****************************************************************************************************************************
-- NAME:  DISC_PROD.AXXESS.GET_AXXESS_HH_AUTHORIZATION 
--
...
WHEN NOT MATCHED THEN
		...
		UPDATE DISC_PROD.AXXESS.HIST_AXXESS_HH_AUTHORIZATION SET
	ETL_DELETED_FLAG=TRUE, ETL_LAST_UPDATED_DATE=CURRENT_TIMESTAMP(),ETL_LAST_UPDATED_BY=CURRENT_USER()
	WHERE Authorization_Detail_Id NOT IN (SELECT A.$1 FROM @DISC_PROD.STAGE.AWSAZSTAGEPROD/Axxess/SFTP_files/AXXESS_HH_Authorization (file_format => DISC_PROD.STAGE.PSV_FORMAT) A);

   CREATE OR REPLACE TABLE DISC_PROD.AXXESS.AXXESS_HH_AUTHORIZATION AS SELECT * FROM DISC_PROD.AXXESS.HIST_AXXESS_HH_AUTHORIZATION WHERE 
   ...
   DISC_PROD.AXXESS.HIST_AXXESS_HH_AUTHORIZATION GROUP BY Authorization_Detail_Id);
END;
';