CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.GET_PREFERRED_SRC_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
,Z2.LNG AS Z2_LNG
,Z2.LAT AS Z2_LAT
FROM DISC_PROD.HHAEXCHANGEPreferred.Stage_Patients AS pt
INNER JOIN DISC_PROD.HHAEXCHANGEPREFERRED.AGENCY_MAPPING AGENCY_CONFIG ON AGENCY_CONFIG.AGENCYID = pt.AGENCYID AND AGENCY_CONFIG.ISACTIVE = TRUE
LEFT JOIN DISC_PROD.HHAEXCHANGEPreferred.TBLVISITS_REPL AS VISIT ON pt.PatientID = visit.PatientID
