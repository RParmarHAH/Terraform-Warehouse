CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.GET_GENERATIONSALLIANCE_SRC_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  GENERATIONSALLIANCE_SRC_CLIENT
--
-- PURPOSE: Creates script for data cleanse for GENERATIONSALLIANCE source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
TRIM( CAST(f.CLIENTID AS VARCHAR)) AS Client_ID, 
UPPER( TRIM(NVL(F.FIRSTNAME, ''''))) AS Client_First_Name, 
UPPER(TRIM(NVL(F.LASTNAME, ''''))) AS Client_Last_Name,
UPPER( IFNULL( TRIM(F.ADDRESS1) , '''') ||'' ''|| IFNULL( TRIM( F.ADDRESS2), '''')) AS SRC_Client_ADDRESS ,
COALESCE(UPPER(TRIM(F.CITY)), GEO.CITY_PRIMARY_NAME) AS SRC_Client_CITY,
COALESCE(GEO.STATE_ISO_CODE, UPPER(NULLIF(TRIM(f.STATE),''''''''))) AS SRC_Client_STATE_CODE,
UPPER( TRIM( F.ZIP))  AS SRC_Client_ZIP,
CAST( NULL AS VARCHAR) AS Src_Client_Zip4,
UPPER( TRIM(F.SSN)) AS Src_Client_SSN,
TRY_TO_DATE(TRIM(F.DATEOFBIRTH)) AS Src_Client_DOB,
UPPER( TRIM( f.PHONE)) AS Src_Client_Home_Phone,
UPPER(TRIM(f.PHONE2)) AS Src_Client_Cell_Phone,
...
MIN( IFNULL(DATE_TRUNC (DAY, SCH."DATE"), CAST( ''2040-12-31'' AS DATE))) AS SRC_Client_Min_Activity_Date,
MAX( IFNULL( DATE_TRUNC (DAY, SCH."DATE"), CAST( ''1900-01-01'' AS DATE))) AS SRC_Client_Max_Activity_Date,
...
FROM DISC_PROD.GENERATIONSALLIANCE.CLIENT F 
LEFT JOIN DW_PROD.HAH.DIM_GEOGRAPHY GEO 
	ON GEO.ZIP_CODE = LEFT(f.ZIP,5)
LEFT JOIN DISC_PROD.GENERATIONSALLIANCE.SCHEDULES SCH ON F.CLIENTID = SCH.CLIENTID 
