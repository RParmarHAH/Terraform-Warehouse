CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.GET_MATRIXCARE_SRC_EMPLOYEE("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
FROM DISC_PROD.MATRIXCARE.STVHC_T_Caregiver car
LEFT JOIN DISC_PROD.MATRIXCARE.STVHC_T_SCHEDULES AS SCH ON sch.SCH_CAREGIVERID = car.car_ID
LEFT JOIN DW_PROD.HAH.DIM_STATE ST1 ON ST1.STATE_ISO_CODE = car.car_STATEORPROVINCE 
LEFT JOIN DW_PROD.HAH.DIM_STATE ST2 ON ST1.STATE_ISO_CODE IS NULL AND UPPER(ST2.STATE_NAME) = UPPER(Car.Car_STATEORPROVINCE)
LEFT JOIN DW_PROD.HAH.DIM_ZIP_CODE Z ON ST1.STATE_ISO_CODE IS NULL AND ST2.STATE_ISO_CODE IS NULL AND Z.ZIP_CODE = LEFT(TRIM(car.car_POSTALCODE), 5)
