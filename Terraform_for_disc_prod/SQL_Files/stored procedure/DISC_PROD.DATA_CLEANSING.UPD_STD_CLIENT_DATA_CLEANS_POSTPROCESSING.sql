CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.UPD_STD_CLIENT_DATA_CLEANS_POSTPROCESSING()
...
/*Update SRC_CLIENT_ADDRESS_PROCESSED_FLAG from CLIENT_STANDARDIZED_ADDRESSES basis of CLIENT_ID  */
	...
	UPDATE DISC_PROD.DATA_CLEANSING."CLIENT_STD_ATTRIBUTES" 
    FROM (
    SELECT 
    ...
    FROM 
    DISC_PROD.DATA_CLEANSING."CLIENT_STANDARDIZED_ADDRESSES" ADD_OUTPT
    JOIN DISC_PROD.DATA_CLEANSING."CLIENT_STD_ATTRIBUTES" STD_ATTRBT
    ...
    WHERE 
    DISC_PROD.DATA_CLEANSING."CLIENT_STD_ATTRIBUTES"."SOURCE_SYSTEM_ID"  = X.SOURCE_SYSTEM_ID
    AND DISC_PROD.DATA_CLEANSING."CLIENT_STD_ATTRIBUTES".SYSTEM_CODE = X.SYSTEM_CODE
    AND DISC_PROD.DATA_CLEANSING."CLIENT_STD_ATTRIBUTES".CLIENT_ID = X.CLIENT_ID ;

	
/* Update SRC_CLIENT_ADDRESS_SUCCESSFULLY_CLEANSED_FLAG where  METHD_UPD is not null */
	
	UPDATE DISC_PROD.DATA_CLEANSING."CLIENT_STD_ATTRIBUTES"  FROM 
	(
	SELECT 
	"SOURCE_SYSTEM_ID" ,
	"SYSTEM_CODE" ,
	"CLIENT_ID" 
	FROM 
	DISC_PROD.DATA_CLEANSING."CLIENT_STD_ATTRIBUTES" 
	WHERE 
	...
	WHERE 
	DISC_PROD.DATA_CLEANSING."CLIENT_STD_ATTRIBUTES"."SOURCE_SYSTEM_ID"  = xx.SOURCE_SYSTEM_ID
	AND DISC_PROD.DATA_CLEANSING."CLIENT_STD_ATTRIBUTES".SYSTEM_CODE = xx.SYSTEM_CODE
	AND DISC_PROD.DATA_CLEANSING."CLIENT_STD_ATTRIBUTES".CLIENT_ID = xx.CLIENT_ID ;
...
/* PROCESSED and CLEANSED FLAG ALONG WITH OTHER ADDRESS WHICH HAS SAME ADDRESS within table and will update whose CLEANSE_FLG is FALSE  */
	...
	SRC_CLIENT_ZIP FROM  DISC_PROD.DATA_CLEANSING."CLIENT_STD_ATTRIBUTES"  
...
/* Update Process flag which has not cleansed successfully with all attribute*/
	...
	(SELECT 
		...
		LONGITUDE ,
		RELEVANCE 
	FROM DISC_PROD.DATA_CLEANSING."CLIENT_STD_ATTRIBUTES"
...
/* FLAG UPDATE FROM  STANDARD ATTRIBUTE TO SOURCE ATTRIBUTE  */
	...
	UPDATE  DISC_PROD.DATA_CLEANSING."CLIENT_SRC_ATTRIBUTES" FROM 
	(SELECT  
	...
	FROM 
	DISC_PROD.DATA_CLEANSING."CLIENT_SRC_ATTRIBUTES" SRC
	JOIN DISC_PROD.DATA_CLEANSING."CLIENT_STD_ATTRIBUTES" STD 
	...
	WHERE 
	DISC_PROD.DATA_CLEANSING."CLIENT_SRC_ATTRIBUTES"."SOURCE_SYSTEM_ID"  = XX.SOURCE_SYSTEM_ID
	AND DISC_PROD.DATA_CLEANSING."CLIENT_SRC_ATTRIBUTES"."SYSTEM_CODE"  = XX.SYSTEM_CODE
	AND DISC_PROD.DATA_CLEANSING."CLIENT_SRC_ATTRIBUTES"."CLIENT_ID"  = XX.CLIENT_ID ;
