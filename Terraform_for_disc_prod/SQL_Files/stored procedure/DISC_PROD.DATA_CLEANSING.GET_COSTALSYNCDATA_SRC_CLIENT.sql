CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.GET_COSTALSYNCDATA_SRC_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
FROM DISC_PROD.COSTALSYNCDATA.CV_PATIENTS AS F
LEFT JOIN DISC_PROD.COSTALSYNCDATA.CV_SCHEDULES AS  SC 
ON TRIM(F.DB) = TRIM(SC.DB) AND trim(SC.PATIENT_NUMBER) = trim(F.PATIENT_NUMBER)
LEFT JOIN DW_PROD.HAH.DIM_ZIP_CODE Z ON Z.ZIP_CODE=TRIM(F.ZIPCODE)
LEFT JOIN DW_PROD.HAH.DIM_STATE S ON S.STATE_ISO_CODE=UPPER(TRIM(F.STATE_CODE))
