CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.GET_SANDATAIMPORT_SRC_CLIENT("STR_ETL_TASK_KEY" VARCHAR(16777216))
...
FROM DISC_PROD.SandataImport.Sandata_Clients AS c
LEFT JOIN DW_PROD.HAH.DIM_STATE S ON S.STATE_ISO_CODE=UPPER(TRIM(C.STATE))
LEFT JOIN DISC_PROD.BI_Repository.SandataVisits AS SV ON sv.AGENCYID = C.AGENCYID AND sv.ClientID = C.ClientID
LEFT JOIN DISC_PROD.SANDATAIMPORT.SANDATA_SCHEDULES AS sch ON sv.AGENCYID = sch.AGENCYID AND sv.SCHEDULEID = sch.SCHEDULEID
