CREATE OR REPLACE PROCEDURE DISC_PROD.PUBLIC.LOAD_SAT_EMPLOYEE_SANDATA("STR_ETL_TASK_KEY" VARCHAR(16777216), "STR_CDC_START" VARCHAR(16777216), "STR_CDC_END" VARCHAR(16777216))
...
BEGIN
	INSERT INTO DATA_VAULT.HUB_EMPLOYEE
					WITH base AS (SELECT st.ssn, st.STAFFAGENCYID AS EMPLOYEE_ID,
						...
						FROM DISC_PROD.SANDATAIMPORT.SANDATA_STAFFS st
						...
						FROM DISC_PROD.SANDATAIMPORT.SANDATA_STAFFS st
						WHERE st.ETL_LAST_UPDATED_DATE >= ''1900-01-01'' and AGENCYID=''8485'' AND SSN=''''
						AND TRY_CAST(st.STAFFAGENCYID AS integer) NOT IN (SELECT E.EMPLOYEE_NUMBER FROM DISC_PROD.TRUSTPOINTDATA.EMPLOYEES E )
						)
					SELECT
...
--						, SDEmp.ID as EMPLOYEE_ID
					...
					FROM DISC_PROD.SANDATAIMPORT.SANDATA_STAFFS SDEmp
...
FROM DISC_PROD.TRUSTPOINTDATA.EMPLOYEES s
)
WHERE SDEmp.AGENCYID=''8485'' AND (SDEmp.ssn NOT LIKE ''000000%'') AND (SDEmp.ssn NOT LIKE ''999%'')
...
SELECT CONCAT (''MESSAGE : '',"number of rows inserted",'' Rows Inserted.'') into :return_result FROM TABLE(RESULT_SCAN(LAST_QUERY_ID()));
					...
					SELECT
						COALESCE(tp.HASH_EMPLOYEE_KEY, MD5(iff(sd.ssn='''',sd.STAFFAGENCYID,sd.ssn))) AS HASH_EMPLOYEE_KEY
						, sd.*
					FROM DISC_PROD.SANDATAIMPORT.SANDATA_STAFFS sd
					LEFT JOIN DISC_PROD.DATA_VAULT.HUB_EMPLOYEE tp ON sd.STAFFAGENCYID = tp.EMPLOYEE_ID
						AND sd.AGENCYID=''8485'' AND tp.RECORD_SOURCE_SYSTEM_ID = ''6''
					WHERE sd.EMAIL NOT LIKE ''%@test.com'' AND sd.AGENCYID=''8485'' 
					AND (sd.ssn<>'''' or sd.STAFFAGENCYID<>'''' ) AND (sd.ssn NOT LIKE ''000000%'') AND (sd.ssn NOT LIKE ''999%'');
				
	
	            CREATE OR REPLACE TABLE DATA_VAULT.VW_SAT_EMPLOYEE_SANDATA AS
					WITH latest as (SELECT HASH_EMPLOYEE_KEY,STAFFAGENCYID, 
									RANK() OVER (PARTITION BY iff(SSN='''',STAFFAGENCYID,SSN) ORDER BY IFF(st.UPDATEDAT::date=''1900-01-01'', ''9999-12-31'',st.UPDATEDAT::date) DESC,
									...
									FROM DISC_PROD.DATA_VAULT.SAT_EMPLOYEE_SANDATA st )
					SELECT sat.*
					FROM DISC_PROD.DATA_VAULT.SAT_EMPLOYEE_SANDATA sat
...
--                      SELECT MD5(NVL(s.AGENCYID,''S'') || ''-'' || TRY_CAST(s.STAFFAGENCYID AS Integer) || ''-'' || ''1900-01-01'' || ''-'' || ''SANDATAIMPORT'') AS LINK_EMPLOYEE_KEY
							 ...
							 , CURRENT_USER AS ETL_INSERTED_BY
							 , 0 AS ETL_DELETED_FLAG
						FROM DISC_PROD.SANDATAIMPORT.SANDATA_STAFFS s
						...
						WHERE s.AGENCYID=''8485'' and  (s.ssn<>'''' or s.STAFFAGENCYID<>'''' ) AND (s.ssn NOT LIKE ''000000%'') AND (s.ssn NOT LIKE ''999%'') 
						      and (TRY_CAST(s.STAFFAGENCYID AS Integer) not in (select tp.EMPLOYEE_NUMBER from DISC_PROD.TRUSTPOINTDATA.EMPLOYEES tp))
					)
					SELECT ct.*
					FROM new_rec ct
