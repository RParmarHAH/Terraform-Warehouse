CREATE OR REPLACE PROCEDURE DISC_PROD.DATA_CLEANSING.MERGE_STAGE_DAT_CLEANSING_STD_CLIENT()
...
MERGE INTO DISC_PROD.DATA_CLEANSING.CLIENT_STD_ATTRIBUTES AS tgt
USING
(
SELECT DISTINCT addr.SOURCE_SYSTEM_ID, addr.SYSTEM_CODE, addr.CLIENT_ID, addr.CLIENT_FIRST_NAME, addr.CLIENT_LAST_NAME, 
       ...
       DISC_PROD.Data_Cleansing.SSN_Validation( SRC_Client_SSN) AS Std_Client_SSN,
       ...
       DISC_PROD.DATA_CLEANSING.PHONE_NUMBER_VALIDATION( addr.SRC_Client_HOME_PHONE) AS Std_Client_Home_Phone,
       ...
       DISC_PROD.DATA_CLEANSING.PHONE_NUMBER_VALIDATION( addr.SRC_Client_Cell_PHONE) AS Std_Client_Cell_Phone,
       ...
       DISC_PROD.DATA_CLEANSING.PHONE_NUMBER_VALIDATION( addr.SRC_Client_Work_PHONE) AS Std_Client_Work_Phone,
       ...
       DISC_PROD.DATA_CLEANSING.EMAIL_VALIDATION( addr.SRC_Client_Personal_Email) AS Std_Client_Personal_Email,
       ...
       DISC_PROD.DATA_CLEANSING.EMAIL_VALIDATION( addr.SRC_Client_Work_Email) AS Std_Client_Work_Email,
...
FROM DISC_PROD.DATA_CLEANSING.CLIENT_SRC_ATTRIBUTES AS addr
LEFT OUTER JOIN 
( SELECT  *  FROM DISC_PROD.DATA_CLEANSING.CLIENT_STANDARDIZED_ADDRESSES 
WHERE 
(source_system_id,system_code,client_number) NOT IN (
SELECT  source_system_id,system_code,client_number
FROM 
DISC_PROD.DATA_CLEANSING.CLIENT_STANDARDIZED_ADDRESSES
WHERE 
(ADDRESS ,CITY,STATE,ZIP )
 IN (SELECT  ADDRESS ,CITY,STATE,ZIP   FROM DISC_PROD.DATA_CLEANSING.CLIENT_STANDARDIZED_ADDRESSES_INCNSTNT )
 OR  COUNTRY_CODE <> ''USA''
 OR zip <> ZIPCODE
 OR LENGTH (zip) <> 5
