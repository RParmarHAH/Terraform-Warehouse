resource "snowflake_procedure" "DISC_SANDATAEXCHANGE_GET_DFIMPORT_DATAFLEXTIMESHEETSANDATAVISITMAPPINGS" {
	name ="GET_DFIMPORT_DATAFLEXTIMESHEETSANDATAVISITMAPPINGS"
...
--*****************************************************************************************************************************
-- NAME:  DISC_${var.SF_ENVIRONMENT}.SanDataExchange.GET_DfImport_DataFlexTimeSheetSanDataVisitMappings 
--
...
BEGIN 
    --TargetSQL
    COPY INTO DISC_${var.SF_ENVIRONMENT}.SanDataExchange.HIST_DfImport_DataFlexTimeSheetSanDataVisitMappings FROM (SELECT t.$1 AS TRACKING_ID, t.$2 AS SCHEDULE_ID, t.$3 AS SYSTEM_CODE, t.$4 AS SANDATA_AGENCY_ID, t.$5 AS SYS_CHANGE_VERSION, t.$6 AS SYS_CHANGE_OPERATION, :TaskKey AS ETL_TASK_KEY, :TaskKey AS ETL_INSERTED_TASK_KEY, current_timestamp::TIMESTAMP_NTZ(9) AS ETL_INSERTED_DATE, current_user() AS ETL_INSERTED_BY, current_timestamp::TIMESTAMP_NTZ(9) AS ETL_LAST_UPDATED_DATE, current_user() AS ETL_LAST_UPDATED_BY, DECODE(t.$6,''D'', True, False) AS ETL_DELETED_FLAG FROM @DISC_${var.SF_ENVIRONMENT}.STAGE.AWSAZSTAGEPROD/To_Be_Processed/SANDATAEXCHANGE/ (PATTERN => ''.*SanDataExchange_DfImport_DataFlexTimeSheetSanDataVisitMappings.*[.]csv.gz'' , FILE_FORMAT=>DISC_${var.SF_ENVIRONMENT}.STAGE.MY_CSV_FORMAT) t);
