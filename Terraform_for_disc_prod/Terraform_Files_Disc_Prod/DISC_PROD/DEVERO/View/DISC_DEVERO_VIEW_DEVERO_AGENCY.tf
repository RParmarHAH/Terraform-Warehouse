resource "snowflake_view" "DISC_DEVERO_VIEW_DEVERO_AGENCY" {
	database = "DISC_${var.SF_ENVIRONMENT}"
	schema = "DEVERO"
	name = "VIEW_DEVERO_AGENCY"
	statement = <<-SQL
	 (
WITH RESULT_DATA AS 
(
SELECT 	DISTINCT
  ...
  FROM DISC_${var.SF_ENVIRONMENT}.DEVERO.HIST_${var.SF_ENVIRONMENT}ERO_AGENCY  A
--WHERE DATE(A.ETL_LAST_UPDATED_DATE) = COALESCE((
--SELECT MAX(DATE(ETL_LAST_UPDATED_DATE)) FROM DISC_${var.SF_ENVIRONMENT}.DEVERO.HIST_${var.SF_ENVIRONMENT}ERO_AGENCY),DATE(GETDATE()))
)
SELECT AGENCY_ID,
