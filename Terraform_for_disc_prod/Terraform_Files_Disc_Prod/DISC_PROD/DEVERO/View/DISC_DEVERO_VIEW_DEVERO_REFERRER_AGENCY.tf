resource "snowflake_view" "DISC_DEVERO_VIEW_DEVERO_REFERRER_AGENCY" {
	database = "DISC_${var.SF_ENVIRONMENT}"
	schema = "DEVERO"
	name = "VIEW_DEVERO_REFERRER_AGENCY"
	statement = <<-SQL
	 (
WITH RESULT_DATA AS
(
SELECT     DISTINCT
  ...
  FROM DISC_${var.SF_ENVIRONMENT}.DEVERO.HIST_${var.SF_ENVIRONMENT}ERO_REFERRER  R
, LATERAL FLATTEN(XMLGET(PARSE_XML(R.REFERRER_XML), 'AGENCY_IDS'):"$":: ARRAY) REFERRER
--WHERE DATE(R.ETL_LAST_UPDATED_DATE) = COALESCE((
--SELECT MAX(DATE(ETL_LAST_UPDATED_DATE)) FROM DISC_${var.SF_ENVIRONMENT}.DEVERO.HIST_${var.SF_ENVIRONMENT}ERO_REFERRER),DATE(GETDATE()))
)
SELECT REFERRER_ID,
    AGENCY_ID,
    DATE_MODIFIED,
