resource "snowflake_view" "DISC_DEVERO_VIEW_DEVERO_FORM" {
	database = "DISC_${var.SF_ENVIRONMENT}"
	schema = "DEVERO"
	name = "VIEW_DEVERO_FORM"
	statement = <<-SQL
	 (
WITH XML_DATA AS 
(
	SELECT FORM_ID,FORM_XML
		...
		, ETL_LAST_UPDATED_BY
		, ETL_DELETED_FLAG
		, ROW_NUMBER() OVER(PARTITION BY FORM_ID ORDER BY DATE_MODIFIED DESC) AS LATEST_MODIFIED FROM DISC_${var.SF_ENVIRONMENT}.DEVERO.HIST_${var.SF_ENVIRONMENT}ERO_FORM 
