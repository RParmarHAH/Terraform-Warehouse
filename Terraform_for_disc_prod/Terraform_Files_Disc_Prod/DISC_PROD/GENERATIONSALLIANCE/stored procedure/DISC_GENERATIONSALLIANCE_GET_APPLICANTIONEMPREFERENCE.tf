resource "snowflake_procedure" "DISC_GENERATIONSALLIANCE_GET_APPLICANTIONEMPREFERENCE" {
	name ="GET_APPLICANTIONEMPREFERENCE"
...
--*****************************************************************************************************************************
-- NAME:  DISC_${var.SF_ENVIRONMENT}.GENERATIONSALLIANCE.GET_ApplicantionEmpReference 
--
...
BEGIN 
    --TargetSQL
    COPY INTO GENERATIONSALLIANCE.ApplicantionEmpReference FROM (SELECT t.$1 AS id, t.$2 AS Name, t.$3 AS Relationship, t.$4 AS YearsKnown, t.$5 AS Phone, t.$6 AS AppId, t.$7 AS SYS_CHANGE_VERSION, t.$8 AS SYS_CHANGE_OPERATION, :TaskKey AS ETL_TASK_KEY, :TaskKey AS ETL_INSERTED_TASK_KEY, current_timestamp::TIMESTAMP_NTZ(9) AS ETL_INSERTED_DATE, current_user() AS ETL_INSERTED_BY, current_timestamp::TIMESTAMP_NTZ(9) AS ETL_LAST_UPDATED_DATE, current_user() AS ETL_LAST_UPDATED_BY, DECODE(t.$8,''D'', True, False) AS ETL_DELETED_FLAG FROM @DISC_${var.SF_ENVIRONMENT}.STAGE.AWSAZSTAGEPROD/To_Be_Processed/GENERATIONSALLIANCE/ (PATTERN => ''.*GENERATIONSALLIANCE_dbo_ApplicantionEmpReference.*[.]csv.gz'' , FILE_FORMAT=>DISC_${var.SF_ENVIRONMENT}.STAGE.PSV_FORMAT) t);
