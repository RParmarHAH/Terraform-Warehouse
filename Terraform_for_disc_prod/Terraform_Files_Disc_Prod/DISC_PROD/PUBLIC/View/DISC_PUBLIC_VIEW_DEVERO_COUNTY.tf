resource "snowflake_view" "DISC_PUBLIC_VIEW_DEVERO_COUNTY" {
...
WHERE DATE(C.ETL_LAST_UPDATED_DATE) = (
SELECT MAX(DATE(ETL_LAST_UPDATED_DATE)) FROM DISC_${var.SF_ENVIRONMENT}.DEVERO.HIST_${var.SF_ENVIRONMENT}ERO_COUNTY)
)
SELECT 
 COUNTY_ID,
	HAS_CODE,
