resource "snowflake_view" "DISC_HHAEXCHANGEOSHAH_VW_OSHAH_REVENUE" {
	database = "DISC_${var.SF_ENVIRONMENT}"
	schema = "HHAEXCHANGEOSHAH"
	name = "VW_OSHAH_REVENUE"
	statement = <<-SQL
...
-- DATE			AUTHOR					NOTES:
-- --------		-------------------		---------------------------------------------------------------------------------------
-- 08/22/2023	Mirisha			        Added Revenue_code logic
--
WITH REVENUE_CODE AS 
(SELECT SC.SERVICECODEID, 
CASE WHEN UPPER(TRIM(SC.REVENUECODE)) ='HOME CARE' THEN 'HC'
WHEN (UPPER(TRIM(SC.REVENUECODE)) IN ('-','--','') OR SC.REVENUECODE IS NULL) THEN 'UNKNOWN'
WHEN UPPER(TRIM(SC.REVENUECODE))='N/A' THEN 'NA'
ELSE UPPER(TRIM(SC.REVENUECODE)) END AS REVENUE_CODE
...
SC.SERVICECODE,
SC.DISCIPLINE,
CASE WHEN SC.DISCIPLINE IN ('HCSS','HSK','PCA','RESP', 'HMK','COMP','Other (Non Skilled)','Other(Non Skilled)','CNA') THEN 'HC'
...
FROM 
DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.SERVICECODES SC
LEFT JOIN REVENUE_CODE RV ON RV.SERVICECODEID= SC.SERVICECODEID 
);
SQL
	or_replace = true 
	is_secure = false 
