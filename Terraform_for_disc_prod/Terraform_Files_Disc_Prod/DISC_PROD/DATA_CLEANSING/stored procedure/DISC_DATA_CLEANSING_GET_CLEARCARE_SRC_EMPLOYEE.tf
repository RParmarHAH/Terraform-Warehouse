resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_CLEARCARE_SRC_EMPLOYEE" {
	name ="GET_CLEARCARE_SRC_EMPLOYEE"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  CLEARCARE_SRC_Employee
--
-- PURPOSE: Creates script for data cleanse for CLEARCARE source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
SELECT DISTINCT 
     ...
     ,CAST(PR.CAREGIVER_ID AS VARCHAR) AS Employee_ID 
     ,UPPER( TRIM( PR.FIRST_NAME)) AS Employee_First_Name
     ,UPPER( TRIM( PR.LAST_NAME)) AS Employee_Last_Name
     ,UPPER( TRIM( IFNULL( TRIM( PR.ADDRESS) , '''') ||'' ''|| IFNULL( TRIM( EM.ADDRESS_LINE_2), ''''))) AS Src_Employee_Address
     ,UPPER(TRIM(PR.CITY)) AS  SRC_Employee_CITY
     ,UPPER( TRIM(EM.STATE)) AS SRC_Employee_STATE_CODE
     ,UPPER(TRIM(EM.ZIP)) AS Src_Employee_Zip
	 ,CAST( NULL AS VARCHAR) AS Src_Employee_Zip4
	 ,CAST( NULL AS VARCHAR)  AS Src_Employee_SSN
     ,TO_DATE(PR.DOB) as Src_Employee_DOB
     ,UPPER( TRIM( PR.PHONE)) AS Src_Employee_Home_Phone
     ,UPPER( TRIM( EM.MOBILE_PHONE)) AS Src_Employee_Cell_Phone
	,UPPER( TRIM( EM.WORK_PHONE)) AS Src_Employee_Work_Phone
	,UPPER( TRIM( EM.EMAIL)) AS Src_Employee_Personal_Email
	,UPPER( TRIM( EM.EMAIL)) AS Src_Employee_Work_Email
    ,MIN( COALESCE( CAST( CC.SHIFT_DATE AS DATE), CAST( CC.CCI_ACTUAL_CLOCK_IN AS DATE))) AS SRC_Employee_Min_Activity_Date
    ,MAX( COALESCE( CAST( CC.SHIFT_DATE AS DATE), CAST(CC.CCI_ACTUAL_CLOCK_OUT AS DATE))) AS SRC_Employee_Max_Activity_Date
 ...
 FROM DISC_${var.SF_ENVIRONMENT}.CLEARCARE.VM_CAREGIVER_CAREGIVER AS PR 
 LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.profile_patient_agencylocation AS EM ON PR.CAREGIVER_ID = EM.CAREGIVER_ID
 LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CLEARCARE.carelogs_carelog CC ON PR.caregiver_id = CC.CAREGIVER_ID
