resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_MATRIXCARE_SRC_CLIENT" {
	name ="GET_MATRIXCARE_SRC_CLIENT"
...
FROM DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.STVHC_T_Client cl
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.MATRIXCARE.STVHC_T_SCHEDULES AS SCH ON sch.sch_clientID = cl.cli_ID
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_STATE ST1 ON ST1.STATE_ISO_CODE = cl.cli_STATEORPROVINCE 
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_STATE ST2 ON ST1.STATE_ISO_CODE IS NULL AND UPPER(ST2.STATE_NAME) = UPPER(Cl.Cli_STATEORPROVINCE)
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_ZIP_CODE Z ON ST1.STATE_ISO_CODE IS NULL AND ST2.STATE_ISO_CODE IS NULL AND Z.ZIP_CODE = LEFT(TRIM(cl.cli_POSTALCODE), 5)
