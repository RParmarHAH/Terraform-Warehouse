resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_GPSYNCDATA_SRC_EMPLOYEE" {
	name ="GET_GPSYNCDATA_SRC_EMPLOYEE"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  GPSyncData_SRC_Employee
--
-- PURPOSE: Creates script for data cleanse for GPSyncData source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
,TRIM( CAST( e.DB AS VARCHAR)) AS SYSTEM_CODE,
TRIM( CAST( e.Employee_ID AS VARCHAR)) AS EMployee_ID, UPPER( TRIM( e.First_Name)) AS Employee_First_Name,
UPPER( TRIM( e.Last_Name)) AS Employee_Last_Name,
UPPER( TRIM( IFNULL( TRIM(e.ADDRESS1) , '''') ||'' ''|| IFNULL( TRIM(e.ADDRESS2), '''') ||'' ''|| IFNULL( TRIM(e.ADDRESS3), ''''))) AS SRC_Employee_ADDRESS
,UPPER( TRIM( e.City)) AS Src_Employee_City
,UPPER( TRIM( e.State)) AS Src_Employee_STATE_CODE
,UPPER( TRIM( e.Zip)) AS Src_Employee_Zip
,CAST( NULL AS VARCHAR) AS Src_Employee_Zip4
,UPPER( TRIM( e.SSN)) AS Src_Employee_SSN
,CAST(e.DOB AS DATE) AS Src_Employee_DOB
,UPPER( TRIM( e.Phone_Number)) AS Src_Employee_Home_Phone
,NULL AS Src_Employee_Cell_Phone
,NULL AS Src_Employee_Work_Phone
--,UPPER( TRIM( e.Personal_Email)) AS Src_Employee_Personal_Email
--,UPPER( TRIM( e.Email))  AS Src_Employee_Work_Email

,NVL(UPPER(TRIM(EMA."Home Email")),UPPER(TRIM(e.PERSONAL_EMAIL))) AS Src_Employee_Personal_Email
,COALESCE(  UPPER(TRIM(EMA."Company Email")), NULLIF(UPPER(TRIM(e.EMAIL)), ''''),UPPER(TRIM(e.PERSONAL_EMAIL)) )  AS Src_Employee_Work_Email

,MIN( CAST( p.PayDate AS DATE)) AS SRC_Employee_Min_Activity_Date
,MAX( CAST( p.PayDate AS DATE)) AS SRC_Employee_Max_Activity_Date
...
FROM DISC_${var.SF_ENVIRONMENT}.GPSyncData.GPAllEmployeesBase AS e
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.GPSyncData.Paystub_Master AS p ON p.EmployeeID= e.Employee_ID AND p.DB = e.DB
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.GPSYNCDATA.GPSYNC_EMPLOYEES_EMAIL EMA
ON TRIM(EMA.SSN) = TRIM(E.SSN)
--LEFT JOIN DISC_Prod.SandataImport.Sandata_SchedulesClients AS sch ON sch.AGENCYID = C.AGENCYID AND sch.CLIENTID = C.CLIENTID
