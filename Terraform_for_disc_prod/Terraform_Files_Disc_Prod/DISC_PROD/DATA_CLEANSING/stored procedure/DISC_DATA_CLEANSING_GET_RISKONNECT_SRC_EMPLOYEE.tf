resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_RISKONNECT_SRC_EMPLOYEE" {
	name ="GET_RISKONNECT_SRC_EMPLOYEE"
...
SELECT DISTINCT 
    ...
    FROM EMPLOYEE EMP     
    LEFT OUTER JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_STATE ST ON UPPER(TRIM(EMP.STATE__C)) = UPPER(TRIM(ST.STATE_ISO_CODE)) OR UPPER(TRIM(EMP.STATE__C)) = UPPER(TRIM(ST.STATE_NAME))
    LEFT OUTER JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_SOURCE_SYSTEM AS SS  ON  SS.SOURCE_SYSTEM_ID = 29 
    WHERE EMP.IND =1 
