resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_EMPEONPREFERRED_SRC_EMPLOYEE" {
	name ="GET_EMPEONPREFERRED_SRC_EMPLOYEE"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME: EMPEONPREFERRED_Employee
--
-- PURPOSE: Creates script for data cleanse for EMPEONPREFERRED source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
TRIM( CAST(EML.ID AS VARCHAR)) AS EMployee_ID,
UPPER( TRIM( EML.FIRSTNAME)) AS Employee_First_Name,
UPPER(TRIM(EML.LASTNAME)) AS Employee_Last_Name,
UPPER( TRIM( IFNULL( TRIM( EML.ADDRESS1) , '''') ||'' ''|| IFNULL( TRIM(EML.ADDRESS2), ''''))) AS SRC_Employee_ADDRESS
,UPPER(TRIM(EML.CITY))   AS SRC_Employee_CITY
,UPPER(TRIM(EML.STATE)) AS SRC_Employee_STATE_CODE
,UPPER(TRIM(EML.ZIP)) AS  Src_Employee_ZIP
,CAST( NULL AS VARCHAR) AS Src_Employee_Zip4
,TRIM( EML.SSN) AS Src_Employee_SSN
,CAST(EML.BIRTHDATE AS DATE) AS Src_Employee_DOB
,UPPER( TRIM(EML.HOMEPHONE)) AS Src_Employee_Home_Phone
,UPPER( TRIM(EML.CELLPHONE)) AS Src_Employee_Cell_Phone
,UPPER( TRIM(EML.WORKPHONE)) AS Src_Employee_Work_Phone
,UPPER( TRIM(EML.EMAILADDRESS)) AS Src_Employee_Personal_Email 
,UPPER( TRIM(EML.WORKEMAIL)) AS Src_Employee_Work_Email
...
FROM DISC_${var.SF_ENVIRONMENT}.EMPEONPREFERRED.EINFO_TABLE EML
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.EMPEONPREFERRED.EPAYHIST EP
ON TRIM(EML.CO)||TRIM(EML.ID) = TRIM(EP.CO)||TRIM(EP.ID)
WHERE EML.ID IS NOT NULL
