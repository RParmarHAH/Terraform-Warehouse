resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_ADPOPAL_SRC_EMPLOYEE" {
	name ="GET_ADPOPAL_SRC_EMPLOYEE"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME: ADPOPAL_SRC_Employee
--
-- PURPOSE: Creates script for data cleanse for ADPOPAL source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
,''ADP-OPAL-'' || EMP.PAYROLL_COMPANY_CODE AS SYSTEM_CODE
--TRIM( CAST(EMP.FILE_NUMBER AS VARCHAR)) AS EMployee_ID
,Right(EMP.FILE_NUMBER,LENGTH(EMP.FILE_NUMBER)-1)AS EMployee_ID
,UPPER( TRIM( EMP.LEGAL_FIRST_NAME)) AS Employee_First_Name
,UPPER(TRIM( EMP.LEGAL_LAST_NAME)) AS Employee_Last_Name
,UPPER( TRIM( IFNULL( TRIM( EMP.LEGAL_PREFERRED_ADDRESS_ADDRESS_LINE_1) , '''') ||'' ''|| IFNULL( TRIM(EMP.LEGAL_PREFERRED_ADDRESS_ADDRESS_LINE_2), ''''))) AS SRC_Employee_ADDRESS
,UPPER(TRIM(EMP.LEGAL_PREFERRED_ADDRESS_CITY))   AS SRC_Employee_CITY
,UPPER(TRIM(EMP.LEGAL_PREFERRED_ADDRESS_STATE_TERRITORY_CODE)) AS SRC_Employee_STATE_CODE
,UPPER(TRIM(EMP.LEGAL_PREFERRED_ADDRESS_ZIP_POSTAL_CODE)) AS  Src_Employee_ZIP
,CAST( NULL AS VARCHAR) AS Src_Employee_Zip4
,TRIM( EMP.TAX_ID_SSN) AS Src_Employee_SSN
,CAST( EMP.BIRTH_DATE AS DATE) AS Src_Employee_DOB
...
,EMP.PERSONAL_CONTACT_PERSONAL_EMAIL AS Src_Employee_Personal_Email 
,EMP.WORK_CONTACT_WORK_EMAIL AS Src_Employee_Work_Email
,MIN( IFNULL(CAST( PAY.PAY_DATE  AS DATE), CAST( ''2040-12-31'' AS DATE))) AS Src_Employee_Min_Activity_Date
,MAX( IFNULL( CAST(PAY.PAY_DATE  AS DATE), CAST( ''1900-01-01'' AS DATE))) AS Src_Employee_Max_Activity_Date
...
FROM DISC_${var.SF_ENVIRONMENT}.ADP.OPALEMPLOYEE 	EMP
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ADP.OPALPAYROLL PAY
ON EMP.FILE_NUMBER::INT = PAY.FILE_NUMBER AND EMP.PAYROLL_COMPANY_CODE  = PAY.COMPANY_CODE
