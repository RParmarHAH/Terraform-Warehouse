resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_AXXESS_SRC_EMPLOYEE" {
	name ="GET_AXXESS_SRC_EMPLOYEE"
...
FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_HH_EMPLOYEEINFO EI
--FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_EMPLOYEEINFO EI
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_GEOGRAPHY GEO ON EI.ADDRESS_ZIPCODE = GEO.ZIP_CODE 
LEFT JOIN 
(
SELECT SCHEDULED_TASK_ID, CAREPERIOD_CASEMANAGER_NAME, CLINICIAN_ID, CLINICIAN_NAME , count(1)
FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_VISITFINANCIALS 
GROUP BY SCHEDULED_TASK_ID, CAREPERIOD_CASEMANAGER_NAME, CLINICIAN_ID,CLINICIAN_NAME
)VF   ON EI.EMPLOYEE_ID = VF.CLINICIAN_ID 
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_TASKDETAILS TD
...
,CAST( FALSE AS BOOLEAN) AS SRC_Employee_Address_Processed_Flag
,CAST( FALSE AS BOOLEAN) AS SRC_Employee_Address_Successfully_Cleansed_Flag
FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_HC_EMPLOYEEINFO WD 
--FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_WORKERDETAILS WD
LEFT JOIN 
(
SELECT SCHEDULED_TASK_ID, CAREPERIOD_CASEMANAGER_NAME, CLINICIAN_ID, CLINICIAN_NAME , count(1)
FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_VISITFINANCIALS 
