resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_ASR_SRC_EMPLOYEE" {
	name ="GET_ASR_SRC_EMPLOYEE"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  ASR_SRC_Employee
--
-- PURPOSE: Creates script for data cleanse for ASR source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
SELECT DISTINCT 
	...
	UPPER(TRIM(E.ADDRESS)) AS SRC_Employee_ADDRESS,
	TRIM(E.CITY) AS Src_Employee_City,
	TRIM(GEO.STATE_ISO_CODE) AS Src_Employee_STATE_CODE,
	UPPER(TRIM(E.POSTALCODE)) AS Src_Employee_Zip,
	CAST( NULL AS VARCHAR) AS Src_Employee_Zip4,
	UPPER(TRIM(E.SSN)) AS Src_Employee_SSN,
	TRY_TO_DATE(TRIM(E.BIRTHDATE),''MM/DD/YYYY HH12:MI:SS AM'') AS Src_EMPLOYEE_DOB ,
	UPPER(TRIM( E.HOMEPHONE)) AS Src_Employee_Home_Phone,
	UPPER(TRIM( E.PHONE2)) AS Src_Employee_Cell_Phone,
	CAST( NULL AS VARCHAR) AS Src_Employee_Work_Phone,
	UPPER( TRIM( E.EMAIL)) AS Src_Employee_Personal_Email,
	CAST( NULL AS VARCHAR) AS Src_Employee_Work_Email
	,MIN( COALESCE( TRY_TO_DATE(TRIM(VI.VISITSTARTDATE),''MM/DD/YYYY HH12:MI:SS AM''), TRY_TO_DATE(TRIM(VI.SCHEDULEDCHECKIN),''MM/DD/YYYY HH12:MI:SS AM''))) AS SRC_Employee_Min_Activity_Date
	,MAX( COALESCE( TRY_TO_DATE(TRIM( VI.VISITENDDATE),''MM/DD/YYYY HH12:MI:SS AM''), TRY_TO_DATE(TRIM(VI.SCHEDULEDCHECKOUT),''MM/DD/YYYY HH12:MI:SS AM''))) AS SRC_Employee_Max_Activity_Date
...
FROM DISC_${var.SF_ENVIRONMENT}.ASR.ASR_EMPLOYEES E
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_GEOGRAPHY GEO ON LEFT(TRIM(E.POSTALCODE),5) = GEO.ZIP_CODE
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ASR.ASR_VISITS VI ON E.EMPLOYEEID = VI.EMPLOYEEID
