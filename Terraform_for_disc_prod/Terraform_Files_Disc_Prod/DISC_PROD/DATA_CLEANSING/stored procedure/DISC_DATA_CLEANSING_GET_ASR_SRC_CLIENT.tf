resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_ASR_SRC_CLIENT" {
	name ="GET_ASR_SRC_CLIENT"
...
,TRIM( CAST( cust.CustomerID AS VARCHAR)) AS Client_ID
,UPPER( TRIM( cust.FirstName)) AS Client_First_Name
,UPPER( TRIM( cust.LastName)) AS Client_Last_Name
,UPPER( TRIM( IFNULL( TRIM(cust.Address) , ''''))) AS SRC_Client_ADDRESS 
,UPPER( TRIM( cust.CITY)) AS SRC_Client_CITY
,UPPER( TRIM( GEO.STATE_ISO_CODE)) AS SRC_Client_STATE_CODE
,UPPER( TRIM( LEFT(cust.PostalCode,5)))  AS SRC_Client_ZIP  
,CAST( NULL AS VARCHAR) AS Src_Client_Zip4
,CAST( SSN AS VARCHAR) AS Src_Client_SSN
,CAST( CLIENT_DOB AS VARCHAR) AS Src_Client_DOB
,UPPER( TRIM( cust.HomePhone)) AS Src_Client_Home_Phone
,UPPER( TRIM( cust.Phone2)) AS Src_Client_Cell_Phone
,UPPER( TRIM( cust.Fax)) AS Src_Client_Work_Phone
,UPPER( TRIM(  cust.Email)) AS Src_Client_Personal_Email
,CAST( NULL AS VARCHAR) AS Src_Client_Work_Email
,MIN( COALESCE( TRY_CAST( LEFT( VI.VISITDate, 10) AS DATE), TRY_CAST( LEFT( VI.TimeCheckedIn,10) AS DATE))) AS SRC_Client_Min_Activity_Date
,MAX( COALESCE( TRY_CAST( LEFT( VI.VISITDate, 10) AS DATE), TRY_CAST( LEFT( VI.TimeCheckedIn,10) AS DATE))) AS SRC_Client_Max_Activity_Date
...
FROM DISC_${var.SF_ENVIRONMENT}.ASR.VW_ASR_CUSTOMERS AS cust
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_GEOGRAPHY GEO ON TRIM(cust.POSTALCODE) = GEO.ZIP_CODE
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ASR.ASR_VISITS VI ON VI.CustomerID = cust.CUSTOMERID
