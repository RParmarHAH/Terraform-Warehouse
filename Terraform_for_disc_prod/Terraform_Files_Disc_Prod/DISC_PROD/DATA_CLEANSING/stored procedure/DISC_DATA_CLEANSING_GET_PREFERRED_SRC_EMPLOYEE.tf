resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_PREFERRED_SRC_EMPLOYEE" {
	name ="GET_PREFERRED_SRC_EMPLOYEE"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  HHAEXCHANGEPreferred_SRC_Employee
--
-- PURPOSE: Creates script for data cleanse for HHAEXCHANGEPreferred source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
,TRIM(( CAST( car.AGENCYID AS VARCHAR))||''''|| TRIM( CAST( car.CaregiverID AS VARCHAR)))AS EMployee_ID
,UPPER( TRIM( car.FirstName)) AS Employee_First_Name
,UPPER( TRIM( LastName)) AS Employee_Last_Name
...
,IFF(LENGTH(REGEXP_REPLACE(TRIM(car.HOMEPHONE),''[^[:digit:]$]'')) = 10,REGEXP_REPLACE(TRIM(car.HOMEPHONE),''[^[:digit:]$]''),
NULL ) AS Src_Employee_Home_Phone
,IFF(LENGTH(REGEXP_REPLACE(TRIM(car.PHONE2),''[^[:digit:]$]'')) = 10,REGEXP_REPLACE(TRIM(car.PHONE2),''[^[:digit:]$]''),
NULL )  AS Src_Employee_Cell_Phone
,IFF(LENGTH(REGEXP_REPLACE(TRIM(car.PHONE3),''[^[:digit:]$]'')) = 10,REGEXP_REPLACE(TRIM(car.PHONE3),''[^[:digit:]$]''), 
...
FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPreferred.Caregivers AS car
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPreferred.TBLVISITS_REPL AS VISIT ON car.CaregiverID = visit.CaregiverID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPreferred.VISITS VI ON VI.MISSEDVISIT = ''N'' AND VI.VISITID = VISIT.VISITID
