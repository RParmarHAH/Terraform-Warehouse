resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_AXXESS_SRC_CLIENT" {
	name ="GET_AXXESS_SRC_CLIENT"
...
FROM DISC_${var.SF_ENVIRONMENT}.Axxess.Axxess_PatientInfo AS cust
LEFT JOIN (SELECT DISTINCT PATIENT_ID ,EMAIL_ADDRESS FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_EPISODESNAPSHOTS) PE ON CUST.PATIENT_ID = PE.PATIENT_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.Axxess.Axxess_VisitInfo VI ON VI.Patient_ID = cust.Patient_ID
...
,CAST( FALSE AS BOOLEAN) AS SRC_Client_Address_Processed_Flag
,CAST( FALSE AS BOOLEAN) AS SRC_Client_Address_Successfully_Cleansed_Flag
FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_CAREPERIODS CA
LEFT JOIN (SELECT DISTINCT PATIENT_ID ,EMAIL_ADDRESS FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_EPISODESNAPSHOTS) PE ON CA.CLIENT_ID = PE.PATIENT_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.Axxess.Axxess_VisitInfo V ON V.Patient_ID = CA.CLIENT_ID
