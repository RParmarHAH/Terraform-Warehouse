resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_SANDATAIMPORT_SRC_CLIENT" {
	name ="GET_SANDATAIMPORT_SRC_CLIENT"
...
,TRIM( CAST( c.ClientID AS VARCHAR)) AS Client_ID 
,UPPER( TRIM( c.FirstName)) AS Client_First_Name 
,UPPER( TRIM( c.LastName)) AS Client_Last_Name
,UPPER( TRIM( IFNULL( TRIM(c.ADDRESS) , '''') ||'' ''|| IFNULL( TRIM(c.ADDRESS2), '''') ||'' ''|| IFNULL( TRIM(c.APTNO), ''''))) AS SRC_Client_ADDRESS
,UPPER( TRIM( c.City)) AS Src_Client_City
,CASE WHEN S.STATE_ISO_CODE IS NOT NULL THEN S.STATE_ISO_CODE ELSE UPPER( TRIM( C.State)) END AS Src_Client_State_Code
,TRIM( c.Zip) AS Src_Client_Zip
,CAST( NULL AS VARCHAR) AS Src_Client_Zip4
,CAST( NULL AS VARCHAR) AS Src_Client_SSN
,CAST(c.DOB AS DATE) AS Src_Client_DOB
,UPPER( TRIM( c.HomePhone)) AS Src_Client_Home_Phone
,UPPER( TRIM( c.MobilePhone)) AS Src_Client_Cell_Phone
,UPPER( TRIM( c.WorkPhone)) AS Src_Client_Work_Phone
,UPPER( TRIM( c.Email)) AS Src_Client_Personal_Email
,CAST( NULL AS VARCHAR) AS Src_Client_Work_Email
,MIN( COALESCE( CAST( sch.EVVTimeIn AS DATE), CAST( sch.ScheduledTimeIn AS DATE))) AS SRC_Client_Min_Activity_Date
,MAX( COALESCE( CAST( sch.EVVTimeIn AS DATE), CAST( sch.ScheduledTimeIn AS DATE))) AS SRC_Client_Max_Activity_Date
...
FROM DISC_${var.SF_ENVIRONMENT}.SandataImport.Sandata_Clients AS c
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_STATE S ON S.STATE_ISO_CODE=UPPER(TRIM(C.STATE))
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.BI_Repository.SandataVisits AS SV ON sv.AGENCYID = C.AGENCYID AND sv.ClientID = C.ClientID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_SCHEDULES AS sch ON sv.AGENCYID = sch.AGENCYID AND sv.SCHEDULEID = sch.SCHEDULEID
