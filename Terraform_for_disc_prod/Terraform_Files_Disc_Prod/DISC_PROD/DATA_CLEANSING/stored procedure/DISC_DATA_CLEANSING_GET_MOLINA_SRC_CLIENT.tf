resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_MOLINA_SRC_CLIENT" {
	name ="GET_MOLINA_SRC_CLIENT"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  MOLINA_SRC_CLIENT
--
-- PURPOSE: Creates script for data cleanse for MOLINA source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
TRIM( CAST(f.EXTERNAL_ID AS VARCHAR)) AS Client_ID, 
UPPER( TRIM(NVL(F.FIRST_NAME, ''''))) AS Client_First_Name, 
UPPER(TRIM(NVL(F.LAST_NAME, ''''))) AS Client_Last_Name,
UPPER( IFNULL( TRIM(F.STREET_ADDRESS) , '''')) AS SRC_Client_ADDRESS ,
UPPER(TRIM(F.CITY)) AS SRC_Client_CITY,
UPPER(TRIM(f.STATE)) AS SRC_Client_STATE_CODE,
UPPER( TRIM( F.ZIP_CODE))  AS SRC_Client_ZIP,
CAST( NULL AS VARCHAR) AS Src_Client_Zip4,
UPPER( TRIM(F.SSN)) AS Src_Client_SSN,
TO_DATE(F.DATE_OF_BIRTH) AS Src_Client_DOB,
UPPER( TRIM( f.PHONE_NUMBER)) AS Src_Client_Home_Phone,
CAST( NULL AS VARCHAR) AS Src_Client_Cell_Phone,
CAST( NULL AS VARCHAR) AS Src_Client_Work_Phone,
UPPER(TRIM(F.EMAIL_ADDRESS)) AS Src_Client_Personal_Email,
CAST( NULL AS VARCHAR) AS Src_Client_Work_Email,
NULL AS Src_Client_Min_Activity_Date,
NULL AS Src_Client_Max_Activity_Date,
...
FROM DISC_${var.SF_ENVIRONMENT}.MOLINA.MOLINA_SOURCE_DATA F
--LEFT JOIN DISC_${var.SF_ENVIRONMENT}.MOLINA.MOLINA_IP_CENSUS C ON F.MEDICAID_ID = C.MEDICAIDID 
