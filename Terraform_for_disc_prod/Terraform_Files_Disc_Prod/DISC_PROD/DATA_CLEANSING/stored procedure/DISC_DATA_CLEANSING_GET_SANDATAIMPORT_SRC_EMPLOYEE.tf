resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_SANDATAIMPORT_SRC_EMPLOYEE" {
	name ="GET_SANDATAIMPORT_SRC_EMPLOYEE"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  SANDATAIMPORT_SRC_Employee
--
-- PURPOSE: Creates script for data cleanse for SANDATAIMPORT source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
,TRIM( CAST( c.AgencyID AS VARCHAR)) AS SYSTEM_CODE,
TRY_CAST(c.STAFFAGENCYID AS INT)::STRING AS Employee_ID,
UPPER( TRIM( c.FirstName)) AS Employee_First_Name, UPPER( TRIM( c.LastName)) AS Employee_Last_Name,
UPPER( TRIM( IFNULL( TRIM(c.ADDRESS) , '''') ||'' ''|| IFNULL( TRIM(c.ADDRESS2), '''')||'' ''|| IFNULL( TRIM(c.APTNO), ''''))) AS SRC_Employee_ADDRESS
,UPPER( TRIM( c.City)) AS Src_Employee_City
,CASE WHEN S.STATE_ISO_CODE IS NOT NULL THEN S.STATE_ISO_CODE ELSE UPPER( TRIM( C.State)) END AS Src_Employee_STATE_CODE
,TRIM( c.Zip) AS Src_Employee_Zip
,CAST( NULL AS VARCHAR) AS Src_Employee_Zip4
,UPPER( TRIM( c.SSN)) AS Src_Employee_SSN
,CAST(c.DOB AS DATE) AS Src_Employee_DOB
,UPPER( TRIM( c.HomePhone)) AS Src_Employee_Home_Phone
,UPPER( TRIM( c.MobilePhone)) AS Src_Employee_Cell_Phone
,UPPER( TRIM( c.WorkPhone)) AS Src_Employee_Work_Phone
,UPPER( TRIM( c.Email)) AS Src_Employee_Personal_Email
,CAST( NULL AS VARCHAR) AS Src_Employee_Work_Email
,MIN( COALESCE( CAST( sch.EVVTimeIn AS DATE), CAST( sch.ScheduledTimeIn AS DATE))) AS SRC_Employee_Min_Activity_Date
,MAX( COALESCE( CAST( sch.EVVTimeIn AS DATE), CAST( sch.ScheduledTimeIn AS DATE))) AS SRC_Employee_Max_Activity_Date
...
FROM DISC_${var.SF_ENVIRONMENT}.SandataImport.Sandata_Staffs AS c
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_STATE S ON S.STATE_ISO_CODE=UPPER(TRIM(C.STATE))
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.SandataImport.SANDATA_SchedulesStaffs AS SS ON ss.AGENCYID = C.AGENCYID AND ss.StaffID = C.StaffID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.SANDATAIMPORT.SANDATA_SCHEDULES AS sch ON ss.AGENCYID = sch.AGENCYID AND ss.SCHEDULEID = sch.SCHEDULEID
--LEFT JOIN DISC_Prod.SandataImport.Sandata_SchedulesEmployees AS sch ON sch.AGENCYID = C.AGENCYID AND sch.EmployeeID = C.EmployeeID
