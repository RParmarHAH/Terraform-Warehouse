resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_DATAFLEXSYNCDATA_SRC_EMPLOYEE" {
	name ="GET_DATAFLEXSYNCDATA_SRC_EMPLOYEE"
...
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_STATE ST ON ST.STATE_ISO_CODE = e.STATE
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.DATAFLEXSYNCDATA.DFTimeSheets AS TS ON e.DBName = ts.DBName AND e.Number = ts.EmployeeNumber
