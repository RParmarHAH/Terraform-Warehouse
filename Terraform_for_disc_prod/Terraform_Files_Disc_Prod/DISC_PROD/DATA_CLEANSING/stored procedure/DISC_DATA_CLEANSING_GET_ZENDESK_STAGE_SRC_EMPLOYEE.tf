resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_ZENDESK_STAGE_SRC_EMPLOYEE" {
	name ="GET_ZENDESK_STAGE_SRC_EMPLOYEE"
...
INSERT OVERWRITE INTO  DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.EMPLOYEE_SRC_ZENDESK_ATTRIBUTES
...
FROM (SELECT Id::VARCHAR(50) AS Employee_ID, * FROM  DISC_${var.SF_ENVIRONMENT}.ZENDESK.USER) Z 
