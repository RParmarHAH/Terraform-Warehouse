resource "snowflake_procedure" "DISC_DATA_CLEANSING_MERGE_STAGE_DAT_CLEANSING_CLIENT_ADDRESS" {
	name ="MERGE_STAGE_DAT_CLEANSING_CLIENT_ADDRESS"
...
INSERT OVERWRITE INTO DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.CLIENT_STG_STANDARDIZED_ADDRESSES 
SELECT * FROM AWS_LANDING_INGEST_DB_${var.SF_ENVIRONMENT}.DATA_CLEANSING.CLIENT_STG_STANDARDIZED_ADDRESSES ;


/* To Update .0 zip with correct value  */

UPDATE DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.CLIENT_STG_STANDARDIZED_ADDRESSES 
FROM 
(
SELECT  
source_system_id,
...
FROM 
DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.CLIENT_STG_STANDARDIZED_ADDRESSES 
...
zip  = x.zip
WHERE 
DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.CLIENT_STG_STANDARDIZED_ADDRESSES.source_system_id = X.source_system_id
AND DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.CLIENT_STG_STANDARDIZED_ADDRESSES.system_code = X.system_code
AND DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.CLIENT_STG_STANDARDIZED_ADDRESSES .CLIENT_NUMBER = X.CLIENT_NUMBER;
...
MERGE INTO DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.CLIENT_STANDARDIZED_ADDRESSES   AS tgt
USING
(
SELECT DISTINCT SOURCE_SYSTEM_ID,
				...
				RELEVANCE,
				INSERTED_AT       
				FROM DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.CLIENT_STG_STANDARDIZED_ADDRESSES 
) AS src
...
INSERT INTO DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.CLIENT_HISTRY_STANDARDIZED_ADDRESSES
SELECT * FROM DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.CLIENT_STG_STANDARDIZED_ADDRESSES  ;
