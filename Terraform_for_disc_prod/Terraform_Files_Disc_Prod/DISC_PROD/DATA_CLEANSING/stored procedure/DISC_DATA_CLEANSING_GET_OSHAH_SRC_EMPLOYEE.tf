resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_OSHAH_SRC_EMPLOYEE" {
	name ="GET_OSHAH_SRC_EMPLOYEE"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  HHAEXCHANGEOSHAH_SRC_Employee
--
-- PURPOSE: Creates script for data cleanse for HHAEXCHANGEOSHAH source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
TRIM( CAST( car.CaregiverID AS VARCHAR)) AS EMployee_ID, UPPER( TRIM( car.FirstName)) AS Employee_First_Name,
...
,UPPER( TRIM( car.ZIP5))  AS SRC_Employee_ZIP  
...
FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.Caregivers AS car
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.OFFICE_OFFICES_REPL  O ON car.OFFICEID = O.OFFICEID AND car.AGENCYID = O.AGENCYID 
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.TBLVISITS_REPL AS VISIT ON car.CaregiverID = visit.CaregiverID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.VISITS VI ON VI.MISSEDVISIT = ''N'' AND VI.VISITID = VISIT.VISITID
