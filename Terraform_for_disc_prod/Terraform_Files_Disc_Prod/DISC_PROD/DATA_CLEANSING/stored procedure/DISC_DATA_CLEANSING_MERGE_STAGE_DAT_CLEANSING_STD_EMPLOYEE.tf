resource "snowflake_procedure" "DISC_DATA_CLEANSING_MERGE_STAGE_DAT_CLEANSING_STD_EMPLOYEE" {
	name ="MERGE_STAGE_DAT_CLEANSING_STD_EMPLOYEE"
...
MERGE INTO DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.EMPLOYEE_STD_ATTRIBUTES  AS tgt
USING
(
SELECT DISTINCT addr.SOURCE_SYSTEM_ID, addr.SYSTEM_CODE, addr.Employee_ID, addr.Employee_FIRST_NAME, COALESCE (addr.Employee_LAST_NAME,'''') as Employee_LAST_NAME, 
	   ...
	   DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.SSN_VALIDATION(SRC_Employee_SSN) AS Std_Employee_SSN,
       ...
       DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.PHONE_NUMBER_VALIDATION(addr.SRC_Employee_HOME_PHONE) AS Std_Employee_Home_Phone,
       ...
       DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.PHONE_NUMBER_VALIDATION( addr.SRC_Employee_Cell_PHONE) AS Std_Employee_Cell_Phone,
       ...
       DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.PHONE_NUMBER_VALIDATION( addr.SRC_Employee_Work_PHONE) AS Std_Employee_Work_Phone,
       ...
       DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.EMAIL_VALIDATION( addr.SRC_Employee_Personal_Email) AS Std_Employee_Personal_Email,
       ...
       DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.EMAIL_VALIDATION( addr.SRC_Employee_Work_Email) AS Std_Employee_Work_Email,
...
FROM DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.Employee_Src_Attributes AS addr
LEFT OUTER JOIN
( SELECT  *  FROM DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.EMPLOYEE_STANDARDIZED_ADDRESSES 
WHERE 
...
(ADDRESS ,CITY,STATE,ZIP )
 IN (SELECT  ADDRESS ,CITY,STATE,ZIP   FROM DISC_${var.SF_ENVIRONMENT}.DATA_CLEANSING.EMPLOYEE_STANDARDIZED_ADDRESSES_INCNSTNT )
 OR  COUNTRY_CODE <> ''USA''
 OR zip <> ZIPCODE
 OR LENGTH (zip) <> 5
