resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_GENERATIONSALLIANCE_SRC_EMPLOYEE" {
	name ="GET_GENERATIONSALLIANCE_SRC_EMPLOYEE"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  ALLIANCE_SRC_Employee
--
-- PURPOSE: Creates script for data cleanse for ALLIANCE source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
,''ALLIANCE'' AS SYSTEM_CODE,
 CAST( MD5(HEX_ENCODE(E.SOCIALSECURITYNUM)) AS VARCHAR) AS EMployee_ID,
 --CAST( E.SOCIALSECURITYNUM AS VARCHAR) AS EMployee_ID,
...
,TRIM( E.SOCIALSECURITYNUM) AS Src_Employee_SSN
,TO_DATE(TRIM(E.DATEOFBIRTH)) AS Src_Employee_DOB
,UPPER( TRIM(E.PHONE1)) AS Src_Employee_Home_Phone
...
,MIN( IFNULL(DATE_TRUNC (DAY, SCH."DATE"), CAST( ''2040-12-31'' AS DATE))) AS Src_Employee_Min_Activity_Date
,MAX( IFNULL( DATE_TRUNC (DAY, SCH."DATE"), CAST( ''1900-01-01'' AS DATE))) AS Src_Employee_Max_Activity_Date
...
FROM DISC_${var.SF_ENVIRONMENT}.GENERATIONSALLIANCE.EMPLOYEE AS  E
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.GENERATIONSALLIANCE.SCHEDULES SCH ON E.SOCIALSECURITYNUM = SCH.SOCIALSECNUM  
