resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_ALAYACARE_SRC_CLIENT" {
	name ="GET_ALAYACARE_SRC_CLIENT"
...
,UPPER( TRIM( c.Profile_Last_Name)) AS Client_Last_Name
,UPPER( IFNULL( TRIM(c.Profile_ADDRESS) , '''') ||'' ''|| IFNULL( TRIM( c.PROFILE_ADDRESS_SUITE), ''''))  AS SRC_Client_ADDRESS
,UPPER( TRIM( c.Profile_City)) AS Src_Client_City
,COALESCE( UPPER( TRIM( z.State_ISO_Code)) ,
          CASE WHEN UPPER(TRIM(c.PROFILE_STATE)) IN (''GEORGIA'',''PORT WENTWORTH'',''SAVANNAH'',''FORSYTH'',''CENTERVILLE'',''MACON'',''FORT VALLEY'',''SAVANNAH'',''HAVANA'',''georgia'',''VALDOSTA'',''GEORGIA'',''SOUTH CAROLINA'',''ALBANY'',''AMERICUS'')
	      THEN ''GA'' 
          WHEN UPPER(TRIM(c.PROFILE_STATE)) = ''SOUTH CAROLINA''
          THEN ''SC'' 
          WHEN UPPER(TRIM(c.PROFILE_STATE)) = ''ALABAMA''
          THEN ''AL'' 
          WHEN UPPER(TRIM(c.PROFILE_STATE)) = ''TEXAS''
          THEN ''TX''
          ELSE UPPER(TRIM(c.PROFILE_STATE))
          END) AS Src_Client_STATE_CODE
,UPPER( TRIM( c.Profile_Zip)) AS Src_Client_Zip
,CAST( NULL AS VARCHAR) AS Src_Client_Zip4
,UPPER( TRIM( c.Profile_SSN)) AS Src_Client_SSN
,CAST( c.Profile_Birthday AS DATE) AS Src_Client_DOB
,CAST( NULL AS VARCHAR) AS Src_Client_Home_Phone
,UPPER( TRIM( c.Profile_Phone_Main)) AS Src_Client_Cell_Phone  
,UPPER( TRIM( c.Profile_Phone_Personal)) AS Src_Client_Work_Phone
,UPPER( TRIM( c.Profile_Email)) AS Src_Client_Personal_Email
,CAST( NULL AS VARCHAR)  AS Src_Client_Work_Email
,MIN( CAST( v.Start_At AS DATE)) AS SRC_Client_Min_Activity_Date
,MAX( CAST( v.Start_At AS DATE)) AS SRC_Client_Max_Activity_Date
...
FROM DISC_${var.SF_ENVIRONMENT}.Alayacare.Client AS c
LEFT OUTER JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_Zip_Code AS z ON z.Zip_Code = c.Profile_Zip
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.Alayacare.Visit AS v ON v.Client_ID = c.Client_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.TBL_GUID_COST_CENTRE_TIER_4 COST_CENTRE_MAPPING ON COST_CENTRE_MAPPING.GUID_TO = C.GUID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.TBL_COST_CENTRES_TIER_4 AS COST_CENTRE ON COST_CENTRE.ID = COST_CENTRE_MAPPING.COST_CENTRE_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.GROUPS AS G ON G.BRANCH_ID = COST_CENTRE.PROPERTIES_BRANCH_ID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ALAYACARE.BRANCH B ON B.BRANCH_ID = COALESCE(C.BRANCH_ID, G.BRANCH_ID)
