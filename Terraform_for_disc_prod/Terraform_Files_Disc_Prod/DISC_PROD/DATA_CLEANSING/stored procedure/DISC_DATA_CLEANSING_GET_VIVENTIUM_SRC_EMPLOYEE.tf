resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_VIVENTIUM_SRC_EMPLOYEE" {
	name ="GET_VIVENTIUM_SRC_EMPLOYEE"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  VIVENTIUM_SRC_Employee
--
-- PURPOSE: Creates script for data cleanse for VIVENTIUM source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
,UPPER( TRIM( IFNULL( TRIM(EM.addressLine1 ) , '''') ||'' ''|| IFNULL( TRIM(EM.addressLine2), '''') )) AS SRC_Employee_ADDRESS
,UPPER(TRIM(EM.city)) AS SRC_Employee_CITY
,UPPER(TRIM(EM.state)) AS SRC_Employee_STATE_CODE
,UPPER(TRIM(EM.zipCode)) AS Src_Employee_ZIP
,CAST( NULL AS VARCHAR) AS Src_Employee_Zip4
,TRIM(EM.SSN)  AS Src_Employee_SSN
,CAST(EM.dateOfBirth AS DATE)  AS Src_Employee_DOB
,UPPER( TRIM(EM.homePhone)) AS Src_Employee_Home_Phone
,UPPER( TRIM(EM.mobilePhone)) AS Src_Employee_Cell_Phone
,UPPER( TRIM(EM.workPhone)) AS Src_Employee_Work_Phone
,UPPER( TRIM(EM.email)) AS Src_Employee_Personal_Email 
,UPPER( TRIM(EM.EMAIL)) AS Src_Employee_Work_Email
,MIN( IFNULL(CAST( EC.CHECKDATE AS DATE), CAST( EC.PERIODBEGINDATE AS DATE))) AS Src_Employee_Min_Activity_Date
,MAX( IFNULL(CAST( EC.CHECKDATE AS DATE), CAST( EC.PERIODENDDATE AS DATE))) AS Src_Employee_Max_Activity_Date
...
FROM  DISC_${var.SF_ENVIRONMENT}.VIVENTIUM.EXPORTEMPLOYEES EM
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.viventium.EXPORTCHECKS_UNFLATTEN EC 
ON EC.COMPANYCODE = EM.COMPANYCODE AND EC.EMPLOYEEKEY = EM.EMPLOYEEKEY
