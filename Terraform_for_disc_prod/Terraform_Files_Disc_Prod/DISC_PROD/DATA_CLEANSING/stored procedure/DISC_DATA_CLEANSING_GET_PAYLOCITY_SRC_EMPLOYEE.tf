resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_PAYLOCITY_SRC_EMPLOYEE" {
	name ="GET_PAYLOCITY_SRC_EMPLOYEE"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  PAYLOCITY_SRC_Employee
--
-- PURPOSE: Creates script for data cleanse for PAYLOCITY source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
,ED.COMPANYID AS SYSTEM_CODE,
 CAST(TRIM(ED.EMPLOYEEID)AS VARCHAR) AS EMployee_ID,
UPPER( TRIM(ED.FIRSTNAME)) AS Employee_First_Name,
UPPER( TRIM(ED.LASTNAME )) AS Employee_Last_Name,
UPPER( TRIM( IFNULL( TRIM( ED.HOMEADDRESS_ADDRESS1) , '''') ||'' ''|| IFNULL( TRIM(ED.HOMEADDRESS_ADDRESS2), ''''))) AS SRC_Employee_ADDRESS
,UPPER(TRIM(ED.HOMEADDRESS_CITY)) AS SRC_Employee_CITY
,UPPER(TRIM(ED.HOMEADDRESS_STATE)) AS SRC_Employee_STATE_CODE
,UPPER(TRIM(ED.HOMEADDRESS_POSTALCODE)) AS Src_Employee_ZIP
,CAST( NULL AS VARCHAR) AS Src_Employee_Zip4
,TRIM(ED.SSN) AS Src_Employee_SSN
,TO_DATE(ED.BIRTHDATE::DATE) AS Src_Employee_DOB
,UPPER( TRIM(ED.HOMEADDRESS_MOBILEPHONE)) AS Src_Employee_Home_Phone
,UPPER( TRIM(ED.HOMEADDRESS_PHONE)) AS Src_Employee_Cell_Phone
,UPPER( TRIM(ED.WORKADDRESS_MOBILEPHONE)) AS Src_Employee_Work_Phone
,UPPER( TRIM(ED.HOMEADDRESS_EMAILADDRESS)) AS Src_Employee_Personal_Email 
,UPPER( TRIM(ED.WORKADDRESS_EMAILADDRESS)) AS Src_Employee_Work_Email
,MIN( IFNULL(PSS.CHECKDATE::DATE, CAST( ''2040-12-31'' AS DATE))) AS Src_Employee_Min_Activity_Date
,MAX( IFNULL( PSS.CHECKDATE::DATE, CAST( ''1900-01-01'' AS DATE))) AS Src_Employee_Max_Activity_Date
...
FROM DISC_${var.SF_ENVIRONMENT}.PAYLOCITY.EMPLOYEE_DETAILS AS  ED
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.PAYLOCITY.PAY_STATEMENT_SUMMARY PSS 
ON TRIM(ED.EMPLOYEEID) = TRIM(PSS.EMPLOYEEID) AND TRIM(ED.COMPANYID) = TRIM(PSS.COMPANYID)
