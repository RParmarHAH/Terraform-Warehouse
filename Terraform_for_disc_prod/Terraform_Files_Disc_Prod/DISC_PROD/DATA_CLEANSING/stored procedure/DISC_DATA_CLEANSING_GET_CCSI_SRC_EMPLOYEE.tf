resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_CCSI_SRC_EMPLOYEE" {
	name ="GET_CCSI_SRC_EMPLOYEE"
...
,''CCSI''  AS SYSTEM_CODE,
E.RECORD_NUMBER||''_''||E.EMPLOYEE_NUMBER||''_''||E.AREA AS EMployee_ID,
UPPER( TRIM( E.FIRST_NAME)) AS Employee_First_Name, 
UPPER( TRIM( E.LAST_NAME)) AS Employee_Last_Name,
UPPER( TRIM( IFNULL( TRIM(E.ADDRESS) , '''') )) AS SRC_Employee_ADDRESS
,UPPER( TRIM( E.CITY )) AS Src_Employee_City
,UPPER( TRIM( E.STATE)) AS Src_Employee_STATE_CODE
,TRIM( E.ZIPCODE) AS Src_Employee_Zip
,CAST( NULL AS VARCHAR) AS Src_Employee_Zip4
,UPPER( TRIM(E.SS)) AS Src_Employee_SSN
,TRY_TO_DATE(E.DATE_OF_BIRTH,''YYYYMMDD'') AS Src_Employee_DOB
,UPPER( TRIM(E.PHONENUMBER)) AS Src_Employee_Home_Phone
,UPPER( TRIM( E.CELLPHONE)) AS Src_Employee_Cell_Phone
,NULL AS Src_Employee_Work_Phone
,NULL AS Src_Employee_Personal_Email
,NULL  AS Src_Employee_Work_Email
,MIN( TRY_TO_DATE(P.PERIOD_DATE, ''MMDDYY'')) AS SRC_Employee_Min_Activity_Date
,MAX( TRY_TO_DATE(P.PERIOD_DATE, ''MMDDYY'')) AS SRC_Employee_Max_Activity_Date
...
FROM DISC_${var.SF_ENVIRONMENT}.CCSI.EMPLOYEE1 E
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.CCSI.RAWVRFP AS p 
