resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_GATOR_SRC_EMPLOYEE" {
	name ="GET_GATOR_SRC_EMPLOYEE"
...
-- NAME: GATOR_Employee
--
-- PURPOSE: Creates script for data cleanse for GATOR source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
TRIM( CAST(A.HEALTHCLOUDGA__INDIVIDUALID__C AS VARCHAR)) AS EMployee_ID,
UPPER( TRIM( A.FIRSTNAME)) AS Employee_First_Name,
UPPER(TRIM(A.LASTNAME)) AS Employee_Last_Name,
UPPER( TRIM( IFNULL( TRIM( A.PRIMARY_ADDRESS__C ) , '''') ||'' ''|| IFNULL( TRIM(A.PERSONMAILINGSTREET), ''''))) AS SRC_Employee_ADDRESS
,UPPER(TRIM(A.PERSONMAILINGCITY))   AS SRC_Employee_CITY
,UPPER(TRIM(s.STATE_ISO_CODE)) AS SRC_Employee_STATE_CODE
,UPPER(TRIM(A.PERSONMAILINGPOSTALCODE)) AS  Src_Employee_ZIP
,CAST( NULL AS VARCHAR) AS Src_Employee_Zip4
,CAST( NULL AS VARCHAR) AS Src_Employee_SSN
,CAST(A.PERSONBIRTHDATE AS DATE) AS Src_Employee_DOB
,UPPER( TRIM(A.PHONE)) AS Src_Employee_Home_Phone
,UPPER( TRIM(A.PERSONMOBILEPHONE)) AS Src_Employee_Cell_Phone
,CAST( NULL AS VARCHAR) AS Src_Employee_Work_Phone
,UPPER( TRIM(A.PERSONEMAIL )) AS Src_Employee_Personal_Email 
,CAST( NULL AS VARCHAR) AS Src_Employee_Work_Email
,MIN( IFNULL(CAST( A.LASTACTIVITYDATE  AS DATE), CAST( ''2040-12-31'' AS DATE))) AS Src_Employee_Min_Activity_Date
,MAX( IFNULL( CAST(A.LASTACTIVITYDATE  AS DATE), CAST( ''1900-01-01'' AS DATE))) AS Src_Employee_Max_Activity_Date
...
FROM DISC_${var.SF_ENVIRONMENT}.HEALTH_NAVIGATOR.ACCOUNT AS A
INNER JOIN DISC_${var.SF_ENVIRONMENT}.HEALTH_NAVIGATOR.RECORDTYPE  r ON r.ID = a.RECORDTYPEID
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_STATE s ON UPPER(s.STATE_NAME) = UPPER(TRIM(a.PERSONMAILINGSTATE))
WHERE r.name = ''Caregiver'' AND A.HEALTHCLOUDGA__INDIVIDUALID__C IS NOT NULL 
