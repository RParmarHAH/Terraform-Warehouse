resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_EVOKE_SRC_EMPLOYEE" {
	name ="GET_EVOKE_SRC_EMPLOYEE"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  Evoke_SRC_Employee
--
-- PURPOSE: Creates script for data cleanse for Evoke source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
TRIM( CAST(CAR.EMPLOYEEKEY AS VARCHAR)) AS EMployee_ID,
UPPER( TRIM( CAR.FIRSTNAME)) AS Employee_First_Name,
UPPER(TRIM( CAR.LASTNAME)) AS Employee_Last_Name,
UPPER( TRIM( IFNULL( TRIM( CAR.HOMESTREETADDRESS) , '''')))AS SRC_Employee_ADDRESS
,UPPER(TRIM( CAR.HOMECITY)) AS SRC_Employee_CITY
,UPPER(TRIM( CAR.HOMESTATE)) AS SRC_Employee_STATE_CODE
,UPPER(TRIM( CAR.HOMEZIPVARCHAR)) AS Src_Employee_ZIP
,CAST( NULL AS VARCHAR) AS Src_Employee_Zip4
,TRIM( CAR.SSN) AS Src_Employee_SSN
,CAST( CAR.DATEOFBIRTH AS DATE) AS Src_Employee_DOB
,UPPER( TRIM(CAR.PRIMARYPHONENUMBER)) AS Src_Employee_Home_Phone
,CAST( NULL AS VARCHAR) AS Src_Employee_Cell_Phone
,CAST( NULL AS VARCHAR) AS Src_Employee_Work_Phone
,UPPER( TRIM( CAR.EMAILADDRESS)) AS Src_Employee_Personal_Email 
...
FROM DISC_${var.SF_ENVIRONMENT}.EVOKEEHR.CAREGIVERS AS  CAR
GROUP BY ALL
)
SELECT 
Source_System_ID 
