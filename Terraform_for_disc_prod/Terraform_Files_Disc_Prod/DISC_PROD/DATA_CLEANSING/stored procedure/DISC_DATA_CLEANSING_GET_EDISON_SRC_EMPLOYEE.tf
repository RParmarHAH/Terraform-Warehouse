resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_EDISON_SRC_EMPLOYEE" {
	name ="GET_EDISON_SRC_EMPLOYEE"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  HHAEXCHANGEEdison_SRC_Employee
--
-- PURPOSE: Creates script for data cleanse for HHAEXCHANGEEdison source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
,TRIM( CAST( car.AGENCYID || car.CaregiverID AS VARCHAR)) AS EMployee_ID
...
FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEdison.Caregivers AS car
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEdison.TBLVISITS_REPL AS VISIT ON car.CaregiverID = visit.CaregiverID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEEdison.VISITS VI ON VI.MISSEDVISIT = ''N'' AND VI.VISITID = VISIT.VISITID
