resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_OSHAH_SRC_CLIENT" {
	name ="GET_OSHAH_SRC_CLIENT"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  OSHAH_SRC_CLIENT
--
-- PURPOSE: Creates script for data cleanse for OSHAH source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.Patients AS pt
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.OFFICE_OFFICES_REPL OFFICE ON pt.OFFICEID = OFFICE.OFFICEID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEOSHAH.TBLVISITS_REPL AS VISIT ON pt.PatientID = visit.PatientID
