resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_EVOKE_SRC_CLIENT" {
	name ="GET_EVOKE_SRC_CLIENT"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  EVOKEEHR_SRC_CLIENT
--
-- PURPOSE: Creates script for data cleanse for EVOKEEHR source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
(
SELECT 
30 AS SOURCE_SYSTEM_ID
,''Evoke'' AS SYSTEM_CODE, 
TRIM( CAST(f.CLIENTKEY AS VARCHAR)) AS Client_ID, 
UPPER( TRIM(NVL(F.FIRSTNAME, ''''))) AS Client_First_Name, 
UPPER(TRIM(NVL(F.LASTNAME, ''''))) AS Client_Last_Name,
UPPER( IFNULL( TRIM(F.HOMESTREETADDRESS) , '''')) AS SRC_Client_ADDRESS ,
UPPER( TRIM( F.HOMECITY)) AS SRC_Client_CITY,
UPPER(TRIM(f.HOMESTATE)) AS SRC_Client_STATE_CODE,
UPPER( TRIM( F.HOMEZIP))  AS SRC_Client_ZIP,
CAST( NULL AS VARCHAR) AS Src_Client_Zip4,
UPPER( TRIM(F.SSN)) AS Src_Client_SSN,
CAST( F.DATEOFBIRTH AS DATE) AS Src_Client_DOB,
UPPER( TRIM( F.PRIMARYPHONENUMBER)) AS Src_Client_Home_Phone,
CAST( NULL AS VARCHAR) AS Src_Client_Cell_Phone,
CAST( NULL AS VARCHAR) AS Src_Client_Work_Phone,
UPPER(TRIM(F.EMAILADDRESS)) AS Src_Client_Personal_Email,
CAST( NULL AS VARCHAR) AS Src_Client_Work_Email,
CAST( ''2040-12-31'' AS DATE)AS SRC_Client_Min_Activity_Date,
CAST( ''1900-01-01'' AS DATE) AS SRC_Client_Max_Activity_Date,
...
FROM DISC_${var.SF_ENVIRONMENT}.EVOKEEHR.CLIENT F
WHERE f.CLIENTKEY IS NOT NULL
GROUP BY ALL
)
SELECT  
Source_System_ID
