resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_RISKONNECT_SRC_CLIENT" {
	name ="GET_RISKONNECT_SRC_CLIENT"
...
,C.ID AS CLI_ID,CLIENT_ID__C ,MEDICAID__C,BIRTHDATE,DATE_OF_DEATH__C,GENDER__C,ETHNIC_ORIGIN__C,EMAIL
,MARITAL_STATUS__C,SALUTATION,FIRSTNAME,MIDDLE_INITIAL__C,LASTNAME,C.NAME,ADDRESS_1__C,ADDRESS_2__C
,C.CITY__C,C.STATE__C,C.ZIP_CODE__C,C.HOMEPHONE,C.PHONE,MOBILEPHONE,C.OTHERPHONE,FAX,CONTACT_EMAIL__C,CLIENT_ID_FORMULA__C
,C.LASTREFERENCEDDATE,C.SERVICE_START_DATE__C,C.SERVICE_END_DATE__C,C.SSN__C,C.ACTIVE_INACTIVE__C
,SUPERVISOR_ID__C,SUPERVISOR_NAME__C,REFERRED_BY__C
,LOCATION__C
       FROM DISC_${var.SF_ENVIRONMENT}.RISKONNECT.CONTACT C
      LEFT OUTER JOIN DISC_${var.SF_ENVIRONMENT}.RISKONNECT.HIERARCHY_NODE H on C.LOCATION__C = H.ID 
      INNER JOIN DISC_${var.SF_ENVIRONMENT}.RISKONNECT.PROPERTY P on H.PROPERTY__C = P.ID  WHERE  CONTACT_TYPE__C = ''Client''  QUALIFY IND =1
)
SELECT 
''29'' AS SOURCE_SYSTEM_ID
,(''RISKONNECT'') AS SYSTEM_CODE, 
CLI_ID||''-''||TRIM( REGEXP_REPLACE(CLI.CLIENT_ID_FORMULA__C,''-'')) AS Client_ID, 
UPPER( TRIM(NVL(CLI.FIRSTNAME, ''''))) AS Client_First_Name, 
UPPER(TRIM(NVL(CLI.LASTNAME, ''''))) AS Client_Last_Name,
UPPER( IFNULL( TRIM(CLI.ADDRESS_1__C) , '''') ||'' ''|| IFNULL( TRIM(CLI.ADDRESS_2__C), '''')) AS SRC_Client_ADDRESS ,
UPPER(TRIM(CLI.CITY__C)) AS SRC_Client_CITY,
UPPER(TRIM(CLI.STATE__C)) AS SRC_Client_STATE_CODE,
UPPER( TRIM(REPLACE(CLI.ZIP_CODE__C,''-'')))  AS SRC_Client_ZIP,
CAST( NULL AS VARCHAR) AS Src_Client_Zip4,
UPPER( TRIM(CLI.SSN__C)) AS Src_Client_SSN,
CAST(CLI.BIRTHDATE AS DATE) AS Src_Client_DOB,
UPPER(COALESCE(REGEXP_REPLACE(TRIM(CLI.HOMEPHONE), ''''),REGEXP_REPLACE(TRIM(CLI.PHONE), ''''))) AS Src_Client_Home_Phone,
REGEXP_REPLACE(TRIM(CLI.MOBILEPHONE), '''') AS Src_Client_Cell_Phone,
REGEXP_REPLACE(TRIM(CLI.OTHERPHONE), '''')AS Src_Client_Work_Phone,
UPPER(TRIM(CLI.EMAIL)) AS Src_Client_Personal_Email,
UPPER(TRIM(CLI.CONTACT_EMAIL__C)) AS Src_Client_Work_Email,
MIN( IFNULL(SERVICE_START_DATE__C, CAST( ''2040-12-31'' AS DATE))) AS SRC_Client_Min_Activity_Date,
MAX( IFNULL( SERVICE_START_DATE__C, CAST( ''1900-01-01'' AS DATE))) AS SRC_Client_Max_Activity_Date,
...
FROM CLIENT CLI  
WHERE Client_ID IS NOT NULL AND  CLI.IND = 1
