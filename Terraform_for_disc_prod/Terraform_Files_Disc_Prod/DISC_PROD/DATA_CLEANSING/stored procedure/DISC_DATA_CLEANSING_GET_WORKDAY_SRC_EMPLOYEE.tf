resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_WORKDAY_SRC_EMPLOYEE" {
	name ="GET_WORKDAY_SRC_EMPLOYEE"
...
,HOME_ADDRESS_DATA AS (
	...
	FROM
		DISC_${var.SF_ENVIRONMENT}.WORKDAY.WORKER_ADDRESS_DATA HAD
...
,CASE
		WHEN PED.HOME_PHONE_${var.SF_ENVIRONMENT}ICE_TYPE = ''LANDLINE'' 
			THEN NVL(CONCAT(PED.HOME_PHONE_AREA_CODE, PED.HOME_PHONE_NUMBER),PED.HOME_PHONE_NUMBER)
	END AS Src_Employee_Home_Phone,
	CASE
		WHEN PED.HOME_PHONE_${var.SF_ENVIRONMENT}ICE_TYPE = ''MOBILE'' OR PED.WORK_PHONE_${var.SF_ENVIRONMENT}ICE_TYPE = ''Mobile'' 
			THEN NVL(
...
FROM DISC_${var.SF_ENVIRONMENT}.WORKDAY.WORKER_SUMMARY WS
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.WORKDAY.WORKER_PERSONAL WP
...
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.WORKDAY.WORKER_STATUS WRS ON WS.EMPLOYEE_ID =WRS.EMPLOYEE_ID
