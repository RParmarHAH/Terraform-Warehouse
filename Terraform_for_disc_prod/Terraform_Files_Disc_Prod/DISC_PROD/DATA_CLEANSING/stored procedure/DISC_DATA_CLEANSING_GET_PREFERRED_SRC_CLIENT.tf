resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_PREFERRED_SRC_CLIENT" {
	name ="GET_PREFERRED_SRC_CLIENT"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  PREFERRED_SRC_CLIENT
--
-- PURPOSE: Creates script for data cleanse for PREFERRED source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
,Z2.LNG AS Z2_LNG
,Z2.LAT AS Z2_LAT
FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPreferred.Stage_Patients AS pt
INNER JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.AGENCY_MAPPING AGENCY_CONFIG ON AGENCY_CONFIG.AGENCYID = pt.AGENCYID AND AGENCY_CONFIG.ISACTIVE = TRUE
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPreferred.TBLVISITS_REPL AS VISIT ON pt.PatientID = visit.PatientID
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPreferred.VISITS VI ON VI.MISSEDVISIT = ''N'' AND VI.VISITID = VISIT.VISITID
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_ZIP_CODE_DETAILED Z1 ON Z1.ZIP = LEFT(pt.ZIP5,5)
LEFT JOIN DW_${var.SF_ENVIRONMENT}.STAGE.PREFERRED_PHYSICAL_OFFICES po on Z1.ZIP IS NOT NULL
...
QUALIFY DENSE_RANK() OVER (PARTITION BY Client_ID ORDER BY ST_DISTANCE(ST_MAKEPOINT(Z1_LNG, Z1_LAT), ST_MAKEPOINT(Z2_LNG, Z2_LAT))) = 1
ORDER BY 1
) AS src
