resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_ADPALLIANCE_SRC_EMPLOYEE" {
	name ="GET_ADPALLIANCE_SRC_EMPLOYEE"
...
TRIM( CAST(GA.EMPLOYEE_NUMBER AS VARCHAR)) AS EMployee_ID,
UPPER( TRIM( GA.EMPLOYEE_FIRST_NAME)) AS Employee_First_Name,
UPPER(TRIM( GA.EMPLOYEE_LAST_NAME)) AS Employee_Last_Name,
UPPER( TRIM( IFNULL( TRIM( EMP.ADDRESS1) , '''') ||'' ''|| IFNULL( TRIM(EMP.ADDRESS2), ''''))) AS SRC_Employee_ADDRESS
,UPPER(TRIM(EMP.CITY))   AS SRC_Employee_CITY
,UPPER(TRIM(EMP.STATE)) AS SRC_Employee_STATE_CODE
,UPPER(TRIM(EMP.ZIP)) AS  Src_Employee_ZIP
,CAST( NULL AS VARCHAR) AS Src_Employee_Zip4
,TRIM( GA.EMPLOYEE_SSN) AS Src_Employee_SSN
,CAST( EMP.DATEOFBIRTH AS DATE) AS Src_Employee_DOB
,UPPER( TRIM(EMP.PHONE1)) AS Src_Employee_Home_Phone
,UPPER( TRIM(EMP.PHONE2)) AS Src_Employee_Cell_Phone
,CAST( NULL AS VARCHAR) AS Src_Employee_Work_Phone
,UPPER( TRIM( EMP.EMAIL)) AS Src_Employee_Personal_Email 
,CAST( NULL AS VARCHAR) AS Src_Employee_Work_Email
,MIN( IFNULL(CAST( sch.PAY_DATE  AS DATE), CAST( ''2040-12-31'' AS DATE))) AS Src_Employee_Min_Activity_Date
,MAX( IFNULL( CAST(sch.PAY_DATE  AS DATE), CAST( ''1900-01-01'' AS DATE))) AS Src_Employee_Max_Activity_Date
...
FROM DISC_${var.SF_ENVIRONMENT}.ADP.ALLIANCE_EMPLOYEE GA 
LEFT JOIN  DISC_${var.SF_ENVIRONMENT}.GENERATIONSALLIANCE.EMPLOYEE EMP ON GA.EMPLOYEE_SSN = EMP.SOCIALSECURITYNUM 
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ADP.ALLIANCE_PAYROLL SCH ON GA.EMPLOYEE_SSN = SCH.TAX_ID 
WHERE GA.EMPLOYEE_NUMBER IS NOT NULL 
