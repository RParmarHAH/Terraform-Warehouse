resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_ADPANSWERCARE_SRC_EMPLOYEE" {
	name ="GET_ADPANSWERCARE_SRC_EMPLOYEE"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME: ADPANSWERCARE_Employee
--
-- PURPOSE: Creates script for data cleanse for ADPANSWERCARE source
--
-- DEVELOPMENT LOG:
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
...
UPPER( TRIM( EML.EMPLOYEE_FIRST_NAME)) AS Employee_First_Name,
UPPER(TRIM( EML.EMPLOYEE_LAST_NAME)) AS Employee_Last_Name,
CAST( NULL AS VARCHAR) AS SRC_Employee_ADDRESS
,CAST( NULL AS VARCHAR)   AS SRC_Employee_CITY
,CAST( NULL AS VARCHAR) AS SRC_Employee_STATE_CODE
,CAST( NULL AS VARCHAR) AS  Src_Employee_ZIP
,CAST( NULL AS VARCHAR) AS Src_Employee_Zip4
,TRIM( EML.EMPLOYEE_SSN) AS Src_Employee_SSN
...
,MIN( IFNULL(CAST( AP.PAY_DATE  AS DATE), CAST( ''2040-12-31'' AS DATE))) AS Src_Employee_Min_Activity_Date
,MAX( IFNULL( CAST(AP.PAY_DATE  AS DATE), CAST( ''1900-01-01'' AS DATE))) AS Src_Employee_Max_Activity_Date
...
FROM DISC_${var.SF_ENVIRONMENT}.ADP.VW_ANSWERCARE_EMPLOYEE EML
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.ADP.ANSWERCARE_PAYROLL AP
ON TRIM(AP.FILE_NUMBER) = TRIM(EML.ID)
