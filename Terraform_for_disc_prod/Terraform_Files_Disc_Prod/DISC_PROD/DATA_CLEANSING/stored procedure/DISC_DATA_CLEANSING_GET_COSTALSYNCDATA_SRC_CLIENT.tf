resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_COSTALSYNCDATA_SRC_CLIENT" {
	name ="GET_COSTALSYNCDATA_SRC_CLIENT"
...
BEGIN
    --*****************************************************************************************************************************
-- NAME:  COSTALSYNCDATA_SRC_CLIENT
--
...
-- DATE        AUTHOR                NOTES:
-- --------    -------------------   -----------------------------------------------------------------------------------------------
-- 08/01/23     Darshan Gosai           Initial development
--*****************************************************************************************************************************
--
...
(
SELECT 
case when TRIM(f.DB) = ''SHC_ALTRUS'' then 1 else 2 end AS SOURCE_SYSTEM_ID,
TRIM(f.DB) AS SYSTEM_CODE ,
TRIM( CAST(f.PATIENT_NUMBER AS VARCHAR)) AS Client_ID, 
UPPER( TRIM(NVL(F.FIRST_NAME, ''''))) AS Client_First_Name, 
UPPER(TRIM(NVL(F.LAST_NAME, ''''))) AS Client_Last_Name,
UPPER( TRIM( IFNULL( TRIM(F.ADDRESS) , '''') ||'' ''|| IFNULL( TRIM(F.STREET_NAME), ''''))) AS SRC_Client_ADDRESS ,
UPPER( TRIM( F.CITY)) AS SRC_Client_CITY,
...
UPPER( TRIM( F.ZIPCODE))  AS SRC_Client_ZIP,
CAST( NULL AS VARCHAR) AS Src_Client_Zip4,
UPPER( TRIM(F.SOCIAL_SECURITY_NUMBER)) AS Src_Client_SSN,
CAST( F.BIRTH_DATE AS DATE) AS Src_Client_DOB,
F.PHONE AS Src_Client_Home_Phone,
UPPER( TRIM( F.CELL_PHONE)) AS Src_Client_Cell_Phone,
CAST( NULL AS VARCHAR) AS Src_Client_Work_Phone,
UPPER(TRIM(F.EMAIL)) AS Src_Client_Personal_Email,
CAST( NULL AS VARCHAR) AS Src_Client_Work_Email,
MIN( IFNULL(SC.SCHEDULE_DATE, CAST( ''2040-12-31'' AS DATE))) AS SRC_Client_Min_Activity_Date,
MAX( IFNULL( SC.SCHEDULE_DATE, CAST( ''1900-01-01'' AS DATE))) AS SRC_Client_Max_Activity_Date,
...
FROM DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.CV_PATIENTS AS F
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.COSTALSYNCDATA.CV_SCHEDULES AS  SC 
ON TRIM(F.DB) = TRIM(SC.DB) AND trim(SC.PATIENT_NUMBER) = trim(F.PATIENT_NUMBER)
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_ZIP_CODE Z ON Z.ZIP_CODE=TRIM(F.ZIPCODE)
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_STATE S ON S.STATE_ISO_CODE=UPPER(TRIM(F.STATE_CODE))
WHERE f.PATIENT_NUMBER IS NOT NULL
