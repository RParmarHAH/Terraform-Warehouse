resource "snowflake_procedure" "DISC_DATA_CLEANSING_GET_GENERATIONSALLIANCE_SRC_CLIENT" {
	name ="GET_GENERATIONSALLIANCE_SRC_CLIENT"
...
FROM DISC_${var.SF_ENVIRONMENT}.GENERATIONSALLIANCE.CLIENT F 
LEFT JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_GEOGRAPHY GEO 
	ON GEO.ZIP_CODE = LEFT(f.ZIP,5)
LEFT JOIN DISC_${var.SF_ENVIRONMENT}.GENERATIONSALLIANCE.SCHEDULES SCH ON F.CLIENTID = SCH.CLIENTID 
