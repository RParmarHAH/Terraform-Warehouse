resource "snowflake_procedure" "DISC_AXXESS_GET_STAGE_AXXESS_DIM_BRANCH" {
	name ="GET_STAGE_AXXESS_DIM_BRANCH"
...
WITH OFFICES AS (
	...
	FROM
		DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_BRANCHINFO
),
BRANCH AS (
SELECT Distinct BRANCH_ID, BRANCH FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_EPISODESNAPSHOTS WHERE BRANCH_ID IS NOT NULL
-- DISC_DEDUPE_${var.SF_ENVIRONMENT}.AXXESS.CLIENT_MASTER_LIST WHERE BRANCH_ID IS NOT NULL 
)
SELECT DISTINCT
...
FROM
	BRANCH CLI
LEFT JOIN
	DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_BRANCHINFO BI ON CLI.BRANCH_ID = BI.BRANCH_ID 
LEFT JOIN
...
SELECT DISTINCT
	...
	0 AS ETL_INFERRED_MEMBER_FLAG
FROM (
SELECT Distinct BRANCH_ID, BRANCH_NAME,BRANCH_STATE_CODE,BRANCH_ZIP_CODE FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_VISITFINANCIALS
