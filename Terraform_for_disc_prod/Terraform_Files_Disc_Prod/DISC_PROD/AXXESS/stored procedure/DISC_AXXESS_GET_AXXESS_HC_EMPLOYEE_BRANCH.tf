resource "snowflake_procedure" "DISC_AXXESS_GET_AXXESS_HC_EMPLOYEE_BRANCH" {
	name ="GET_AXXESS_HC_EMPLOYEE_BRANCH"
...
--*****************************************************************************************************************************
-- NAME:  DISC_${var.SF_ENVIRONMENT}.AXXESS.GET_AXXESS_HC_EMPLOYEE_BRANCH 
--
...
MERGE INTO DISC_${var.SF_ENVIRONMENT}.AXXESS.HIST_AXXESS_HC_EMPLOYEE_BRANCH TARGET 
USING (
    SELECT 
        A.$1 AS Account_Id, 
        A.$2 AS Account_Name, 
    ...
    FROM @DISC_${var.SF_ENVIRONMENT}.stage.AWSAZSTAGEPROD/Axxess/SFTP_files/AXXESS_HC_EMPLOYEE_BRANCH(file_format => DISC_${var.SF_ENVIRONMENT}.STAGE.PSV_FORMAT) A
) SOURCE
...
WHEN NOT MATCHED THEN
 ...
 UPDATE DISC_${var.SF_ENVIRONMENT}.AXXESS.HIST_AXXESS_HC_EMPLOYEE_BRANCH
 SET ETL_DELETED_FLAG = TRUE, ETL_LAST_UPDATED_DATE = CURRENT_TIMESTAMP(),ETL_LAST_UPDATED_BY = CURRENT_USER()
 WHERE Account_Id NOT IN ( SELECT A.$1 FROM @DISC_${var.SF_ENVIRONMENT}.stage.AWSAZSTAGEPROD/Axxess/SFTP_files/AXXESS_HC_EMPLOYEE_BRANCH(file_format => DISC_${var.SF_ENVIRONMENT}.STAGE.PSV_ZIPCODES) A);


 CREATE OR REPLACE TABLE DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_HC_EMPLOYEE_BRANCH AS SELECT * FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.HIST_AXXESS_HC_EMPLOYEE_BRANCH WHERE 
 ETL_DELETED_FLAG=FALSE AND (Account_Id, ETL_INSERTED_DATE) IN (SELECT Account_Id, MAX(ETL_INSERTED_DATE)FROM 
 DISC_${var.SF_ENVIRONMENT}.AXXESS.HIST_AXXESS_HC_EMPLOYEE_BRANCH GROUP BY Account_Id);
