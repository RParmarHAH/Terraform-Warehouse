resource "snowflake_procedure" "DISC_AXXESS_GET_STAGE_AXXESS_FACT_REVENUE" {
...
LEFT JOIN (SELECT  BRANCH_ID, BRANCH FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_EPISODESNAPSHOTS WHERE BRANCH_ID IS NOT NULL GROUP BY  BRANCH_ID, BRANCH)
...
JOIN DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_FINANCIALTRANSACTIONS FIN ON FIN.CLAIM_ID = VISIT.CLAIM_ID
 )T
GROUP BY
REVENUE_KEY
,REPORT_DATE
