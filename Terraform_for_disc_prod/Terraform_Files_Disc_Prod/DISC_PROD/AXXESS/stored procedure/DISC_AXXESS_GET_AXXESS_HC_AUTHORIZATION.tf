resource "snowflake_procedure" "DISC_AXXESS_GET_AXXESS_HC_AUTHORIZATION" {
	name ="GET_AXXESS_HC_AUTHORIZATION"
...
--*****************************************************************************************************************************
-- NAME:  DISC_${var.SF_ENVIRONMENT}.AXXESS.GET_AXXESS_HC_AUTHORIZATION 
--
...
WHEN NOT MATCHED THEN
		...
		UPDATE DISC_${var.SF_ENVIRONMENT}.AXXESS.HIST_AXXESS_HC_AUTHORIZATION SET
	ETL_DELETED_FLAG=TRUE, ETL_LAST_UPDATED_DATE=CURRENT_TIMESTAMP(),ETL_LAST_UPDATED_BY=CURRENT_USER()
	WHERE Authorization_Detail_Id NOT IN (SELECT A.$1 FROM @DISC_${var.SF_ENVIRONMENT}.STAGE.AWSAZSTAGEPROD/Axxess/SFTP_files/AXXESS_HC_Authorization (file_format => DISC_${var.SF_ENVIRONMENT}.STAGE.PSV_FORMAT) A);

   CREATE OR REPLACE TABLE DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_HC_AUTHORIZATION AS SELECT * FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.HIST_AXXESS_HC_AUTHORIZATION WHERE 
   ...
   DISC_${var.SF_ENVIRONMENT}.AXXESS.HIST_AXXESS_HC_AUTHORIZATION GROUP BY Authorization_Detail_Id);
END;
