resource "snowflake_procedure" "DISC_AXXESS_GET_AXXESS_HC_BRANCHINFO" {
	name ="GET_AXXESS_HC_BRANCHINFO"
...
--*****************************************************************************************************************************
-- NAME:  DISC_${var.SF_ENVIRONMENT}.AXXESS.GET_AXXESS_HC_BranchInfo 
--
...
BEGIN 
	...
	UPDATE DISC_${var.SF_ENVIRONMENT}.AXXESS.HIST_AXXESS_HC_BRANCHINFO SET
	ETL_DELETED_FLAG=TRUE, ETL_LAST_UPDATED_DATE=CURRENT_TIMESTAMP(),ETL_LAST_UPDATED_BY=CURRENT_USER()
	WHERE Branch_Id NOT IN (SELECT A.$1 FROM @DISC_${var.SF_ENVIRONMENT}.stage.AWSAZSTAGEPROD/Axxess/SFTP_files/AXXESS_HC_BRANCHINFO(file_format => DISC_${var.SF_ENVIRONMENT}.STAGE.PSV_FORMAT) A);
	
	CREATE OR REPLACE TABLE DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_HC_BRANCHINFO AS SELECT * FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.HIST_AXXESS_HC_BRANCHINFO WHERE 
   ETL_DELETED_FLAG=FALSE AND (Branch_Id,ETL_INSERTED_DATE) IN (SELECT Branch_Id,MAX(ETL_INSERTED_DATE)FROM 
   DISC_${var.SF_ENVIRONMENT}.AXXESS.HIST_AXXESS_HC_BRANCHINFO GROUP BY Branch_Id);
