resource "snowflake_procedure" "DISC_AXXESS_GET_AXXESS_HH_AGENCYINFO" {
	name ="GET_AXXESS_HH_AGENCYINFO"
...
--*****************************************************************************************************************************
-- NAME:  DISC_${var.SF_ENVIRONMENT}.AXXESS.GET_AXXESS_HH_AGENCYINFO 
--
...
--*****************************************************************************************************************************
	...
	MERGE INTO DISC_${var.SF_ENVIRONMENT}.AXXESS.HIST_AXXESS_HH_AGENCYINFO TARGET
USING (
		SELECT 
			A.$1 AS Agency_Id, 
		...
		FROM @DISC_${var.SF_ENVIRONMENT}.stage.AWSAZSTAGEPROD/Axxess/SFTP_files/AXXESS_HH_AGENCYINFO(file_format => DISC_${var.SF_ENVIRONMENT}.STAGE.PSV_FORMAT) A
	) SOURCE
...
WHEN NOT MATCHED THEN
 ...
 UPDATE DISC_${var.SF_ENVIRONMENT}.AXXESS.HIST_AXXESS_HH_AGENCYINFO
 SET ETL_DELETED_FLAG = TRUE, ETL_LAST_UPDATED_DATE = CURRENT_TIMESTAMP(),ETL_LAST_UPDATED_BY = CURRENT_USER()
 WHERE Agency_Id NOT IN ( SELECT A.$1 FROM @DISC_${var.SF_ENVIRONMENT}.stage.AWSAZSTAGEPROD/Axxess/SFTP_files/AXXESS_HH_AGENCYINFO(file_format => DISC_${var.SF_ENVIRONMENT}.STAGE.PSV_ZIPCODES) A);
 
 CREATE OR REPLACE TABLE DISC_${var.SF_ENVIRONMENT}.AXXESS.AXXESS_HH_AGENCYINFO AS 
 SELECT * FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.HIST_AXXESS_HH_AGENCYINFO
 WHERE ETL_DELETED_FLAG=FALSE AND (Agency_Id, ETL_INSERTED_DATE) IN (SELECT Agency_Id, MAX(ETL_INSERTED_DATE) FROM DISC_${var.SF_ENVIRONMENT}.AXXESS.HIST_AXXESS_HH_AGENCYINFO GROUP BY Agency_Id);
