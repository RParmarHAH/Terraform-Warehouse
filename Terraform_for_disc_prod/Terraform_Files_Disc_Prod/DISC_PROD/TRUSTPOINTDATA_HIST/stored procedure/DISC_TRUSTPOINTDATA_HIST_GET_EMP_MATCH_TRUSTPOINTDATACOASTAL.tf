resource "snowflake_procedure" "DISC_TRUSTPOINTDATA_HIST_GET_EMP_MATCH_TRUSTPOINTDATACOASTAL" {
	name ="GET_EMP_MATCH_TRUSTPOINTDATACOASTAL"
	...
	statement = <<-EOT
        ...
        var sql = `
INSERT OVERWRITE INTO DISC_DEDUPE_${var.SF_ENVIRONMENT}.TRUSTPOINTDATA.EMP_MATCH_TRUSTPOINTDATACOASTAL
WITH DEDUPE_RECORD_IDS AS 
(
	SELECT RECORD_ID,CLUSTER_ID,FILENAME
	FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.TRUSTPOINTDATA.TRUSTPOINTDATAcoastal_EMP_DEDUPE_SOURCE
	WHERE 
...
(
	...
	FROM DISC_DEDUPE_${var.SF_ENVIRONMENT}.TRUSTPOINTDATA.TRUSTPOINTDATAcoastal_EMP_DEDUPE_SOURCE DEDUPE
	INNER JOIN DISC_${var.SF_ENVIRONMENT}.TRUSTPOINTDATA.EMPLOYEES EMPLOYEE ON
