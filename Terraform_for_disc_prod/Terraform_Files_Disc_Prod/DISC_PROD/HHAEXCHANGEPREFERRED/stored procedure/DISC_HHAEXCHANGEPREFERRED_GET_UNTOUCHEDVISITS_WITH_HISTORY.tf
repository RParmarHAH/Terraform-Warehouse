resource "snowflake_procedure" "DISC_HHAEXCHANGEPREFERRED_GET_UNTOUCHEDVISITS_WITH_HISTORY" {
...
BEGIN
	...
	UPDATE DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.HIST_UNTOUCHEDVISITS_WITH_HISTORY SET ETL_DELETED_FLAG=TRUE,ETL_LAST_UPDATED_DATE=CURRENT_TIMESTAMP(),ETL_LAST_UPDATED_BY=CURRENT_USER() 
		WHERE VISITDATE>=(SELECT MIN(t.$7) FROM @DISC_${var.SF_ENVIRONMENT}.STAGE.AWSAZSTAGEPROD/To_Be_Processed/ (file_format => DISC_${var.SF_ENVIRONMENT}.STAGE.MY_CSV_FORMAT,PATTERN => ''.*HHAEXCHANGEPREFERRED_dbo_UntouchedVisits.*[.]csv.gz'') T WHERE t.$1 IN (243, 371)) 
		AND (AGENCYID,VISITID) NOT IN (SELECT DISTINCT t.$1 AS AgencyID, t.$29 AS VisitID FROM @DISC_${var.SF_ENVIRONMENT}.STAGE.AWSAZSTAGEPROD/To_Be_Processed/ (file_format => DISC_${var.SF_ENVIRONMENT}.STAGE.MY_CSV_FORMAT,PATTERN => ''.*HHAEXCHANGEPREFERRED_dbo_UntouchedVisits.*[.]csv.gz'')T WHERE t.$1 IN (243, 371));
	
	UPDATE DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.HIST_UNTOUCHEDVISITS_WITH_HISTORY SET ETL_DELETED_FLAG=FALSE,ETL_LAST_UPDATED_DATE=CURRENT_TIMESTAMP(),ETL_LAST_UPDATED_BY=CURRENT_USER() WHERE ETL_DELETED_FLAG=TRUE AND (AGENCYID,VISITID) IN (SELECT DISTINCT t.$1 AS AgencyID, t.$29 AS VisitID FROM @DISC_${var.SF_ENVIRONMENT}.STAGE.AWSAZSTAGEPROD/To_Be_Processed/ (file_format => DISC_${var.SF_ENVIRONMENT}.STAGE.MY_CSV_FORMAT,PATTERN => ''.*HHAEXCHANGEPREFERRED_dbo_UntouchedVisits.*[.]csv.gz'')T WHERE t.$1 IN (243, 371));

	CREATE OR REPLACE TABLE DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.UNTOUCHEDVISITS_WITH_HISTORY AS SELECT * FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.HIST_UNTOUCHEDVISITS_WITH_HISTORY WHERE ETL_DELETED_FLAG=FALSE AND (AGENCYID,VISITID,ETL_INSERTED_DATE) IN (SELECT AGENCYID,VISITID,MAX(ETL_INSERTED_DATE) FROM DISC_${var.SF_ENVIRONMENT}.HHAEXCHANGEPREFERRED.HIST_UNTOUCHEDVISITS_WITH_HISTORY GROUP BY AGENCYID,VISITID);
