resource "snowflake_view" "DISC_JAZZHR_PARADOX_JAZZHR_ACROSS_DEDUPE" {
	database = "DISC_${var.SF_ENVIRONMENT}"
	schema = "JAZZHR"
	name = "PARADOX_JAZZHR_ACROSS_DEDUPE"
	statement = <<-SQL
	 with hire as (
select distinct APPLICANT_ID,max(HIRED_DATE) as HIRED_DATE  from DISC_${var.SF_ENVIRONMENT}.JAZZHR.HIRES group by 1
) 
 select 
...
from DW_${var.SF_ENVIRONMENT}.HAH.DIM_APPLICANT APPLICANT 
...
from DW_${var.SF_ENVIRONMENT}.HAH.DIM_APPLICANT APPLICANT1 
JOIN (select APPLICANT_KEY,max(HIRED_DATETIME) HIRED_DATETIME from DW_${var.SF_ENVIRONMENT}.HAH.FACT_APPLICANT_STATUS group by 1) status ON APPLICANT1.applicant_key = status.applicant_key  
